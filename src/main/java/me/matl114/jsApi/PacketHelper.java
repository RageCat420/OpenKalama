...
public class PacketHelper {
    ...
    public static void sendInteractEntityAt(Object entity, Object pos, boolean sneaking, boolean offhand) {
        ...
        mc.interactionManager.sendSequencedPacket(
                ...
                seq -> new PlayerInteractEntityC2SPacket(
                        var5, sneaking, new InteractAtHandler(offhand ? Hand.OFF_HAND : Hand.MAIN_HAND, var7)));
    }
    ...
    public static void sendInteractItem(boolean offHand, float pitch, float yaw) {
        ...
        mc.interactionManager.sendSequencedPacket(
                mc.world,
                seq -> new PlayerInteractItemC2SPacket(offHand ? Hand.OFF_HAND : Hand.MAIN_HAND, seq, yaw, pitch));
    }
    ...
    public static void sendInteractBlock(Object pos, Object direction, boolean offhand) {
        ...
        mc.interactionManager.sendSequencedPacket(mc.world, seq -> {
            PlayerInteractBlockC2SPacket var3x =
                    new PlayerInteractBlockC2SPacket(offhand ? Hand.OFF_HAND : Hand.MAIN_HAND, var5, seq);
            if (var3x instanceof PlayerInteractBlockC2SPacketAccess var4x) {
                var4x.setUseContext(new PlayerInteractBlockC2SPacketAccess.UseContext(
                        mc.player
    ...
    public static List<String> getAllPacketTypes() {
        return Listener.K().keySet().stream()
                .map(PacketType::id)
    ...
    public static void sendInteractEntity(Object entity, boolean sneaking, boolean offhand) {
        ...
        mc.interactionManager.sendSequencedPacket(
                mc.world,
                seq -> new PlayerInteractEntityC2SPacket(
                        var4, sneaking, new InteractHandler(offhand ? Hand.OFF_HAND : Hand.MAIN_HAND)));
    }
    ...
    public static void sendAttackBlock(Object pos, Object direction) {
        ...
        if (mc.player.getAbilities().creativeMode) {
            ...
            if (var6) {
                mc.interactionManager.sendSequencedPacket(
                        mc.world,
                        sequence -> new PlayerActionC2SPacket(Action.START_DESTROY_BLOCK, var3, var2, sequence));
                mc.execute(() -> mc.interactionManager.breakBlock(var3));
            } else if (!Objects.equals(
                    var3, PlayerInteractionAccess.of(mc.interactionManager).getCurrentMiningPos())) {
