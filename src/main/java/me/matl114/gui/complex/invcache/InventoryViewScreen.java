...
public class InventoryViewScreen extends GenericBackGroundScreen {
   ...
   protected void init() {
      super.init();
      ExecutableWidget.instance(this.x + 40, this.y + 20 + 2, 16, 16).<ExecutableWidget>eV(this.V).addTo(this);
      AbstractElement var1;
      if (this.Q != null) {
         var1 = LabelElement.instance(
               Text.literal("%s [%d, %d, %d] ".formatted(this.R.getRegistryKey().getValue().toString(), this.Q.getX(), this.Q.getY(), this.Q.getZ()))
                  .append(Text.translatable("widget.gui.inventory-view-screen.click-slot"))
            )
            .cF(
               KalamaHelperHelperP.aA(
                  () -> MinecraftClient.getInstance().keyboard.setClipboard("%d %d %d".formatted(this.Q.getX(), this.Q.getY(), this.Q.getZ()))
               )
            )
            .aO(
               TooltipHandler.ap(
                  Streams.concat(
                        new Stream[]{
                           ChatUtils.parseTranslation("widget.gui.inventory-view-screen.click-slot.tooltips", "").stream(),
                           ChatUtils.parseTranslation("widget.gui.inventory-view-screen.copy-coordinate.tooltips", "").stream()
                        }
                     )
                     .toList()
               )
         ...
         var1 = LabelElement.instance(
               Text.translatable("widget.gui.inventory-view-screen.virtual-screen").append(Text.translatable("widget.gui.inventory-view-screen.click-slot"))
            )
            .aO(TooltipHandler.ap(ChatUtils.parseTranslation("widget.gui.inventory-view-screen.click-slot.tooltips", "")));
      }

      ExecutableWidget.instance(this.x + 60, this.y + 20, 140, 20).<ExecutableWidget>eV(var1).addTo(this);
      new ContentDelegateWidget<GridSubScreen<DrawableWidget>>(this.x, this.y, 0, 0).setContentDelegate(this.S).addTo(this);
      ExecutableWidget.instance(this.x + 40, this.y + 20 + 20 + 120, 160, 20)
         .<ExecutableWidget>eV(
            LabelElement.instance(
                  Text.translatable("widget.gui.inventory-view-screen.player-inventory-view")
                     .append(Text.translatable("widget.gui.inventory-view-screen.click-slot"))
               )
               .aO(TooltipHandler.ap(ChatUtils.parseTranslation("widget.gui.inventory-view-screen.player-inventory-view.tooltips", "")))
         )
         .addTo(this);
      new ContentDelegateWidget<GridSubScreen<DrawableWidget>>(this.x, this.y, 0, 0).setContentDelegate(this.T).addTo(this);
      DisplayWidget.instance(0, 0, this.width, this.height)
         ...
         .addTo(this);
      ExecutableWidget.instance(0, 0, this.width, this.height).<ExecutableWidget>eT((element, mouseX, mouseY, button) -> {
         this.Y = ItemStack.EMPTY;
         return false;
      }).<DrawableWidget>setPriority(-1).addTo(this);
   }

   public InventoryViewScreen(HandledScreen<?> handledScreen) {
      this(
         handledScreen.getScreenHandler().slots.stream().filter(i -> !(i.inventory instanceof PlayerInventory)).toList(),
   ...
   protected DrawableWidget makeIcon(Slot screen) {
      return ExecutableWidget.instance(0, 0, 16, 16).eV(new SlotElement(screen.inventory, screen.getIndex(), (stack, i) -> {
   ...
   protected DrawableWidget as(Slot screen) {
      return ExecutableWidget.instance(0, 0, 16, 16)
         .eV(new SlotElement(screen.inventory, screen.getIndex(), (stack, i) -> this.pickupItem(screen)).aN(() -> this.Y == null || this.Y.isEmpty()));
   }
   ...
   private static List<Slot> streamInventoryToSlot(Inventory inventory, int size) {
      return IntStream.range(0, size).mapToObj(i -> new Slot(inventory, i, 0, 0)).toList();
   }
