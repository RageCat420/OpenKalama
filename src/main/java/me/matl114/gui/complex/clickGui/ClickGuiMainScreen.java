   ...
   protected void init() {
      ...
      for (String var6 : this.dE.keySet()) {
         AbstractElement var8 = new ButtonElement(
               TextProvider.c(Text.translatableWithFallback("widget.click-gui.selection." + var6, var6)), ButtonAction.a(() -> this.setGlobal(var6))
            )
            .cA(ButtonElement.bH)
            .cC(ButtonElement.bI)
            .cw(el -> Objects.equals(this.dF, var6))
            .aO(TooltipHandler.ap(ChatUtils.parseTranslation("widget.click-gui.selection." + var6 + ".tooltips", "暂无介绍")));
         ExecutableWidget.instance(var2 + var4 * var3, 0, var3, 12).<ExecutableWidget>eV(var8).addTo(this);
         var4++;
      }
      ...
      this.dF = null;
      this.setGlobal(var9);
      this.dI = new ContentDelegateWidget<DrawableWidget>(0, 12, this.width, this.height - 12).setContentDelegate(this.be).addTo(this);
   }
