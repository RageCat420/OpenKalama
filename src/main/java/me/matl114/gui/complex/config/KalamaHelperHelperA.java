...
public class KalamaHelperHelperA<W> extends KalamaHelperHelperE<W> {
   ...
   public KalamaHelperHelperA(int x, int y, int dx, int dy, int dKey, int dblank, int dvalue, Ref<W> kv, AttrKeyValue<W> attr) {
      this(x, y, dx, dy, dKey, dblank, dvalue, kv.hasDefaultValue() ? Optional.of((W)kv.getDefaultValue()) : Optional.empty(), attr);
   }

   public KalamaHelperHelperA(int x, int y, int dx, int dy, int dKey, int dblank, int dvalue, Optional<W> kv, AttrKeyValue<W> attr) {
   ...
   protected void af() {
      ...
      ExecutableWidget.instance(this.dr + this.ds + this.dt + 1, 1, this.dy - 2, this.dy - 2)
         .<ExecutableWidget>eV(
            new me.matl114.gui.elements.KalamaHelperHelperD(
