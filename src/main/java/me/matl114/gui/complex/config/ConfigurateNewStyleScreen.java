...
public class ConfigurateNewStyleScreen extends IndexedScreen<Config, ConfigureListWidget> {
    ...
    protected ConfigureListWidget bd(Config val) {
        ...
        return ConfigureListWidget.createConfigConfigure(
                val, 20, 0, 100, 140, 0, 220, 20, this.width - 100 - 30, this.height - 20, this.filterWidget);
    }
    ...
    protected ElementHandler be(Config val) {
        return new ButtonElement(
                        TextProvider.c(Text.literal(val.getConfigName())), ButtonAction.a(() -> this.setGlobal(val)))
                .cA(ButtonElement.bH)
                .cC(ButtonElement.bI)
