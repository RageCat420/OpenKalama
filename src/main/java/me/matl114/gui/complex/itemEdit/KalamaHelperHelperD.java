...
public class KalamaHelperHelperD extends KalamaHelperHelperQ {
   ...
   protected void init() {
      this.bi = AttrKeyValue.registry("widget.gui.item-edit-screen.nbt-editor.generic.item-id", Registries.ITEM, this.M.dO.getItem());
      this.bj = AttrKeyValue.integer("widget.gui.item-edit-screen.nbt-editor.generic.count", this.M.dO.getCount());
      this.bl = AttrKeyValue.integer("widget.gui.item-edit-screen.nbt-editor.generic.durability", this.M.dO.getDamage());
      ...
      this.bk = AttrKeyValue.bool("widget.gui.item-edit-screen.nbt-editor.generic.unbreakable-flag", ItemStackUtils.getIsUnbreakable(this.M.dO));
   ...
   protected void saveChanges() {
      ...
      if (var1 != null && !var1.isEmpty()) {
         if (this.bn != null) {
            PropertyMap var2 = BukkitItemStackUtils.buildPropertyMap(VRecord.getGameProfileProperties(this.bn), var1);
            ItemStackUtils.setOrRemoveChange(this.M.dO, DataComponentTypes.PROFILE, VRecord.withProperty(this.bn, var2));
         } else {
            ItemStackUtils.setOrRemoveChange(this.M.dO, DataComponentTypes.PROFILE, BukkitItemStackUtils.e(var1));
         }
      } else if (this.bn != null) {
         ItemStackUtils.setOrRemoveChange(this.M.dO, DataComponentTypes.PROFILE, VRecord.withProperty(this.bn, VRecord.k()));
      } else {
