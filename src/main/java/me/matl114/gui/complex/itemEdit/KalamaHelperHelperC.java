...
public class KalamaHelperHelperC extends KalamaHelperHelperQ {
   ...
   protected void ag() {
      ItemStackUtils.applyEntityModifier(
         this.M.dO, new AttributeModifiersComponent(this.bb.stream().map(KalamaHelperHelperP::value).filter(Objects::nonNull).toList(), this.showInTooltips)
      );
   }
   ...
   protected void init() {
      ...
      var1.modifiers()
         .forEach(var -> this.bb.add(new KalamaHelperHelperP(this, ItemStackUtils.solveDynamic(var.attribute()).toString(), var.modifier(), var.slot())));
      new me.matl114.gui.complex.config.KalamaHelperHelperD(
            ListEntryWidgetController.mutable(this.bb, () -> new KalamaHelperHelperP(this), KalamaHelperHelperP::b, 60, 180),
            10,
            0,
            260,
