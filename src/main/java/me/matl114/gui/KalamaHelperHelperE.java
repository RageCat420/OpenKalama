...
public class KalamaHelperHelperE<T extends ClickableWidget> extends ContentDelegateWidget<T> {
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        if (this.Z && this.ef() != null) {
            ...
            TextFieldAccess.of(var10)
                    .dragSelect(
                            (int) ((mouseX - this.getX() - var10.getX()) / var11),
                            (int) ((mouseY - this.getY() - var10.getY()) / var11),
                            true);
        }
    ...
    public boolean startDrag(Screen screen, double mouseX, double mouseY) {
        if (this.ef() != null
                && TextFieldAccess.of(this.ef()).canStartDrag(mouseX - this.getX(), mouseY - this.getY())) {
            this.Z = true;
            return true;
        } else {
            return false;
