...
public class ListRegistrySelectWidget<T> extends KalamaHelperHelperD<Triplet<Text, Identifier, T>> {
    ...
    public static <T> List<Triplet<Text, Identifier, T>> list(
        ...
        return lst.stream()
                .map(s -> new Triplet<Text, net.minecraft.util.Identifier, T>(
                        (Text) localization.apply(s), registry.getId(s), s))
                .toList();
    }
    ...
    public ListRegistrySelectWidget(
        ...
        super(
                ...
                renderFactory,
                filterInput,
                (BiPredicate<Triplet<Text, Identifier, T>, String>) (BiPredicate) filter,
                x,
                y,
                dx,
                dy,
