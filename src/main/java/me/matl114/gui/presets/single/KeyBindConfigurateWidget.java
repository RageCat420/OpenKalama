...
public class KeyBindConfigurateWidget extends KalamaHelperHelperCX {
   ...
   private void onAnyKeyPressed(int keyCode) {
      ...
      if (var3.isEmpty() || !Objects.equals(var3.get(var3.size() - 1), var2)) {
         var3.add(var2);
         this.ackChange(var3, this.dJ.getOriginValue().j(), this.dJ.getOriginValue().k());
      }
   }
   ...
   private void init() {
      this.dL = ExecutableWidget.instance(0, 1, this.dx - 3 * this.dy - 2, this.dy - 2)
         ...
         })).aO(TooltipHandler.ap(ChatUtils.parseTranslation("widget.gui.key-bind-configurate-widget.keycode-configure.tooltips", ""))))
         .addToSub(this);
      ExecutableWidget.instance(this.dx - 3 * this.dy - 1, 1, this.dy - 2, this.dy - 2)
         .<ExecutableWidget>eV(
            new ButtonElement(TextProvider.c(Text.literal("T")), (element, widget, mouseButton) -> {
               ...
               .aO(TooltipHandler.ap(ChatUtils.parseTranslation("widget.gui.key-bind-configurate-widget.keycode-t.tooltips", "")))
         )
         .addToSub(this);
      ExecutableWidget.instance(this.dx - 2 * this.dy - 1, 1, this.dy - 2, this.dy - 2)
         .<ExecutableWidget>eV(
            new ButtonElement(TextProvider.c(Text.literal("V")), (element, widget, mouseButton) -> {
               ...
               .aO(TooltipHandler.ap(ChatUtils.parseTranslation("widget.gui.key-bind-configurate-widget.keycode-v.tooltips", "")))
         )
         .addToSub(this);
      this.dM = ExecutableWidget.instance(this.dx - this.dy - 1, 1, this.dy - 2, this.dy - 2)
         .<ExecutableWidget>eV(new ButtonElement(TextProvider.c(Text.literal("D")), (element, widget, mouseButton) -> {
            this.fi();
            return false;
         }).aO(TooltipHandler.ap(ChatUtils.parseTranslation("widget.gui.key-bind-configurate-widget.keycode-d.tooltips", ""))))
         .addToSub(this);
   }
   ...
   private void fi() {
      ...
      if (!var1.isEmpty()) {
         ...
         this.ackChange(var1, this.dJ.getOriginValue().j(), this.dJ.getOriginValue().k());
      }
   }
   ...
   private Text createKeyDisplay(DrawableWidget el) {
      ...
      return this.isFocused() && el == this.B ? Text.literal("> " + var3 + " <").formatted(Formatting.GOLD) : Text.literal(var3);
   }
