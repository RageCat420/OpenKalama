...
public class ColorSelectScreen extends ConfirmingBigScreen {
    ...
    protected void init() {
        ...
        KalamaHelperHelperCX var1 =
                new KalamaHelperHelperCX(this.x, this.y, this.backgroundWidth, this.backgroundHeight);
        ...
        ExecutableWidget.instance(350, 60, 40, 40)
                .<ExecutableWidget>eV(new ColorSelectIcon(
                        ValueAccessor.of(() -> TextColor.fromRgb(ColorUtils.c(this.cr, this.cs, this.ct))),
                        ButtonAction.c()))
                .addToSub(var1);
        ExecutableWidget.instance(310, 120, 120, 30)
                .<ExecutableWidget>eV(new RawTextElement(
                        el -> Text.literal("R: %d, G: %d, B: %d".formatted(this.cr, this.cs, this.ct)),
    ...
    protected boolean canConfirm(ElementHandler elementHandler) {
        return ColorUtils.getColorInt(this.cr, this.cs, this.ct, 0)
                != this.source.getValue().getRgb();
    }
}
