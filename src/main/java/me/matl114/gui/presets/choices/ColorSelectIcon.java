...
public class ColorSelectIcon extends BoxElement {
    ...
    public ColorSelectIcon(ValueAccessor<TextColor> color) {
        ...
        this.cE(TooltipHandler.ar(() -> {
            ...
            var1.add(Text.translatable(
                    "widget.gui.color-select-icon.current-color",
                    new Object[] {((TextColor) color.getValue()).getName()}));
    ...
    public void renderCentered0(
        ...
        context.fill(
                1, 1, element.getTextureWidth() - 1, element.getTextureHeight() - 1, ColorUtils.j(var8.getRgb(), 255));
    }

    public ColorSelectIcon(ValueAccessor<TextColor> color, ButtonAction buttonAction) {
        super(buttonAction);
        this.color = color;
        this.cE(TooltipHandler.ar(() -> List.of(Text.translatable(
                "widget.gui.color-select-icon.current-color",
                new Object[] {((TextColor) color.getValue()).getName()}))));
    }
