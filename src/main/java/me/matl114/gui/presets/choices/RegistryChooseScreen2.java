...
public class RegistryChooseScreen2<T> extends ConfirmingBigScreen {
    ...
    public RegistryChooseScreen2(Registry<T> registry, Set<T> currentSelection, Consumer<Set<T>> callback) {
        ...
        this.cm = ListRegistryMultiSelectWidget.registry(
                this.aq, currentSelection, ValueAccessor.holder(""), 0, CONTENT_START_Y + 20, 240, 240, 20);
    }

    @Override
    protected void init() {
        super.init();
        this.as = new ContentDelegateWidget<ListRegistryMultiSelectWidget<T>>(
