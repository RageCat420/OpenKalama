   ...
   public static void onDebugRenderTick(Event<MatrixStack> event) {
      if (d && a.player != null) {
         try {
            ...
            RenderUtils.drawOutlinedBox((MatrixStack)event.e(), var1.toCenterPos().add(l), var1.toCenterPos().add(n), Color.GREEN);
            Vec3d var2 = a.player.getPos();
            Vec3d var3 = var2.subtract(0.0, 0.500001F, 0.0);
            BlockPos var4 = BlockPos.ofFloored(var3);
            RenderUtils.drawOutlinedBox((MatrixStack)event.e(), var4.toCenterPos().add(l), var4.toCenterPos().add(n), Color.MAGENTA);
         } finally {
      ...
      if (j) {
         i(
            new KalamaHelperHelperCX(
               DEBUG_TICK,
               new KalamaHelperHelperDX(Vec3d.of(packetHitResult.getBlockPos()), Vec3d.of(packetHitResult.getBlockPos()).add(1.0, 1.0, 1.0), Color.WHITE)
            )
         );
         i(
            new KalamaHelperHelperCX(
               DEBUG_TICK, new KalamaHelperHelperDX(packetHitResult.getPos().add(-0.1, -0.1, -0.1), packetHitResult.getPos().add(0.1, 0.1, 0.1), Color.RED)
            )
         );
      }
   }
