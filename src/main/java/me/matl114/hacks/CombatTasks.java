...
public class CombatTasks {
    ...
    public static boolean isWeaponForMCPlayer(ItemStack itemStack) {
        AttributeModifiersComponent var1 =
                (AttributeModifiersComponent) itemStack.get(DataComponentTypes.ATTRIBUTE_MODIFIERS);
        if (var1 != null && !var1.modifiers().isEmpty()) {
            ...
            return var2 != null
                    && (ItemStackUtils.getEnchantmentLevel(var2, Enchantments.SHARPNESS) > 0
                            || ItemStackUtils.getEnchantmentLevel(var2, Enchantments.SMITE) > 0);
        }
    }
        ...
        if (!(var3 > 10.0) && !(velocity > 10.0F)) {
            ...
            Vec2f var15 = new Vec2f(
                    (float) (-Math.toDegrees(
                            Math.atan((var10 - Math.sqrt(var11 - 0.05F * (0.05F * var8 + 2.0 * var12 * var10)))
                                    / (0.05F * var6)))),
            ...
            if (var3 < 1.0E-4) {
                ...
                Vec3d var21 = var19 > 1.0E-4
                        ? new Vec3d(targetVec.x / var19, 0.0, targetVec.z / var19)
                ...
                if (var19 < 1.0E-4) {
                    ...
                    return var36
                            ? new Vec2f((float) (-Math.toDegrees(var22)), (float) (Math.toDegrees(var24) - 90.0))
                            : var15;
                }
            }
        } else {
    ...
    public static boolean notSuitableForAttack(ItemStack item) {
        return item.isEmpty()
                || !ItemStackUtils.g(item, DataComponentTypes.ATTRIBUTE_MODIFIERS)
                        && !VItem.w().c(item)
                || VItem.w().e(item);
    }
