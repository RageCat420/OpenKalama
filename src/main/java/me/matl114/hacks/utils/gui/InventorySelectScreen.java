...
public class InventorySelectScreen extends GenericBackGroundScreen {
   private static final List<Text> bD = List.of(Text.literal("点击切换容器过滤规则"), Text.empty(), Text.literal("当前过滤规则: 拒绝虚拟容器(即不存在实体方块的容器)"));
   private static final List<Text> bC = List.of(Text.literal("点击切换容器过滤规则"), Text.empty(), Text.literal("当前过滤规则: 接受虚拟容器(即不存在实体方块的容器)"));
   ...
   protected DrawableWidget makeIcon(InvSubHelperV screen) {
      ...
      if (screen.i().isPresent()) {
         ...
         var2.add(Text.literal("记录位置: ").append(Text.literal("[%d, %d, %d]".formatted(var5.getX(), var5.getY(), var5.getZ())).formatted(Formatting.GREEN)));
         var2.add(Text.literal("记录世界: ").append(Text.literal(var4.world().getValue().toString())));
      } else {
      ...
      return ExecutableWidget.instance(0, 0, 16, 16)
         .eV(
            SlotElement.aI(var3 == null ? InvTasks.d : var3)
               .aK(false)
               .cF(
                  KalamaHelperHelperP.au(
                     l -> {
                        if (l) {
                           this.openInventoryViewScreen(screen);
                        } else if (screen instanceof TileInventory var3x
                           && !var3x.isVirtual()
                           && WorldUtils.areWorldEquals(MinecraftClient.getInstance().world, var3x.getWorld())) {
                           ContainerPosition var4x = var3x.getContainerPosition();
                           RenderTasks.i(
                              new KalamaHelperHelperCX(120, new KalamaHelperHelperDX(var4x.vG(), Color.GREEN), new KalamaHelperHelperSX(var4x.vF(), Color.RED))
                           );
                           this.close();
                        }
                     }
   ...
   protected void init() {
      ...
      this.grid.dE(var1);
      new ContentDelegateWidget<KalamaHelperHelperA<InvSubHelperV>>(this.x, this.y, 0, 0).setContentDelegate(this.grid).addTo(this);
   }
   ...
   protected void openInventoryViewScreen(InvSubHelperV screen) {
      if (screen.f() != null) {
         ...
         ScreenAccess.of(new InventoryViewScreen(screen.d(), screen.g().orElse(Text.empty()), screen.h().<ItemStack>map(ItemStack::new).orElse(InvTasks.d)))
            .openFromCurrent();
      }
   }
   ...
   protected boolean filterInventory(String value, InvSubHelperV screen) {
      return (!this.filterVirtual || screen.i().isPresent())
         && (
            FilterService.nameMatch(screen.g().orElse(Text.empty()).getString().replace("§.", ""), value)
