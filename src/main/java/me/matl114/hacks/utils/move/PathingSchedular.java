...
public class PathingSchedular {
    ...
    private boolean isEmptyShulkerBox(ItemStack stack) {
        return stack.getItem() instanceof BlockItem var3
                ...
                && ((ContainerComponent) stack.get(DataComponentTypes.CONTAINER))
                        .streamNonEmpty()
                        .findFirst()
                        .isEmpty();
    }
    ...
    private boolean isContainerStillValid(ContainerPosition pos) {
        ...
        return var3 instanceof BarrelBlockEntity
                || var3 instanceof ChestBlockEntity
                || var3 instanceof ShulkerBoxBlockEntity;
    }
                            ...
                            if (Math.max(Math.abs(var9), Math.abs(var10)) == var8) {
                                ...
                                for (int var13 = var1; var13 >= var2; var13--) {
                                    ...
                                    if (isValidLandingSpot(var3, var4)) {
                                        double var14 = var4.toCenterPos().squaredDistanceTo(targetPos.toCenterPos())
                                                + var8 * 0.01;
                                        if (var14 < var5) {
                                            var5 = var14;
    ...
    private boolean canUseToExpandStorage(ItemStack stack) {
        return SchedularSettings.INSTANCE.enableAutoExpandShulker.get() && this.isEmptyShulkerBox(stack)
                ? true
                : SchedularSettings.INSTANCE.enableAutoExpandChest.get()
                        && stack.getItem() instanceof BlockItem var3
                        && var3.getBlock() instanceof ChestBlock;
    }

    private static boolean isPassableForPath(BlockPos pos) {
        if (a.world != null && a.player != null) {
            BlockState var1 = a.world.getBlockState(pos);
            return var1.isLiquid()
                    ? false
                    : var1.getCollisionShape(a.world, pos, ShapeContext.of(a.player))
                            .isEmpty();
        } else {
            return false;
        }
    ...
    public int onStateReplenish(StateMachine machine) {
        if (this.A != null && !this.isContainerStillValid(this.A)) {
            if (!this.A.isDouble()) {
                ContainerPosition var2 =
                        ContainerPosition.resolve(a.world, this.A.vO().YO());
                if (var2.isDouble()) {
                    this.s.add(var2);
                }
            }
                        ...
                        for (int var7 = 0; var7 < var5.size(); var7++) {
                            if (!var5.getStack(var7).isEmpty()) {
                                a.interactionManager.clickSlot(
                                        var9.getScreenHandler().syncId, var7, 0, SlotActionType.QUICK_MOVE, a.player);
                                if (--var6 == 0) {
                                    break;
                                }
                            }
    ...
    public static IPathGoal pathToOrNearStopGoal(BlockPos pos, double distance) {
        if (pos == null) {
            ...
            if (var3 != null) {
                return var3.toCenterPos().squaredDistanceTo(a.player.getPos()) < MathUtils.a(distance)
                        ? null
                        : new GoalNear(var3, distance);
            } else {
                Box var4 = new Box(pos);
                return var4.squaredMagnitude(a.player.getEyePos()) < MathUtils.a(distance)
                        ? null
                        : new GoalNear(pos, distance);
            }
        }
    }
    ...
    public int onStateDischarge(StateMachine machine) {
        if (this.B != null && !this.isContainerStillValid(this.B)) {
            if (!this.B.isDouble()) {
                ContainerPosition var2 =
                        ContainerPosition.resolve(a.world, this.B.vO().YO());
                if (var2.isDouble()) {
                    this.t.add(var2);
                }
            }
        ...
        Predicate<me.matl114.utils.collections.KalamaHelperHelperK<ItemStack>> var3 = entry ->
                entry.index() >= SchedularSettings.INSTANCE.hotbarProtect.get() && var14.test((ItemStack) entry.val());
                ...
                if (this.B == null) {
                    this.B = this.t.stream()
                            ...
                            .min(Comparator.comparingDouble(
                                    s -> a.player.getPos().squaredDistanceTo(s.vF())))
                            .orElse(null);
                }

                if (this.B != null) {
                    BlockPos var4 = this.B.vO().YO();
                    if (a.currentScreen instanceof TileInventory var6
                            && Objects.equals(var6.getContainerPosition(), this.B)) {
                        ...
                        boolean var16 = InventoryUtils.findItem(
                                        var7, item -> item.isEmpty() || item.getCount() < item.getMaxCount(), true)
                                != null;
                        if (var16) {
                            if (!var11) {
                                this.B = null;
    ...
    private void analysisContainer(ContainerPosition pos, Inventory handledScreen) {
        ...
        if (SchedularSettings.INSTANCE.enableAutoExpandChest.get()
                && InventoryUtils.G(handledScreen, Items.CHEST) != null) {
            this.u.add(pos);
        }
    }
    ...
    public int onStateExpandStorage(StateMachine machine) {
        ...
        if (this.D != null) {
            if (a.world.getBlockEntity(this.D) != null) {
                if (a.currentScreen instanceof TileInventory var12
                        && var12.getContainerPosition() != null
                        && var12.getContainerPosition().contains(this.D)) {
                    this.D = null;
        ...
        if (var3 != null) {
            ...
            Pair var15 = (var10 ? KitReplenish.INSTANCE.bJ() : KitReplenish.INSTANCE.bK(false))
                    .findFirst()
                    .orElse(null);
            if (var15 != null) {
            ...
            if (this.u != null && !this.u.isEmpty()) {
                if (this.C == null) {
                    this.C = this.u.stream()
                            .min(Comparator.comparingDouble(s -> s.vF().squaredDistanceTo(a.player.getPos())))
                            .orElseThrow();
                }

                if (a.currentScreen instanceof TileInventory var2
                        && Objects.equals(var2.getContainerPosition(), this.C)) {
                    ...
                    for (int var8 = 0; var8 < var6; var8++) {
                        ...
                        if (this.canUseToExpandStorage(var9)) {
                            var7 = true;
                            a.interactionManager.clickSlot(
                                    var14.getScreenHandler().syncId, var8, 0, SlotActionType.QUICK_MOVE, a.player);
                            break;
                        }
                    }
