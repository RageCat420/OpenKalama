...
public class PrimitiveMap<T, W> implements NBTParsable<PrimitiveMap<T, W>> {
    ...
    public Primitive<T> createNewKeyPrimitive() {
        return this.defaultKeyPrimitive
                ...
                .orElseGet(() -> Primitive.of(this.keyType, this.keyType.createEmpty()));
    }

    public Primitive<W> createNewValuePrimitive() {
        return this.defaultValuePrimitive
                .map(this::copyValuePrimitive)
                .orElseGet(() -> Primitive.of(this.valueType, this.valueType.createEmpty()));
    }
    ...
    public <R> Optional<PrimitiveMap<T, W>> tryTypeConvert(Ref<R> ref) {
        return ref instanceof NBTRef<?> nbt
                ...
                ? Optional.of(this.withDefault(
                        (Map<T, W>) primitiveMap.map(), this.defaultKeyPrimitive, this.defaultValuePrimitive))
                : Optional.empty();
    }
    ...
    public int hashCode() {
        return Objects.hash(
                this.keyType, this.valueType, this.defaultKeyPrimitive, this.defaultValuePrimitive, this.map);
    }
