...
class HackUtilHelperE<T> extends RegistryChooseScreen<T> {
   @Override
   protected boolean canConfirm(ElementHandler elementHandler) {
      return this.cn.isValidate();
   }

   HackUtilHelperE(Registry var1, Consumer var2, AttrKeyValue var3, AttrKeyValue var4, AttrKeyValue var5) {
      super(var1, var2);
      this.cn = var3;
      this.co = var4;
      this.cp = var5;
   ...
   public DrawableWidget dp(KalamaHelperHelperCX subScreenWidget) {
      return ExecutableWidget.instance(subScreenWidget.getWidth(), -20, 20, 20)
         .eV(IconElement.cm(KalamaHelperHelperB.g, ButtonAction.a(() -> {})).aO(TooltipHandler.ap(((RegistryRegex)(Object)this.cp.getOriginValue()).getRules())));
   }
   AttrKeyValue cn;
   AttrKeyValue co;
   AttrKeyValue cp;
