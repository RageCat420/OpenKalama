...
public class WidgetPos implements NBTParsable<WidgetPos> {
   ...
   public static NBTType<WidgetPos> TYPE = new NBTType<>(
      ...
      (w, x, y, dx, dy) -> {
         ...
         widget.Q(ExecutableWidget.instance(0, 0, 2 * dy, dy).eV(new ButtonElement(el -> {
            return switch (((WidgetPos)w.getOriginValue()).getType()) {
               case 0 -> Text.translatableWithFallback("widget.nbt-parsable.widget-pos.percentage", "Per");
               case 1 -> Text.translatableWithFallback("widget.nbt-parsable.widget-pos.absolute-length", "Abs");
               default -> Text.empty();
            };
         }, ButtonAction.a(() -> {
            int total = 2;
            int type = ((WidgetPos)w.getOriginValue()).getType();
            w.valueChangeInternal(null, ((WidgetPos)w.getOriginValue()).withType((type + 1) % total));
         }))));
         TypeConvertAttrKeyValue<WidgetPos, Vec2> percentageSel = new TypeConvertAttrKeyValue<>(
            w, WrapperFactory.of(s -> ((WidgetPos)w.getOriginValue()).withPercentage(s), WidgetPos::toPercentage), NBTTypes.p
         );
         ...
         percentage1.Q(generateWidgetPosSelectScreenButton(dx - 3 * dy, 0, dy, dy, () -> ((WidgetPos)w.getOriginValue()).getFPoint(), el -> {
            ...
            Vec2 percentage2x = new Vec2(MathHelper.clamp(Math.round(mulWidth) / 100.0, 0.0, 1.0), MathHelper.clamp(Math.round(mulHeight) / 100.0, 0.0, 1.0));
         ...
         TypeConvertAttrKeyValue<WidgetPos, Vec2> absoluteSel = new TypeConvertAttrKeyValue<>(
            w, WrapperFactory.of(s -> ((WidgetPos)w.getOriginValue()).withLength(s), WidgetPos::toLength), NBTTypes.p
         );
         ...
         percentage2.Q(
            generateWidgetPosSelectScreenButton(
               dx - 3 * dy, 0, dy, dy, () -> ((WidgetPos)w.getOriginValue()).getFPoint(), el -> absoluteSel.valueChangeInternal(null, new Vec2(el.a, el.b))
            )
         );
   ...
   public static DrawableWidget generateWidgetPosSelectScreenButton(
      int x, int y, int dx, int dy, Supplier<KalamaHelperHelperM> current, Consumer<KalamaHelperHelperM> consumer
   ) {
      return ExecutableWidget.instance(x, y, dx, dy)
         .eV(
            IconElement.cm(
                  KalamaHelperHelperB.g, ButtonAction.a(() -> ScreenAccess.of(new WidgetPosSelectScreen(320, current.get(), consumer)).openFromCurrent())
               )
               .aO(TooltipHandler.ap(ChatUtils.parseTranslation("widget.nbt-parsable.widget-pos.open-select-screen.tooltips", "")))
         );
   }
   ...
   public WidgetPos withType(int type) {
      return this.type == type ? this : new WidgetPos(type, this.percentageX, this.percentageY, this.lengthX, this.lengthY);
   }
}
