...
public record WeakHolder<T>(Identifier registry, Identifier location) implements NBTParsable<WeakHolder<T>> {
    ...
    public static NBTType<WeakHolder> TYPE = new NBTType<>(
            ...
            (w, x, y, dx, dy) -> {
                ...
                if (optionalLookup.isPresent()) {
                    ...
                    return RegistryAttrKeyValue.generateTextInputWithRegistrySearch(
                            (Registry) lookupValue, wrapper, x, y, dx, dy);
                } else {
    ...
    public static <T> DataResult<WeakHolder<T>> parse(String s) {
        ...
        if (split.length == 2) {
            ...
            return identifier != null && location != null
                    ? DataResult.success(new WeakHolder(identifier, location))
                    : DataResult.error(() -> "Invalid format");
        } else {
    ...
    public Optional<RegistryEntry<T>> getEntry() {
        ...
        return ItemStackUtils.registry()
                .get(this.toRegistryKey().getRegistryRef())
                .getEntry(key)
                .map(s -> (RegistryEntry<T>) s);
    }
