...
public class WeakEntryPrimitiveMap<T, W> extends PrimitiveMap<WeakHolder<T>, W> {
    ...
    public Map<RegistryKey<T>, W> keyMap() {
        ...
        return this.map().entrySet().stream()
                .collect(Collectors.toMap(
                        entry -> RegistryKey.of(var1, entry.getKey().location()), Entry::getValue));
    }
    ...
    public static <T, W> NBTType<WeakEntryPrimitiveMap<T, W>> createEntry() {
        ...
        AttrKeyValue.CustomWidgetFactory<WeakEntryPrimitiveMap<T, W>> var2 =
                (attr, x, y, dx, dy) -> var1.customWidgetFactory()
                        .generateWidget(new TypeConvertAttrKeyValue<>(attr, var0, var1), x, y, dx, dy);
        return new NBTType<>(
                "weakentryprimitivemap", var0.wrapCodecXmap(var1.typeCodec()), var2, (WeakEntryPrimitiveMap<T, W>)
                        new WeakEntryPrimitiveMap<>(Map.of(), RegistryKeys.BLOCK.getValue(), NBTTypes.g));
    }
