...
public class StringFormat implements NBTParsable<StringFormat> {
    ...
    public static NBTType<StringFormat> TYPE = new NBTType<>(
            ...
            (s, x, y, dx, dy) -> {
                ...
                AttrKeyValue<String> wrapper = new TypeConvertAttrKeyValue<>(
                        s, WrapperFactory.of(original::withFormatString, StringFormat::formatString), NBTTypes.g);
                ...
                if (hasFormatArgument) {
                    subScreenWidget.Q(new ExecutableWidget(width, 0, dy, dy)
                            .eV(IconElement.cm(KalamaHelperHelperB.e, ButtonAction.c())
                                    .aO(TooltipHandler.ap(
                                            generateTooltipsForArgument(original.formattingArgument())))));
                }

                if (original.colorString()) {
                    subScreenWidget.Q(new ExecutableWidget(width + dy, 0, dy, dy)
                            .eV(IconElement.cm(KalamaHelperHelperB.g, ButtonAction.a(StringFormat::openWikiColorString))
                                    .aO(TooltipHandler.aq(
                                            el -> ((StringFormat) s.getOriginValue()).generateColorStringPreview()))));
                }
    ...
    public static List<Text> generateTooltipsForArgument(List<String> formattingArgument) {
        List<Text> tooltips = new ArrayList<>(
                ChatUtils.parseTranslation("widget.nbt-parsable.string-format.argument-info.tooltips", ""));
    ...
    public List<Text> generateColorStringPreview() {
        List<Text> tooltips = new ArrayList<>(
                ChatUtils.parseTranslation("widget.nbt-parsable.string-format.color-string-info.tooltips", ""));
