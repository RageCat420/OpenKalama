...
public class EnumPrimitiveList<T extends ConfigEnum, W> implements NBTParsable<EnumPrimitiveList<T, W>> {
    ...
    public EnumPrimitiveList(List<Pair<String, Primitive<W>>> lazyEntryList, String type, NBTType<W> primitiveType) {
        ...
        this.lazyEntryList = lazyEntryList.stream()
                .<EnumPrimitiveList<T, W>.LazyEntry>map(
                        p -> this.new LazyEntry((String) p.getFirst(), ((Primitive<W>) p.getSecond()).value()))
                .toList();
    }
    ...
    public EnumPrimitiveList(String type, NBTType<W> primitiveType, List<Pair<T, W>> lazyEntryList) {
        ...
        this.primitiveType = primitiveType;
        this.lazyEntryList = lazyEntryList.stream()
                .<EnumPrimitiveList<T, W>.LazyEntry>map(s -> this.new LazyEntry((T) s.getFirst(), s.getSecond()))
                .toList();
    }
    ...
    public static <T extends ConfigEnum, W> EnumPrimitiveList<T, W> of(
        ...
        return new EnumPrimitiveList<>(
                ...
                lazyEntryList.stream()
                        .map(s -> Pair.of((T) ((ConfigEnum) s.getFirst()), ((Primitive<W>) s.getSecond()).value()))
                        .toList());
    }
    ...
    private List<Pair<String, Primitive<W>>> toPrimitive() {
        return this.lazyEntryList.stream()
        ...
        Codec<EnumPrimitiveList<T, W>> codec = RecordCodecBuilder.create(oInstance -> oInstance
                .group(
                        CodecUtils.pairListCodec(
                                        Codec.STRING, Primitive.TYPE.cast().typeCodec())
                                .fieldOf("data")
                                .forGetter(el -> (List) el.toPrimitive()),
                ...
                .apply(oInstance, (data, type, primitiveType) -> {
                    EnumPrimitiveList<T, W> list = new EnumPrimitiveList<T, W>(
                            java.util.Collections.<Pair<String, Primitive<W>>>emptyList(), type, primitiveType);
                    List<Pair<String, Primitive<W>>> dataPairs = (List<Pair<String, Primitive<W>>>) (List<?>) data;
                    list.lazyEntryList = dataPairs.stream()
                            .map(s -> list
                            .new LazyEntry(s.getFirst(), s.getSecond().value()))
