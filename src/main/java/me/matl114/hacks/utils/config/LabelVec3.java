    ...
    public static final NBTType<LabelVec3> TYPE = new NBTType<>(
            ...
            (s, x, y, dx, dy) -> {
                ...
                WrapperFactory<Double, LabelVec3> firstWrapper =
                        WrapperFactory.of(d -> ((LabelVec3) s.getOriginValue()).withX(d), LabelVec3::x);
                WrapperFactory<Double, LabelVec3> secondWrapper =
                        WrapperFactory.of(d -> ((LabelVec3) s.getOriginValue()).withY(d), LabelVec3::y);
                WrapperFactory<Double, LabelVec3> thirdWrapper =
                        WrapperFactory.of(d -> ((LabelVec3) s.getOriginValue()).withZ(d), LabelVec3::z);
                return subScreenWidget
                        .Q(DisplayWidget.instance(0, 0, label, dy)
                                .setRenderHandler(new ButtonElement(
                                        ...
                                        .aO(TooltipHandler.ap(
                                                ChatUtils.parseTranslation(original.xLabel() + ".tooltips", "")))))
                        .Q(new TypeConvertAttrKeyValue<>(s, firstWrapper, NBTTypes.e)
                                .generateValueWidget(label, 0, half - label, dy))
                        .Q(DisplayWidget.instance(half, 0, label, dy)
                                .setRenderHandler(new ButtonElement(
                                                TextProvider.c(Text.translatableWithFallback(
                                                        original.yLabel(), original.yLabel())),
                                                ButtonAction.c())
                                        .aO(TooltipHandler.ap(
                                                ChatUtils.parseTranslation(original.yLabel() + ".tooltips", "")))))
                        .Q(new TypeConvertAttrKeyValue<>(s, secondWrapper, NBTTypes.e)
                                .generateValueWidget(half + label, 0, half - label, dy))
                        .Q(DisplayWidget.instance(2 * half, 0, label, dy)
                                .setRenderHandler(new ButtonElement(
                                                TextProvider.c(Text.translatableWithFallback(
                                                        original.zLabel(), original.zLabel())),
                                                ButtonAction.c())
                                        .aO(TooltipHandler.ap(
                                                ChatUtils.parseTranslation(original.zLabel() + ".tooltips", "")))))
                        .Q(new TypeConvertAttrKeyValue<>(s, thirdWrapper, NBTTypes.e)
