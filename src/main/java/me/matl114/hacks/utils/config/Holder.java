...
public record Holder<T>(Registry<T> registry, @Nullable T entry, String asString) implements NBTParsable<Holder<T>> {
   ...
   public static final NBTType<Holder<Object>> TYPE = create();

   public static <W> Class<Holder<W>> uA() {
      return (Class<Holder<W>>)(Class<?>)Holder.class;
   }
   ...
   public static <T> Holder<T> of(Registry<T> registry, T entry) {
      ...
      if (entry == null) {
         ...
         if (entryIdentifier != null) {
            return new Holder<>(registry, entry, string + "|" + entryIdentifier.toString());
         } else {
            ...
            if (registry != null) {
               ...
               if (entryIdentifier != null) {
                  if (Objects.equals(entryIdentifier, DEFAULT_EMPTY)) {
                     ...
                     T val = (T)registry.get(entryIdentifier);
                     return val != null
                        ? DataResult.success(new Holder(registry, val, s))
                        : DataResult.error(() -> "Can not find registry entry for " + entryIdentifier);
                  }
               } else {
   ...
   private static <T> NBTType<Holder<T>> create() {
      return new NBTType<>(
         ...
         (w, x, y, dx, dy) -> {
            Holder<T> holder = (Holder<T>)w.getOriginValue();
            Registry<T> registry = holder.registry;
            WrapperFactory<Optional<T>, Holder<T>> wrapperFactory = WrapperFactory.of(s -> of(registry, s.orElse(null)), t -> Optional.ofNullable(t.entry()));
            ...
            WrapperFactory<String, Optional<T>> stringifyFactoryWithDefault = WrapperFactory.of(
               s -> Objects.equals(s, DEFAULT_EMPTY_STRING) ? Optional.empty() : Optional.of(stringifyFactory.create(s)),
               t -> t.map(stringifyFactory::get).orElse(DEFAULT_EMPTY_STRING)
            );
            AttrKeyValue.CustomWidgetFactory<Optional<T>> widgetFactory = (s, x1, y1, dx1, dy1) -> RegistryAttrKeyValue.generateTextInputWithRegistrySearch(
               registry, s, x1, y1, dx1, dy1
            );
   ...
   public boolean isSameType(NBTParsable<?> type) {
      return NBTParsable.super.isSameType(type) && type instanceof Holder<?> holder && holder.registry == holder.registry;
   }
