    ...
    private static <T> NBTType<Holder<T>> create() {
        return new NBTType<Holder<T>>(
                ...
                (w, x, y, dx, dy) -> {
                    ...
                    WrapperFactory<Optional<T>, Holder<T>> wrapperFactory =
                            WrapperFactory.of(s -> of(registry, s.orElse(null)), t -> Optional.ofNullable(t.entry()));
                    ...
                    WrapperFactory<String, Optional<T>> stringifyFactoryWithDefault = WrapperFactory.of(
                            s -> Objects.equals(s, DEFAULT_EMPTY_STRING)
                                    ? Optional.empty()
                                    : Optional.of(stringifyFactory.create(s)),
                    ...
                    AttrKeyValue.CustomWidgetFactory<Optional<T>> widgetFactory = (s, x1, y1, dx1, dy1) ->
                            RegistryAttrKeyValue.generateTextInputWithRegistrySearch(registry, s, x1, y1, dx1, dy1);
