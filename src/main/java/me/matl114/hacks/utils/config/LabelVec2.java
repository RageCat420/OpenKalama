...
public record LabelVec2(String xLabel, String yLabel, Vec2 data) implements NBTParsable<LabelVec2> {
                        ...
                        .Q(DisplayWidget.instance(0, 0, 2 * dy, dy)
                                .setRenderHandler(new ButtonElement(
                                        ...
                                        .aO(TooltipHandler.ap(
                                                ChatUtils.parseTranslation(originalLabel.xLabel() + ".tooltips", "")))))
                        .Q(new TypeConvertAttrKeyValue<>(s, pairFactory.asFirstWrapper(s::getOriginValue), NBTTypes.e)
                                .generateValueWidget(2 * dy, 0, half - 2 * dy, dy))
                        .Q(DisplayWidget.instance(half, 0, 2 * dy, dy)
                                .setRenderHandler(new ButtonElement(
                                                TextProvider.c(Text.translatableWithFallback(
                                                        originalLabel.yLabel(), originalLabel.yLabel())),
                                                ButtonAction.c())
                                        .aO(TooltipHandler.ap(
                                                ChatUtils.parseTranslation(originalLabel.yLabel() + ".tooltips", "")))))
                        .Q(new TypeConvertAttrKeyValue<>(s, pairFactory.asSecondWrapper(s::getOriginValue), NBTTypes.e)
    ...
    public boolean isSameType(NBTParsable<?> type) {
        return type instanceof LabelVec2 label2
                && Objects.equals(this.xLabel, label2.xLabel())
                && Objects.equals(this.yLabel, label2.yLabel());
    }
