    ...
    public static final NBTType<Vec3> TYPE = new NBTType<>(
            ...
            (s, x, y, dx, dy) -> {
                ...
                WrapperFactory<Double, Vec3> firstWrapper =
                        WrapperFactory.of(d -> ((Vec3) s.getOriginValue()).withX(d), Vec3::x);
                WrapperFactory<Double, Vec3> secondWrapper =
                        WrapperFactory.of(d -> ((Vec3) s.getOriginValue()).withY(d), Vec3::y);
                WrapperFactory<Double, Vec3> thirdWrapper =
                        WrapperFactory.of(d -> ((Vec3) s.getOriginValue()).withZ(d), Vec3::z);
