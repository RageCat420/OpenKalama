...
public record InvSubHelperB(
        String name, List<KalamaHelperHelperK<NbtCompound>> itemNBT, int maxSize, InvSubHelperL rule) {
    ...
    public static Codec<InvSubHelperB> dt = RecordCodecBuilder.create(oinstance -> oinstance
            .group(
    ...
    public static InvSubHelperB fromItem(
            String name, List<KalamaHelperHelperK<ItemStack>> itemNBT, int maxSize, InvSubHelperL rule) {
        return new InvSubHelperB(
                name,
                itemNBT.stream()
                        .filter(s -> !((ItemStack) s.val()).isEmpty())
                        .map(s -> new KalamaHelperHelperK<>(
                                s.index(), VItem.w().k((ItemStack) s.val(), ItemStackUtils.registry())))
                        .toList(),
                maxSize,
                rule);
    }
    ...
    public List<KalamaHelperHelperK<ItemStack>> hy() {
        return this.itemNBT.stream()
                .map(s -> new KalamaHelperHelperK<>(s.index(), VItem.w().j(s.val(), ItemStackUtils.registry())))
                .toList();
    }
}
