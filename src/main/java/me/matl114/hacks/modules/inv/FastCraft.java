...
public class FastCraft extends BaseModule {
    ...
    public final FlagRef ae =
            this.flagBuilder(this.pO.add("enable-fastcraft-buttons")).build();
    ...
    private void addInventoryButton(InventoryScreen screen) {
        ...
        ExecutableWidget var5 = ExecutableWidget.instance(150, screen.height / 2 - 25, 24, 12)
                .<ExecutableWidget>eV(new ButtonElement(
                                TextProvider.c(Text.translatable("widget.fast-craft.lock")), ButtonAction.a(this::yj))
        ...
        ExecutableWidget var7 = ExecutableWidget.instance(150, screen.height / 2 - 72, 24, 12)
                .<ExecutableWidget>eV(new ButtonElement(
                                TextProvider.c(Text.translatable("widget.fast-craft.toggle-drop")),
                                ButtonAction.a(var6))
                        .aO(TooltipHandler.ap(
                                ChatUtils.parseTranslation("widget.fast-craft.toggle-drop.tooltips", ""))))
                .addToSub(var3);
        DrawableWidget var8 = DisplayWidget.instance(132, var2.getScreenY() + 55, 18, 18)
                .<DrawableWidget>setRenderHandler(
                        new SlotElement(this::getDisplayItemStack).aK(false).aL(false))
                .addToSub(var3);
        DrawableWidget var9 = DisplayWidget.instance(142, var2.getScreenY() + 55 + 10, 7, 7)
                .<DrawableWidget>setRenderHandler(
                        new SlotElement(this::getLockItem).aL(false).aK(false).n(v -> this.isLock()))
                .addToSub(var3);
    ...
    public void placeLastCraftingRecipe(
        ...
        if (var3 != null) {
            mc.interactionManager.clickRecipe(
                    ((AbstractRecipeScreenHandler) craftingScreen.getScreenHandler()).syncId, var3, true);
            if (doCraft) {
                int var4 = 64;

                for (Ingredient var6 : RecipeTasks.getIngredients(var3)) {
                    for (ItemStack var8 :
                            RecipeTasks.streamIngredientOptions(var6).toList()) {
                        var4 = Math.min(var4, var8.getMaxCount());
                    }
                }

                int var9 =
                        ((AbstractRecipeScreenHandler) craftingScreen.getScreenHandler()).getCraftingResultSlotIndex();
