...
public class SaveItem extends BaseModule {
    ...
    public SaveItem() {
        super("SaveItem");
        this.saveSlotItem = this.hotkey(this.mH.add("save-slot-item"))
                .defaultValue(new MultiKeyBind())
                .registerHotkey(HotKeyUtils.e(this::saveItem))
                .build();
        this.nV = false;
                ...
                .xmap(
                        lst -> lst.stream()
                                .collect(Collectors.toMap(
                                        Function.identity(),
                                        InvTasks.as()::getDataFromCodecId,
                                        (k, v) -> v,
                                        LinkedHashMap::new)),
    ...
    public void onSave() {
        if (this.kK) {
            ...
            try {
                JsonElement var1 = (JsonElement)
                        (Object) this.nY.encodeStart(JsonOps.INSTANCE, this.nW).getOrThrow();
    ...
    public void onLoad() {
        try {
            ...
            this.nW.clear();
            Map var3 = (Map)
                    ((Pair) (Object) this.nY.decode(JsonOps.INSTANCE, var2).getOrThrow()).getFirst();
            this.nW.putAll(var3);
            this.kK = false;
