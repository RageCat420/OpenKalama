...
public class NbtTooltips extends BaseModule {
   ...
   public NbtTooltips() {
      ...
      this.showHotkey = this.hotkey(this.Ta.add("show-hotkey")).defaultValue(new MultiKeyBind(342)).build();
      this.width = this.intBuilder(this.Ta.add("width")).defaultValue(360).validator(Configs.d).build();
      this.formatIndent = this.intBuilder(this.Ta.add("format-indent")).defaultValue(0).validator(Configs.d).build();
