...
public class ChestHistory extends BaseModule {
   ...
   public ChestHistory() {
      ...
      this.ignoreContainerWithTitle = this.builder(this.BA.add("ignore-container-with-title"), Regex.class)
         ...
         .build();
      this.showTitle = this.flagBuilder(this.BA.add("show-title")).defaultValue(false).build();
      this.enablePersistentStorage = this.flagBuilder(this.BA.add("enable-persistent-storage")).build();
      this.BO = 0;
      INSTANCE = this;
   }
            ...
            for (Entry var7 : this.BC.entrySet()) {
               if (((ContainerPosition)var7.getKey()).isInRenderRange(var3, 64.0)) {
                  ...
                  if (!var5.contains(var8)) {
                     ...
                     int var10 = (int)InventoryUtils.streamInventory(((InvSubHelperV)((MutableEntry)var7.getValue()).b).d()).filter(s -> !s.isEmpty()).count();
                     MutableText var11 = ((InvSubHelperV)((MutableEntry)var7.getValue()).b())
                        .g()
                        .orElse(Text.empty())
                        .copy()
                        .append(Text.literal("(x%d)".formatted(var10)).formatted(Formatting.YELLOW));
                     VRender.getInstance().drawTextCameraCoord(var11.asOrderedText(), var2, Vec3d.ZERO, E, Color.WHITE, VRender.a);
                     var2.pop();
                  }
               }
            }
   ...
   public void Qb(ContainerPosition containerPosition) {
      this.BC.remove(containerPosition);
      BlockStorage var2 = ServerStorage.IZ().n(containerPosition.world(), containerPosition.vO().YO(), false);
      if (var2 != null) {
         var2.f(BM, null);
   ...
   public void onTick(Event<ClientPlayerEntity> event) {
      if (++this.BO >= 40) {
         if (this.BH != null && this.BH.f != null) {
            if (ClientPlayerAccess.of(mc.player).getServerScreenHandler().syncId == this.BH.f.getScreenHandler().syncId) {
               this.BH.b = true;
            } else {
               this.BH.b(this.BH.f);
               this.BH = null;
               ...
               if (WorldUtils.n(var5.x, var5.z)) {
                  ...
                  if (var6 != null && var6 != Blocks.AIR) {
                     if (!((ContainerPosition)var4.getKey()).isDouble()) {
                        Block var7 = mc.world.getBlockState(((ContainerPosition)var4.getKey()).vO().YO()).getBlock();
                        if (var7 != var6) {
                           var3.remove();
   ...
   public void onPlaceShulkerBox(Event<PlayerInteractBlockC2SPacket> eventInteract) {
      PlayerInteractBlockC2SPacketAccess var2 = PlayerInteractBlockC2SPacketAccess.of((PlayerInteractBlockC2SPacket)eventInteract.b);
      ...
      if (var3 != null
         ...
         BlockPos var10 = var3.getPlaceBlockPos(
            ((PlayerInteractBlockC2SPacket)eventInteract.b).getHand(), ((PlayerInteractBlockC2SPacket)eventInteract.b).getBlockHitResult()
         );
         BlockState var6 = mc.world.getBlockState(var10);
         if (var6.getBlock() == var5.getBlock() && mc.world.getBlockEntity(var10) instanceof ShulkerBoxBlockEntity var8) {
            ...
            InvSubHelperV var9 = new InvSubHelperV(
               InventoryUtils.getInventoryEntries(var8), var8.size(), Optional.ofNullable(var8.getName()), Optional.of(var5.getBlock()), Optional.of(var12)
            );
   ...
   public List<InvSubHelperV> PQ() {
      return Stream.concat(this.BC.values().stream().map(MutableEntry::b), this.BD.stream()).toList();
   }
