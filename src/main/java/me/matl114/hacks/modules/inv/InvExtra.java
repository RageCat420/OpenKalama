...
public class InvExtra extends BaseModule {
   ...
   public final IntRef mI = this.intBuilder(this.mH.add("packet-limit")).defaultValue(40).validator(Configs.e).build();
   ...
   public InvExtra() {
      super("InvExtra");
      this.moveClickGrimFix = this.flagBuilder(this.mH.add("move-click-grim-fix")).build();
      this.sprintClickGrimFix = this.flagBuilder(this.mH.add("sprint-click-grim-fix")).build();
      this.expandBackpackInventory = this.flagBuilder(this.mH.add("expand-backpack-inventory")).build();
      this.ghostHandAttributeSync = this.flagBuilder(this.mH.add("ghost-hand-attribute-sync")).build();
      this.pickItem = this.hotkey(this.mH.add("pick-item")).defaultValue(new MultiKeyBind(341, -98)).registerHotkey(HotKeyUtils.e(this::onPickItem)).build();
   ...
   public void onInvClick(int syncId) {
      if (mc.currentScreen instanceof HandledScreen var3 && var3.getScreenHandler().syncId == syncId && this.sprintClickGrimFix.get()) {
   ...
   public void onCloseScreen(Event<CloseHandledScreenC2SPacket> closeS2C) {
      if (this.expandBackpackInventory.get() && ((CloseHandledScreenC2SPacket)closeS2C.b).getSyncId() == mc.player.playerScreenHandler.syncId) {
         closeS2C.cancel();
      }
   }
         ...
         if (!var1.isCreative() && InvTasks.getCurrentServerScreen(var1) instanceof HandledScreen var3) {
            ...
            if (var5 != null) {
               if (var5.inventory instanceof PlayerInventory) {
                  if (var5.getIndex() >= 36) {
                  ...
                  } else if (ViaFabricPlusHooks.getInstance().isViaEnabled() && ViaFabricPlusHooks.getInstance().getCurrentVersion().c(21, 3)) {
                     ViaFabricPlusHooks.ViaPacketWrapper var6 = ViaFabricPlusHooks.getInstance().createViaPacket();
                     ...
                     Debug.b("No Longer support this feat in version " + ViaFabricPlusHooks.getInstance().getCurrentVersion());
                  }
