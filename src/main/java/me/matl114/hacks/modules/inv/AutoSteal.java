...
public class AutoSteal extends BaseModule {
            ...
            if (var5 != null && !(var5 instanceof CreativeScreenHandler)) {
               for (Slot var7 : var5.slots) {
                  ...
                  if (!var11.isEmpty() && this.zl.get().test(var11.getItem())) {
                     mc.interactionManager.clickSlot(var5.syncId, var7.getIndex(), 0, SlotActionType.QUICK_MOVE, mc.player);
                  }
               }
            }
         }
   ...
   public AutoSteal() {
      ...
      this.toggleKey = this.toggleHotkey(this.zi.add("toggle-key"), new MultiKeyBind(), this.zi.add("enable")).build();
      this.titleRegex = this.builder(this.zi.add("title-regex"), Regex.class).defaultValue(new Regex(".*")).build();
      this.zl = this.builder(this.zi.add("item-filter"), EntrySet.<Item>parameter()).defaultValue(new EntrySet<Item>(new Regex(".*"), Registries.ITEM)).build();
