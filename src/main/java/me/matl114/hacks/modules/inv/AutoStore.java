...
public class AutoStore extends BaseModule {
            ...
            while (var13.hasNext()) {
               ...
               if (var10 != null && !var10.isEmpty() && var10.getCount() >= 4) {
                  if (!var5.getCursorStack().isEmpty()) {
                     ...
                     if (var16 >= 0) {
                        InvTasks.at().execute(() -> mc.interactionManager.clickSlot(var4.getScreenHandler().syncId, var16, 0, SlotActionType.PICKUP, var2));
                     } else {
                        InvTasks.at().execute(() -> mc.interactionManager.clickSlot(var4.getScreenHandler().syncId, var16, 0, SlotActionType.THROW, var2));
                     }
                  ...
                  if (var12 >= 0) {
                     InvTasks.at().execute(() -> {
                        mc.interactionManager.clickSlot(var4.getScreenHandler().syncId, var14, 1, SlotActionType.PICKUP, var2);
                        mc.interactionManager.clickSlot(var4.getScreenHandler().syncId, var12, 0, SlotActionType.PICKUP, var2);
                     });
                     return;
                  }
               }
   ...
   private static int anyMatch(DefaultedList<Slot> slots, ItemStack stack, int amount, int... index) {
      for (int var7 : index) {
         ...
         if (var8 != null && (var8.isEmpty() || var8.getCount() + amount <= var8.getMaxCount() && ItemStack.areItemsAndComponentsEqual(stack, var8))) {
            return var7;
         }
      }
