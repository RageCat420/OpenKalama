...
public class AutoShulker extends BaseModule {
    ...
    public void registerAll() {
        ...
        this.registerListener(
                Listener.ao().getChannel(PlayerInteractBlockC2SPacket.class), this::onClickShulkerBoxOrPlaceShulkerBox);
    }
    ...
    public void onClickShulkerBoxOrPlaceShulkerBox(Event<PlayerInteractBlockC2SPacket> event) {
        if (!event.d()) {
            if (this.yk.get()) {
                ...
                if (var5.getBlock() instanceof ShulkerBoxBlock
                        && !var4
                        && mc.world.getBlockEntity(var3.getBlockPos()) instanceof ShulkerBoxBlockEntity var7) {
                            ...
                            InvTasks.executePredictInventoryAction(var7, handler -> {
                                for (int var2x = 0; var2x < var11; var2x++) {
                                    mc.interactionManager.clickSlot(
                                            handler.syncId, var2x, 0, SlotActionType.QUICK_MOVE, mc.player);
                                }
                            });
                            int var12 = Tasks.b();
    ...
    public AutoShulker() {
        super("AutoShulker");
        this.toggleKey = this.toggleHotkey(this.yj.add("toggle-key"), new MultiKeyBind(), this.yj.add("enable"))
                .build();
