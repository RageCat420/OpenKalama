...
public class AutoMine extends BaseModule {
   ...
   private boolean Un() {
      return this.ae.get() && this.mode.get() == AutoMine$Mode.BATCH && this.enableBaritone.get() && this.EE != SurvivalSubHelperM.yc;
   }
   ...
   private BlockPos findMinePosAround(BlockPos standPos, boolean requireReachable) {
      for (int var3 = this.mineHeight.get() - 1; var3 >= 0; var3--) {
         for (Vector2i var5 : InteractExtra.INSTANCE.getBlocksAround()) {
            ...
            if (this.isInsidePlan(var6) && (requireReachable ? this.checkDistanceAndCondition(var6) : this.isMineable(mc.world.getBlockState(var6)))) {
               return var6;
            }
         }
      }
   ...
   private boolean checkDistanceAndCondition(BlockPos newPos) {
      if (newPos == null) {
         ...
         if (Objects.equals(var2.getCurrentFailBreakPos(), newPos)) {
            ...
            return !this.isMineable(mc.world.getBlockState(newPos)) ? false : !MineTasks.distanceOutOfReach(newPos, mc.player.getEyePos());
         }
      }
   }
   ...
   private ItemEntity findCollectTarget(boolean dynamic) {
      Box var2 = mc.player.getBoundingBox().expand(this.collectSearchRadius.get(), 2.0, this.collectSearchRadius.get());
      return mc.world
         .getEntitiesByType(EntityType.ITEM, var2, item -> !this.isCollectibleDrop(item) ? false : dynamic || this.EJ.contains(item.getUuid()))
         .stream()
   ...
   private boolean shouldDischargeStack(ItemStack stack) {
      return !stack.isEmpty() && stack.getItem() instanceof BlockItem var3 ? this.EL.get().test(var3.getBlock()) : false;
   }
   ...
   private boolean shouldEnterCollectMode() {
      if (!this.hasCollectTarget(true)) {
         ...
         return this.EH >= this.EG.size()
            ? true
            : mc.player.getPos().squaredDistanceTo(this.EG.get(this.EH).toCenterPos()) > MathUtils.a(this.collectEnterDistance.get());
      }
   }
   ...
   private int onMineCommon(Supplier<BlockPos> posFinder) {
      ...
      do {
         ...
         KalamaHelperHelperK var5 = this.autoSwap.get()
            ? InventoryUtils.v(
               stack -> this.isDurabilityOk(stack) ? (double)WorldUtils.getPlayerBlockBreakingSpeedWithCanMineMultiply(mc.player, var4, stack) : null,
               true,
               true
            )
         ...
         switch ((Configs$MineTargetingMode)this.legalMode.get()) {
            case SWING_HAND_AND_ROT:
               ...
               if (var13.dotProduct(var7) < 0.0) {
                  ...
                  mc.getNetworkHandler().sendPacket(VPacket.h(mc.player.getYaw(), mc.player.getPitch(), mc.player.isOnGround(), mc.player.horizontalCollision));
               }
               break;
         ...
         if (!MineExtra.INSTANCE.adl(var6)) {
            ...
            if (!this.useDoubleBreak.get() || !Objects.equals(var12.getCurrentMiningPos(), this.W) || !var12.isFailBreakEmpty()) {
               break;
            }
   ...
   public AutoMine() {
      ...
      this.mode = this.builder(this.aD.add("mode"), AutoMine$Mode.class).defaultValue(AutoMine$Mode.BATCH).build();
      this.enableBaritone = this.flagBuilder(this.aD.add("enable-baritone")).defaultValue(true).build();
      this.EL = this.builder(this.aD.add("whitelist"), EntrySet.<Block>parameter())
         .defaultValue(new EntrySet<Block>(new Regex("^(sand|red_sand|gravel|clay|dirt|grass_block)$"), Registries.BLOCK))
         .build();
      this.startDownOffset = this.intBuilder(this.aD.add("start-down-offset")).defaultValue(3).validator(Configs.d).build();
      this.heightSearchLimit = this.intBuilder(this.aD.add("height-search-limit")).defaultValue(24).validator(Configs.e).build();
      this.horizontalRange = this.intBuilder(this.aD.add("horizontal-range")).defaultValue(8).validator(Configs.d).build();
      this.mineHeight = this.intBuilder(this.aD.add("mine-height")).defaultValue(6).validator(Configs.e).build();
      this.routeSpacing = this.intBuilder(this.aD.add("route-spacing")).defaultValue(3).validator(Configs.e).build();
      this.collectEnterDistance = this.doubleBuilder(this.aD.add("collect-enter-distance")).defaultValue(8.0).build();
      this.collectSearchRadius = this.doubleBuilder(this.aD.add("collect-search-radius")).defaultValue(8.0).build();
      this.mineDuringCollect = this.flagBuilder(this.aD.add("mine-during-collect")).defaultValue(true).build();
      this.refreshCollectDrops = this.flagBuilder(this.aD.add("refresh-collect-drops")).defaultValue(true).build();
      this.requiredEmptySlots = this.intBuilder(this.aD.add("required-empty-slots")).defaultValue(1).validator(Configs.d).build();
      this.legalMode = this.builder(this.aD.add("legal-mode"), Configs$MineTargetingMode.class).defaultValue(Configs$MineTargetingMode.NO_BYPASS).build();
      this.considerCooldown = this.builder(this.aD.add("consider-cooldown"), Boolean.class).defaultValue(true).build();
      this.autoSwap = this.flagBuilder(this.aD.add("auto-swap")).defaultValue(true).build();
      this.durabilityProtect = this.flagBuilder(this.aD.add("durability-protect")).defaultValue(true).build();
   ...
   private void enterCollectMode() {
      ...
      if (!this.refreshCollectDrops.get()) {
         Box var1 = mc.player.getBoundingBox().expand(this.collectSearchRadius.get(), 2.0, this.collectSearchRadius.get());
   ...
   private boolean isCollectibleDrop(ItemEntity itemEntity) {
      return itemEntity != null && !itemEntity.isRemoved() && !itemEntity.getStack().isEmpty();
   }
