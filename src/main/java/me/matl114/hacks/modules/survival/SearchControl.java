...
public class SearchControl extends BaseModule {
   ...
   public void onPreTick(Event<ClientPlayerEntity> event) {
      if (this.ae.get()) {
         ...
         if (this.CD != null) {
            ...
            if (mc.player.getPos().squaredDistanceTo(this.centerPos.get().to().toCenterPos()) < MathUtils.a(this.maxDistance.get())) {
               this.CD.run();
   ...
   public SearchControl() {
      ...
      this.J = this.toggleHotkey(this.Cu.addHotkey(), new MultiKeyBind(), this.Cu.addEnable()).build();
      this.searchMode = this.builder(this.Cu.add("search-mode"), SearchControl$Mode.class)
         ...
         .build();
      this.centerPos = this.builder(this.Cu.add("center-pos"), Pos3.class).defaultValue(new Pos3(0, 0, 0)).build();
      ...
      this.maxDistance = this.doubleBuilder(this.Cu.add("max-distance")).defaultValue(3.0E8).build();
