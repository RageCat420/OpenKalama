...
public class BlockFarm extends BaseModule {
        ...
        if (this.ae.get()) {
            ...
            if (hitResultEvent.b instanceof BlockHitResult var3) {
                ...
                if (var7 != null && !(var4 = mc.world.getBlockState(var7)).isAir() && !var4.isLiquid()) {
                    ...
                    if (var5 instanceof BlockItem var6
                            && var6 != Items.AIR
                            && (!this.whiteListEnable.get()
                                    || this.whiteList.get().test(var5))) {
                        this.lf = var6;
                    }
                }
            }
    ...
    public BlockFarm() {
        ...
        this.whiteListEnable =
                this.flagBuilder(this.aD.add("white-list-enable")).build();
            ...
            if (this.ae.get() && this.lf != null) {
                ...
                if (var4.getBlock() == this.lf.getBlock()) {
                    if (this.timer++ > this.delay.get()
                            && new Box(var3).squaredMagnitude(mc.player.getEyePos())
                                    < MathUtils.a(AttributeUtils.getPlayerBlockInteractionRange(mc.player))) {
                        this.timer = 0;
                            ...
                            new HackUtilHelperB(
                                    Text.literal("Farm: %s"
                                            .formatted(Registries.ITEM
                                                    .getId(this.lf)
                                                    .getPath())),
                                    var3.toCenterPos().add(0.0, 0.6, 0.0),
                                    0.66F),
                            -1);
                }
            }
