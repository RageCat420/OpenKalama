...
public class AntiAXray extends BaseModule {
    ...
    public AntiAXray() {
        ...
        this.enable =
                this.flagBuilder(this.VY.add("enable")).defaultValue(false).build();
        this.hotkey = this.toggleHotkey(this.VY.add("hotkey"), new MultiKeyBind(), this.VY.add("enable"))
                .build();
        this.packetLimit = this.builder(this.VY.add("packet-limit"), IntRef.TYPE)
                .defaultValue(30)
                .validator(Configs.e)
                .build();
            ...
            if (!MineTasks.distanceOutOfReach(var5, mc.player.getEyePos()) && !simpleDetection.contains(var5)) {
                ...
                if (this.legal.get()) {
                    mc.interactionManager.sendSequencedPacket(
                            mc.world,
                            sequence -> new PlayerActionC2SPacket(Action.START_DESTROY_BLOCK, var5, var7, sequence));
                    PlayerInteractionAccess.of(mc.interactionManager).sendAbortBreakPacket();
                } else {
                    mc.interactionManager.sendSequencedPacket(
                            mc.world,
                            sequence -> new PlayerActionC2SPacket(Action.STOP_DESTROY_BLOCK, var5, var7, sequence));
                }
