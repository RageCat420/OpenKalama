...
public class WorldManager extends BaseModule {
    ...
    public final FlagRef Op = this.builder(this.aD.add("enable-entity"), Boolean.class)
            .defaultValue(true)
            .build();
    ...
    public void Qe(Event<TaskSubHelperJ> event) {
        ...
        for (EntityStorage var5 : var2.m()) {
            if (var5.contains("kalama:world_manager/entity_data_storage")) {
                SurvivalSubHelperH var6 =
                        var5.c("kalama:world_manager/entity_data_storage", SurvivalSubHelperH.CODEC, var3);
                if (var6 != null) {
        ...
        for (BlockStorage var8 : var2.g()) {
            if (var8.contains("kalama:world_manager/block_data_storage")) {
                SurvivalSubHelperI var9 =
                        var8.c("kalama:world_manager/block_data_storage", SurvivalSubHelperI.CODEC, var3);
                if (var9 != null) {
    ...
    public WorldManager() {
        super("WorldManager");
        this.enableBlockEntities = this.builder(this.aD.add("enable-block-entities"), Boolean.class)
                .defaultValue(true)
                .build();
    ...
    public SurvivalSubHelperH aff(LivingEntity entity, boolean create) {
        if (entity.getHealth() > 0.0F) {
            return create
                    ? this.Or.computeIfAbsent(entity.getUuid(), SurvivalSubHelperH::new)
                    : this.Or.get(entity.getUuid());
        } else {
            return null;
        }
    }
    ...
    public boolean aeY(VillagerEntity villager) {
        ...
        return var2 != null
                && NBTUtils.resolve(var2.h(), "kalama:villager/trade_info", "kalama:trade_lock") instanceof NbtByte var4
                && var4.byteValue() == 1;
    }
    ...
    public List<SurvivalSubHelperS> afb(VillagerEntity villager) {
        SurvivalSubHelperH var2 = this.aff(villager, false);
        return var2 != null
                        && NBTUtils.resolve(var2.h(), "kalama:villager/trade_info", "kalama:trade_list")
                                instanceof NbtList var4
                ? NBTUtils.toValue(
                        var4,
                        Codec.list(SurvivalSubHelperS.CODEC),
        ...
        if (((SetTradeOffersS2CPacket) eventSetTrade.b).getSyncId() == mc.player.currentScreenHandler.syncId
            ...
            this.Ot.execute(() -> {
                ...
                List var5 = var3x.stream()
                        .map(offer -> new SurvivalSubHelperS(
                                offer.getSellItem(),
                                offer.getFirstBuyItem().itemStack(),
                                offer.getSecondBuyItem()
                                        .<ItemStack>map(TradedItem::itemStack)
    ...
    public void afd(Event<SerializedEntry<?>> eventDataUpdate) {
        if (eventDataUpdate.getArgs(0) instanceof VillagerEntity var3
            ...
            this.Ot.execute(() -> {
                ...
                if (!Objects.equals(var3x, VillagerProfession.NONE)
                        && !Objects.equals(var3x, VillagerProfession.NITWIT)) {
                    if (var5.getLevel() > 1) {
                        this.aeX(var3, true);
                    }
                } else {
