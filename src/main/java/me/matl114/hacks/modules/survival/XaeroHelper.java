...
public class XaeroHelper extends BaseModule {
    ...
    public final FlagRef Ag = this.flagBuilder(this.aD.add("loaded-chunk-render"))
            .updateListener(this::Ov)
            .build();
    ...
    public XaeroHelper() {
        super("XaeroHelper");
        this.loadedChunkRenderColor = this.builder(this.aD.add("loaded-chunk-render-color"), WrapColor.class)
                .defaultValue(new WrapColor(Formatting.RED))
                .build();
        this.xplusBaritoneElytraPathFix =
                this.flagBuilder(this.aD.add("xplus-baritone-elytra-path-fix")).build();
        this.enableXaeroRightClickCommand = this.flagBuilder(this.aD.add("enable-xaero-right-click-command"))
                .build();
        this.xaeroRightClickCommandList = this.builder(
                        this.aD.add("xaero-right-click-command-list"), PrimitiveList.type(StringFormat.class))
                .defaultValue(new PrimitiveList<>(
                        NBTTypes.x,
                        List.of(new StringFormat(Ak, "/tp {pos}"), new StringFormat(Ak, "/!!travel to {pos}")),
                        new StringFormat(Ak, "")))
                .build();
        this.xaeroRightClickSuggestList = this.builder(
                        this.aD.add("xaero-right-click-suggest-list"), PrimitiveList.type(StringFormat.class))
    ...
    public void OB(Event<ArrayList<MapClickContext>> event) {
        if (this.enableXaeroRightClickCommand.get()) {
            ...
            for (StringFormat var6 : this.xaeroRightClickCommandList.get().list()) {
                String var7 = ChatUtils.l(Text.translatable(
                        "message.module.xaero-helper.right-click-command.command", new Object[] {var6.formatText(As)}));
            ...
            for (StringFormat var9 : this.xaeroRightClickSuggestList.get().list()) {
                String var10 = ChatUtils.l(Text.translatable(
                        "message.module.xaero-helper.right-click-command.suggest", new Object[] {var9.formatText(As)}));
                ...
                if (!Objects.equals(var2.getCurrentWorld(), mc.world.getRegistryKey())) {
                    ...
                    if (this.travelGoalSync.get()) {
                        if (Travel.CW == null) {
                            ...
                            BlockPos var5 =
                                    new BlockPos((int) var4.x, (int) Math.clamp(var4.y, -512.0, 512.0), (int) var4.z);
                            if (this.Au == null) {
                                this.Au = var2.createWaypoint(
                                        var5.getX(),
                                        var5.getY(),
                                        var5.getZ(),
                                        "[SFH] Travel",
        ...
        if (XaeroHooks.getInstance().isXaeroPlusEnable()) {
            if (bl) {
                this.Ap = XaeroHooks.getInstance()
                        .getMapDrawFactory()
                        .lines(
                                "slimefun_xaerohelper_loaded_chunk_render",
                                this::Ow,
                                () -> this.loadedChunkRenderColor.get().withAlpha(255),
                                () -> 0.1F,
                                100);
                this.Ap.register();
