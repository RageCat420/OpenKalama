...
public class SchedularSettings extends BaseModule {
    ...
    public void amB() {
        if (this.warn.get()) {
            this.VV.b(
                    100,
                    () -> this.logI18N(
                            "message.module.schedular-settings.logic-error.no-suitable-container-source",
                            new Object[0]));
        }
    }
    ...
    public SchedularSettings() {
        super("Schedular");
        this.enginType = this.builder(this.aD.add("engin-type"), SchedularSettings$EngineType.class)
                .defaultValue(SchedularSettings$EngineType.BARITONE)
                .build();
        this.replenish = this.builder(this.aD.add("replenish"), Boolean.class)
                .defaultValue(true)
                .build();
        this.discharge = this.builder(this.aD.add("discharge"), Boolean.class)
                .defaultValue(true)
                .build();
        this.invSpeedLimit =
                this.intBuilder(this.aD.add("inv-speed-limit")).defaultValue(27).build();
        this.hotbarProtect =
                this.intBuilder(this.aD.add("hotbar-protect")).defaultValue(0).build();
        this.scannChestRange = this.builder(this.aD.add("scann-chest-range"), Vec3.class)
                .defaultValue(new Vec3(20.0, 5.0, 20.0))
                .build();
        this.enableAutoExpandShulker = this.builder(this.aD.add("enable-auto-expand-shulker"), Boolean.class)
                .defaultValue(true)
                .build();
        this.enableAutoExpandChest =
                this.flagBuilder(this.aD.add("enable-auto-expand-chest")).build();
        this.enableContainerHotReload =
                this.flagBuilder(this.aD.add("enable-container-hot-reload")).build();
        this.warn = this.builder(this.aD.add("warn"), Boolean.class)
                .defaultValue(true)
                .build();
        this.detailedLog = this.flagBuilder(this.aD.add("detailed-log")).build();
        this.pause = this.flagBuilder(this.aD.add("pause")).build();
        this.pauseHotkey =
                this.hotkey(this.aD.add("pause-hotkey"), new MultiKeyBind()).build();
        this.holdReset =
                this.hotkey(this.aD.add("hold-reset"), new MultiKeyBind()).build();
        this.enableRender = this.builder(this.aD.add("enable-render"), Boolean.class)
                .defaultValue(true)
                .build();
        this.replenishmentColor = this.builder(this.aD.add("replenishment-color"), WrapColor.class)
                .defaultValue(new WrapColor(Formatting.BLUE))
                .build();
        this.dischargeColor = this.builder(this.aD.add("discharge-color"), WrapColor.class)
                .defaultValue(new WrapColor(Formatting.RED))
                .build();
        this.shulkerSupportColor = this.builder(this.aD.add("shulker-support-color"), WrapColor.class)
                .defaultValue(new WrapColor(Formatting.YELLOW))
                .build();
        this.goalColor = this.builder(this.aD.add("goal-color"), WrapColor.class)
                .defaultValue(new WrapColor(Formatting.GREEN))
                .build();
        this.colorLines = this.builder(this.aD.add("color-lines"), WrapColor.class)
    ...
    public void amA() {
        if (this.warn.get()) {
            this.VV.b(
                    100,
                    () -> this.logI18N(
                            "message.module.schedular-settings.logic-error.no-container-source", new Object[0]));
        }
    }
    ...
    public void amC() {
        if (this.warn.get()) {
            this.VW.b(
                    100,
                    () -> this.logI18N(
                            "message.module.schedular-settings.logic.hot-reload-container-source", new Object[0]));
        }
    }
}
