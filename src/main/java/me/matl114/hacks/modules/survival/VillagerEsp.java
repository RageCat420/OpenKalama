...
public class VillagerEsp extends BaseModule {
    ...
    private Text buildEnchantmentTradeText(SurvivalSubHelperS trade) {
        ...
        if (var2.isOf(Items.ENCHANTED_BOOK) && var2.contains(DataComponentTypes.STORED_ENCHANTMENTS)) {
            ...
            if (var3 == null) {
                ...
                return ((Enchantment) var4.value())
                        .description()
                        .copy()
                        .append(Text.literal(String.valueOf(var5)))
                        .append(Text.literal(" " + var6));
            }
        } else {
            return null;
        }
    ...
    public VillagerEsp() {
        super("VillagerEsp");
        this.J = this.moduleEntry(this.Wi.addHotkey(), new MultiKeyBind(), this.Wi.addEnable())
                .build();
        this.textScale = this.doubleBuilder(this.Wi.add("text-scale"))
                .defaultValue(0.75)
                .validator(Configs.doubleRange(0.1, 4.0))
                .build();
        this.color = this.builder(this.Wi.add("color"), WrapColor.class)
