...
public class BaritoneFix extends BaseModule {
   ...
   public void xf(Event<String> eventCommandSay) {
      if (BaritoneHooks.getInstance().isEnabled() && ((String)eventCommandSay.b).startsWith("/")) {
         ...
         if (((String)eventCommandSay.b).startsWith(var2) && BaritoneHooks.getInstance().handleCommand((String)eventCommandSay.b)) {
   ...
   private boolean xb() {
      ...
      if (var1.afr()) {
         ...
         return var1.enable.get() ? true : var2.getItem() == Items.ELYTRA && var2.getMaxDamage() - var2.getDamage() >= this.oX.getValue();
      }
   }
      ...
      if (this.enableBaritoneCommandProtect.get() && ((String)chatEvent.e()).startsWith("#")) {
         if (!BaritoneHooks.getInstance().isEnabled()) {
            ...
            if (!((String)chatEvent.e()).startsWith(this.oZ.getValue())) {
               this.logI18N("message.module.baritone-fix.command-prefix-mismatch", new Object[]{this.oZ.getValue()});
               chatEvent.cancel();
            }
         }
      }
   ...
   public BaritoneFix() {
      ...
      this.emergencyLandingFix = this.flagBuilder(this.oD.add("emergency-landing-fix")).build();
      this.disableInventoryCheck = this.flagBuilder(this.oD.add("disable-inventory-check")).build();
      this.enableInventoryFireworks = this.flagBuilder(this.oD.add("enable-inventory-fireworks")).build();
      this.enableFireworkSwap = this.flagBuilder(this.oD.add("enable-firework-swap")).build();
      this.enableBaritoneCommandProtect = this.flagBuilder(this.oD.add("enable-baritone-command-protect")).build();
      this.changeLandingToElytraFlight = this.flagBuilder(this.oD.add("change-landing-to-elytra-flight")).build();
      this.autoJumpFix = this.flagBuilder(this.oD.add("auto-jump-fix")).build();
      this.changeLandingToLog = this.flagBuilder(this.oD.add("change-landing-to-log")).build();
      this.baritoneConditionalPause = this.flagBuilder(this.oD.add("baritone-conditional-pause")).build();
      this.baritonePauseHotkey = this.builder(this.oD.add("baritone-pause-hotkey"), KeyBindRef.TYPE).defaultValue(new MultiKeyBind()).build();
      this.fixBaritoneSimulateError = this.flagBuilder(this.oD.add("fix-baritone-simulate-error")).build();
      this.fixWhenFailCalculate = this.flagBuilder(this.oD.add("fix-when-fail-calculate")).build();
      this.baritoneExperiment1 = this.flagBuilder(this.oD.add("baritone-experiment-1")).build();
      this.baritoneExperimentHeight1 = this.doubleBuilder(this.oD.add("baritone-experiment-height-1")).defaultValue(36.0).build();
      this.baritoneExperiment2 = this.flagBuilder(this.oD.add("baritone-experiment-2")).build();
      this.exp2MinHeight = this.doubleBuilder(this.oD.add("exp-2-min-height")).defaultValue(38.0).build();
      this.exp2MaxHeight = this.doubleBuilder(this.oD.add("exp-2-max-height")).defaultValue(42.0).build();
      INSTANCE = this;
   }
   ...
   public void addCustomWidgets(Consumer<DrawableWidget> acceptor, int dx, int dy, int dblank) {
      acceptor.accept(
         this.createTitleLabel(
            BaritoneHooks.getInstance().isBaritoneAPISupported() ? "widget.baritone-fix.baritone-api-support" : "widget.baritone-fix.baritone-api-not-support",
            0,
            dblank,
            dx,
            dy
      ...
      acceptor.accept(
         this.createTitleLabel(
            BaritoneHooks.getInstance().isBaritoneVersionSupported() ? "widget.baritone-fix.baritone-support" : "widget.baritone-fix.baritone-not-support",
            0,
            dblank,
            dx,
            dy
   ...
   private boolean xc() {
      ...
      return InventoryUtils.computePlayerInventory(stack -> var2.afG(stack) ? (double)stack.getCount() : null, false) >= var1;
   }
