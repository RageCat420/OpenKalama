...
public class RenderExtra extends BaseModule {
    ...
    public RenderExtra() {
        ...
        this.ignoreServerRequest =
                this.flagBuilder(this.ov.add("ignore-server-request")).build();
        this.nightvision = this.builder(this.ow.add("nightvision"), Boolean.class)
                .defaultValue(true)
                .build();
        this.noWorldBobView = this.builder(this.ow.add("no-world-bob-view"), FlagRef.TYPE)
                .defaultValue(true)
                .build();
        this.disableWurstHud =
                this.flagBuilder(this.ow.add("disable-wurst-hud")).build();
        this.enhancedDebugHud =
                this.flagBuilder(this.ow.add("enhanced-debug-hud")).build();
        INSTANCE = this;
    }
    ...
    public void onResourceRequest(Event<ResourcePackSendS2CPacket> resourceEvent) {
        if (this.ignoreServerRequest.get()) {
            ...
            Debug.chat(
                    ...
                    Text.literal(var3.url())
                            .setStyle(Style.EMPTY.withClickEvent(new ClickEvent(Action.OPEN_URL, var3.url())))
