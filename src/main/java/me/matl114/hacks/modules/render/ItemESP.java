...
public class ItemESP extends BaseModule {
            ...
            if (this.ae.get()) {
                ...
                if (var2 || var3) {
                    for (Entity var11 : mc.world.getEntities()) {
                        if (var11 instanceof ItemEntity var12
                                || this.enableFrame.get() && var11 instanceof ItemFrameEntity) {
                            if (var2
                                ...
                                if (this.nameDisplaySpecial.get()) {
                                    this.gP.submit(
                                            new HackUtilHelperB(
                                                    var19,
                                                    var11.getBoundingBox().getCenter(),
                                                    (float) this.nameScale.get()),
                                            var7);
                                }
                            } else if (var3) {
                                if (var8.box()) {
                                ...
                                if (this.nameDisplay.get()) {
                                    MutableText var17;
                                    if (var11 instanceof ItemEntity var15
                                            && !var15.getStack().isEmpty()) {
                                        ItemStack var16 = var15.getStack();
                                        var17 = VItem.w()
                                                .l(var16)
                                                .append(ChatUtils.textFromLegacyString(
                                                        "&ex%d".formatted(var16.getCount())));
                                    } else if (var11 instanceof ItemFrameEntity var18
                                            && !var18.getHeldItemStack().isEmpty()) {
    ...
    public void onItemEntity(Entity itemEntity, ItemStack stack) {
        if (!stack.isEmpty() && this.testItem(stack)) {
            this.kl(
                    itemEntity,
                    VItem.w().l(stack).append(ChatUtils.textFromLegacyString("&ex%d".formatted(stack.getCount()))));
        } else {
            this.km(itemEntity);
        }
    }
    ...
    public void handleItemEntityItemData(Event<SerializedEntry<?>> entryUpdateEvent) {
        if (this.enableItem.get()) {
            ...
            if (var2.id() == 8
                    && var2.value() instanceof ItemStack var4
                    && entryUpdateEvent.getArgs(0) instanceof ItemEntity var5) {
        ...
        try {
            NbtCompound var3 = var2.isEmpty()
                    ...
                    : (NbtCompound) ComponentChanges.CODEC
                            .encodeStart(ItemStackUtils.registry().getOps(NbtOps.INSTANCE), var2)
                            .getOrThrow();
    ...
    public ItemESP() {
        ...
        this.enableItem = this.flagBuilder(this.gx.add("enable-item"))
                .updateListener(s -> this.kn())
                .build();
        this.enableFrame = this.flagBuilder(this.gx.add("enable-frame"))
                .updateListener(s -> this.kn())
                .build();
        this.drawName = this.flagBuilder(this.gx.add("draw-name")).build();
        this.options = this.builder(this.gx.add("options"), TracingOption.class)
        ...
        varB.defaultValue(new EntrySet(
                new Regex(
        ...
        this.specialOptions = this.builder(this.gx.add("special-options"), TracingOption.class)
                .defaultValue(new TracingOption(true, true))
                .build();
        this.nameDisplaySpecial = this.builder(this.gx.add("name-display-special"), Boolean.class)
                .defaultValue(true)
                .build();
        this.color = this.builder(this.gx.add("color"), WrapColor.class)
                .defaultValue(new WrapColor(Formatting.YELLOW))
                .build();
        this.specialColor = this.builder(this.gx.add("special-color"), WrapColor.class)
                .defaultValue(new WrapColor("#ED0355"))
                .build();
        this.nameDisplayColor = this.builder(this.gx.add("name-display-color"), WrapColor.class)
        ...
        this.specialNameDisplayColor = this.builder(this.gx.add("special-name-display-color"), WrapColor.class)
                ...
                .build();
        this.nameScale =
                this.doubleBuilder(this.gx.add("name-scale")).defaultValue(0.75).build();
    ...
    public void kq(Event<SerializedEntry<?>> entryUpdateEvent) {
        if (this.enableItem.get() && this.enableFrame.get()) {
            ...
            if (var2.id() == 9
                    && var2.value() instanceof ItemStack var4
                    && entryUpdateEvent.getArgs(0) instanceof ItemFrameEntity var5) {
                this.onItemEntity(var5, var4);
            }
        }
    }
