...
public class Hud extends IRender2DColoredModule {
    ...
    public void handleIcon(VDrawContext vdraw) {
        if (this.right2.get()) {
            vdraw.w(
                    Identifier.tryParse("kalama:textures/custom/genshin_impact.png"),
                    -80,
                    0,
                    0,
                    27,
    ...
    public Hud() {
        super("Hud");
        this.elements = this.builder(this.he.add("elements"), Hud$HudElementSelectSet.class)
                .defaultValue(new Hud$HudElementSelectSet())
                .build();
    ...
    public void handleCommonInfo(VDrawContext vdraw) {
        ...
        MutableText var5 = ChatUtils.textFromLegacyString(
                "&aMCv" + var2 + (Objects.equals(var2, SupportVersion.CURRENT) ? "" : "(Via)") + " Fps:"
                        + mc.getCurrentFps() + " " + var3 + "ms");
