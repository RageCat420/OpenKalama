...
public class NoRender extends BaseModule {
    ...
    public void doCancelEffect(Event<EntityStatusEffectS2CPacket> packet) {
        if (!checkNull()) {
            if (this.ae.get()
                    && this.forceNo.get()
                    && this.to.get().test((StatusEffect) ((EntityStatusEffectS2CPacket) packet.b)
                            .getEffectId()
                            .value())) {
                packet.cancel();
            }
    ...
    public void doCancelSpawn(Event<EntitySpawnS2CPacket> packet) {
        if (!checkNull()) {
            if (this.ae.get()
                    && this.forceNo2.get()
                    && this.types.get().test(((EntitySpawnS2CPacket) packet.b).getEntityType())) {
                packet.cancel();
            }
        }
    }
    ...
    public NoRender() {
        ...
        this.blockRandomEffect =
                this.flagBuilder(this.sV.add("block-random-effect")).build();
        ...
        this.to = this.builder(this.sU.add("types"), EntrySet.<StatusEffect>parameter())
                .defaultValue(new EntrySet<StatusEffect>(
                        new Regex("^(blindness|darkness|nausea)$"), Registries.STATUS_EFFECT))
                .build();
        ...
        this.types = this.builder(this.sX.add("types"), EntityTypeRegex.class)
        ...
        this.tx = this.builder(this.sY.add("types"), EntrySet.<ParticleType<?>>parameter())
