...
public class MineESP extends BaseModule {
   ...
   private void renderProjectedGrid(VDrawContext vdraw, RenderSubHelperJ[][] states, double centerX, double centerY) {
      ...
      this.fillLimited(vdraw, var12 - 2, var12 - 2, var12 + var11 + 2, var12 + var11 + 2, var10, var13, var15, var14, var16);
   ...
   public float predictGhostHandBreakSpeed(PlayerEntity player, BlockPos pos) {
      ...
      ItemStack var5 = var3.j
         .stream()
         .max(Comparator.comparingDouble(s -> WorldUtils.getPlayerBlockBreakingSpeedWithCanMineMultiply(player, var4, s.fS())))
   ...
   private RenderSubHelperJ projectColumnState(int x, int z, int minY, int maxYExclusive, int[] occupiedBlocks) {
      ...
      for (int var10 = minY; var10 < maxYExclusive; var10++) {
         ...
         for (MineSubHelperH var14 : MiningProgressManager.INSTANCE.getBreakingMap().values()) {
   ...
   private void renderGridOverlay(VDrawContext vdraw, RenderSubHelperJ[][] cells, float rotationRadians, float playerOffsetX, float playerOffsetY) {
      for (int var6 = 0; var6 < 5; var6++) {
         for (int var7 = 0; var7 < 5; var7++) {
            RenderSubHelperJ var8 = cells[var6][var7];
            if (var8.UG() >= 0) {
               RenderSubHelperD var9 = this.rotatePoint(this.Gy(var7, playerOffsetX), this.Gz(var6, playerOffsetY), rotationRadians);
   ...
   public void onUpdate(Event<ClientPlayerEntity> eventUpdate) {
      ...
      if (!checkNull()) {
         if (this.ae.get()) {
            for (MineSubHelperH var3 : MiningProgressManager.INSTANCE.getBreakingMap().values()) {
               if (var3.b != null) {
                  ...
                  if (this.renderName.get()) {
                     MutableText var14 = ChatUtils.textFromLegacyString(var3.a.getNameForScoreboard() + "\n" + var10);
                     this.lg.submit(new HackUtilHelperB(var14, var4.toCenterPos().add(0.0, 0.2, 0.0), 0.5F), this.nameColor.get().withAlpha(255));
                  }
                  ...
                  if (this.renderBox.get()) {
                     ...
                     this.vn
                        .submit(
                           new Box(var4.toCenterPos().add(RenderTasks.l.multiply(var15)), var4.toCenterPos().add(RenderTasks.n.multiply(var15))),
               ...
               if (var3.e != null) {
                  ...
                  if (this.renderName.get()) {
                     ...
                     this.lg.submit(new HackUtilHelperB(var16, var11.toCenterPos().add(0.0, 0.2, 0.0), 0.5F), this.nameColor.get().withAlpha(255));
                  }
                  ...
                  if (this.renderBox.get()) {
                     ...
                     this.vn
                        .submit(
                           new Box(var11.toCenterPos().add(RenderTasks.l.multiply(var17)), var11.toCenterPos().add(RenderTasks.n.multiply(var17))),
                           this.progressColor.get().withAlpha(64)
                        );
                  }
               }
   ...
   public MineESP() {
      ...
      this.ghostHandPredict = this.flagBuilder(this.aD.add("ghost-hand-predict")).build();
      this.nameColor = this.builder(this.aD.add("name-color"), WrapColor.class).defaultValue(new WrapColor(Formatting.WHITE)).build();
      this.frameColor = this.builder(this.aD.add("frame-color"), WrapColor.class).defaultValue(new WrapColor(Formatting.AQUA)).build();
      this.frameDoubleColor = this.builder(this.aD.add("frame-double-color"), WrapColor.class).defaultValue(new WrapColor(Formatting.AQUA)).build();
      this.progressColor = this.builder(this.aD.add("progress-color"), WrapColor.class).defaultValue(new WrapColor(Formatting.GOLD)).build();
      this.renderGrid2d = this.flagBuilder(this.aD.add("render-grid-2d")).build();
      this.gridPos = this.builder(this.aD.add("grid-pos"), WidgetPos.class).defaultValue(new WidgetPos(0, 0.5, 0.7, 0, 0)).build();
      this.gridCellSize = this.intBuilder(this.aD.add("grid-cell-size")).defaultValue(12).validator(Configs.intRange(6, 40)).build();
      this.gridGap = this.intBuilder(this.aD.add("grid-gap")).defaultValue(1).validator(Configs.intRange(0, 6)).build();
      this.gridBreakingColor = this.builder(this.aD.add("grid-breaking-color"), WrapColor.class).defaultValue(new WrapColor(Formatting.RED)).build();
      this.gridDoubleColor = this.builder(this.aD.add("grid-double-color"), WrapColor.class).defaultValue(new WrapColor(Formatting.AQUA)).build();
      this.gridSolidColor = this.builder(this.aD.add("grid-solid-color"), WrapColor.class).defaultValue(new WrapColor(new Color(96, 96, 96))).build();
      this.gridAirColor = this.builder(this.aD.add("grid-air-color"), WrapColor.class).defaultValue(new WrapColor(new Color(24, 24, 24))).build();
   ...
   private void fillLimited(VDrawContext vdraw, int x1, int y1, int x2, int y2, int color, int limitX1, int limitY1, int limitX2, int limitY2) {
