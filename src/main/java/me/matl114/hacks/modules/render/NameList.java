...
public class NameList extends INameTag {
            ...
            for (int var4 = 0; var4 < var3.length; var4++) {
                if (var3[var4].isEmpty()) {
                    ...
                    if (this.equipmentDamage.get().isNotIn(new ConfigEnum[] {ItemStackDisplayUtils$DamageDisplay.NONE})
                            && var6.getCount() == 1
                            && var6.isDamageable()) {
                        var7 = ItemStackDisplayUtils.getDamageShowText(var6, this.equipmentDamage.get())
                                .getString();
                        var6 = var6.copy();
                        var6.setDamage(0);
                    }
    ...
    public void handleTooManyPlayerList(VDrawContext vdraw) {
        vdraw.b();
        OrderedText var2 = Text.literal("......(" + (this.GU.size() - this.playerListMaxLength.get()) + " more)")
                .asOrderedText();
    ...
    public void handleEffectDisplayList(VDrawContext vdraw, RenderSubHelperIX player) {
        if (player.g != null) {
            ...
            for (int var5 = 0; var5 < var4; var5++) {
                ...
                vdraw.z(
                        mc.textRenderer,
                        ((Text) var6.getValue()).asOrderedText(),
                        9,
                        0,
    ...
    protected void initializeModuleSettings() {
        this.playerListMaxLength = this.builder(this.GE.add("player-list-max-length"), IntRef.TYPE)
                .defaultValue(20)
                .build();
        this.listRight = this.flagBuilder(this.GE.add("list-right")).build();
        this.listPos = this.builder(this.GE.add("list-pos"), WidgetPos.class)
                .defaultValue(new WidgetPos(0, 0.02, 0.02, 5, 5))
                .build();
    }
