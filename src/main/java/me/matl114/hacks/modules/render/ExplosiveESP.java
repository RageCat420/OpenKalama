...
public class ExplosiveESP extends BaseModule {
    ...
    private void submitDamageText(Vec3d explosionPos, float power, int color) {
        float var4 = power == 5.0F
                ? ExplosionUtils.respawnAnchorDamage(
                        mc.player.getBoundingBox(), explosionPos, mc.world, ExplosionUtils.g)
                : ExplosionUtils.c(mc.player.getBoundingBox(), explosionPos, mc.world, ExplosionUtils.g);
        this.gP.submit(
                new HackUtilHelperB(
                        Text.literal("%.1f/%.1f".formatted(var4, ExplosionUtils.o(mc.world, var4))),
                        explosionPos,
                        0.66F),
                ColorUtils.j(color, 255));
    }
    ...
    public ExplosiveESP() {
        ...
        this.J = this.moduleEntry(this.aD.addHotkey(), new MultiKeyBind(), this.aD.addEnable())
                .build();
        this.distance = this.intBuilder(this.aD.add("distance"))
                .defaultValue(16)
                .validator(Configs.e)
                .build();
        this.options = this.builder(this.aD.add("options"), TracingOption.class)
                .defaultValue(new TracingOption(true, false))
                .build();
        this.anchorColor = this.builder(this.aD.add("anchor-color"), WrapColor.class)
                .defaultValue(new WrapColor(Formatting.GOLD))
                .build();
        this.crystalColor = this.builder(this.aD.add("crystal-color"), WrapColor.class)
