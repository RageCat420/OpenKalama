...
public class EntityESP extends BaseModule {
   ...
   public EntityESP() {
      super("EntityESP");
      this.pv = this.moduleEntry(Configs.i, this.HD.add("hotkey").toPath(), new MultiKeyBind(), this.HD.add("enable").toPath()).build();
      this.renderMode = this.builder(this.HD.add("render-mode"), RenderMode.class).defaultValue(RenderMode.RENDER_3D).build();
      ...
      this.color = this.builder(this.HD.add("color"), NBTType.<EntryPrimitiveMap<EntityType<?>, TextColor>>parameter(EntryPrimitiveMap.class))
         ...
         .build();
      this.boxingOption = this.builder(this.HD.add("boxing-option"), NBTType.<EntryPrimitiveMap<EntityType<?>, Boolean>>parameter(EntryPrimitiveMap.class))
         .defaultValue(
            new EntryPrimitiveMap<>(
               Registries.ENTITY_TYPE, NBTTypes.f, Map.of(EntityType.PLAYER, true, EntityType.END_CRYSTAL, true, EntityType.WITHER, true), false
            )
         )
         .build();
      this.traceOption = this.builder(this.HD.add("trace-option"), NBTType.<EntryPrimitiveMap<EntityType<?>, Boolean>>parameter(EntryPrimitiveMap.class))
         .defaultValue(
            new EntryPrimitiveMap<>(
               Registries.ENTITY_TYPE, NBTTypes.f, Map.of(EntityType.PLAYER, true, EntityType.END_CRYSTAL, false, EntityType.WITHER, true), false
            )
         )
         .build();
      this.highlightOption = this.builder(this.HD.add("highlight-option"), NBTType.<EntryPrimitiveMap<EntityType<?>, Boolean>>parameter(EntryPrimitiveMap.class))
         .defaultValue(
            new EntryPrimitiveMap<>(
               Registries.ENTITY_TYPE, NBTTypes.f, Map.of(EntityType.PLAYER, true, EntityType.END_CRYSTAL, true, EntityType.WITHER, true), true
            )
         )
         .build();
      this.HI = new ArrayList<>();
