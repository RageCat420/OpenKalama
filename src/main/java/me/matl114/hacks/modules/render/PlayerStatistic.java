...
public class PlayerStatistic extends IRender2DColoredModule {
   ...
   private static final RegistryDisplays$IIcon<StatusEffect> statusEffectRenderer = RegistryDisplays.f(StatusEffect.class);
   ...
   public PlayerStatistic() {
      ...
      this.itemTypes = this.builder(this.he.add("item-types"), EntrySet.<Item>parameter())
         .defaultValue(new EntrySet<Item>(Registries.ITEM, List.of(Items.TOTEM_OF_UNDYING, Items.FIREWORK_ROCKET)))
         .build();
   ...
   public void handleEffects(VDrawContext vdraw) {
      for (StatusEffectInstance var3 : mc.player.getStatusEffects()) {
         OrderedText var4 = StatusEffectUtil.getDurationText(var3, 1.0F, mc.world.getTickManager().getTickRate()).asOrderedText();
         ...
         statusEffectRenderer.a(1, 1, vdraw, (StatusEffect)var3.getEffectType().value());
   ...
   public void handleTurtle(VDrawContext vdraw, Potion potionType) {
      ...
      int var4;
      if (var3 != null) {
         var4 = var3.entrySet().stream().filter(s -> this.isTurtle(((ItemStackSample)s.getKey()).fS(), potionType)).mapToInt(Entry::getValue).sum();
      } else {
         var4 = 0;
      }

      this.drawItemStatistic(vdraw, PotionContentsComponent.createStack(Items.POTION, Registries.POTION.getEntry(potionType)), var4);
   }
   ...
   public void AK(VDrawContext vdraw, Item itemType) {
      ...
      int var4;
      if (var3 != null) {
         var4 = var3.entrySet().stream().filter(s -> ((ItemStackSample)s.getKey()).fS().isOf(itemType)).mapToInt(Entry::getValue).sum();
      } else {
         var4 = 0;
      }
