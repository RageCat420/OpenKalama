...
public class InvHud extends BaseModule {
    ...
    public void onPostTick(Event<ClientPlayerEntity> event) {
        if (this.ae.get()) {
            ...
            Map var2;
            if ((var2 = this.showShulkerItems.get() ? PlayerStateManager.INSTANCE.jN : PlayerStateManager.INSTANCE.jM)
                    != null) {
                for (Entry var4 : ((java.util.Set<Entry>) (var2).entrySet())) {
                    if (this.showItems
                            .get()
                            .test(((ItemStackSample) var4.getKey()).fS().getItem())) {
                        ItemStack var5 =
                                ((ItemStackSample) var4.getKey()).fS().copyWithCount((Integer) var4.getValue());
                        this.Cq.add(var5);
                    }
                }
            }
    ...
    public InvHud() {
        super("InvHud");
        this.hotkey = this.moduleEntry(this.Cn.add("hotkey"), new MultiKeyBind(), this.Cn.add("enable"))
                .build();
        ...
        this.showItems = this.builder(this.Cn.add("show-items"), NBTType.parameter(RegistryRegex.class))
                ...
                .<NBTRef>build();
        this.countPerLine =
                this.intBuilder(this.Cn.add("count-per-line")).defaultValue(9).build();
        this.showShulkerItems = this.builder(this.Cn.add("show-shulker-items"), Boolean.class)
                .defaultValue(true)
                .build();
