...
public class ProjectileESP extends BaseModule {
      ...
      if (vec3ds.size() > 3) {
         ...
         if (!vec3ds.isEmpty()) {
            if (result != null && result.getType() == Type.ENTITY) {
               ...
               RenderUtils.r(stack, var4.add(RenderTasks.m), var4.add(RenderTasks.o), ColorUtils.k(Color.GREEN, 0.25F));
            }
         }
      }
   }
   ...
   public ProjectileESP() {
      ...
      this.renderProjectile = this.flagBuilder(this.gw.add("render-projectile")).build();
   ...
   private static void drawClassicArrowTrajectory(MatrixStack stack, List<Vec3d> vec3ds, Color clr) {
      if (vec3ds.size() > 3) {
         ...
         if (!vec3ds.isEmpty()) {
            ...
            RenderUtils.r(stack, var3.add(RenderTasks.m), var3.add(RenderTasks.o), ColorUtils.k(Color.GREEN, 0.25F));
         }
      }
   }
         ...
         if (this.ae.get()) {
            ...
            try {
               for (Entity var7 : mc.world.getEntities()) {
                  ...
                  if (var3 && var7 instanceof AbstractSkeletonEntity var15 && !(var15 instanceof WitherSkeletonEntity)) {
   ...
   public static ArrayList<Vec3d> predictFireballTrace(ExplosiveProjectileEntity fireball) {
      ...
      for (int var7 = 0; var7 < 400; var7++) {
         ...
         if (var1.size() > 2) {
            ...
            if (RaycastUtils.raycastAnySolidBlock(fireball, var3, var2) || RaycastUtils.raycastHitAnyEntityExceptPlayer(fireball, var3, var2)) {
               break;
            }
         }
      }
   ...
   private static void renderPlayerProjectile(MatrixStack stack, PlayerEntity player, float tickDelta) {
      for (Hand var6 : Hand.values()) {
         if (player.getStackInHand(var6).getItem() instanceof RangedWeaponItem var8) {
            ...
            drawArrowTrajectoryWithHitResult(stack, (List<Vec3d>)var9.getFirst(), (HitResult)var9.getSecond(), tickDelta);
            return;
         }
      }
   }
   ...
   private static void renderArrowProjectile(MatrixStack stack, PersistentProjectileEntity arrow, float tickDelta) {
      if (!arrow.isOnGround() && arrow.getVelocity().lengthSquared() > 1.0E-5) {
         drawClassicArrowTrajectory(stack, RenderSubHelperW.of(arrow, tickDelta).predictLine(400), Color.RED);
      }
   }

   private static void renderCrossbowProjectile(MatrixStack stack, CrossbowUser pillagerEntity, float tickDelta) {
      if (pillagerEntity instanceof LivingEntity var3 && var3.isUsingItem() && var3.getActiveItem().getItem() instanceof RangedWeaponItem var5) {
         drawClassicArrowTrajectory(stack, RenderSubHelperW.i(pillagerEntity, tickDelta).predictLine(400), Color.YELLOW);
      }
   }
   ...
   private static void renderSkeletonProjectile(MatrixStack stack, AbstractSkeletonEntity entity, float tickDelta) {
      if (entity.isUsingItem() && entity.getActiveItem().getItem() instanceof BowItem) {
         drawClassicArrowTrajectory(stack, RenderSubHelperW.d(entity, tickDelta).predictLine(400), Color.YELLOW);
      }
   }
}
