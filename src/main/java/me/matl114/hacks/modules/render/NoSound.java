...
public class NoSound extends BaseModule {
    ...
    public NoSound() {
        ...
        this.cancelSoundPlay =
                this.flagBuilder(this.aD.add("cancel-sound-play")).build();
        this.cancelHudDisplay =
                this.flagBuilder(this.aD.add("cancel-hud-display")).build();
        this.noSounds = this.builder(this.aD.add("no-sounds"), EntrySet.<SoundEvent>parameter())
                .defaultValue(new EntrySet<>(
                        Registries.SOUND_EVENT,
                        Set.of((SoundEvent) SoundEvents.ITEM_ARMOR_EQUIP_NETHERITE.value(), (SoundEvent)
