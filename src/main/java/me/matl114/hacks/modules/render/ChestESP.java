    ...
    public ChestESP() {
        ...
        this.FA = this.builder(this.Mf.add("enable-types"), EntrySet.<BlockEntityType<?>>parameter())
                .defaultValue(new EntrySet<BlockEntityType<?>>(
                        new Regex("^(.*chest|barrel|.*box)$"), Registries.BLOCK_ENTITY_TYPE))
                .build();
        this.espTraceOptions = this.builder(this.Mf.add("esp-trace-options"), TracingOption.class)
        ...
        this.colorMap = this.builder(this.Mf.add("color-map"), EntryPrimitiveMap.<BlockEntityType<?>, TextColor>uA())
                .defaultValue(new EntryPrimitiveMap<BlockEntityType<?>, TextColor>(
