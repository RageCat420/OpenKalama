...
public class StorageDisplay extends BaseModule {
    ...
    public final FlagRef nb = this.builder(this.iv.add("enable-storage-display"), Boolean.class)
            .defaultValue(true)
            .build();
    ...
    public static String handlePureChickenDNAInfo(ItemStack item) {
        ...
        if (var2 != null
                && var2.startsWith("GCE_")
                && (var1 = ItemStackUtils.getBukkitValue(var1)) != null
                && var1.contains(ny)) {
            try {
    ...
    public static BukkitItemStack getNetworkStoraged(NbtCompound tag) {
        try {
            if (tag != null) {
                ...
                } else if (tag.contains(nj)
                        && tag.get(nj) instanceof NbtCompound var11
                        && var11.get(nl) instanceof NbtByteArray var14) {
    ...
    private static boolean hasAnyStorage(NbtCompound tag) {
        return tag != null
                ...
                && tag.getKeys().size() > 1
                && tag.getKeys().stream().anyMatch(nx::contains);
    }
    ...
    public void ve(Event<List<GuiModel>> event) {
        if (this.nb.get()) {
            ...
            if (hasAnyStorage(var3)) {
                RenderSubHelperF var4 = this.asyncUpdateItemInfo(var2, st -> {
    ...
    public static ItemStack handleElectricSpawnerInfo(ItemStack item, String sfid) {
        if (sfid != null && sfid.startsWith("ELECTRIC_SPAWNER_")) {
            ...
            EntityType var3 = (EntityType) Registries.ENTITY_TYPE
                    .getOrEmpty(new Identifier("minecraft", var2.toLowerCase(Locale.ROOT)))
                    .orElse(null);
            if (var3 != null) {
    ...
    public void vf(Event<List<GuiModel>> event) {
        if (this.enableInfoDisplay.get()) {
            ...
            if (var3 != null) {
                ItemStack var4;
                if ((var4 = vq(var2)) == null
                        && (var4 = vu(var2, var3)) == null
                        && (var4 = handleElectricSpawnerInfo(var2, var3)) == null) {
                    return;
                }
    ...
    public void onContainerVanilla(Event<List<GuiModel>> event) {
        if (this.enableShulkerDisplay.get()) {
            ...
            if (var3 != null) {
                RenderSubHelperF var4 = this.asyncUpdateItemInfo(var2, st0 -> {
                            ...
                            if (!var5x.isEmpty()) {
                                for (Entry<ItemStackSample, Integer> var7 : var3x.entrySet()) {
                                    if (ItemStackUtils.matchItemWithout(
                                            var5x, ((ItemStackSample) var7.getKey()).fS(), false, false, false)) {
    ...
    private RenderSubHelperF asyncUpdateItemInfo(ItemStack stack, Function<ItemStack, List<ItemStack>> func) {
        ...
        if (var3 == null || var3.lastUpdated < System.currentTimeMillis() - 10000L) {
            ...
            CompletableFuture.<List>supplyAsync(() -> (List) func.apply(var4))
                    .thenAccept(s -> var5.itemStack = (List<ItemStack>) s);
        }
    ...
    public StorageDisplay() {
        ...
        this.enableInfoDisplay = this.builder(this.iv.add("enable-info-display"), Boolean.class)
                .defaultValue(true)
                .build();
        this.enableShulkerDisplay =
                this.flagBuilder(this.iv.add("enable-shulker-display")).build();
        this.displayMode = this.builder(this.iv.add("display-mode"), StorageDisplay$Mode.class)
                .defaultValue(StorageDisplay$Mode.ALL)
                .build();
    ...
    public static BukkitItemStack getLogitechSingularity(NbtCompound tag) {
        try {
            if (tag != null
                    && tag.contains(nt)
                    && tag.get(nt) instanceof NbtCompound var2
                    && var2.get(ns) instanceof NbtByteArray var3) {
                byte[] var6 = var3.getByteArray();
                return BukkitItemStackUtils.a.fromPrimitive(var6);
            } else {
                return null;
            ...
            if (tag != null) {
                if (tag.contains(nm)) {
                    if (tag.get(nm) instanceof NbtCompound var2 && var2.get(no) instanceof NbtByteArray var3) {
                        ...
                        && tag.get(nn) instanceof NbtCompound var10
                        && var10.get(np) instanceof NbtByteArray var11) {
        ...
        if (bakedModelEvent.e() == null) {
            if (this.enableInfoDisplay.get()) {
                ...
                if (var3 != null) {
                    ...
                    if (var4 != null) {
                        RenderListener.c(new Identifier("kalama", "gce/" + var4))
