...
public record SlimefunSubHelperQ(String id, ItemStackDataWithAmount icon) {
   public static final SlimefunSubHelperQ Wl = new SlimefunSubHelperQ("NULL", new ItemStackDataWithAmount(ItemStackData.Xa(RecipeDatabase.Ip), 1));
   public static Codec<SlimefunSubHelperQ> dt = RecordCodecBuilder.create(
      instance -> instance.group(
            Codec.STRING.optionalFieldOf("rid", "").forGetter(SlimefunSubHelperQ::id),
            InvTasks.D.optionalFieldOf("icon", ItemStackDataWithAmount.EMPTY).forGetter(SlimefunSubHelperQ::icon)
         )
