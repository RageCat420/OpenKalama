...
public class RecipeDatabase extends BaseModule {
    ...
    public void onLoad() {
        ...
        try {
            ...
            Map var3 = (Map)
                    ((Pair) (Object) this.Il.decode(JsonOps.INSTANCE, var2).getOrThrow()).getFirst();
            this.Ih.putAll(var3);
            this.Ii = false;
        ...
        try {
            ...
            Map var8 = (Map)
                    ((Pair) (Object) this.Im.decode(JsonOps.INSTANCE, var7).getOrThrow()).getFirst();
            this.Ij.putAll(var8);
            this.Ik = false;
            ...
            if (var2.size() >= 27
                ...
                if (var4 != null) {
                    ...
                    if (var5) {
                        ...
                        String var14 = var12.isEmpty()
                                ? "NULL_RECIPE"
                                : var12.getName().getString().replace("§.", "");
                        this.WD(var14, var12);
    ...
    public void onSave() {
        if (this.saveData.get()) {
            if (this.Ii) {
                ...
                try {
                    JsonElement var1 = (JsonElement) (Object)
                            this.Il.encodeStart(JsonOps.INSTANCE, this.Ih).getOrThrow();
            ...
            if (this.Ik) {
                ...
                try {
                    JsonElement var4 = (JsonElement) (Object)
                            this.Im.encodeStart(JsonOps.INSTANCE, this.Ij).getOrThrow();
                    CompletableFuture.runAsync(() -> {
    ...
    public RecipeDatabase() {
        ...
        this.saveData = this.builder(this.HZ.add("save-data"), FlagRef.TYPE)
                .defaultValue(true)
                .build();
        this.lockExisting = this.flagBuilder(this.HZ.add("lock-existing")).build();
        this.lockCurrentData =
                this.flagBuilder(this.HZ.add("lock-current-data")).build();
        this.multiblockPattern = this.builder(this.HZ.add("multiblock-pattern"), StringRef.TYPE)
                .defaultValue("^(多方块结构|MultiBlock)$")
                .validator(Configs.a)
                .build();
        this.rpTitle = this.builder(this.HZ.add("rp-title"), Regex.class)
                ...
                if (this.ae.get()
                    ...
                    if (var4 != null) {
                        ...
                        if (this.rpTitle.get().test(var4)) {
                            Listener.C(new TimedPacketCatcherImpl<InventoryS2CPacket>(
                                    InventoryS2CPacket.class, 20, packetEvent -> {
                                        InventoryS2CPacket var3x = packetEvent.e();
                                        if (var3x.getSyncId()
                                                == ((GenericContainerScreenHandler) var3.getScreenHandler()).syncId) {
