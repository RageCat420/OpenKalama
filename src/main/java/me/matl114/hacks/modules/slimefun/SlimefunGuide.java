...
public class SlimefunGuide extends BaseModule {
   ...
   public static final List<Text> NG = List.of(Text.literal("左键获得一组该物品(仅限创造)"), Text.literal("shift左键拷贝/give指令"), Text.literal("右键打开物品编辑器"));
   ...
   public static final List<Text> NC = List.of(
      Text.literal("左键查看当前物品合成表"), Text.literal("右键查看包含当前物品的合成表"), Text.literal("Shift右键的时候会同时显示原版物品配方"), Text.literal("中键的时候会尝试获取物品")
   );
   ...
   public void aea() {
      if (!this.handleNotEnable()) {
         SlimefunTasks.openOrSwitch(
            new SlimefunChoiceScreen<>(
                  NF,
                  NG,
                  () -> InvTasks.aq().getSavedItemDataMap().keySet().stream().map(SlimefunTasks::e).toList(),
                  entry -> new ExecutableWidget(0, 0, 16, 16).eV(SlotElement.instance(entry.copyWithCount(1), (item, button) -> {
                     if (button == 0) {
                        this.tryGetItemStack(item);
                        return true;
                     } else if (button == 1) {
                        SlimefunTasks.openOrSwitch(SlimefunEntryListScreen.mapToWidget(List.of(entry), iv -> new SavedItemWidget(0, 0, iv, null)));
                        return true;
                     } else {
                        return false;
                     }
   ...
   public void adY(SlimefunSubHelperQ type) {
      if (!this.handleNotEnable()) {
         SlimefunTasks.openOrSwitch(
            SlimefunEntryListScreen.eF(
               SlimefunTasks.n().values().stream().filter(i -> Objects.equals(i.Co(), type.id())).map(IRecipeEntry.class::cast).toList()
            )
         );
      }
   }
   ...
   public void handleAutoEnable() {
      ...
      Debug.b(Text.literal("注意: 在1.20.5以上的物品数据和1.20.4及以下不互通,如果你进入了via支持的服务器,请注意这一点!").formatted(Formatting.YELLOW));
   }
      ...
      if (!this.handleNotEnable()) {
         SlimefunTasks.openOrSwitch(
            new SlimefunChoiceScreen<>(
                  ...
                  rp -> new ExecutableWidget(0, 0, 16, 16).eV(SlotElement.instance(rp.Ct().copyWithCount(1), (item, button) -> {
                     if (button == 0) {
                        this.adX(rp);
                        return true;
                     } else if (button == 1) {
   ...
   public void tryGetItemStack(ItemStack item) {
      ...
      } else if (mc.player != null && mc.interactionManager.getCurrentGameMode().isCreative()) {
      ...
      if (!this.handleNotEnable()) {
         SlimefunTasks.openOrSwitch(
            new SlimefunChoiceScreen<>(
                  ...
                  ct -> new ExecutableWidget(0, 0, 16, 16).eV(SlotElement.aI(ct.amY()).cF(KalamaHelperHelperP.au(t -> this.adY(ct)))),
      ...
      if (!this.handleNotEnable()) {
         SlimefunTasks.openOrSwitch(
            new SlimefunChoiceScreen<>(
                  ...
                  entry -> new ExecutableWidget(0, 0, 16, 16).eV(SlotElement.instance(entry.Ct().copyWithCount(1), (item, button) -> {
                     if (button == 0) {
                        this.adX(entry);
                        return true;
                     } else if (button == 1) {
