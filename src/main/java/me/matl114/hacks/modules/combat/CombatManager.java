...
public class CombatManager extends BaseModule {
    ...
    public void onUpdatePlayerPosition() {
        ...
        this.Kq.entrySet()
                .removeIf(re -> Math.abs(re.getKey().getX() - var1.getX()) > 1
                        || Math.abs(re.getKey().getZ() - var1.getZ()) > 1
                        || Math.abs(re.getKey().getY() - var1.getY()) > 1);
        this.Kv.removeIf(re -> Math.abs(re.getX() - var1.getX()) > 1
                || Math.abs(re.getZ() - var1.getZ()) > 1
                || Math.abs(re.getY() - var1.getY()) > 1);

        for (int var2 = -1; var2 <= 1; var2++) {
            for (int var3 = -1; var3 <= 1; var3++) {
                for (int var4 = -1; var4 <= 1; var4++) {
                    ChunkSectionPos var5 =
                            ChunkSectionPos.from(var1.getX() + var2, var1.getY() + var3, var1.getZ() + var4);
    ...
    private synchronized void updateTrackedMaps(
            Map<ChunkSectionPos, CombatSubHelperAa> updateMap, boolean trust, CombatSubHelperMX service) {
