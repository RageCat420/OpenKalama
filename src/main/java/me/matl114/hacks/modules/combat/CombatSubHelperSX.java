...
public class CombatSubHelperSX {
   ...
   protected void antiSpear(OptionalPrimitive<Double> op) {
      ...
      if (var2.lengthSquared() < 0.01) {
         double var3 = 3.0;
         if (!(MinecraftClient.getInstance().player.getPos().subtract(this.t.uI.getPos()).horizontalLength() < var3)) {
            return;
         }
   ...
   protected boolean willUseAntiSpear(OptionalPrimitive<Double> op, Vec3d predictorPos) {
      return op.isPresent()
         ...
         && this.t.DC()
         && MinecraftClient.getInstance().player.getPos().squaredDistanceTo(predictorPos) < MathUtils.a(this.t.combatSpearRange.get());
   }
   ...
   public Entity searchTarget() {
      return CombatTasks.l().akK(this.t.range.get(), true, this.t.playerOnly.get() ? e -> e instanceof PlayerEntity : null);
   }
}
