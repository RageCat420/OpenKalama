...
public class TotemLog extends BaseModule {
   ...
   public TotemLog() {
      ...
      this.logTotalCount = this.builder(this.hg.add("log-total-count"), Boolean.class).defaultValue(true).build();
      this.logDeath = this.builder(this.hg.add("log-death"), Boolean.class).defaultValue(true).build();
   ...
   public void onTotemPop(Event<PlayerEntity> event) {
      if (this.logTotem.get() && this.logDeath.get()) {
         ...
         if (var2 != null) {
            int var3 = var2;
            this.logSub("Totem", this.logDeathFormat.get().formatText(((PlayerEntity)event.b).getNameForScoreboard(), var3));
         }
      }
   }
