...
public class TargetSelector extends BaseModule {
    ...
    public final FlagRef bV =
            this.flagBuilder(this.Ju.add("use-grim-expand-eye-height")).build();
    ...
    private boolean isSameTeam(PlayerEntity pl, EquipmentSlot slot) {
        ...
        if (var3.contains(DataComponentTypes.DYED_COLOR)) {
            ...
            if (var4.contains(DataComponentTypes.DYED_COLOR)
                    && Objects.equals(
                            var3.get(DataComponentTypes.DYED_COLOR), var4.get(DataComponentTypes.DYED_COLOR))) {
                return true;
            }
        }
    ...
    public TargetSelector() {
        ...
        this.friends = this.builder(this.Ju.add("friends"), Regex.class)
        ...
        this.sendOnAddFriend = this.builder(this.Ju.add("send-on-add-friend"), OptionalPrimitive.type(String.class))
                .defaultValue(
                        new OptionalPrimitive<>(false, NBTTypes.g, "/msg %s I have just added you to my friends list!"))
                .build();
        this.attFriend = this.builder(this.Ju.add("att-friend"), FlagRef.TYPE)
                .defaultValue(true)
                .build();
        this.attFriendHotkey = this.moduleEntry(
                        this.Ju.add("att-friend-hotkey"), new MultiKeyBind(), this.Ju.add("att-friend"))
                .build();
        this.attNamed = this.builder(this.Ju.add("att-named"), Boolean.class)
                .defaultValue(true)
                .build();
        this.attTeammate = this.builder(this.Ju.add("att-teammate"), Boolean.class)
                .defaultValue(true)
                .build();
        this.attHostile = this.builder(this.Ju.add("att-hostile"), Boolean.class)
                .defaultValue(true)
                .build();
        this.attInvulnerable = this.flagBuilder(this.Ju.add("att-invulnerable")).build();
        this.oppositeAttackMultiply =
                this.flagBuilder(this.Ju.add("opposite-attack-multiply")).build();
        this.playerAttackMultiply = this.builder(this.Ju.add("player-attack-multiply"), DoubleRef.TYPE)
                ...
                .build();
        this.fakePlayerAndNpcDetect =
                this.flagBuilder(this.Ju.add("fake-player-and-npc-detect")).build();
    ...
    public void akt() {
        ...
        NBTRef<PrimitivePairList<String, String>> var2 = new NBTRef<>(new PrimitivePairList<String, String>(
                "widget.friend-list.friend-name", "widget.friend-list.friend-alias", NBTTypes.g, NBTTypes.g, var1));
    ...
    public DoubleStream getPotentialEyeHeights() {
        if (this.bV.get()) {
            ...
            return !mc.player.isFallFlying() && !mc.player.isUsingRiptide() && !mc.player.isSwimming()
                    ? DoubleStream.concat(
                            Arrays.stream(cd).map(s -> s * var1), DoubleStream.of(mc.player.dimensions.eyeHeight()))
                    : DoubleStream.concat(
                            Arrays.stream(cc).map(s -> s * var1), DoubleStream.of(mc.player.dimensions.eyeHeight()));
        } else {
    ...
    public boolean isTargetInRange(Entity e, double nearby, int ticks) {
        if (mc.player == null) {
            ...
            Vec3d var5 =
                    mc.player.getPos().add(mc.player.getVelocity().multiply(mc.player.isFallFlying() ? ticks : 0.0));
    ...
    public Entity searchAttack(
        ...
        if (mc.player == null) {
            ...
            if (var8 != null
                    && var8.getType() == Type.ENTITY
                    && combinedPredicate.test(((EntityHitResult) var8).getEntity())) {
    ...
    public void addCustomWidgets(Consumer<DrawableWidget> acceptor, int dx, int dy, int dblank) {
        acceptor.accept(this.createExecuteButton(
                "widget.friend-list.edit-friend-list", ButtonAction.a(this::akt), 0, dblank, dx, dy));
    ...
    private double getEntityWeight(Entity e, PlayerEntity player) {
        ...
        if (this.oppositeAttackMultiply.get()) {
            if (var6 >= 0.0) {
                return -(var6 / (e.getPos().subtract(var3).horizontalLength() + 1.0E-10)
                        + (e instanceof PlayerEntity ? this.playerAttackMultiply.get() : 0.0));
            } else {
                double var8 = var6 / (var5.length() * var4.length() + 1.0E-10);
                return -(var8 + (e instanceof PlayerEntity ? this.playerAttackMultiply.get() : 0.0));
            }
        } else {
            return -(Math.abs(var6 / (e.getPos().subtract(var3).horizontalLength() + 1.0E-10))
                    + (e instanceof PlayerEntity ? this.playerAttackMultiply.get() : 0.0));
        }
    }
    ...
    private void akS(MainCommand mainCommand) {
        ...
        var2.subBuilder(SubCommand.bp())
                .u("friends")
                .z(m -> m.<KalamaHelperHelperD, TaskSubCommand>subBuilder(SubCommand.bo())
                        .u("list")
            ...
            if (var6 != null && var6.getType() == Type.ENTITY) {
                ...
                if (var3.isEmpty()) {
                    ...
                    var3.sort(Comparator.comparingDouble(
                            e -> -e.getEyePos().subtract(var4).normalize().dotProduct(var5)));
    ...
    public Optional<Vec3d> getBestAttackEyePos(Vec3d pos, Box box, double range) {
        if (box.squaredMagnitude(pos) > MathUtils.a(range + 3.0 + mc.player.dimensions.eyeHeight())) {
            ...
            List<Vec3d> var5 = this.getPotentialEyeHeights()
                    .mapToObj(s -> pos.add(0.0, s, 0.0))
                    .toList();
    ...
    public Vec3d akn(Vec3d pos, Box box) {
        List<Vec3d> var3 = this.getPotentialEyeHeights()
                .mapToObj(s -> pos.add(0.0, s, 0.0))
                .toList();
