...
public abstract class CombatSubHelperOX extends CombatSubHelperSX implements CombatSubHelperG {
    ...
    public int onStateNone(StateMachine machine) {
        if (this.t.uI != null) {
            return PlayerStateManager.INSTANCE.jh > 4.0
                            && MinecraftClient.getInstance().player.getPos().getY()
                                    > this.t.uI.getPos().getY() + 4.0
                    ? 2
                    : 1;
        } else {
            this.u = Vec3d.ZERO;
    ...
    public boolean H() {
        ...
        if (var1) {
            ...
            return this.t.maceCombatConsiderUse.get() && CombatTasks.o().aaq()
                    ? false
                    : !Attack.INSTANCE.willUseMaceAttack(false);
        } else {
            return false;
        }
    }
        ...
        if (!MinecraftClient.getInstance().player.isFallFlying()
            ...
            if (this.attackFlag) {
                if (this.t.uI != null) {
                    ...
                    boolean var2 = this.J();
                    if (var1) {
                        CombatSubHelperPX var3 =
                                CombatTasks.n().createAttackSettings().yX(false);
                        if (this.F()) {
                            var3 = var3.za(true);
                        }
    ...
    public boolean I() {
        boolean var1 = this.r < Tasks.b() || Attack.INSTANCE.willUseMaceAttack(false);
        return var1
                && PlayerStateManager.INSTANCE.jh
                        > this.t.maceMinFallDistance.get().orElse(1.5);
    }
    ...
    public void onElytra(Event<KalamaHelperHelperI<FlightVelocity>> event) {
        if (this.u != null && this.u.lengthSquared() > 1.0E-9) {
            ...
            ((FlightVelocity) ((KalamaHelperHelperI) event.b).b())
                    .velocity(var2.normalize().multiply(var6));
        }
    }
        ...
        if (this.t.uQ != CombatSubHelperV.Rp && this.t.uQ != CombatSubHelperV.Ro) {
            ...
            Vec3d var2 = this.t.macePullUpUsePredictor.get()
                    ? PositionPredict.INSTANCE
                            ...
                            .get()
                            .predict(this.t.uI)
                            .withAxis(Axis.Y, this.t.uI.getY())
                    : this.t.uI.getPos();
