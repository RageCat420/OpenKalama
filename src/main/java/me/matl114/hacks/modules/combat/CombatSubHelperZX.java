...
class CombatSubHelperZX implements HackUtilHelperJ {
    ...
    public void gz(Event<LegalMovementManager> movementManagerEvent) {
        if (this.mE) {
            ...
            if (!var2.isFallFlying()) {
                PlayerInputUtils.a(MinecraftClient.getInstance().player)
                        .rD(false)
                        .applyInput(MinecraftClient.getInstance().player);
                        ...
                        if (elytraExtra.afV()) {
                            MinecraftClient.getInstance()
                                    ...
                                    .sendPacket(new ClientCommandC2SPacket(
                                            MinecraftClient.getInstance().player, Mode.START_FALL_FLYING));
                            elytraExtra.afN(elytraExtra.PI);
                            elytraExtra.PH = -1;
                            EntityInternalAccess.of(MinecraftClient.getInstance().player)
                                    .setDataFlag(7, true);
                        }
                    });
                } else if (this.yN != -1) {
    ...
    public void bb(Event<LegalMovementManager> movementManagerEvent) {
        ...
        if (this.yG && var2.isFallFlying()) {
            ...
            Vec3d var7 = TargetSelector.INSTANCE.akn(
                    MinecraftClient.getInstance().player.getPos(), this.yJ);
            if (this.yG || var2.isFallFlying()) {
            ...
            if (this.yO.tpReach.get().positive() && this.yJ.squaredMagnitude(var7) > MathUtils.a(this.mF)) {
                Vec3d var5 = MovTasks.tpAttackSearch(var8, this.et.getBoundingBox(), this.mF, 9.9, 1).stream()
                        .findFirst()
                        .orElse(null);
            ...
            if (this.mD) {
                Vec3d var9 = this.yJ
                        .getCenter()
                        .add(
                                this.yO.DO.nextDouble(-0.05, 0.05),
                                this.yO.DO.nextDouble(-0.05, 0.05),
                                this.yO.DO.nextDouble(-0.05, 0.05));
                ...
                if (RenderTasks.f) {
                    RenderTasks.i(
                            new KalamaHelperHelperCX(RenderTasks.DEBUG_TICK, new KalamaHelperHelperBX(var7, var6)));
                }
