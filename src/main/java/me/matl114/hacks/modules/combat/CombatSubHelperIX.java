...
public class CombatSubHelperIX extends CombatSubHelperSX implements CombatSubHelperG {
   ...
   int f;
   StateMachine stateMachine = new StateMachine(0, this::onStateUpdate, this::onStateNone, this::onStateFollow, this::onStateNearFollow, this::onStatePullOver);
   ...
   private boolean x(Vec3d targetPosition) {
      ...
      return var2.isPresent() && MinecraftClient.getInstance().player.getEyePos().squaredDistanceTo(targetPosition) < MathUtils.a((Double)var2.getValue());
   }
   ...
   public int onStateNearFollow(StateMachine machine) {
      if (this.x(this.getTargetPosition())) {
         return 3;
      } else if ((this.t.uR && this.t.uQ == CombatSubHelperV.Rn || this.t.uQ == CombatSubHelperV.Rm || this.t.uQ == CombatSubHelperV.Rp)
         ...
         if (MinecraftClient.getInstance().player.getEyePos().squaredDistanceTo(var2) > MathUtils.a(this.v())) {
            ...
            if (this.canAdjustMovement() && this.adjustMovementForSpear((PlayerEntity)(Object)this.t.uI, var3, false)) {
               return 2;
            } else {
   ...
   private boolean adjustMovementForSpear(PlayerEntity otherShit, Vec3d originalLook, boolean expand) {
      if (otherShit.squaredDistanceTo(MinecraftClient.getInstance().player.getPos()) < MathUtils.a(this.v() * 2.0 + this.t.spearAntiSpearExtraDistance.get())) {
         return Tasks.b() % 5 < 2 ? this.moveAdjust(originalLook) : this.q(originalLook);
      } else {
         Box var4 = expand ? MinecraftClient.getInstance().player.getBoundingBox().expand(0.85, 0.85, 0.85) : MinecraftClient.getInstance().player.getBoundingBox();
         ...
         Debug.g("Spear judgement", var5, var6, MinecraftClient.getInstance().player.getPos());
   ...
   public int onStatePullOver(StateMachine machine) {
      ...
      if (this.h && var2.isPresent()) {
         ...
         if (this.t.uQ != null) {
            if (this.t.uQ == CombatSubHelperV.Rp || this.t.uQ == CombatSubHelperV.Ro || this.t.uR && this.t.uQ != CombatSubHelperV.Rl) {
               double var11 = this.y();
   ...
   private boolean moveAdjust(Vec3d originalLook) {
      ...
      Vec3d var7 = MovTasks.simulateMovement(MinecraftClient.getInstance().player, MinecraftClient.getInstance().player.getPos(), var6, false);
      if (var7.squaredDistanceTo(var6) < 0.1) {
         this.u = var5;
         Debug.g("JudgeA", this.u);
         RenderTasks.g(MinecraftClient.getInstance().player.getBoundingBox(), var5.normalize().multiply(1.7), 50, Color.BLUE);
         return true;
      } else {
         var5 = var5.negate();
         var6 = var6.negate();
         var7 = MovTasks.simulateMovement(MinecraftClient.getInstance().player, MinecraftClient.getInstance().player.getPos(), var6, false);
         if (var7.squaredDistanceTo(var6) < 0.1) {
            this.u = var5;
   ...
   public int onStateNone(StateMachine machine) {
      if (this.t.uI != null) {
         if (!VItem.w().b(MinecraftClient.getInstance().player.getMainHandStack()) && !VItem.w().b(MinecraftClient.getInstance().player.getOffHandStack())) {
   ...
   private double y() {
      ...
      return var1.isPresent() ? Math.max(this.t.spearPullOverDistance.get(), (Double)var1.getValue()) : this.t.spearPullOverDistance.get();
   }
   ...
   private Vec3d getTargetPosition() {
      ...
      return this.t.uS ? this.t.uI.getEyePos().add(var2) : this.t.uI.getBoundingBox().getCenter().add(var2);
   }
}
