...
public class ElytraBot extends BaseModule {
    ...
    public void x(Event<Void> event) {
        ...
        if (!checkNull()) {
            if (this.uJ != null) {
                this.uU = mc.world.getDimension().hasCeiling()
                        && mc.player.getY()
    ...
    public void DM(Event<VDrawContext> eventVDraw) {
        if (this.enable.get() && this.render.get() && this.uJ != null && this.uI != null) {
            ...
            var2.A(
                    mc.textRenderer,
                    "Action: %s, Combating: %s".formatted(this.uQ, String.valueOf(this.uR)),
                    0,
                    0,
                    -1,
                    true);
    ...
    public void De() {
        if (this.skipFollow.get()) {
            this.mode.set(
                    this.mode.get() == ElytraBot$Mode.MACE_ARUA ? ElytraBot$Mode.SPEAR_ARUA : ElytraBot$Mode.MACE_ARUA);
        } else {
            this.mode.next();
        }

        this.logI18N(
                "message.module.elytra-bot.mode-switch",
                new Object[] {this.mode.get().resultAsString()});
    }

    public boolean DA() {
        return this.enable.get()
                && this.autoControlElytra.get()
                && this.uJ != null
                && this.uJ.u != null
                && this.uJ.u.lengthSquared() > 1.0E-9;
    }
    ...
    public boolean DH() {
        return this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.SPEAR_ARUA})
                || this.mode.get() == ElytraBot$Mode.MXS && !this.uO.V();
    }
    ...
    public void dd() {
        if (!EntityUtils.isEntityValid(this.uI)
                || this.uI.getPos().squaredDistanceTo(mc.player.getPos()) > this.range.get()) {
            this.uI = null;
        }
    ...
    public void jt(Event<EntityDamageS2CPacket> e) {
        if (!checkNull()) {
            if (this.uJ instanceof CombatSubHelperG var3
                ...
                RegistryKey var4 = (RegistryKey)
                        ((EntityDamageS2CPacket) e.b).sourceType().getKey().orElse(null);
        ...
        if (this.uI != null) {
            ...
            if (this.uI instanceof PlayerEntity var4) {
                if (this.uS) {
                    ...
                    List var30 = ((PlayerInternalAccess) (Object) this.uI)
                            .getPredictorImpl()
                            .getLastKnownPositions(3);
                    if (var30.size() < 2) {
    ...
    public void DO(Event<PlayerInteractEntityC2SPacket> attack) {
        if (this.enable.get()
                && this.uJ != null
                && PlayerInteractEntityC2SPacketAccess.of((PlayerInteractEntityC2SPacket) attack.b)
                        .isAttack()) {
            Entity var2 = mc.world.getEntityById(
                    PlayerInteractEntityC2SPacketAccess.of((PlayerInteractEntityC2SPacket) attack.b)
                            .getEntityId());
            if (var2 != null) {
                this.uJ.f(var2);
    ...
    public boolean DG() {
        return this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.MACE_ARUA})
                || this.mode.get() == ElytraBot$Mode.MXS && this.uO.V();
    }
    ...
    public double DE() {
        return this.uI instanceof PlayerEntity var2 && SpearEnhance.isUsingSpear(var2)
                ? this.combatSpearRange.get()
                : this.combatRange.get();
    }
                ...
                if (this.uJ != null) {
                    ...
                    if (var3 != null) {
                        RenderUtils.drawOutlinedBox(
                                var2, var3.add(RenderTasks.l), var3.add(RenderTasks.n), Color.MAGENTA);
                    }
                }
            } finally {
    ...
    public void addCustomWidgets(Consumer<DrawableWidget> acceptor, int dx, int dy, int dblank) {
        ...
        acceptor.accept(new KalamaHelperHelperJ<>(
                () -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.MACE_ARUA}) ? var5 : null, 0, 0));
    }

    public CombatSubHelperSX DD() {
        if (this.enable.get()) {
            return (CombatSubHelperSX)
                    (switch ((ElytraBot$Mode) this.mode.get()) {
                        case FOLLOW -> this.uK;
    ...
    public ElytraBot() {
        super("ElytraBot");
        this.mode = this.builder(this.tL.add("mode"), ElytraBot$Mode.class)
                .defaultValue(ElytraBot$Mode.FOLLOW)
                .build();
        this.enable = this.flagBuilder(this.tL.add("enable")).build();
        this.hotkey = this.moduleEntry(
                        this.tL.add("hotkey"), new MultiKeyBind(), this.tL.add("enable"), moduleMeta(() -> this.mode))
                .build();
        this.range = this.intBuilder(this.tL.add("range"))
                .defaultValue(80)
                .validator(Configs.e)
                .build();
        this.playerOnly = this.builder(this.tL.add("player-only"), FlagRef.TYPE)
                .defaultValue(true)
                .build();
        this.autoControlElytra =
                this.flagBuilder(this.tL.add("auto-control-elytra")).build();
        ...
        this.combatRange = this.builder(this.tL.add("combat-range"), DoubleRef.TYPE)
                .defaultValue(10.0)
                .build();
        this.combatSpearRange = this.builder(this.tL.add("combat-spear-range"), DoubleRef.TYPE)
                .defaultValue(11.0)
                .build();
        this.followerFollowFriend = this.flagBuilder(this.tL.add("follower-follow-friend"))
                ...
                .build();
        this.followOnGroundHeightExtra = this.builder(
                        this.tL.add("follow-on-ground-height-extra"), OptionalPrimitive.DOUBLE_TYPE)
        ...
        this.macePullUpUsePredictor = this.flagBuilder(this.tL.add("mace-pull-up-use-predictor"))
                .show(() -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.MACE_ARUA}))
                .build();
        this.maceCustomPullUpAngle = this.builder(
                        this.tL.add("mace-custom-pull-up-angle"), OptionalPrimitive.DOUBLE_TYPE)
        ...
        this.combatAngleOptimizeRange = this.builder(this.tL.add("combat-angle-optimize-range"), LabelVec3.class)
                ...
                .show(() -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.MACE_ARUA})
                        ...
                        && ElytraFlight.INSTANCE.useAutoRescale.get())
                .build();
        this.combatAngleOptimizeRadicalFollow = this.builder(
                        this.tL.add("combat-angle-optimize-radical-follow"), OptionalPrimitive.DOUBLE_TYPE)
                ...
                .show(() -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.MACE_ARUA})
                        ...
                        && ElytraFlight.INSTANCE.useAutoRescale.get())
                .build();
        this.combatPullUpAngleOptimize = this.builder(
                        this.tL.add("combat-pull-up-angle-optimize"), OptionalPrimitive.type(Vec2.class))
                ...
                .show(() -> SlimefunHelper.DEV_ENV
                        ...
                        && ElytraFlight.INSTANCE.useAutoRescale.get())
                .build();
        this.combatMaceChaseFollowYBias = this.builder(
                        this.tL.add("combat-mace-chase-follow-y-bias"), OptionalPrimitive.DOUBLE_TYPE)
                .show(() -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.MACE_ARUA}))
        ...
        this.flyAntiSpear = this.builder(this.tL.add("fly-anti-spear"), OptionalPrimitive.DOUBLE_TYPE)
                ...
                .show(() -> this.mode.get().isNotIn(new ConfigEnum[] {ElytraBot$Mode.SPEAR_ARUA}))
                .build();
        this.logSpearHit = this.flagBuilder(this.tL.add("log-spear-hit"))
        ...
        this.spearNoEntryDistance = this.builder(this.tL.add("spear-no-entry-distance"), OptionalPrimitive.DOUBLE_TYPE)
                ...
                .show(() -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.SPEAR_ARUA}))
                .build();
        this.spearPullBackDistance = this.builder(
                        this.tL.add("spear-pull-back-distance"), OptionalPrimitive.DOUBLE_TYPE)
        ...
        this.spearPullOverDistance = this.doubleBuilder(this.tL.add("spear-pull-over-distance"))
                ...
                .show(() -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.SPEAR_ARUA}))
                .build();
        this.flyAntiSpearWhenPullUp = this.builder(
                        this.tL.add("fly-anti-spear-when-pull-up"), OptionalPrimitive.DOUBLE_TYPE)
                .defaultValue(new OptionalPrimitive<>(false, NBTTypes.e, 1.0))
                .show(() -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.MACE_ARUA}))
                .build();
        this.flyAntiSpearUseSpearResetWhenFollow = this.flagBuilder(
                        this.tL.add("fly-anti-spear-use-spear-reset-when-follow"))
                .show(() -> this.mode.get().isIn(new ConfigEnum[] {ElytraBot$Mode.MACE_ARUA}))
                .build();
        this.render = this.flagBuilder(this.tL.add("render")).build();
        this.switchHotkey = this.hotkey(this.tL.add("switch-hotkey"))
                .defaultValue(new MultiKeyBind())
                .registerHotkey(HotKeyUtils.b(this::De))
                .build();
        this.skipFollow =
                this.flagBuilder(this.tL.add("skip-follow")).defaultValue(false).build();
