...
public class AutoTotem extends BaseModule {
      ...
      if (!checkNull()) {
         if (this.ae.get()
            ...
            for (int var6 = var5.size() - 1; var6 >= 0; var6--) {
               if (var6 != var3
                  && (((Slot)var5.get(var6)).inventory instanceof PlayerInventory || var4 == mc.player.playerScreenHandler)
   ...
   public void onTotemLazy() {
      if (this.Dk.a(this.totemSwapCooldown.get())) {
         if (!this.canBeAccepted(mc.player.getOffHandStack())) {
            ...
            int var1 = this.smartAutoTotem.get() && mc.player.getMainHandStack().isEmpty() && !mc.player.getOffHandStack().isEmpty()
            ...
            for (int var4 = 0; var4 < var3.size(); var4++) {
               if ((((Slot)var3.get(var4)).inventory instanceof PlayerInventory || var2 == mc.player.playerScreenHandler)
   ...
   public AutoTotem() {
      super("AutoTotem");
      this.autoTotemHotkey = this.toggleHotkey(this.hg.add("auto-totem-hotkey"), new MultiKeyBind(), this.hg.add("auto-totem")).build();
      this.autoTotemLog = this.builder(this.hg.add("auto-totem-log"), Boolean.class).defaultValue(true).build();
      this.totemSwapCooldown = this.builder(this.hg.add("totem-swap-cooldown"), IntRef.TYPE).defaultValue(1).build();
      this.smartAutoTotem = this.flagBuilder(this.hg.add("smart-auto-totem")).build();
      this.Df = this.builder(this.hg.add("enable-hand-items"), EntrySet.<Item>parameter()).defaultValue(new EntrySet<Item>(new Regex("^()$"), Registries.ITEM)).build();
