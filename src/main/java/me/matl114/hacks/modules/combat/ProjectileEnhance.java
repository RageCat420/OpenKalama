...
public class ProjectileEnhance extends BaseModule {
    ...
    public void onPlayerInteractItem(Event<PlayerInteractItemC2SPacket> packetMutableObject) {
        if (!packetMutableObject.d()) {
            if (this.ae.get()) {
                ...
                ItemStack var4 = PlayerInteractItemC2SPacketAccess.of(
                                (PlayerInteractItemC2SPacket) packetMutableObject.b)
                        .getItemStack();
                if (var4 != null && !var4.isEmpty()) {
                    if (this.aimEnable.get()) {
                        ...
                        if (var5) {
                            ...
                            if (var7 != null) {
                                Debug.b(Text.literal("[Proj Aim] Aim at %s"
                                                .formatted(var7 instanceof PlayerEntity var8 ? "player " : "entity "))
                                        .append(EntityUtils.getEntityDisplayable(var7))
                                        .formatted(Formatting.GREEN));
                                Vec3d var29 = CombatTasks.m()
                                        .predictAimPositionForEntity(var7, var6)
                                        .subtract(mc.player.getEyePos());
                                Vec2f var9 = CombatTasks.calculatePitchYawPredict(var6, Vec3d.ZERO, var29);
                                if (!Float.isNaN(var9.x)
                                        && !Float.isInfinite(var9.x)
                                        && !Float.isNaN(var9.y)
                                        && !Float.isInfinite(var9.y)) {
                    ...
                    if (this.tpEnable.get()
                        ...
                        KalamaHelperHelperFX var16 = new KalamaHelperHelperFX(
                                mc.player, var15, var15.add(var13.multiply(var10 + 1.0)), true);
                        double var17 = var10;
                            ...
                            if (!(var17 > 10.0)) {
                                ...
                                while (true) {
                                    ...
                                    if (MovTasks.validMovementAsServer(var20, var21)) {
                                        Vec3d var22 = var16.simulateMovement(
                                                mc.player, var15.add(var21), new Vec3d(0.0, var33.y, 0.0));
                        ...
                        if (var14.lengthSquared() > 1.0E-4) {
                            ...
                            if (!var31.isEmpty()) {
                                Debug.b(Text.literal(
                                                "[Proj TP] Projectile Velocity Simulate %.2f".formatted(var14.length()))
                                        .formatted(Formatting.GREEN));
                                ...
                                for (int var34 = 0; var34 < var25; var34++) {
                                    var24.add(
                                            var34 == 0
                                                    ? MovTasks$MovInfo.adA((Vec3d) var31.get(var34))
                                                    : MovTasks$MovInfo.adz((Vec3d) var31.get(var34)));
                                }

                                var24.add(MovTasks$MovInfo.adz(var15.add(0.0, 9.0E-8, 0.0)));
                                MovTasks.scheduleFarawayMoveInternal(
                                        var24, false, KalamaHelperHelperIX.create(var15), true);
                                MovTasks.Y();
                                break label79;
                            }
                        }
    ...
    public ProjectileEnhance() {
        ...
        this.projectileEnhanceHotkey = this.toggleHotkey(
                        this.RY.add("projectile-enhance-hotkey"), new MultiKeyBind(), this.RY.add("projectile-enhance"))
                .build();
        ...
        this.targetingMode = this.builder(this.RY.add("targeting-mode"), Configs$LegalInteractMode.class)
                ...
                .build();
        this.tpAccelerate = this.builder(this.RY.add("tp-accelerate"), DoubleRef.TYPE)
                .defaultValue(150.0)
                .show(this.tpEnable::get)
                .build();
        this.tpAccelerateExactTp = this.flagBuilder(this.RY.add("tp-accelerate-exact-tp"))
                .show(this.tpEnable::get)
                .build();
        this.tpAccelerateExactTp2 = this.builder(this.RY.add("tp-accelerate-exact-tp"), Regex.class)
                .defaultValue(new Regex("^(LOGITECH_LASER_GUN)$"))
                .build();
        this.tridentAutoDupe =
                this.flagBuilder(this.RY.add("trident-auto-dupe")).build();
