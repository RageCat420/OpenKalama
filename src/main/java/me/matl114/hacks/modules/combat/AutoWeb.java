...
public class AutoWeb extends BaseModule {
    ...
    private boolean ahb(PlayerEntity target, BlockPos pos) {
        if (!this.NK(pos)) {
            ...
            if (var3.isOf(Blocks.COBWEB)) {
                ...
                return !var3.isAir() && !var3.isLiquid() && !var3.isReplaceable()
                        ? false
                        : Qg.canPlaceAt(mc.world, pos);
            }
        }
    }
    ...
    private List<PlayerEntity> agY() {
        return TargetSelector.INSTANCE == null
                ? List.of()
                : TargetSelector.INSTANCE.akC(this.interactRange.get()).stream()
    ...
    public AutoWeb() {
        ...
        this.selfWebOnlySlow =
                this.flagBuilder(this.aD.add("self-web-only-slow")).build();
    ...
    private List<BlockPos> agZ(PlayerEntity target) {
        ...
        if (this.ceiling.get() && !target.isOnGround()) {
            ...
            MathUtils.getOccupiedBlockPositions(var3.stretch(0.0, 0.75, 0.0)).stream()
                    .filter(pos -> pos.getY() > var4)
                    .forEach(var2::add);
        }
        ...
        var2.stream()
                ...
                .sorted(Comparator.comparingDouble(this::ahd))
                .forEach(var5::add);
        return var5;
    }
    ...
    private FlagEntry<BlockHitResult> aha(PlayerEntity target, BlockPos pos) {
        if (!this.ahb(target, pos)) {
            ...
            FlagEntry var3 = InteractionTasks.q(
                    pos, Qg, this.airPlace.get(), !this.mode.get().isLegal());
    ...
    private boolean ahe() {
        return this.agZ(mc.player).stream()
