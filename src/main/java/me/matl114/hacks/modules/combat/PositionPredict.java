...
public class PositionPredict extends BaseModule {
   ...
   public Vec3d predictPlayerMove(PlayerInputUtils$Input input) {
      ...
      if (mc.player.isFallFlying()) {
         ...
         return mc.player.isInFluid() ? mc.player.getVelocity() : var2.calculateLastMoveVelocity(input.ro(), input.rp());
      }
   }
   ...
   public PositionPredict() {
      ...
      this.attackPredictArgument = this.builder(this.Ju.add("attack-predict-argument"), PositionPredict$PredictArgument.class)
         .defaultValue(new PositionPredict$PredictArgument(2.0, 5, PositionPredict$Mode.NO_PREDICT))
         .build();
      this.flyPredictArgument = this.builder(this.Ju.add("fly-predict-argument"), PositionPredict$PredictArgument.class)
         .defaultValue(new PositionPredict$PredictArgument(2.0, 5, PositionPredict$Mode.PREDICTOR_NV))
         .build();
      this.spearPredictArgument = this.builder(this.Ju.add("spear-predict-argument"), PositionPredict$PredictArgument.class)
         .defaultValue(new PositionPredict$PredictArgument(2.0, 5, PositionPredict$Mode.PREDICTOR_NV))
         .build();
      this.exactTpAntiShield = this.builder(this.KE.add("exact-tp-anti-shield"), Boolean.class).defaultValue(false).build();
      this.debugRenderPrediction = this.flagBuilder(this.Ju.add("debug-render-prediction")).build();
      this.Mw = new Int2ObjectArrayMap();
      this.placeRecorder = this.flagBuilder(this.KE.add("place-recorder")).updateListener(s -> this.Mw.clear()).build();
      this.placeRecorderHotkey = this.moduleEntry(this.KE.add("place-recorder-hotkey"), new MultiKeyBind(), this.KE.add("place-recorder")).build();
      INSTANCE = this;
   }
      ...
      if (this.debugRenderPrediction.get()) {
         ...
         try {
            ...
            VRender.getInstance().h((operation, vertexConsumer) -> {
               for (Box var5x : var2) {
                  operation.a((MatrixStack)event.b, vertexConsumer, var5x.getMinPos(), var5x.getMaxPos(), Color.MAGENTA.getRGB());
               }
            });
            ...
            while (var11.hasNext()) {
               ...
               int var7 = ColorUtils.j(Hashing.sha256().hashInt(var12.getIntKey()).hashCode(), 255);
               VRender.getInstance().h((operation, vertexConsumer) -> {
                  for (Vec3d var7x : var13) {
                     var7x = var7x.add(var3);
                     operation.a((MatrixStack)event.b, vertexConsumer, var7x.add(-0.2, -0.2, -0.2), var7x.add(0.2, 0.2, 0.2), var7);
                  }
               });
               VRender.getInstance()
                  .i(
                     (operation, vertexConsumer) -> operation.drawLines(
                        (MatrixStack)event.b, vertexConsumer, var13.stream().map(s -> s.add(var3)).toList(), var7
                     )
                  );
            }
         } finally {
   ...
   public boolean considerAntiShield(Entity target) {
      return this.exactTpAntiShield.get() && target instanceof LivingEntity var2 && var2.isUsingItem() && var2.getActiveItem().getItem() instanceof ShieldItem;
   }
   ...
   public void onPostEntityPos(Event<EntityPositionS2CPacket> event) {
      if (!checkNull()) {
         if (mc.world.getEntityById(((EntityPositionS2CPacket)event.b).getEntityId()) instanceof PlayerInternalAccess var3) {
   ...
   public Vec3d predictAimPositionForEntity(Entity entity, float finalVelocity) {
      ...
      return entity.getEyePos().subtract(entity.getPos()).multiply(0.75).add(this.flyPredictArgument.get().predictWithExtraTicks(entity, var6));
   }
}
