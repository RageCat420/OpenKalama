    ...
    public CombatLog() {
        ...
        this.logHitFormat = this.builder(this.yv.add("log-hit-format"), StringFormat.class)
                .defaultValue(new StringFormat(
                        ...
                        "{attacker} hit {target}, type: {damageType}",
                        true))
                .build();
        ...
        if (!checkNull()) {
            if (this.ae.get()
                ...
                this.logDamage("you", var3.getNameForScoreboard(), var7);
                    ...
                    && mc.world.getEntityById(((EntityDamageS2CPacket) e.b).sourceCauseId())
                            instanceof PlayerEntity var4
                    && this.logOtherHitMe.get()) {
                RegistryKey var6 = (RegistryKey)
                        ((EntityDamageS2CPacket) e.b).sourceType().getKey().orElse(null);
