...
public class AutoCity extends BaseModule {
   ...
   private void onMine() {
      ...
      Predicate<net.minecraft.util.math.BlockPos> var7 = np -> MathUtils.r(np).squaredMagnitude(var4) <= MathUtils.a(var5);
      var3.addAll(MathUtils.getOccupiedBlockPositions(var1).stream().sorted(Comparator.comparingInt(Vec3i::getY)).toList());
      Comparator<net.minecraft.util.math.BlockPos> var8 = Comparator.comparingDouble(v -> MathUtils.r(v).squaredMagnitude(var4));
      if (this.headTarget.get()) {
         Box var9 = var1.stretch(0.0, 0.75, 0.0);
         MathUtils.getOccupiedBlockPositions(var9).stream().filter(var7).sorted(var8).forEach(var2::add);
      }

      if (this.down.get()) {
         Box var24 = var1.stretch(0.0, -0.75, 0.0);
         MathUtils.getOccupiedBlockPositions(var24).stream().filter(var7).sorted(var8).forEach(var2::add);
      }
      ...
      if (this.surround.get()) {
         ...
         } else if (var1.maxY > var12.getY() + 1 && PacketMine.INSTANCE.isMineable(mc.world.getBlockState(var12.up()))) {
   ...
   public AutoCity() {
      ...
      this.hotkey = this.toggleHotkey(this.eb.add("hotkey"), new MultiKeyBind(), this.eb.add("enable")).build();
      this.burrowFirst = this.builder(this.eb.add("burrow-first"), Boolean.class).defaultValue(true).build();
      this.headTarget = this.flagBuilder(this.eb.add("head-target")).build();
      this.down = this.flagBuilder(this.eb.add("down")).build();
      this.surround = this.builder(this.eb.add("surround"), Boolean.class).defaultValue(true).build();
   ...
   public void refreshTarget() {
      ...
      if (!EntityUtils.isEntityValid(this.eh) || this.eh.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > MathUtils.a(var1)) {
         this.eh = null;
         this.ei = null;
      }

      if (this.eh == null && CombatTasks.l().akK(var1, true, pl -> pl instanceof PlayerEntity) instanceof PlayerEntity var4 && var4 != mc.player) {
         this.eh = var4;
      }
   }
