...
public class CombatSubHelperO extends CombatSubHelperSX {
   public synchronized void onUpdate() {
      super.h();
      if (this.t.uI != null) {
         this.u = this.t.uI.getPos().subtract(MinecraftClient.getInstance().player.getPos());
         if (this.t.uS) {
   ...
   public boolean canBeAttack(Entity entity) {
      return entity instanceof PlayerEntity var2
         ...
         ? true
         : TargetSelector.INSTANCE.canAttack(entity) && (!this.t.playerOnly.get() || entity instanceof PlayerEntity);
   }
