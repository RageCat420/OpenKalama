...
public record PositionPredict$PredictArgument(double ticksLater, int ticksHistory, PositionPredict$Mode mode)
    ...
    public static NBTType<PositionPredict$PredictArgument> TYPE = new NBTType<>(
            ...
            RecordCodecBuilder.create(s -> s.group(
                            Codec.withAlternative(Codec.DOUBLE, Codec.INT.xmap(t -> (double) t.intValue(), t ->
                                            (int) t.doubleValue()))
                                    .fieldOf("ticks")
                            ...
                            CodecUtils.enumCodec(PositionPredict$Mode.class)
                                    .fieldOf("mode")
                                    .forGetter(PositionPredict$PredictArgument::mode))
            ...
            (s, x, y, dx, dy) -> {
                ...
                WrapperFactory<Double, PositionPredict$PredictArgument> firstWrapper = WrapperFactory.of(
                        d -> ((PositionPredict$PredictArgument) s.getOriginValue()).withTicksLater(d),
                        PositionPredict$PredictArgument::ticksLater);
                WrapperFactory<Integer, PositionPredict$PredictArgument> secondWrapper = WrapperFactory.of(
                        d -> ((PositionPredict$PredictArgument) s.getOriginValue()).withTicksHistory(d),
                        PositionPredict$PredictArgument::ticksHistory);
                WrapperFactory<PositionPredict$Mode, PositionPredict$PredictArgument> thirdWrapper = WrapperFactory.of(
                        d -> ((PositionPredict$PredictArgument) s.getOriginValue()).withMode(d),
                        PositionPredict$PredictArgument::mode);
                return subScreenWidget
                        .Q(DisplayWidget.instance(0, 0, dy, dy)
                        ...
                        .Q(new TypeConvertAttrKeyValue<>(s, firstWrapper, NBTTypes.e)
                                .generateValueWidget(dy, 0, half - dy, dy))
                        .Q(DisplayWidget.instance(half, 0, dy, dy)
                                .setRenderHandler(new ButtonElement(
                                                TextProvider.c(Text.translatableWithFallback(
                        ...
                        .Q(new TypeConvertAttrKeyValue<>(s, secondWrapper, NBTTypes.c)
                                .generateValueWidget(half + dy, 0, half - dy, dy))
                        .Q(DisplayWidget.instance(2 * half, 0, dy, dy)
                                .setRenderHandler(new ButtonElement(
                                                TextProvider.c(Text.translatableWithFallback(
    ...
    public Vec3d predict0(Entity entity, double ticksLater) {
        ...
        Vec3d floorPos = EntityInternalAccess.of(entity)
                .getPositionPredictor()
                .predict(floor, this.mode.ordinal(), this.ticksHistory);
        if (Math.abs(floor - ticksLater) < 0.01) {
            return floorPos;
        } else {
            Vec3d roofPos = EntityInternalAccess.of(entity)
                    .getPositionPredictor()
                    .predict(floor + 1, this.mode.ordinal(), this.ticksHistory);
    ...
    public PositionPredict$PredictArgument withTicksLater(double ticksLater) {
        return this.ticksLater == ticksLater
                ? this
                : new PositionPredict$PredictArgument(ticksLater, this.ticksHistory, this.mode);
    }

    public PositionPredict$PredictArgument withTicksHistory(int ticksHistory) {
        return this.ticksHistory == ticksHistory
                ? this
                : new PositionPredict$PredictArgument(this.ticksLater, ticksHistory, this.mode);
    }
