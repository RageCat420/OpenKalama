...
public class Blink extends BaseModule {
        ...
        if (this.enable.get()) {
            ...
            if (this.onEnermyNear.get() != Blink$Action.NONE && this.wa != null) {
                ...
                for (Entity var7 : var3) {
                    if (var7.getBoundingBox().squaredMagnitude(var5) < MathUtils.a(this.enermyNearRange.get())
                            && TargetSelector.INSTANCE.canAttack(var7)) {
                        var2 = true;
                        break;
                    }
                }
    ...
    public Blink() {
        ...
        this.hotkey = this.toggleHotkey(this.vL.add("hotkey"), new MultiKeyBind(), this.vL.add("enable"))
                .build();
        this.revert = this.hotkey(this.vL.add("revert"))
                .defaultValue(new MultiKeyBind())
                .registerHotkey(HotKeyUtils.b(this::GU))
                .build();
        this.render = this.flagBuilder(this.vL.add("render")).build();
        this.closeOnDelay = this.flagBuilder(this.vL.add("close-on-delay")).build();
        this.closeDelay =
                this.intBuilder(this.vL.add("close-delay")).defaultValue(50).build();
        this.autoFlush = this.flagBuilder(this.vL.add("auto-flush")).build();
        this.autoFlushPeriod = this.intBuilder(this.vL.add("auto-flush-period"))
                .defaultValue(20)
                .build();
        this.attackBehaviour = this.builder(this.vL.add("attack-behaviour"), Blink$Action.class)
                .defaultValue(Blink$Action.FLUSH)
                .build();
        this.onHurtBehaviour = this.builder(this.vL.add("on-hurt-behaviour"), Blink$Action.class)
                .defaultValue(Blink$Action.NONE)
                .build();
        this.onVelocityBehaviour = this.builder(this.vL.add("on-velocity-behaviour"), Blink$Action.class)
                .defaultValue(Blink$Action.NONE)
                .build();
        this.onInventoryBehaviour = this.builder(this.vL.add("on-inventory-behaviour"), Blink$Action.class)
                .defaultValue(Blink$Action.NONE)
                .build();
        this.onTotemBehaviour = this.builder(this.vL.add("on-totem-behaviour"), Blink$Action.class)
                .defaultValue(Blink$Action.NONE)
                .build();
        this.onEnermyNear = this.builder(this.vL.add("on-enermy-near"), Blink$Action.class)
                .defaultValue(Blink$Action.NONE)
                .build();
        this.enermyNearRange = this.builder(this.vL.add("enermy-near-range"), DoubleRef.TYPE)
                .defaultValue(3.5)
                .build();
        this.onNearTarget = this.builder(this.vL.add("on-near-target"), Blink$Action.class)
                .defaultValue(Blink$Action.NONE)
                .build();
    ...
    public void onPacketHurt(Event<EntityDamageS2CPacket> damage) {
        if (this.enable.get()
                && mc.player != null
                && ((EntityDamageS2CPacket) damage.b).entityId() == mc.player.getId()) {
