...
public class SpearAttack extends BaseModule implements HackUtilHelperJ {
   ...
   public final FlagRef ae = this.builder(this.dI.add("spear-attack-enable"), Boolean.class).defaultValue(true).build();
   ...
   private void renderPlayerSpearTarget(Event<MatrixStack> event) {
      if (this.ae.get()) {
         ...
         if (this.renderTarget.get()) {
            ...
            try {
               if (this.iZ()) {
                  Entity var4 = CombatTasks.l().akK(this.spearMotionSimulation.get(), false, SpearAttack::isSpearable);
                  if (var4 != null) {
   ...
   public Pair<List<Vec3d>, List<Vec3d>> isValidTpLocation(KalamaHelperHelperMX context, Vec3d playerLocation, Vec3d tpLocation, double maxDistance) {
   ...
   public SpearAttack() {
      super("SpearAttack");
      this.spearAttackHotkey = this.hotkey(this.dI.add("spear-attack-hotkey")).defaultValue(new MultiKeyBind()).registerHotkey(HotKeyUtils.c(this::iY)).build();
      this.spearMotionSimulation = this.builder(this.dI.add("spear-motion-simulation"), DoubleRef.TYPE).defaultValue(50.0).build();
      this.spearMaxTp = this.builder(this.dI.add("spear-max-tp"), DoubleRef.TYPE).defaultValue(100.0).build();
      this.renderTarget = this.flagBuilder(this.dI.add("render-target")).build();
      this.spearServerTickDelay = this.intBuilder(this.dI.add("spear-server-tick-delay")).defaultValue(2).validator(Configs.e).build();
   ...
   public static boolean isSpearable(Entity entity) {
      if (mc.player.getEyePos().subtract(entity.getEyePos()).lengthSquared() <= MathUtils.a(2.0)) {
         ...
         BlockHitResult var1 = mc.world
            .raycast(new RaycastContext(mc.player.getEyePos(), entity.getEyePos(), ShapeType.COLLIDER, FluidHandling.NONE, mc.player));
   ...
   public Pair<List<Vec3d>, List<Vec3d>> findValidTpPosition(
      Vec3d currentPlayerPos, Vec3d tpDirection, Vec3d expandDirection, double maxDistance, double distance, double minDistance
   ) {
   ...
   public boolean spearAttack() {
      if (this.spearMotionSimulation.get() < 0.0) {
         ...
         if (var1 == null) {
            ...
            Vec3d var5 = var4.add(0.0, var1.getEyeHeight(var1.getPose()), 0.0).subtract(mc.player.getEyePos());
            if (RenderTasks.i) {
               RenderTasks.i(new KalamaHelperHelperCX(RenderTasks.DEBUG_TICK, new KalamaHelperHelperBX(mc.player.getEyePos(), var5).g(Color.MAGENTA)));
            }
            ...
            Pair var10 = this.findValidTpPosition(var2, var8, var9, this.spearMaxTp.get(), this.spearMotionSimulation.get(), var6);
            if (var10 != null && ((List)var10.getSecond()).size() == 2) {
               ...
               if (RenderTasks.i) {
                  RenderTasks.i(
                     new KalamaHelperHelperCX(
                        RenderTasks.DEBUG_TICK, new KalamaHelperHelperDX(var18.add(RenderTasks.l), var18.add(RenderTasks.n), Color.MAGENTA)
                     )
                  );
                  RenderTasks.i(new KalamaHelperHelperCX(RenderTasks.DEBUG_TICK, new KalamaHelperHelperSX(var18, Color.MAGENTA)));
               }
               ...
               Debug.b(Text.literal("[Spear] simulate delay %.2f".formatted(var2.subtract(var18).dotProduct(var5))).formatted(Formatting.GREEN));
