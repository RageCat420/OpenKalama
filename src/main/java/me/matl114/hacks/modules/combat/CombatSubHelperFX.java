...
public class CombatSubHelperFX extends CombatSubHelperOX implements CombatSubHelperG {
    ...
    protected void e(Vec3d targetPos) {
        ...
        } else if (!this.t.combatAngleOptimizeFollow.get()
                && this.t.combatSmoothFlight3.get()
                && targetPos.y <= MinecraftClient.getInstance().player.getY()) {
            Vec3d var17 = this.t.uI.dimensions.getBoxAt(targetPos).getCenter();
            double var4 = this.t.combatSmoothFlightArgument2.get();
            Pair var6 = MathUtils.C(
                    var17, var4, MinecraftClient.getInstance().player.getEyePos());
            ...
            if (var13 > var11) {
                ...
                if (var15 > 0.1
                        && (!this.t.combatAngleOptimizeRadicalFollow.get().isPresent()
                                || Math.abs(this.u.y)
                                        < this.u.horizontalLength()
                                                * Math.tan(Math.toRadians(this.t
                                                        .combatAngleOptimizeRadicalFollow
                                                        .get()
                                                        .getValue())))
    ...
    protected void d(Vec3d predictor) {
        ...
        if (this.t.combatSmoothFlight.get() && !var3) {
            ...
            if (var6) {
                ...
                Pair var12 = MathUtils.C(
                        var9, var10, MinecraftClient.getInstance().player.getEyePos());
                ...
                if (Math.abs(this.t.combatSmoothFlightArgument11.get()) > 1.0E-6
                        && (this.t.combatSmoothFlightArgument15.get()
                                        ? var9.subtract(MinecraftClient.getInstance()
                                                        .player
                                                        .getEyePos())
                                                .horizontalLengthSquared()
                                        : var9.squaredDistanceTo(MinecraftClient.getInstance()
                                                .player
                                                .getEyePos()))
                                < MathUtils.a(this.t.combatSmoothFlightArgument16.get())) {
                    var15 = var15.normalize();
                    Vec3d var16 =
                            MinecraftClient.getInstance().player.getEyePos().subtract(var9);
                    Vec3d var17 = new Vec3d(var16.x, 0.0, var16.z)
                            .normalize()
                            .multiply(this.t.combatSmoothFlightArgument11.get());
        ...
        if (var2 == null) {
            var2 = predictor
                    .withAxis(Axis.Y, predictor.getY() + this.t.maceHeight.get())
                    .subtract(MinecraftClient.getInstance().player.getPos());
            if (var2.length() < 5.0) {
    ...
    public int onStatePullUp(StateMachine machine) {
        ...
        Vec3d var3 = MovTasks.simulateMovement(
        ...
        boolean var5 = this.G();
        Vec3d var6 = this.t.macePullUpUsePredictor.get()
                ? PositionPredict.INSTANCE.attackPredictArgument.get().predict(this.t.uI)
                : this.t.uI.getPos();
        if (var5) {
            ...
            if (var9
                    && this.t.combatMaceChaseFollowYBias.get().isPresent()
                    && MinecraftClient.getInstance().player.getY() > this.t.uI.getY()
                    && !this.t.uR) {
    ...
    public int onStateFollow(StateMachine machine) {
        if (PlayerStateManager.INSTANCE.jh < 1.0E-6 && this.s > 1.0E-6) {
            ...
            if (var7
                    && (this.t.uS
                            || MinecraftClient.getInstance().player.getY() < var2.getY() + this.t.minAttackHeight.get())
                    && var8) {
                this.e(var2);
                this.M();
                machine.e();
                ...
                Vec3d var10 = MovTasks.simulateMovement(
                        MinecraftClient.getInstance().player,
                        MinecraftClient.getInstance().player.getPos(),
                        var9,
                        true);
                ...
                if (!var11
                        && this.t.uI.getY()
                                > MinecraftClient.getInstance().player.getY() + var13) {
                    var12 = true;
                }
