...
public class CombatExtra extends BaseModule {
   ...
   public final DoubleRef sg = this.doubleBuilder(this.sf.add("att-range")).defaultValue(0.0).build();
   ...
   public CombatExtra() {
      super("CombatExtra");
      this.boatReachRange = this.doubleBuilder(this.sf.add("boat-reach-range")).defaultValue(0.0).build();
      this.shieldingSetbackLog = this.flagBuilder(this.sf.add("shielding-setback-log")).build();
   ...
   public void onShieldSetback(Event<EntityTrackerUpdateS2CPacket> trackerUpdateS2CPacketEvent) {
      if (!trackerUpdateS2CPacketEvent.d()) {
         ...
         if (this.shieldingSetbackLog.get()
            ...
            && !mc.player.getItemCooldownManager().isCoolingDown(mc.player.getActiveItem().getItem())) {
   ...
   public double getAttackAtTargetRange(Entity entity) {
      double var2 = !this.ridingBypass(mc.player) && !this.ridingBypass(entity) ? this.sg.get() : this.boatReachRange.get();
