...
public class Attack extends BaseModule {
    ...
    public boolean Yj() {
        return this.willUseMaceAttack(
                this.maceSwap.get().isPresent() && this.maceSwap.get().getValue() <= PlayerStateManager.INSTANCE.jh);
    }
    ...
    private boolean processLegacySnapAttack(Entity target, CombatSubHelperPX settings) {
        ...
        boolean var6 =
                RaycastUtils.x(mc.player, PlayerStateManager.INSTANCE.jl, PlayerStateManager.INSTANCE.jm, target, var4);
        if (settings.yV() && var6) {
            ...
            if (this.tpReach.get().positive() && !var9) {
                Vec3d var10 = MovTasks.tpAttackSearch(var7, target.getBoundingBox(), var4, 9.9, 1).stream()
                        .findFirst()
                        .orElse(null);
    ...
    public static void Ye(PlayerEntity player, Entity target, CombatSubHelperPX attackSettings) {
        ...
        if (player.hasVehicle()) {
            ...
            if (var3.ru()) {
                PlayerInputUtils$Input var4 =
                        var3.rw().rx(false).ry(false).rz(false).rA(false);
        ...
        if (attackSettings.antiShieldSwap() && shouldUseAntiShield(target) && (var5 = Yb()) != null) {
                ...
                && (var5 = InventoryUtils.v(
                                ex -> {
                                    if (VItem.w().c(ex)) {
                                        ...
                                        return var3x == null
                                                ? null
                                                : -(var3x.intValue() * 1.0E8)
                                                        + KalamaHelperHelperB.i(player, var6, ex)
                                                                * KalamaHelperHelperB.e(player, ex);
                                    } else {
                                        return null;
                                    }
                                },
            ...
            var8 = InvExtra.INSTANCE.swapInventoryIndexToHand(var5.index());
                ...
                && (var5 = InventoryUtils.v(
                                ex -> ex.getItem() == Items.MACE ? KalamaHelperHelperB.k(var7, ex) : null,
                                false,
                                false))
                        != null) {
            var8 = InvExtra.INSTANCE.swapInventoryIndexToHand(var5.index());
    ...
    public boolean Yi(Entity target, CombatSubHelperPX settings) {
        return this.legalTargeting.get().isLegal()
                ? this.processLegalAttack(target, settings)
                : this.processIllegalAttack(target, settings);
    }
    ...
    private boolean processIllegalAttack(Entity target, CombatSubHelperPX settings) {
        ...
        if (var3 == null) {
            ...
            boolean var11 = settings.selectWeapon()
                    && this.exactTp.get()
                    && (!var10 || CombatTasks.m().considerAntiShield(target));
            boolean var12 = false;
            boolean var13 = true;
            boolean var14 = false;
            boolean var15 = false;
            ...
            if (var13) {
                ...
                List var33 = Streams.concat(new Stream[] {var7.stream(), var8.stream()})
                        .toList();
                ...
                if (var34 > 1) {
                    ...
                    if (Math.abs(var25 - var27)
                            > var3.getAttributeValue(EntityAttributes.GENERIC_SAFE_FALL_DISTANCE) - 1.0) {
            ...
            try {
                ...
                if (var5 != null) {
                    ...
                    RenderUtils.r(
                            ...
                            var7.getMinPos(),
                            var7.getMaxPos(),
                            ColorUtils.h(this.renderTargetColor.get().color(), var6));
                }
            } finally {
    ...
    private boolean processDelayMovementAttack(Entity target, CombatSubHelperPX settings) {
        ...
        if (settings.yV() && var9) {
            ...
            ClientPlayerAccess.of(mc.player)
                    ...
                    .i(new CombatSubHelperZX(
                            this, var6, var7, target, var4, settings, var8, var13, var11, var3, var10));
            return true;
        }
    }
        ...
        switch (var2) {
            ...
            case fe:
                if (this.tpReach.get().getValue() < 0.0) {
                    this.tpReach.set(
                            this.tpReach.get().withValue(-this.tpReach.get().getValue()));
                }
                break;
            default:
                if (this.tpReach.get().getValue() > 0.0) {
    ...
    public double getTpSelectRange() {
        return CombatTasks.j().getAttackRange()
                + (this.Sd() ? Math.max(0.0, this.tpReach.get().getValue()) : 0.0);
    }
    ...
    public Attack() {
        ...
        this.alwaysAttHotkey = this.toggleHotkey(
                        this.Ju.add("always-att-hotkey"), new MultiKeyBind(), this.Ju.add("always-att"))
                .build();
        this.legalTargeting = this.builder(this.Ju.add("legal-targeting"), Configs$LegalTargetingMode.class)
                ...
                .build();
        this.tpReach = this.builder(this.Ju.add("tp-reach"), OptionalPrimitive.DOUBLE_TYPE)
                .defaultValue(new OptionalPrimitive<>(true, NBTTypes.e, 0.0))
                .build();
        this.maceHeightMultiply = this.builder(this.Ju.add("mace-height-multiply"), OptionalPrimitive.DOUBLE_TYPE)
                ...
                .build();
        this.exactTp = this.flagBuilder(this.Ju.add("exact-tp"))
                .show(() -> !this.legalTargeting.get().isLegal()
                        && this.tpReach.get().isPresent())
                .build();
        this.useDelayMovementPosPredict = this.flagBuilder(this.Ju.add("use-delay-movement-pos-predict"))
                .show(() -> !this.legalTargeting.get().isLegal()
                        && this.legalTargeting.get().isIn(new ConfigEnum[] {Configs$LegalTargetingMode.DELAY_MOVEMENT}))
                .build();
        ...
        this.attackSelectBestWeapon =
                this.flagBuilder(this.Ju.add("attack-select-best-weapon")).build();
        this.autoHandleUseWhenAttack =
                this.flagBuilder(this.Ju.add("auto-handle-use-when-attack")).build();
        this.maceSwap = this.builder(this.Ju.add("mace-swap"), OptionalPrimitive.DOUBLE_TYPE)
                ...
                .build();
        this.swingHand = this.builder(this.Ju.add("swing-hand"), Boolean.class)
                .defaultValue(true)
                .build();
        this.renderTarget = this.flagBuilder(this.Ju.add("render-target")).build();
        this.renderTargetColor = this.builder(this.Ju.add("render-target-color"), WrapColor.class)
    ...
    private boolean processMaceAttack(
            ...
            CombatSubHelperPX attackSettings) {
        if (attackSettings.useAttack() && this.willUseMaceAttack(attackSettings.invSwap())) {
            ...
            double var11 = MovTasks.searchFirstNoCollisionSpaceYHeight(
                    var8.add(0.0, var6, 0.0), 0.0, var6 - 2.0 - var9, false);
            double var13 = var6 + var11;
            double var15 = 0.0;
            if (var13 - var15 > 2.0) {
                Debug.b(Text.literal("[Attack Bot] Mace Attack Simulation: simulate height %.2f".formatted(var13))
    ...
    private boolean processCommonTpAttack(
        ...
        if (alreadyAtTarget) {
            return true;
                ...
                && target.getBoundingBox().squaredMagnitude(var7.add(0.0, mc.player.getStandingEyeHeight(), 0.0))
                        > MathUtils.a(var8)) {
            List<Vec3d> var10 = MovTasks.tpAttackSearch(var7, target.getBoundingBox(), var8 - 0.25, 135.0, 1);
            if (!var10.isEmpty()
                    && target.getBoundingBox().squaredMagnitude((Vec3d) var10.get(var10.size() - 1))
                            < MathUtils.a(var8)) {
    ...
    private static boolean processVanillaAttack(
        ...
        return alreadAtTarget
                ? true
                : target.getBoundingBox().squaredMagnitude(var5.add(0.0, mc.player.getStandingEyeHeight(), 0.0))
                        <= MathUtils.a(CombatTasks.j().getAttackRange());
    }

    public CombatSubHelperPX createAttackSettings() {
        boolean var1 = this.Sd();
        boolean var2 = this.maceSwap.get().isPresent()
                && PlayerStateManager.INSTANCE.jh >= this.maceSwap.get().getValue();
        ...
        if (var10
                && this.maceSwap.get().isPresent()
                && this.maceHeightMultiply.get().getValue()
                        >= this.maceSwap.get().getValue()) {
            var2 = true;
        }
