...
public class SpearEnhance extends BaseModule {
    ...
    private static final Optional<RegistryEntry<SoundEvent>> dY =
    ...
    public final FlagRef dJ =
            this.flagBuilder(this.dI.add("spear-auto-restart")).build();
    ...
    private static final Optional<RegistryEntry<SoundEvent>> dX =
        ...
        if (event.getArgs(1).equals(new Identifier("minecraft", "blocks"))) {
            ((Set) event.e())
                    .addAll(ResourceUtils.lookupResources(
                            event.getArgs(0), "kalama", "kalama", "textures", ".png", s -> s.startsWith("spear")));
        }
    }
    ...
    public boolean onPiercing(IntSupplier seq) {
        if (VItem.w().b(mc.player.getStackInHand(Hand.MAIN_HAND))
                && ViaFabricPlusHooks.getInstance().isViaEnabled()) {
            if (SupportVersion.CURRENT.b(21, 6)) {
                ViaFabricPlusHooks.ViaPacketWrapper var2 =
                ...
                PlayerActionC2SPacket var12 =
                        new PlayerActionC2SPacket(Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, Direction.DOWN);
    ...
    public void fV(Event<Set<Identifier>> event) {
        ((Set) event.e())
                .addAll(ResourceUtils.lookupResources(
                        event.getArgs(0), "kalama", "kalama", "models", ".json", s -> s.startsWith("spear")));
    }
                    ...
                    Tasks.q(
                            () -> {
                                if (var4.getAndIncrement() > 20) {
                                    ...
                                    if (mc.player.getActiveHand() == var2
                                            && ItemStack.areItemsAndComponentsEqual(var3, mc.player.getActiveItem())) {
                                        dY.ifPresent(sound -> mc.player
                                                .getEntityWorld()
                                                .playSound(
                                                        mc.player,
                                                        mc.player.getX(),
    ...
    public SpearEnhance() {
        super("SpearEnhance");
        this.renderKineticPlayers =
                this.flagBuilder(this.dI.add("render-kinetic-players")).build();
        this.replaceViaSpearModel = this.builder(this.dI.add("replace-via-spear-model"), Boolean.class)
                .defaultValue(true)
                .build();
        this.fixOldVersionSpear = this.builder(this.dI.add("fix-old-version-spear"), Boolean.class)
                .defaultValue(true)
                .build();
        this.fixOldVersionSpearPiercing = this.builder(this.dI.add("fix-old-version-spear-piercing"), Boolean.class)
                .defaultValue(true)
                .build();
        this.fixOldVersionSpearSound = this.builder(this.dI.add("fix-old-version-spear-sound"), Boolean.class)
                .defaultValue(true)
                .build();
        this.renderKineticPlayersColor = this.builder(this.dI.add("render-kinetic-players-color"), WrapColor.class)
                ...
                .build();
        this.resetSpearSpeedRotEnable =
                this.flagBuilder(this.dI.add("reset-spear-speed-rot-enable")).build();
        this.resetSpearSpeedRotHotkey = this.toggleHotkey(
                        this.dI.add("reset-spear-speed-rot-hotkey"),
                        new MultiKeyBind(),
                        this.dI.add("reset-spear-speed-rot-enable"))
                .build();
        this.resetSpearSpeedAuto =
                this.flagBuilder(this.dI.add("reset-spear-speed-auto")).build();
        this.resetSpearSpeedOnlyCombat =
                this.flagBuilder(this.dI.add("reset-spear-speed-only-combat")).build();
        this.resetSpearAutoFocusTarget =
                this.flagBuilder(this.dI.add("reset-spear-auto-focus-target")).build();
        this.dU = new HashMap<>();
            ...
            if (this.fixOldVersionSpearSound.get() && ((EntityStatusS2CPacket) eventPost.b).getStatus() == 2) {
                ...
                if (var2 instanceof LivingEntity var3 && var3.isUsingItem()) {
                    ...
                    if (this.dU.containsKey(var4.getItem()) && VItem.w().b(var4)) {
                        dX.ifPresent(hitSound -> mc.world.playSoundFromEntity(
                                var3, (SoundEvent) hitSound.value(), var2.getSoundCategory(), 1.0F, 1.0F));
                    }
                }
            }
        }
    ...
    public void onPostTick(Event<ClientPlayerEntity> eventPostTick) {
        if (!checkNull()) {
            if (eventPostTick.b == mc.player
                    && (this.resetSpearSpeedRotEnable.get() || this.forceSpearReset)
                ...
                if (this.resetSpearSpeedOnlyCombat.get()) {
                    boolean var3 = TargetSelector.INSTANCE.akC(25.0).stream()
                            .anyMatch(s -> s instanceof PlayerEntity var1 && isUsingSpear(var1));
                    if (!var3) {
                        var2 = false;
                    }
                }
