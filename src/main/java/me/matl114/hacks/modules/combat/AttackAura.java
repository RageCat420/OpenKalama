...
public class AttackAura extends BaseModule {
      ...
      if (mc.player != null) {
         if (this.autoAtt.get()) {
            ...
            if (var4 || var5 <= this.BO) {
               if (var4 || !var2.legalTargeting.get().isLegal() && (!var3 || !this.weapon.get()) && (var3 || !this.hand.get())) {
   ...
   public AttackAura() {
      ...
      this.autoAttHotkey = this.toggleHotkey(this.KE.add("auto-att-hotkey"), new MultiKeyBind(), this.KE.add("auto-att")).build();
      this.maxAtOnce = this.intBuilder(this.KE.add("max-at-once")).defaultValue(1).validator(Configs.e).build();
      this.weapon = this.builder(this.KF.add("weapon"), Boolean.class).defaultValue(true).build();
      this.hand = this.builder(this.KF.add("hand"), Boolean.class).defaultValue(true).build();
      this.autoAttRate = this.intBuilder(this.KE.add("auto-att-rate")).defaultValue(0).validator(Configs.d).build();
      this.stopAttackWhenEat = this.flagBuilder(this.KE.add("stop-attack-when-eat")).build();
      this.stopAttackWhenSpear = this.flagBuilder(this.KE.add("stop-attack-when-spear")).build();
      this.airKill = this.flagBuilder(this.KE.add("air-kill")).build();
      this.bindFlag(this.autoAtt);
   }

   public boolean aap() {
      return this.stopAttackWhenSpear.get() && mc.player.isUsingItem() && VItem.w().b(mc.player.getActiveItem());
   }

   public boolean aao() {
      return this.stopAttackWhenEat.get() && mc.player.isUsingItem() && VItem.w().h(mc.player.getActiveItem());
   }
}
