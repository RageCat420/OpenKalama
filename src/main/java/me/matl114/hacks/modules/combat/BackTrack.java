...
public class BackTrack extends BaseModule {
   ...
   public void refreshTarget() {
      if (this.ae.get()) {
         Entity var1 = TargetSelector.INSTANCE
            .akK(this.maxDistance.get(), true, this.playerOnly.get() ? ev -> ev instanceof PlayerEntity : ev -> ev instanceof LivingEntity);
         if (var1 != this.Jr) {
            if (var1 != null) {
               this.setTarget(var1);
            } else {
         ...
         } else if (!TargetSelector.INSTANCE.akm(mc.player.getPos(), this.Jr.getBoundingBox(), CombatExtra.INSTANCE.getAttackAtTargetRange(this.Jr))) {
            this.XL();
         }
      } else {
         this.XK();
   ...
   public void onQueuePlayerPosition(Event<PacketStorage> event) {
      if (this.ae.get() && this.Jr != null) {
         ...
         if (var2 instanceof KalamaHelperHelperH var3) {
            ...
            if (var4 instanceof EntityS2CPacket var9 && var9.getEntity(mc.world) == this.Jr && var9.isPositionChanged()) {
            ...
            if (var4 instanceof EntityStatusS2CPacket var10 && var10.getStatus() == 35 && var10.getEntity(mc.world) == mc.player) {
               this.XL();
               this.XI();
               return;
            }
   ...
   public void onRender(Event<MatrixStack> eventMatrixStack) {
      if (this.renderOld.get() && this.shouldDelay && this.Js != null && this.Jr != null) {
         ...
         try {
            RenderUtils.drawOutlinedBox((MatrixStack)eventMatrixStack.b, var2.getMinPos(), var2.getMaxPos(), Color.ORANGE);
         } finally {
   ...
   public BackTrack() {
      ...
      this.J = this.toggleHotkey(this.Jo.addHotkey(), new MultiKeyBind(), this.Jo.addEnable()).build();
      this.maxDelay = this.doubleBuilder(this.Jo.add("max-delay")).defaultValue(50.0).build();
      this.renderOld = this.flagBuilder(this.Jo.add("render-old")).build();
      this.maxDistance = this.doubleBuilder(this.Jo.add("max-distance")).defaultValue(5.0).build();
