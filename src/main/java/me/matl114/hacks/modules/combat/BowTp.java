...
public class BowTp extends BaseModule {
    ...
    public void onBowAction(Event<PlayerActionC2SPacket> packetEvent) {
        if (!packetEvent.d()) {
            ...
            if (var2.getAction() == Action.RELEASE_USE_ITEM
                ...
                Debug.b(Text.literal("[Bow Attack] Aim at %s"
                                .formatted(var5 instanceof PlayerEntity ? "player " : "entity "))
    ...
    public BowTp() {
        super("BowTp");
        this.bowtpEnableHotkey = this.toggleHotkey(
                        this.CZ.add("bowtp-enable-hotkey"), new MultiKeyBind(), this.CZ.add("bowtp-enable"))
                .build();
        this.bowtpDistance = this.builder(this.CZ.add("bowtp-distance"), DoubleRef.TYPE)
                .defaultValue(80.0)
                .build();
        this.bowtpTargetDistance = this.builder(this.CZ.add("bowtp-target-distance"), DoubleRef.TYPE)
                .defaultValue(80.0)
                .build();
        this.bowtpRenderTarget =
                this.flagBuilder(this.CZ.add("bowtp-render-target")).build();
    ...
    private double getFireArrowPositionHeight(PlayerEntity player, Entity target) {
        return target.getBoundingBox().getLengthY()
                - player.getEyeHeight(player.getPose())
                + 0.11
                + this.bowtpStartDeltaY.get();
    }
    ...
    private boolean processBowVClip(
        ...
        if (this.canTp()) {
            ...
            double var10 = MovTasks.searchFirstNoCollisionSpaceYHeight(
                    var7.add(0.0, var5, 0.0), 0.0, var5 - 2.0 - var8, false);
            double var12 = var5 + var10;
            double var14 = 0.0;
            if (var12 - var14 > 0.0) {
                Debug.b(Text.literal("[Bow Attack] Bow Attack Simulation: simulate height %.2f".formatted(var12))
                        .formatted(Formatting.GREEN));
                movementStack.addLast(MovTasks$MovInfo.adB(var7.add(0.0, var12, 0.0)));
                movementStack.addLast(new MovTasks$MovInfo(
                        var7.add(0.0, var14, 0.0), null, false, new Vec2f(89.0F, mc.player.getYaw())));
                return true;
            }
        }
