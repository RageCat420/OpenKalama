...
public class Criticals extends BaseModule implements HackUtilHelperJ {
    ...
    public void applyBeforeMovementPacketModify(Event<LegalMovementManager> movementManagerEvent) {
        ...
        if (this.enable.get() && this.mode.get() == Criticals$Mode.FREEZE && this.akb() && !mc.player.isFallFlying()) {
            ...
            label121:
            {
                ...
                if (this.inWallFreeze.get() && (PlayerStateManager.INSTANCE.jH || PlayerStateManager.INSTANCE.jG)) {
                    ...
                    List var8 =
                            CollisionUtil.getIntersectingBlockPositions(mc.world, var6.offset(0.0, -var4, 0.0), false);
        ...
        if (this.enable.get()
            ...
            if (this.SB <= 0) {
                boolean var21 = mc.player
                                        .getPos()
                                        .subtract(((LegalMovementManager) movementManagerEvent.b).c.g)
                                        .horizontalLengthSquared()
                                > MathUtils.a(2.0E-4)
                        || Math.abs(mc.player.getPos().y - ((LegalMovementManager) movementManagerEvent.b).c.g.y)
                                > 2.0E-4;
                double var14 = 1.0E-5;
                double var16 = 1.0E-4;
    ...
    public Criticals() {
        ...
        this.mode = this.builder(this.Sg.add("mode"), Criticals$Mode.class)
                .defaultValue(Criticals$Mode.PACKET)
                .build();
        this.hotkey = this.moduleEntry(
                        this.Sg.add("hotkey"), new MultiKeyBind(), this.Sg.add("enable"), moduleMeta(() -> this.mode))
                .build();
        this.groundOnly = this.flagBuilder(this.Sg.add("ground-only"))
                .show(() -> this.mode
                        .get()
                        .isIn(new ConfigEnum[] {Criticals$Mode.FREEZE, Criticals$Mode.GRIM_GROUND_SIMULATION}))
                .build();
        ...
        this.customInWallPacketHeight = this.builder(this.Sg.add("custom-in-wall-packet-height"), Double.class)
                ...
                .build();
        this.criticalCooldown = this.builder(this.Sg.add("critical-cooldown"), Integer.class)
                .defaultValue(10)
                .build();
        this.Sv = 0;
        this.Sw = false;
                    ...
                    if (!CombatTasks.n().Sd()) {
                        if (mc.player.isSprinting()) {
                            mc.getNetworkHandler()
                                    .sendPacket(new ClientCommandC2SPacket(mc.player, Mode.STOP_SPRINTING));
                            ...
                            case CRASH_PACKETS:
                                mc.getNetworkHandler()
                                        .sendPacket(PlayerMoveC2SPacketAccess.setCause(
                                                VPacket.g(var2, Double.POSITIVE_INFINITY, var6, false, false),
                                                PlayerMoveC2SPacketAccess.Cause.TRIGGER_SIMULATION));
                                break;
                            case SIMULATION:
                                mc.getNetworkHandler()
                                        .sendPacket(PlayerMoveC2SPacketAccess.setCause(
                                                VPacket.g(var2, var4 + 1.0, var6, false, false),
                                                PlayerMoveC2SPacketAccess.Cause.TRIGGER_SIMULATION));
                        }
    ...
    public void handleCriticalWall(Event<PlayerInteractEntityC2SPacket> event) {
        ...
        double var6 = mc.player.getZ();
        mc.getNetworkHandler()
                .sendPacket(VPacket.g(var2, var4 + this.customInWallPacketHeight.get(), var6, false, false));
        mc.getNetworkHandler()
                .sendPacket(VPacket.g(var2, var4 + this.customInWallPacketHeight.get() * 0.75, var6, false, false));
        ClientPlayerAccess.of(mc.player).resyncPos();
    }
    ...
    public void onPlayerAttack(Event<PlayerInteractEntityC2SPacket> event) {
        if (!checkNull() && !event.d() && !this.canNotCrit()) {
            if (!(PlayerStateManager.INSTANCE.jh > 1.0E-6)) {
                ...
                if (var2) {
                    if (this.enable.get()
                            && PlayerInteractEntityC2SPacketAccess.of((PlayerInteractEntityC2SPacket) event.b)
                                    .isAttack()
                            && mc.world != null
                            && mc.world.getEntityById(PlayerInteractEntityC2SPacketAccess.of(
                                                    (PlayerInteractEntityC2SPacket) event.b)
                                            .getEntityId())
                                    instanceof LivingEntity var4) {
    ...
    public void onTeleportConfirmPre(Event<PlayerMoveC2SPacket> event) {
        if (this.Sz != null
                && event.b instanceof PlayerMoveC2SPacketAccess var3
                && var3.getCause() == PlayerMoveC2SPacketAccess.Cause.SET_BACK) {
            this.SB = 1;
            Entity var11 = mc.world.getEntityById(
                    PlayerInteractEntityC2SPacketAccess.of(this.Sz).getEntityId());
            PlayerMoveC2SPacket var4 = (PlayerMoveC2SPacket) event.b;
            Vec2f var5 = null;
            if (var11 != null) {
                boolean var6 = RaycastUtils.w(
                        mc.player,
                        var4.getPitch(PlayerStateManager.INSTANCE.jl),
                        var4.getYaw(PlayerStateManager.INSTANCE.jm),
                        var11);
                if (!var6) {
            ...
            PacketManager.b((PlayerMoveC2SPacket) event.b, () -> {
                ...
                Runnable var5x = null;
                if (this.delaySwap.get()
                        && var13 != null
                        && !var13.isEmpty()
                        && !ItemStack.areItemsAndComponentsEqual(var4x, var13)) {
                    KalamaHelperHelperK var6x =
                            InventoryUtils.p(it -> ItemStack.areItemsAndComponentsEqual(it, var13), true, false);
                    if (var6x != null) {
