...
public class ChatSpamFix extends BaseModule {
      ...
      if (this.ae.get()) {
         ...
         if (this.regexList.get().test(var2)) {
            ...
            if (this.logHiddenMessages.get()) {
               String var3 = ((Text)event.e()).getString().replaceAll("\r", "\\\\r").replaceAll("\n", "\\\\n");
               String var4 = (String)Nullables.map((MessageIndicator)event.getArgs(1), MessageIndicator::loggedName);
               if (var4 != null) {
   ...
   public ChatSpamFix() {
      super("ChatSpamFix");
      this.regexList = this.builder(this.ro.add("regex-list"), RegexList.class).defaultValue(new RegexList(List.of())).build();
      this.logHiddenMessages = this.flagBuilder(this.ro.add("log-hidden-messages")).build();
