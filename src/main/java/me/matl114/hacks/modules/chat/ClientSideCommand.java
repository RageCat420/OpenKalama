...
public class ClientSideCommand extends BaseModule {
   ...
   public final FlagRef ae = this.flagBuilder(this.vE.add("client-side-command-override")).build();
   ...
   private boolean handleClientSideGiveCommand(Map<String, ParsedArgument<ClientCommandSource, ?>> argsMap, String command) throws CommandSyntaxException {
      if (this.clientSideGive.get()) {
         if (mc.interactionManager.getCurrentGameMode().isCreative()) {
            ...
            if (!var4.isSenderOnly() && !Objects.equals(mc.player.getNameForScoreboard(), command.substring(var5.getStart(), var5.getEnd()))) {
               ...
               int var7 = argsMap.containsKey("count") ? (Integer)((ParsedArgument)argsMap.get("count")).getResult() : 1;
   ...
   private static Text getErrorMessage(CommandSyntaxException e) {
      ...
      return (Text)(var2 != null ? Text.translatable("command.context.parse_error", new Object[]{var1, e.getCursor(), var2}) : var1);
   }
   ...
   private <T extends CommandSource> void addOurCommandNodesInRoot(RootCommandNode<T> node) {
      if (this.clientSideGive.get()) {
         ...
         if (var2 == null) {
            CommandRegistryAccess var8 = CommandRegistryAccess.of(ItemStackUtils.l(), FeatureFlags.DEFAULT_ENABLED_FEATURES);
            ArgumentCommandNode var5 = new ArgumentCommandNode("targets", EntityArgumentType.players(), null, vG, null, null, false, null);
            var3.addChild(var5);
            ArgumentCommandNode var6 = new ArgumentCommandNode("item", ItemStackArgumentType.itemStack(var8), vH, vG, null, null, false, null);
            var5.addChild(var6);
            ArgumentCommandNode var7 = new ArgumentCommandNode("count", IntegerArgumentType.integer(1), vH, vG, null, null, false, null);
            var6.addChild(var7);
         }
      }
   }
   ...
   private boolean dispatchVanillaCommand(String command) {
      if (mc.player == null) {
         ...
         try {
            ParseResults var2 = mc.getNetworkHandler().getCommandDispatcher().parse(command, mc.getNetworkHandler().getCommandSource());
            if (var2.getReader().canRead()) {
               if (var2.getExceptions().size() == 1) {
                  throw (CommandSyntaxException)var2.getExceptions().values().iterator().next();
               }

               if (var2.getContext().getRange().isEmpty()) {
                  throw CommandSyntaxException.BUILT_IN_EXCEPTIONS.dispatcherUnknownCommand().createWithContext(var2.getReader());
               }

               throw CommandSyntaxException.BUILT_IN_EXCEPTIONS.dispatcherUnknownArgument().createWithContext(var2.getReader());
            }
