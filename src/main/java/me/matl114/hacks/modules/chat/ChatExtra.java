...
public class ChatExtra extends BaseModule {
    ...
    public void iI(Event<String> stringEvent) {
        if (this.dontSendEmptyMessage.get()) {
            ...
            if (var2.isEmpty()
                    || Objects.equals(var2, "/")
                    || Objects.equals(var2, MainCommand.l)
                    || Objects.equals(var2, "/" + MainCommand.l)) {
                stringEvent.cancel();
            }
        }
    ...
    public ChatExtra() {
        ...
        this.ignoreChatLenLimit =
                this.flagBuilder(this.ev.add("ignore-chat-len-limit")).build();
        this.escapeTrimChat = this.flagBuilder(this.ev.add("escape-trim-chat")).build();
        this.escapeNormalizeSpaceChat =
                this.flagBuilder(this.ev.add("escape-normalize-space-chat")).build();
        this.checkChatLen = this.flagBuilder(this.ev.add("check-chat-len")).build();
        this.chatLenLimit =
                this.intBuilder(this.ev.add("chat-len-limit")).defaultValue(256).build();
        this.checkCommandLen =
                this.flagBuilder(this.ev.add("check-command-len")).build();
        this.commandLenLimit = this.intBuilder(this.ev.add("command-len-limit"))
                .defaultValue(32760)
                .build();
        this.limitWarnFormat = this.builder(this.ev.add("limit-warn-format"), String.class)
                .defaultValue("&c你的输入内容太长了! %d / %d")
                .build();
        this.overrideChatHistoryLen =
                this.flagBuilder(this.ev.add("override-chat-history-len")).build();
        this.chatHistoryLen = this.intBuilder(this.ev.add("chat-history-len"))
                .defaultValue(100)
                .validator(Configs.d)
                .build();
        this.addToHistoryWhenClose =
                this.flagBuilder(this.ev.add("add-to-history-when-close")).build();
        this.dontSendEmptyMessage =
                this.flagBuilder(this.ev.add("dont-send-empty-message")).build();
        ...
        this.passwordEncryptLength = this.intBuilder(this.ew.add("password-encrypt-length"))
                .defaultValue(20)
                .build();
        this.passwordEncryptSalt = this.builder(this.ew.add("password-encrypt-salt"), String.class)
                .defaultValue("")
                .build();
        this.enableChatMessageFormat =
                this.flagBuilder(this.ev.add("enable-chat-message-format")).build();
        this.chatMessageFormatStr = this.builder(this.ev.add("chat-message-format-str"), StringFormat.class)
                .defaultValue(
                        new StringFormat(List.of("message", "random+数字"), "{message}喵 | WurstV91 client | {random6}"))
                .build();
        this.chatMessageEscapeFormat = this.builder(this.ev.add("chat-message-escape-format"), Regex.class)
    ...
    private String generateRandomString(int length) {
        ...
        for (int var3 = 0; var3 < length; var3++) {
            var2.append("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
                    .charAt(this.eU.nextInt(
    ...
    public void onChatScreenInitialized(Event<Screen> screenEvent) {
        if (this.ignoreChatLenLimit.get()) {
            ...
            } else if (screenEvent.e() instanceof AnvilScreen var5
                    && var5.getFocused() instanceof TextFieldWidget var8) {
