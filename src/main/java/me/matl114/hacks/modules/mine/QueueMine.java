...
public class QueueMine extends BaseModule {
   ...
   public boolean sumitMine(BlockPos pos) {
      if (!this.pm.isEmpty() && this.pm.stream().anyMatch(pos::equals)) {
         return false;
      } else if (Objects.equals(PlayerInteractionAccess.of(mc.interactionManager).getCurrentFailBreakPos(), pos)) {
         return false;
      } else {
         this.pm.add(pos);
         return true;
   ...
   public void onPostInputEvent(Event<Void> eventVoid) {
      ...
      if (!checkNull()) {
         ...
         if (this.doubleBreak.get() && this.doubleBreakGhostHand.get() && PlayerInteractionAccess.of(mc.interactionManager).getCurrentFailBreakPos() != null) {
            PacketMine.INSTANCE.tickGhostHandDoubleBreak(null, false);
         }

         for (BlockPos var3 : this.pm) {
            this.po.submit(new Box(var3).expand(-0.2), this.renderColor.get().withAlpha(255));
         }
      }
   }
   ...
   public QueueMine() {
      ...
      this.queueSize = this.intBuilder(this.H.add("queue-size")).defaultValue(2).build();
      this.doubleBreak = this.flagBuilder(this.H.add("double-break")).build();
      this.doubleBreakGhostHand = this.flagBuilder(this.H.add("double-break-ghost-hand")).build();
      this.render = this.flagBuilder(this.H.add("render")).build();
      this.renderColor = this.builder(this.H.add("render-color"), WrapColor.class).defaultValue(new WrapColor(Color.GREEN)).build();
         ...
         if (var1 != null) {
            if (!this.doubleBreak.get() || MineExtra.INSTANCE.adc(5) || MineExtra.INSTANCE.add(1)) {
               ...
               while (!this.pm.isEmpty()) {
                  ...
                  if (new Box(var3).squaredMagnitude(mc.player.getEyePos()) > MathUtils.a(mc.player.getBlockInteractionRange() + 1.0)) {
                     this.pm.poll();
                  } else {
