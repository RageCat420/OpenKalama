...
public class MineExtra extends BaseModule {
                ...
                if (mc.interactionManager != null && mc.player != null && mc.world != null) {
                    ...
                    if (mc.player.getPos().squaredDistanceTo(var3) < 40000.0 && this.adp()) {
                        RenderUtils.drawOutlinedBox(
                                ...
                                var3,
                                var3.add(1.0, 1.0, 1.0),
                                ColorUtils.h(this.renderFrameColor.get().color(), 1.0F));
                        BlockState var4 = mc.world.getBlockState(var2);
                        KalamaHelperHelperK var5 = this.acU(var4);
                        float var6 = PlayerInteractionAccess.of(mc.interactionManager)
                                .getCurrentMiningProgress((ItemStack) var5.val());
                        if (var6 > 0.0F) {
                            Box var7;
                            if (var4.isAir()) {
                            ...
                            Vec3d var19 =
                                    var7.getMaxPos().subtract(var7.getMinPos()).multiply(0.5);
                    ...
                    if (var15 != null) {
                        ...
                        if (mc.player.getPos().squaredDistanceTo(var16) < 40000.0 && !Objects.equals(var16, var3)) {
                            float var17 = PlayerInteractionAccess.of(mc.interactionManager)
                                    .getFailBreakMiningProgress();
                            RenderUtils.drawOutlinedBox(
                                    (MatrixStack) renderEvent.b,
                                    var16,
                                    var16.add(1.0, 1.0, 1.0),
                                    ColorUtils.h(this.doubleBreakColor.get().color(), 1.0F));
                            if (var17 > 0.0F) {
                                ...
                                if (var18.isAir()) {
                                    ...
                                    var20 = var21.isEmpty()
                                            ...
                                            : var21.getBoundingBox();
                                }

                                Vec3d var22 = var20.getMaxPos()
                                        .subtract(var20.getMinPos())
                                        .multiply(0.5);
                                ...
                                RenderUtils.r(
                                        ...
                                        var23.add(var22.multiply(var11)),
                                        ColorUtils.h(
                                                this.renderProgressColor.get().color(), 0.25F));
                            }
                        }
                    }
                }
    ...
    public void acW(Event<PlayerActionC2SPacket> packetEvent) {
        ...
        if (var2.getAction() != Action.ABORT_DESTROY_BLOCK) {
            if (this.MY
                    && (var2.getDirection() != this.MX || Objects.equals(this.MW, var2.getPos()))
                    && this.fixMultiBreak.get()) {
                PacketManager.d(
                        var2,
        ...
        if (var2.bypassMode.getValue() == MineExtra$Mode.BYPASS_GRIM_BAD_PACKETS) {
            ...
            if (this.Ne > var3 && var2.bypassMode.getValue() == MineExtra$Mode.BYPASS_GRIM_LEGIT && mc.player != null) {
                ...
                if (!mc.player.getAbilities().creativeMode) {
                    ...
                    Direction var5 = Direction.getFacing(pos.toCenterPos().subtract(var4.getEyePos()))
                            .getOpposite();

                    for (int var6 = 0; var6 < 20; var6++) {
                        mc.interactionManager.sendSequencedPacket(
                                mc.world,
                                sequence -> new PlayerActionC2SPacket(Action.STOP_DESTROY_BLOCK, pos, var5, sequence));
                    }
                }
            }
        }
        ...
        if (this.MA.get()) {
            if (this.MZ != 0) {
                if (!instaBreak) {
                    ...
                    if (this.Nb > var7
                            && var6
                            && this.bypassMode.getValue() == MineExtra$Mode.BYPASS_GRIM_LEGIT
                            && mc.player != null) {
                        this.Nb = 150;
                        if (!mc.player.getAbilities().creativeMode) {
                            ClientPlayerEntity var8 = MinecraftClient.getInstance().player;
                            Direction var9 = Direction.getFacing(
                                            pos.toCenterPos().subtract(var8.getEyePos()))
                                    .getOpposite();
                            ...
                            for (int var10 = 0; var10 < 20; var10++) {
                                mc.interactionManager.sendSequencedPacket(
                                        ...
                                        sequence -> new PlayerActionC2SPacket(
                                                Action.START_DESTROY_BLOCK, pos, var9, sequence));
                            }
                        }
                    }
    ...
    public void acZ(Event<PlayerActionC2SPacket> eventPlayerAction) {
        if (this.silentBreak.get()
                ...
                && Objects.equals(
                        PlayerInteractionAccess.of(mc.interactionManager).getCurrentMiningPos(),
                        ((PlayerActionC2SPacket) eventPlayerAction.b).getPos())) {
    ...
    public float adq(BlockPos pos) {
        if (mc.player.isCreative()) {
            ...
            float var4 =
                    WorldUtils.getPlayerBlockBreakingSpeedWithCanMineMultiply(mc.player, var2, (ItemStack) var3.val());
    ...
    public MineExtra() {
        ...
        this.MB = this.moduleEntry(this.Mz.addHotkey(), new MultiKeyBind(), this.Mz.addEnable())
                .build();
        this.useFakeInstantBreak =
                this.flagBuilder(this.Mz.add("use-fake-instant-break")).build();
        this.bypassMode = this.builder(this.Mz.add("bypass-mode"), MineExtra$Mode.class)
                .defaultValue(MineExtra$Mode.NO_BYPASS)
                .build();
        this.grimPunishmentThreshold = this.builder(this.Mz.add("grim-punishment-threshold"), Integer.class)
                ...
                .build();
        this.breakThreshold = this.builder(this.Mz.add("break-threshold"), Double.class)
                .defaultValue(0.99)
                .validator(Configs.doubleRange(0.0, 1.1))
                .build();
        this.breakCooldown = this.builder(this.Mz.add("break-cooldown"), Integer.class)
                .defaultValue(5)
                .validator(Configs.d)
                .build();
        this.vanillaBreak = this.builder(this.Mz.add("vanilla-break"), Boolean.class)
                .defaultValue(true)
                .build();
        this.doubleBreak = this.flagBuilder(this.Mz.add("double-break")).build();
        this.sameBlockOptimize =
                this.flagBuilder(this.Mz.add("same-block-optimize")).build();
        this.ghostHandMine = this.flagBuilder(this.Mz.add("ghost-hand-mine")).build();
        this.ghostHandSwapWhenStart = this.flagBuilder(this.Mz.add("ghost-hand-swap-when-start"))
                .show(this.ghostHandMine::get)
                .build();
        ...
        this.renderCurrentBreakPos =
                this.flagBuilder(this.Mz.add("render-current-break-pos")).build();
        this.renderFrameColor = this.builder(this.Mz.add("render-frame-color"), WrapColor.class)
                .defaultValue(new WrapColor(Color.BLUE))
                .build();
        this.renderProgressColor = this.builder(this.Mz.add("render-progress-color"), WrapColor.class)
                .defaultValue(new WrapColor(Color.YELLOW))
                .build();
        this.doubleBreakColor = this.builder(this.Mz.add("double-break-color"), WrapColor.class)
                .defaultValue(new WrapColor(Color.MAGENTA))
                .build();
        this.renderOnlyWhenMine =
                this.flagBuilder(this.Mz.add("render-only-when-mine")).build();
        this.MT = 0;
        this.Nc = 0;
        this.Nd = false;
        this.Nf = 0;
    ...
    public void acY(Event<PlayerActionC2SPacket> event) {
        if (this.MA.get() && this.bypassMode.get() == MineExtra$Mode.BYPASS_GRIM_BAD_PACKETS && mc.player != null) {
            ...
            if (var2.getAction() == Action.START_DESTROY_BLOCK
                    ...
                    && Objects.equals(
                            PlayerInteractionAccess.of(mc.interactionManager).getCurrentMiningPos(), var2.getPos())) {
                if (mc.player.getAbilities().creativeMode) {
                    this.Nb = 150;
                    return;
                }
    ...
    public KalamaHelperHelperK<ItemStack> acU(BlockState currentState) {
        ...
        if (!this.ghostHandMine.get()) {
            ...
            BlockState var3 = !currentState.isAir() && !currentState.isLiquid()
                    ? currentState
                    : Blocks.OBSIDIAN.getDefaultState();
            double var4 =
                    WorldUtils.getPlayerBlockBreakingSpeedWithCanMineMultiply(mc.player, var3, (ItemStack) var2.val());
            KalamaHelperHelperK var6 = InventoryUtils.v(
                    item -> {
                        if (!item.isEmpty()
                            ...
                            double var4x =
                                    WorldUtils.getPlayerBlockBreakingSpeedWithCanMineMultiply(mc.player, var3, item);
    ...
    public void ada(Event<ClientPlayerEntity> tickEvent) {
        if (this.MA.get()
                && this.bypassMode.get() == MineExtra$Mode.BYPASS_GRIM_BAD_PACKETS
                && mc.player != null
                && this.MU != null
                && Tasks.b() - this.MZ == 6) {
            if (mc.player.getAbilities().creativeMode) {
