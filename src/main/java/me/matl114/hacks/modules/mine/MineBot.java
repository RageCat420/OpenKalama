...
public class MineBot extends BaseModule {
   ...
   public MineBot() {
      ...
      this.minDy = this.intBuilder(this.NI.add("min-dy"))
         ...
         .show(() -> this.mineMode.get().isNotIn(new ConfigEnum[]{MineBot$MineBotMode.AUTO_TOOL}))
         .build();
      this.maxDy = this.intBuilder(this.NI.add("max-dy")).defaultValue(6).show(() -> this.mineMode.get() != MineBot$MineBotMode.AUTO_TOOL).build();
      this.maxWidth = this.intBuilder(this.NI.add("max-width"))
         .defaultValue(1)
         .show(() -> this.mineMode.get().isIn(new ConfigEnum[]{MineBot$MineBotMode.SQUARE, MineBot$MineBotMode.TUNNEL}))
         .validator(Configs.d)
         .build();
      this.maxInstantMine = this.intBuilder(this.NI.add("max-instant-mine")).defaultValue(30).build();
      this.useDoubleBreak = this.flagBuilder(this.NI.add("use-double-break")).build();
      this.considerCooldown = this.builder(this.NI.add("consider-cooldown"), Boolean.class).defaultValue(true).build();
      this.autoSwap = this.flagBuilder(this.NI.add("auto-swap")).defaultValue(true).build();
      this.durabilityProtect = this.builder(this.NI.add("durability-protect"), Boolean.class).defaultValue(true).build();
      this.W = null;
   ...
   public int onMineCommon(Supplier<BlockPos> posFinder) {
      ...
      do {
         ...
         KalamaHelperHelperK var5 = this.autoSwap.get()
            ? InventoryUtils.v(
               s -> this.isDurabilityOk(s) ? (double)WorldUtils.getPlayerBlockBreakingSpeedWithCanMineMultiply(mc.player, var4, s) : null, true, true
            )
   ...
   private boolean checkDistanceAndCondition(BlockPos newPos) {
      ...
      if (Objects.equals(var2.getCurrentFailBreakPos(), newPos)) {
         ...
         return MineTasks.distanceOutOfReach(newPos, mc.player.getEyePos()) ? false : this.isMineable(mc.world.getBlockState(newPos));
      }
   }
