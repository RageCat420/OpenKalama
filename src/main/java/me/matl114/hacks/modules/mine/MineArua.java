...
public class MineArua extends BaseModule {
   ...
   public void onMineBlockAction(Event<HitResult> event) {
      if (mc.player != null && this.isActive()) {
         ...
         if (var2 != this.sq) {
            if (var2 != null) {
               Debug.b(Text.literal("[Mine Arua] Redirect mine target ").append(ChatUtils.t(Vec3d.of(var2))).formatted(Formatting.GREEN));
            }
         ...
         if (this.sq != null) {
            Direction var3 = Direction.getFacing(this.sq.toCenterPos().subtract(mc.player.getEyePos())).getOpposite();
   ...
   private BlockPos refreshMineAruaTarget() {
      if (mc.player != null && mc.world != null) {
         ...
         if (this.sq != null && this.isMineAruaTarget(mc.world, this.sq) && !MineTasks.distanceOutOfReach(this.sq, var1)) {
            return this.sq;
         }
   ...
   public MineArua() {
      ...
      this.el = this.toggleHotkey(this.ss.addHotkey(), new MultiKeyBind(), this.ss.addEnable()).build();
