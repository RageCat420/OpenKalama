...
public class PacketMine extends BaseModule {
   ...
   public BlockPos getCurrentMiningPos() {
      return mc.interactionManager == null ? null : PlayerInteractionAccess.of(mc.interactionManager).getCurrentMiningPos();
   }
   ...
   public void tickMine() {
      if (mc.interactionManager != null && mc.player != null) {
         ...
         if (var5 <= MathUtils.a(mc.player.getBlockInteractionRange() + 1.0)) {
            ...
            if (this.canMine(var7, var9)) {
               ...
               if (!var10.d()) {
                  if (this.groundDeceive.get() && !mc.player.isOnGround()) {
                     boolean var11 = true;
                     if (this.groundOnlyWhenNoControl.get() && !PlayerInputUtils.of(mc.options).rv()) {
                        var11 = false;
                     }

                     if (var11) {
                        mc.getNetworkHandler()
                           .sendPacket(LegacySnapRotManager.INSTANCE.ahx(PlayerStateManager.INSTANCE.jl, PlayerStateManager.INSTANCE.jm, true));
                  ...
                  var4 = PlayerInteractionAccess.of(mc.interactionManager).predictCurrentMiningProgressWithTool(var9);
         ...
         if (this.autoPickaxeDoubleBreak.get() && PlayerInteractionAccess.of(mc.interactionManager).getCurrentFailBreakPos() != null) {
   ...
   public PacketMine() {
      super("PacketMine");
      this.J = this.toggleHotkey(this.H.addHotkey(), new MultiKeyBind(), this.H.addEnable()).build();
      this.multiplePackets = this.intBuilder(this.H.add("multiple-packets")).defaultValue(1).validator(Configs.e).build();
      this.simulateRealBreak = this.flagBuilder(this.H.add("simulate-real-break")).build();
      this.considerAirBreak = this.flagBuilder(this.H.add("consider-air-break")).build();
      ...
      this.autoPickaxeDoubleBreak = this.flagBuilder(this.H.add("auto-pickaxe-double-break")).build();
      this.groundDeceive = this.flagBuilder(this.H.add("ground-deceive")).build();
      this.groundOnlyWhenNoControl = this.flagBuilder(this.H.add("ground-only-when-no-control")).build();
      this.onlyMineOncePerClick = this.flagBuilder(this.H.add("only-mine-once-per-click")).build();
      this.enableMineWhiteList = this.flagBuilder(this.H.add("enable-mine-white-list")).build();
      this.V = this.builder(this.H.add("mine-white-list"), EntrySet.<Block>parameter()).defaultValue(new EntrySet<Block>(new Regex("^()$"), Registries.BLOCK)).build();
      this.X = null;
   ...
   public KalamaHelperHelperK<ItemStack> getCurrentUsableTool(BlockState currentState) {
      if (this.autoPickaxe.get()) {
         ...
         KalamaHelperHelperK var3 = InventoryUtils.v(
            item -> (double)WorldUtils.getPlayerBlockBreakingSpeedWithCanMineMultiply(mc.player, var2, item), true, true
         );
         if (var3 != null) {
            return var3;
         }
