...
public class NewStyleModel extends BaseModule {
    ...
    public final FlagRef ch = this.builder(this.cg.add("enable-enchant-book"), Boolean.class)
            .defaultValue(true)
            .build();
    ...
    public boolean shouldEnableNewStyle(ItemStack item) {
        return this.enableNewStyleItem.get() && this.cq.containsKey(item.getItem())
                || this.enableNewStyleNbt.get() && isNewVersion(item);
    }

    public NewStyleModel() {
        super("NewStyleModel");
        this.enableNewStyleItem = this.builder(this.cg.add("enable-new-style-item"), Boolean.class)
                .defaultValue(false)
                .build();
        this.enableNewStyleNbt = this.builder(this.cg.add("enable-new-style-nbt"), Boolean.class)
                .defaultValue(true)
                .build();
        this.cp = new HashMap<>();
        this.cq = new HashMap<>();
    ...
    public void onRefreshCache(Event<ResourceManager> event) {
        ...
        for (Item var3 : Registries.ITEM) {
            Identifier var4 =
                    new Identifier(cl, ck + "/" + Registries.ITEM.getId(var3).getPath());
                            ...
                            Identifier var11 = var10 == 1
                                    ...
                                    : (var10 == var9
                                            ...
                                            : (var10 > var9
                                                    ...
                                                    : new Identifier(
                                                            cl, "enchanted_book/" + var8.getPath() + "_" + var10)));
