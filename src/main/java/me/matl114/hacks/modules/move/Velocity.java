...
public class Velocity extends BaseModule implements HackUtilHelperJ {
   ...
   public void x(Event<ClientPlayerEntity> eventPreTick) {
      if (this.ae.get()) {
            ...
            mc.interactionManager
               .sendSequencedPacket(
                  ...
                  seq -> new PlayerActionC2SPacket(
                     Action.STOP_DESTROY_BLOCK, mc.player.isCrawling() ? mc.player.getBlockPos() : mc.player.getBlockPos().up(), Direction.DOWN, seq
                  )
               );
            if (this.fS + this.grimResetKbTick.get() <= Tasks.b()) {
               this.fV = false;
            } else if (!this.grimFreezeIfWalk.get() && PlayerInputUtils.of(mc.options).rv()) {
         ...
         if (this.mode.get() == Velocity$Mode.FREEZE
            ...
            && (!this.pauseWhenWasd.get() || !PlayerInputUtils.of(mc.options).rv())) {
            FloatingUtils.INSTANCE.SB(true);
         }
      }
   }
   ...
   public void onPlayerVelocity(Event<Vec3d> event) {
      if (this.ae.get() && mc.player != null) {
         if (this.executeDuringFireworks.get() && mc.player.isFallFlying() && MovTasks.ax().agj()) {
            this.ju();
            event.cancel();
            return;
         }
         ...
         if (var2) {
            ...
            if (this.fT.horizontalLength() >= this.horizontalThreshold.get() || Math.abs(this.fT.y) >= this.verticalThreshold.get()) {
               if ((mc.player.isTouchingWater() || mc.player.isSubmergedInWater() || mc.player.isInLava()) && this.notInWater.get()) {
                  return;
               }

               if (!mc.player.isFallFlying() && (!this.onGroundOnly.get() || mc.player.isOnGround()) && this.mode.get() == Velocity$Mode.GRIM_LEGACY_GROUND) {
                  this.jz(event);
                  return;
               }

               if (!mc.player.isFallFlying() && (!this.onGroundOnly.get() || mc.player.isOnGround()) && this.mode.get() == Velocity$Mode.GRIM_NEW_GROUND) {
                  this.jC(event);
                  return;
               }
   ...
   public Velocity() {
      ...
      this.grimResetKbTick = this.intBuilder(this.fv.add("grim-reset-kb-tick"))
         ...
         .show(() -> this.mode.get().isIn(new ConfigEnum[]{Velocity$Mode.GRIM_LEGACY_GROUND}))
         .build();
      this.pauseWhenWasd = this.flagBuilder(this.fv.add("pause-when-wasd")).show(() -> this.mode.get().isIn(new ConfigEnum[]{Velocity$Mode.FREEZE})).build();
      this.freezeTime = this.doubleBuilder(this.fv.add("freeze-time"))
         .show(() -> this.mode.get().isIn(new ConfigEnum[]{Velocity$Mode.FREEZE}))
         .defaultValue(5.0)
         .build();
      this.onGroundOnly = this.flagBuilder(this.fv.add("on-ground-only")).show(() -> this.mode.get().isNotIn(new ConfigEnum[]{Velocity$Mode.NONE})).build();
      this.notInWater = this.flagBuilder(this.fv.add("not-in-water")).show(() -> this.mode.get().isNotIn(new ConfigEnum[]{Velocity$Mode.NONE})).build();
      this.executeDuringFireworks = this.builder(this.fv.add("execute-during-fireworks"), Boolean.class).defaultValue(true).build();
      this.executeInWall = this.builder(this.fv.add("execute-in-wall"), Boolean.class).defaultValue(true).build();
      ...
      this.noLiquidFlowPush = this.flagBuilder(this.fv.add("no-liquid-flow-push")).build();
