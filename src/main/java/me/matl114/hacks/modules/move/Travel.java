...
public class Travel extends BaseModule implements HackUtilHelperJ {
    ...
    public KeyBindRef J = this.hotkey(this.iN.add("toggle-auto-speed"))
            .defaultValue(new MultiKeyBind())
            .registerHotkey(HotKeyUtils.b(this::QO))
            .build();
    ...
    public boolean QW(MoveSubHelperSX ti) {
        if (CW == ti
                && ti.f == this
                && !(mc.player.getPos().subtract(ti.getCurrentFlyingTarget()).horizontalLengthSquared() < 900.0)) {
            return false;
        } else {
            this.QR(ti);
            if (mc.player != null) {
    ...
    public void QK(MainCommand mainCommand) {
        ...
        var2.subBuilder(SubCommand.bp())
                ...
                .z(s -> s.<KalamaHelperHelperD, TaskSubCommand>subBuilder(SubCommand.bo())
                        ...
                        .A(KalamaHelperHelperA.<KalamaHelperHelperUX, ExecutePos>b(KalamaHelperHelperUX::new)
                                .B("target")
                                .v())
    ...
    public Travel() {
        ...
        this.speed = this.builder(this.iN.add("speed"), DoubleRef.TYPE)
                .show(() -> this.controlType
                        .get()
                        .isIn(new ConfigEnum[] {TravellingControl$Type.MOV_VOID, TravellingControl$Type.MOV_VOID_2}))
                .defaultValue(9.9)
                .build();
        this.elytraSpeed = this.builder(this.iN.add("elytra-speed"), DoubleRef.TYPE)
                ...
                .build();
        this.minHeight = this.builder(this.iN.add("min-height"), IntRef.TYPE)
                .defaultValue(256)
                .build();
        this.maxHeight = this.builder(this.iN.add("max-height"), IntRef.TYPE)
                .defaultValue(400)
                .build();
        ...
        this.pitch40EndSafety = this.flagBuilder(this.iN.add("pitch-40-end-safety"))
                .show(() -> this.controlType.get().isIn(new ConfigEnum[] {
                    TravellingControl$Type.ELYTRA_PITCH40, TravellingControl$Type.ELYTRA_GRIM_FLY40
                }))
                .build();
        ...
        this.pitch40HeightLimit = this.builder(this.iN.add("pitch-40-height-limit"), OptionalPrimitive.DOUBLE_TYPE)
                ...
                .build();
        this.clearTargetWhenExit =
                this.flagBuilder(this.iN.add("clear-target-when-exit")).build();
        this.CV = false;
        if (cy == null) {
        ...
        if (ti != null && ti.startingTime != 0L) {
            ...
            if (mc.player != null) {
                ...
                Debug.chat(
                        ...
                        var2,
                        "s, 运行距离: ",
                        String.format("%.2f", var4),
                        ", 平均速度: ",
                        String.format("%.2f", var6),
                        "m/s");
                mc.player.setOnGround(false);
            }
        }
