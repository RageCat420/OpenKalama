...
public class MovExtra extends BaseModule {
    ...
    public void sendPacketsForPreStartFallFlying() {
        if (this.grimac1212InputFeatures.get()
                && ViaFabricPlusHooks.isSupportEndTick()
                && !PlayerStateManager.INSTANCE.jJ.rI()) {
    ...
    public MovExtra() {
        super("MovExtra");
        this.grimac1212InputFeatures =
                this.flagBuilder(this.gQ.add("grimac-1-21-2-input-features")).build();
        this.grimacSprintFeatures = this.builder(this.gQ.add("grimac-sprint-features"), Boolean.class)
                .defaultValue(true)
                .build();
        this.disableStepheightFeature =
                this.flagBuilder(this.gQ.add("disable-stepheight-feature")).build();
