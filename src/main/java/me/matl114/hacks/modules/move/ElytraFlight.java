...
public class ElytraFlight extends BaseModule implements HackUtilHelperJ {
    ...
    public boolean alu() {
        ...
        Debug.chat(
                ChatUtils.textFromLegacyString("&c[ElytraFlight] &fMode switch to"),
                this.flightMode.get().resultAsString());
        return false;
    }
    ...
    public void bb(Event<LegalMovementManager> movementManagerEvent) {
            ...
            if (var2.isFallFlying()) {
                ...
                switch ((ElytraFlight$Mode) this.flightMode.get()) {
                    ...
                    case ROTATION:
                        boolean var9 = true;
                        Vec3d var10 =
                                EntityUtils.lookCoordToPos(var2.getPitch(), var2.getYaw(), var8.rp(), 0.0, var8.ro());
                ...
                if (var4) {
                    ...
                    mc.player.setVelocity(
                            this.motionMode.get() == ElytraExtra$MotionMode.FIRE_WORKS && this.useAutoRescale.get()
                                    ? ElytraExtra.INSTANCE.agA(
                                            var24, mc.player.getPitch(), mc.player.getYaw(), !mc.player.hasNoGravity())
                                    : var24);
                }
        ...
        if (this.Ty && !mc.player.isFallFlying() && mc.player.isOnGround()) {
            if (this.enableControl.get() && this.landAutoClose.get()) {
                HotKeyUtils.f(this.Tj.add("enable-control").toPath(), this.enableControl)
                        .run();
            }
    ...
    public ElytraFlight() {
        ...
        this.flightMode = this.builder(this.Tj.add("flight-mode"), ElytraFlight$Mode.class)
                .defaultValue(ElytraFlight$Mode.CONTROL)
                .build();
        this.enableControlHotkey = this.moduleEntry(
                        this.Tj.add("enable-control-hotkey"),
                        new MultiKeyBind(),
                        this.Tj.add("enable-control"),
                        moduleMeta(() -> this.flightMode))
                .build();
        this.motionAmount = this.builder(this.Tk.add("motion-amount"), DoubleRef.TYPE)
                .defaultValue(0.05)
                .validator(Configs.doubleRange(0.0, 10000.0))
                .build();
        this.motionMode = this.builder(this.Tj.add("motion-mode"), ElytraExtra$MotionMode.class)
                .defaultValue(ElytraExtra$MotionMode.VOID)
                .build();
        this.flightToggle = this.hotkey(this.Tj.add("flight-toggle"))
                .defaultValue(new MultiKeyBind())
                .registerHotkey(HotKeyUtils.c(this::alu))
                .build();
        this.useFloatingUtils =
                this.flagBuilder(this.Tj.add("use-floating-utils")).build();
        this.horizontalNoGravity = this.builder(this.Tj.add("horizontal-no-gravity"), Boolean.class)
                .defaultValue(true)
                .build();
        this.pauseWhenAccelerate = this.builder(this.Tj.add("pause-when-accelerate"), Boolean.class)
                .defaultValue(true)
                .build();
        ...
        this.holdJumpTakeoffTicks = this.builder(this.Tj.add("hold-jump-takeoff-ticks"), OptionalPrimitive.INT_TYPE)
                ...
                .build();
        this.motionLerpArgument = this.builder(this.Tj.add("motion-lerp-argument"), DoubleRef.TYPE)
                .defaultValue(1.0)
                .build();
        this.motionLerpStarting = this.builder(this.Tj.add("motion-lerp-starting"), Boolean.class)
                .defaultValue(false)
                .build();
        this.useAutoRescale = this.flagBuilder(this.Tj.add("use-auto-rescale"))
                .show(() -> ElytraExtra.INSTANCE.autoRescaleFireworkBox.get())
                .build();
        ...
        this.autoFlyHotkey = this.moduleEntry(
                        this.Tj.add("auto-fly-hotkey"), new MultiKeyBind(), this.Tj.add("auto-fly"))
                .build();
        this.autoFlyAutoJumpOff =
                this.flagBuilder(this.Tj.add("auto-fly-auto-jump-off")).build();
        this.autoFlyLandAutoClose =
                this.flagBuilder(this.Tj.add("auto-fly-land-auto-close")).build();
        this.fT = null;
        this.TD = 0;
        this.TE = null;
        if (cy == null) {
