   ...
   public void applyBeforeMovementPacketModify(Event<LegalMovementManager> movementManagerEvent) {
      if (this.module.isActive()) {
         ...
         if (this.UO && this.Mc && MinecraftClient.getInstance().player.isOnGround()) {
            ...
            this.Mc = false;
            this.UN = false;
            MinecraftClient.getInstance().player.setPosition(MinecraftClient.getInstance().player.getPos().withAxis(Axis.Y, ((LegalMovementManager)movementManagerEvent.b).c.g.y + 9.0E-5));
            MinecraftClient.getInstance().player.setOnGround(false);
            ClientPlayerAccess.of(MinecraftClient.getInstance().player).resyncPos();
         }

         if (var4 && MinecraftClient.getInstance().player.isOnGround() && !((LegalMovementManager)movementManagerEvent.b).c.b) {
            MinecraftClient.getInstance().player.setOnGround(false);
            this.module.Uw = MinecraftClient.getInstance().player.getY();
            MinecraftClient.getInstance().player.setPosition(((LegalMovementManager)movementManagerEvent.b).c.g.add(0.0, 9.0E-8, 0.0));
            ClientPlayerAccess.of(MinecraftClient.getInstance().player).resyncPos();
            this.UO = true;
         }
      } else {
