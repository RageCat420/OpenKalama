...
public class FloatingUtils extends BaseModule implements HackUtilHelperJ {
   ...
   public void iC(Event<LegalMovementManager> movementManagerEvent) {
      if (this.SG()) {
         ...
         if (var2) {
            ...
            this.Ei = false;
            this.Ef = LegacySnapRotManager.INSTANCE.ahx(mc.player.getPitch(), mc.player.getYaw(), PlayerStateManager.INSTANCE.jI);
         } else {
            this.Ef = VPacket.h(mc.player.getYaw(), mc.player.getPitch(), mc.player.isOnGround(), mc.player.horizontalCollision);
            if (this.Ee) {
   ...
   public FloatingUtils() {
      ...
      this.DZ = this.toggleHotkey(this.DX.addHotkey(), new MultiKeyBind(), this.DX.addEnable()).build();
   ...
   public boolean postModify(Event<LegalMovementManager> movementManagerEvent, boolean enabledThisTick) {
      ...
      if (this.Ef != null) {
         if (!this.Eh && (!this.Eg || PlayerStateManager.INSTANCE.nQ(this.Ef.getPitch(mc.player.getPitch()), this.Ef.getYaw(mc.player.getYaw())))) {
