...
public class ElytraExtra extends BaseModule implements HackUtilHelperJ {
        ...
        if (var2.isFallFlying()
            ...
            if (var5) {
                if (EntityUtils.isEntityValid(this.PT)) {
                    this.Qc = VPacket.g(
                            var2.getX(),
                            ((LegalMovementManager) movementManagerEvent.b).c.g.y + 9.0E-8,
                            var2.getZ(),
                            false,
    ...
    public int afC() {
        if (ClientPlayerAccess.of(mc.player).getServerScreenHandler() == mc.player.playerScreenHandler) {
            ...
            if (var1 != null) {
                return mc.player
                        .playerScreenHandler
                        .getSlotIndex(mc.player.getInventory(), var1.index())
                        .orElse(-1);
            }
        }
            ...
            if (value != -1) {
                ...
                if (VItem.w().a(var2)) {
                                ...
                                if (entry.index() >= 36 && entry.index() != 40) {
                                    ...
                                    if (var1.isEmpty()) {
                                        ...
                                        return mc.player.getPreferredEquipmentSlot(var1) == EquipmentSlot.CHEST
                                                ? KalamaHelperHelperB.c(mc.player, var1, EquipmentSlot.CHEST)
                                                        * KalamaHelperHelperB.d(mc.player, var1, EquipmentSlot.CHEST)
                                                : null;
                                    }
                                }
                            },
                    ...
                    if (var3 != null) {
                        int var4 = mc.player
                                .playerScreenHandler
                                .getSlotIndex(mc.player.getInventory(), var3.index())
                                .orElse(-1);
                        if (var4 != -1) {
                            this.PI = var4;
                            this.PK = -1;
        ...
        if (!movementManagerEvent.d()) {
            ...
            if (this.noKineticMode.get().isPresent() && this.PI == -1 && var3.isFallFlying()) {
                ...
                if (this.noKineticMode.get().getValue().get().hasAc()) {
                    ...
                    if ((!(var5.horizontalLength() > 0.3) || MathHelper.approximatelyEquals(var6.x, var5.x))
                            && MathHelper.approximatelyEquals(var6.z, var5.z)) {
                        if (this.PR > 0) {
                            this.PR--;
                            ((LegalMovementManager) movementManagerEvent.b).c();
                            if (var4) {
    ...
    public static boolean afR() {
        return InventoryUtils.p(vv -> mc.player.getPreferredEquipmentSlot(vv) == EquipmentSlot.CHEST, false, false)
                != null;
    }
    ...
    public void afB(Event<PlayerInteractEntityC2SPacket> interactPacket) {
        if (!interactPacket.d()) {
            ...
            if (this.maceHitFixMode.get().isPresent()
                ...
                if (!this.afr()) {
                    PacketManager.d(
                            (PlayerInteractEntityC2SPacket) interactPacket.b, () -> this.PD.add(Tasks.b() + 20));
                }
            }
        }
    }
    ...
    public void age(Event<Boolean> takeOff) {
        if ((Boolean) takeOff.b
                && (this.flyRocketOnFirstOff.get()
                        || MovTasks.ay().alw()
                        || this.autoTakeOffFireworksFromGround.get() && this.Qa)) {
            this.PP = Tasks.b();
        }
    }
    ...
    public void afW(boolean continueFly) {
        if (this.PI != -1) {
            ...
            if (this.PH == -1
                    && this.PL
                    && this.armorMode.get().isNotIn(new ConfigEnum[] {ElytraExtra$ArmorFlyMode.LAZY})) {
                this.PL = false;
                var3 = true;
            }
        ...
        if (ClientPlayerAccess.of(mc.player).getServerScreenHandler() == mc.player.playerScreenHandler) {
            ...
            KalamaHelperHelperK var2 = InventoryUtils.findBestPlayerInventory(
                    item -> (item.index() < 36 || item.index() == 40) && var1.test(item.val())
                            ? KalamaHelperHelperB.c(mc.player, item.val(), EquipmentSlot.CHEST)
                                    * KalamaHelperHelperB.d(mc.player, item.val(), EquipmentSlot.CHEST)
                            : null,
                    true,
                    true);
            if (var2 != null) {
                return mc.player
                        .playerScreenHandler
                        .getSlotIndex(mc.player.getInventory(), var2.index())
                        .orElse(-1);
            }
        }
    ...
    public void jJ(Event<LegalMovementManager> movementManagerEvent, Event<Vec3d> moveEvent) {
        ...
        if (this.PS) {
            ...
            if (!var4 && !var5) {
                ...
                if (!MathHelper.approximatelyEquals(var7.x, var6.x)
                        || !MathHelper.approximatelyEquals(var7.z, var6.z)) {
                    this.PR = 2;
                    ((LegalMovementManager) movementManagerEvent.b).c();
                    if (var5) {
                        EntityUtils.setEntityPitchSafe(var3, this.PR % 2 == 0 ? -89.999F : 89.999F);
    ...
    public void agc() {
        ...
        try {
            ...
            while (!this.PE.isEmpty()) {
                ...
                if (!var5.isEmpty()) {
                    KalamaHelperHelperK var6 =
                            InventoryUtils.p(it -> ItemStack.areItemsAndComponentsEqual(var5, it), false, false);
                    if (var6 != null) {
                        if (var6.index() == InventoryUtils.getSelectedSlot()) {
                            this.Pi.d();
                            Listener.sendPacketNoEvents(new PlayerInteractItemC2SPacket(
                                    Hand.MAIN_HAND, NetworkUtils.generateNextSequence(), var3, var4));
                        } else {
                            var2 = InvExtra.INSTANCE.uh(var6.index());
                            if (var2 != null) {
                                this.Pi.d();
                                Listener.sendPacketNoEvents(new PlayerInteractItemC2SPacket(
                                        Hand.OFF_HAND, NetworkUtils.generateNextSequence(), var3, var4));
                                var2.run();
                            }
                        }
                    }
        ...
        if (!this.autoRescaleFireworkBox.get()) {
            ...
            Vec3d var6 = !PlayerStateManager.INSTANCE.jz && !PlayerStateManager.INSTANCE.jy
                    ...
                    : EntityUtils.simulateTravelInFluidVelocity(
                            var5, PlayerStateManager.INSTANCE.jz, PlayerStateManager.INSTANCE.jy, true);
            ...
            if (!ViaFabricPlusHooks.isSupportEndTick() && var48 > 1.0E-6) {
                ...
                if (var56 < 0.04 * currentMotion.y) {
                    double var58 = EntityUtils.calculateGlidingVelocity(
                                    mc.player,
                                    currentMotion.multiply((var54 + this.autoRescaleAxisZeroPointThree.get()) / var54),
                                    var4,
                                    true)
                            .y;
            ...
            if (this.speedDropOptimization.get()) {
                ...
                var60 = new Vec3d(
                        agx(currentMotion.x, var6.x, var42, var44),
                        agx(currentMotion.y, var6.y, var46, var48),
                        agx(currentMotion.z, var6.z, var50, var52));
        ...
        if (!serializedEntryUpdateEvent.d()) {
            if (serializedEntryUpdateEvent.f().length > 0
                                ...
                                if (afO()) {
                                    if (afQ()) {
                                        serializedEntryUpdateEvent.context(
                                                new SerializedEntry(var8.id(), var8.handler(), (byte) (var4 | 128)));
                                        MovTasks.aj().Xj();
                                        mc.getNetworkHandler()
                                                .sendPacket(
                                                        new ClientCommandC2SPacket(mc.player, Mode.START_FALL_FLYING));
                                        MovTasks.aj().sendPacketsForPreStartFallFlying();
                                    }
                                } else {
                                    this.PD.clear();
                ...
                if (this.aft() && var9) {
                    ...
                    if (afO()) {
                        serializedEntryUpdateEvent.context(
                                new SerializedEntry(var14.id(), var14.handler(), (byte) (var6 | 128)));
                        mc.getNetworkHandler()
                                .sendPacket(new ClientCommandC2SPacket(mc.player, Mode.START_FALL_FLYING));
                        this.PM = true;
                        if (this.PA != -1) {
                    ...
                    if (this.armorMode.get() == ElytraExtra$ArmorFlyMode.LAZY) {
                        if (this.afV()) {
                            serializedEntryUpdateEvent.context(
                                    new SerializedEntry(var7.id(), var7.handler(), (byte) (var6 | 128)));
                            mc.getNetworkHandler()
                                    .sendPacket(new ClientCommandC2SPacket(mc.player, Mode.START_FALL_FLYING));
                            this.PM = true;
                            this.agc();
    ...
    public Vec3d agw(Vec3d velocity) {
        if (this.speedLimitEnable.get() && velocity != null) {
            ...
            if (Double.isFinite(var2) && !(var2 <= 0.0)) {
                ...
                return Double.isFinite(var4) && !(var4 <= var2 * var2)
                        ? velocity.multiply(var2 / Math.sqrt(var4))
                        : velocity;
            } else {
    ...
    public void agf(Event<ClientCommandC2SPacket> event) {
        if (!checkNull()) {
            if (this.PP == Tasks.b()
                    && mc.player.isFallFlying()
                    && ((ClientCommandC2SPacket) event.b).getMode() == Mode.START_FALL_FLYING
                    && this.agq(false)) {
                this.PN.state(true);
    ...
    public ElytraExtra() {
        ...
        this.enable2 = this.flagBuilder(this.OE.add("enable"))
                .updateListener(this::afZ)
                .build();
        this.armorMode = this.builder(this.OE.add("armor-mode"), ElytraExtra$ArmorFlyMode.class)
                .defaultValue(ElytraExtra$ArmorFlyMode.TICK)
                .build();
        this.noKineticMode = this.builder(
                        this.OC.add("no-kinetic-mode"), OptionalPrimitive.configEnum(Configs$BypassMode.class))
                .defaultValue(
                        new OptionalPrimitive<>(false, NBTTypes.o.cast(), new WrapEnum<>(Configs$BypassMode.NO_BYPASS)))
                .show(() -> !this.enable2.get())
                .build();
        this.maceHitFixMode = this.builder(
                        this.OC.add("mace-hit-fix-mode"), OptionalPrimitive.configEnum(Configs$BypassMode.class))
                .defaultValue(
                        new OptionalPrimitive<>(false, NBTTypes.o.cast(), new WrapEnum<>(Configs$BypassMode.NO_BYPASS)))
                .show(() -> !this.enable2.get()
                        || !this.armorMode.get().isIn(new ConfigEnum[] {ElytraExtra$ArmorFlyMode.TICK}))
                .build();
        this.noFallWhenLanding =
                this.flagBuilder(this.OC.add("no-fall-when-landing")).build();
        this.autoSwitch = this.flagBuilder(this.OC.add("auto-switch")).build();
        this.autoTakeOff = this.hotkey(this.OC.add("auto-take-off"), new MultiKeyBind())
                .registerHotkey(HotKeyUtils.c(this::afo))
                .build();
        this.autoTakeOffFireworksFromGround = this.flagBuilder(this.OC.add("auto-take-off-fireworks-from-ground"))
                .build();
        this.autoTakeOffRetryTicks = this.intBuilder(this.OC.add("auto-take-off-retry-ticks"))
                .defaultValue(3)
                .build();
        this.autoStartFlyJoinServer =
                this.flagBuilder(this.OC.add("auto-start-fly-join-server")).build();
        this.fireworkLiquidFly = this.flagBuilder(this.OC.add("firework-liquid-fly"))
                .hideConfig()
                .build();
        this.speedLimitEnable = this.flagBuilder(this.OC.add("speed-limit-enable"))
                .defaultValue(false)
                .build();
        ...
        this.OS = this.moduleEntry(this.OD.addHotkey(), new MultiKeyBind(), this.OD.addEnable())
                .build();
        this.period = this.intBuilder(this.OD.add("period"))
                .defaultValue(16)
                .validator(Configs.e)
                .build();
        this.resetVanilla = this.flagBuilder(this.OD.add("reset-vanilla")).build();
        this.enableHotkey = this.toggleHotkey(this.OE.add("enable-hotkey"), new MultiKeyBind(), this.OE.add("enable"))
                .build();
        this.forceNoElytra = this.flagBuilder(this.OE.add("force-no-elytra")).build();
        this.enableLiquidFly =
                this.flagBuilder(this.OE.add("enable-liquid-fly")).build();
        this.liquidFlyOnlyFireworks =
                this.flagBuilder(this.OE.add("liquid-fly-only-fireworks")).build();
        this.onGroundFlyOnlyFireworks =
                this.flagBuilder(this.OE.add("on-ground-fly-only-fireworks")).build();
        ...
        this.fireworkItemId = this.builder(this.OF.add("firework-item-id"), Regex.class)
                .defaultValue(new Regex("^()$"))
                .build();
        this.fireworkDelayMultiplyVanilla = this.intBuilder(this.OF.add("firework-delay-multiply-vanilla"))
                .defaultValue(10)
                .validator(Configs.d)
                .build();
        this.Pi = new MoveSubHelperAX(this.fireworkDelayMultiplyVanilla);
        this.fireworkAutoUseVanilla =
                this.flagBuilder(this.OF.add("firework-auto-use-vanilla")).build();
        this.autoRescaleFireworkBox =
                this.flagBuilder(this.OF.add("auto-rescale-firework-box")).build();
        ...
        this.speedDropOptimization = this.flagBuilder(this.OF.add("speed-drop-optimization"))
                .defaultValue(true)
                .show(() -> this.autoRescaleFireworkBox.get()
                        && this.autoRescaleFireworkAl.get().isIn(new ConfigEnum[] {ElytraExtra$Al.V2, ElytraExtra$Al.V3
                        }))
                .build();
        this.switchAutoRescaleFireworkAlKey = this.hotkey(
                        this.OF.add("switch-auto-rescale-firework-al-key"), new MultiKeyBind())
                .registerHotkey(HotKeyUtils.b(HackUtilHelperF::a))
                .experimental()
                .build();
        this.autoRescaleAxisZeroPointThree = this.builder(
                        this.OF.add("auto-rescale-axis-zero-point-three"), Double.class)
                .defaultValue(0.03)
                .show(this.autoRescaleFireworkBox::get)
                .build();
        this.fireworkBoostEnable =
                this.flagBuilder(this.OF.add("firework-boost-enable")).build();
        this.fireworkBoostHotkey = this.moduleEntry(
                        this.OF.add("firework-boost-hotkey"), new MultiKeyBind(), this.OF.add("firework-boost-enable"))
                .build();
        this.fireworkBoostSpeed = this.doubleBuilder(this.OF.add("firework-boost-speed"))
                .defaultValue(1.7)
                .validator(Configs.doubleRange(0.0, 10000.0))
                .build();
        this.fireworkBoostUseRescale = this.flagBuilder(this.OF.add("firework-boost-use-rescale"))
                .show(this.autoRescaleFireworkBox::get)
                .build();
        this.flyRocketOnFirstOff =
                this.flagBuilder(this.OF.add("fly-rocket-on-first-off")).build();
        this.flyRocketOnFirstOffHotkey = this.moduleEntry(
                        this.OF.add("fly-rocket-on-first-off-hotkey"),
                        new MultiKeyBind(),
                        this.OF.add("fly-rocket-on-first-off"))
                .build();
        this.clickFireworkUse = this.hotkey(this.OF.add("click-firework-use"))
                .defaultValue(new MultiKeyBind())
                .registerHotkey(HotKeyUtils.c(this::afq))
                .build();
    ...
    public void afH(Event<PlayerInteractItemC2SPacket> packet) {
        if (!this.PF && (this.afr() || this.Pz && this.PA != -1)) {
            ItemStack var2 = PlayerInteractItemC2SPacketAccess.of((PlayerInteractItemC2SPacket) packet.b)
                    .getItemStack();
    ...
    public void le(Event<KalamaHelperHelperI<ModulePreset>> presetEvent) {
        switch ((ModulePreset) ((KalamaHelperHelperI) presetEvent.b).b()) {
            case fg:
            case fh:
                this.noKineticMode.set(
                        this.noKineticMode.get().withValue(new WrapEnum<>(Configs$BypassMode.BYPASS_GRIM)));
                break;
            default:
                this.noKineticMode.set(
                        this.noKineticMode.get().withValue(new WrapEnum<>(Configs$BypassMode.NO_BYPASS)));
        }

        switch ((ModulePreset) ((KalamaHelperHelperI) presetEvent.b).b()) {
            case fg:
            case fh:
                this.maceHitFixMode.set(
                        this.maceHitFixMode.get().withValue(new WrapEnum<>(Configs$BypassMode.BYPASS_GRIM)));
                break;
            default:
                this.maceHitFixMode.set(
                        this.maceHitFixMode.get().withValue(new WrapEnum<>(Configs$BypassMode.NO_BYPASS)));
        }
    ...
    public Vec3d agB(Vec3d currentMotion, float pitch, float yaw, boolean applyGravity) {
        if (!this.autoRescaleFireworkBox.get()) {
            ...
            double var28 = Math.abs(var27.y) < 1.0E-6
                    ? 0.0
                    : (var27.y < 0.0 ? (-var27.y - var25 + 0.02) / -var24.minY : (var27.y + var25 + 0.02) / var24.maxY);
            double var30 =
                    Math.abs(var27.x) < 1.0E-6 ? 0.0 : (var27.x < 0.0 ? var27.x / var24.minX : var27.x / var24.maxX);
            double var32 =
                    Math.abs(var27.z) < 1.0E-6 ? 0.0 : (var27.z < 0.0 ? var27.z / var24.minZ : var27.z / var24.maxZ);
    ...
    public Vec3d agy(
            ...
            double maxY,
            double maxZ) {
        return this.agw(new Vec3d(
                agx(requested.x, fallback.x, minX, maxX),
                agx(requested.y, fallback.y, minY, maxY),
                agx(requested.z, fallback.z, minZ, maxZ)));
    }
