...
public class ElytraJump extends BaseModule implements HackUtilHelperJ {
    ...
    public ElytraJump() {
        ...
        this.J = this.toggleHotkey(this.aD.addHotkey(), new MultiKeyBind(), this.aD.addEnable())
                .build();
        this.conditionalSprint =
                this.flagBuilder(this.aD.add("conditional-sprint")).build();
        ...
        this.groundHeight = this.doubleBuilder(this.aD.add("ground-height"))
                .defaultValue(3.0)
                .build();
        this.zv = false;
        this.jn = false;
    ...
    public void bb(Event<LegalMovementManager> movementManagerEvent) {
        this.zv = false;
        if (this.ae.get()) {
            this.zv = !CollisionUtil.getIntersectingBlockPositions(
                            mc.world, mc.player.getBoundingBox().stretch(0.0, -this.groundHeight.get(), 0.0), false)
                    .isEmpty();
        }
