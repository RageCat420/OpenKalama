...
public class MovTest extends BaseModule implements HackUtilHelperJ {
   ...
   public boolean postModify(Event<LegalMovementManager> movementManagerEvent, boolean enabledThisTick) {
      ...
      if (this.enable()) {
         ...
         if (var3 != null) {
            Debug.chat("Speed last one sec :", mc.player.getPos().subtract(var3).length());
         }
   ...
   public void onVelocityPacket(Event<EntityVelocityUpdateS2CPacket> event) {
      if (this.enable() && !checkNull() && ((EntityVelocityUpdateS2CPacket)event.b).getEntityId() == mc.player.getId()) {
      }
   }
