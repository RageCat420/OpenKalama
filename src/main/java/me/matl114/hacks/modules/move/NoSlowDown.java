...
public class NoSlowDown extends BaseModule implements HackUtilHelperJ {
   ...
   public void preSwap(boolean v3) {
      ...
      if (!var5.isEmpty()) {
         ...
         KalamaHelperHelperK var9 = InventoryUtils.findScreenSlot(var6.slots, sl -> {
         ...
         if (var9 != null) {
            ...
            this.Gt = () -> mc.interactionManager.clickSlot(var6.syncId, var10, var7, SlotActionType.SWAP, mc.player);
            ...
            if (var11.isPresent()) {
               ...
               mc.interactionManager.clickSlot(var6.syncId, var11.getAsInt(), var3, SlotActionType.SWAP, mc.player);
               this.Gt = () -> {
                  mc.interactionManager.clickSlot(var6.syncId, var11.getAsInt(), var3, SlotActionType.SWAP, mc.player);
                  mc.interactionManager.clickSlot(var6.syncId, var11.getAsInt(), 0, SlotActionType.PICKUP, mc.player);
               };
            ...
            KalamaHelperHelperK var17 = InventoryUtils.x(
               var6.slots, sl -> sl.getStack().isEmpty() && sl.canInsert(var8) && !(sl.inventory instanceof PlayerInventory), true
            );
            ...
            if (var17 != null && var12.isPresent()) {
               ...
               mc.interactionManager.clickSlot(var6.syncId, var12.getAsInt(), var3, SlotActionType.SWAP, mc.player);
               this.Gt = () -> {
                  mc.interactionManager.clickSlot(var6.syncId, var12.getAsInt(), var3, SlotActionType.SWAP, mc.player);
                  mc.interactionManager.clickSlot(var6.syncId, var17.index(), var16, SlotActionType.SWAP, mc.player);
               };
         ...
         if (var14.isPresent()) {
            mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, var14.getAsInt(), var13, SlotActionType.SWAP, mc.player);
            ClientPlayerAccess.of(mc.player).resyncPos();
            this.Gt = () -> mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, var14.getAsInt(), var13, SlotActionType.SWAP, mc.player);
         }
      }
            ...
            switch (var1) {
               ...
               case INTERACT:
                  ...
                  if (!var5 && var1 != NoSlowDown$PacketSneakMode.BAD_PACKET) {
                     if (var10 == null
                        || var10.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > MathUtils.a(mc.player.getEntityInteractionRange() + 0.5)) {
                        Debug.b("[NoSlow] 当前模式下需要一个实体以交互");
                        return;
                     }

                     Entity var15 = Objects.requireNonNull(var10);
                     ClientPlayerAccess.of(mc.player).getLegalMovementManager().i(new MoveSubHelperCX(this, var15));
                  } else {
                     ...
                     mc.interactionManager
                        .sendSequencedPacket(
                           mc.world, seq -> new PlayerInteractEntityC2SPacket(var14, true, new InteractAtHandler(Hand.MAIN_HAND, mc.player.getPos()))
                        );
                     this.Gs = true;
   ...
   public NoSlowDown() {
      ...
      this.whenSpecialBlock = this.flagBuilder(this.Gb.add("when-special-block")).build();
      this.fakeSneak = this.flagBuilder(this.Gb.add("fake-sneak")).build();
      this.fakeSneakHotkey = this.toggleHotkey(this.Gb.add("fake-sneak-hotkey"), new MultiKeyBind(), this.Gb.add("fake-sneak")).build();
      ...
      this.useItemSwapItemDelay = this.builder(this.Gb.add("use-item-swap-item-delay"), Integer.class)
         .show(() -> this.useItemBypass.get().isIn(new ConfigEnum[]{NoSlowDown$UseBypassMode.BYPASS_GRIM_LAZY, NoSlowDown$UseBypassMode.BYPASS_GRIM_LAZY_V3}))
         .defaultValue(1)
         .build();
      this.useItemSwapNoSprint = this.flagBuilder(this.Gb.add("use-item-swap-no-sprint"))
         .show(() -> this.useItemBypass.get().isIn(new ConfigEnum[]{NoSlowDown$UseBypassMode.BYPASS_GRIM_LAZY_V3}))
         .build();
      this.blockInBypass = this.builder(this.Gb.add("block-in-bypass"), Configs$BypassMode.class).defaultValue(Configs$BypassMode.NO_BYPASS).build();
      ...
      this.blockInMineWhenJump = this.flagBuilder(this.Gb.add("block-in-mine-when-jump"))
         .show(() -> this.blockInBypass.get().isIn(new ConfigEnum[]{Configs$BypassMode.BYPASS_GRIM}))
         .build();
      this.fakeSneakMode = this.builder(this.Gb.add("fake-sneak-mode"), Configs$BypassMode.class).defaultValue(Configs$BypassMode.NO_BYPASS).build();
      this.Gq = this.hotkey(this.Gc).defaultValue(new MultiKeyBind()).registerHotkey(HotKeyUtils.b(this::onSneakStatus)).build();
      ...
      if (this.whenInBlock.get()) {
         ...
         switch ((Configs$BypassMode)this.blockInBypass.get()) {
            case BYPASS_GRIM:
               ...
               if (this.blockInMineWhenJump.get() && !mc.player.isFallFlying() && (mc.player.getVelocity().y >= 0.0 || mc.player.isOnGround()) && var3.rI()) {
                  mc.interactionManager.sendSequencedPacket(mc.world, seq -> new PlayerActionC2SPacket(Action.START_DESTROY_BLOCK, var2, Direction.UP, seq));
                  mc.interactionManager.sendSequencedPacket(mc.world, seq -> new PlayerActionC2SPacket(Action.STOP_DESTROY_BLOCK, var2, Direction.UP, seq));
                  slowMovement.cancel();
                  return;
               }
         ...
         return switch ((NoSlowDown$UseBypassMode)this.useItemBypass.get()) {
            ...
            case BYPASS_GRIM_LAZY -> {
               if (this.Gu) {
                  ...
                  if (var1 && !mc.player.hasVehicle() && PlayerInputUtils.a(mc.player).ru() && this.getActiveItemSpeedMultiplier() < 0.99F) {
            ...
            case BYPASS_GRIM_LAZY_V3 -> {
               if (mc.player.hasVehicle() || !PlayerInputUtils.a(mc.player).ru() || !(this.getActiveItemSpeedMultiplier() < 0.99F)) {
                  yield false;
               } else if (this.Gw) {
                  this.Gw = false;
                  yield true;
