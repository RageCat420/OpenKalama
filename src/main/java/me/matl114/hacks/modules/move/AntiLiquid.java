...
public class AntiLiquid extends BaseModule implements HackUtilHelperJ {
    ...
    public void applyBeforeMovementPacketModify(Event<LegalMovementManager> movementManagerEvent) {
        ...
        if (this.ae.get() && this.mode.get() != AntiLiquid$Mode.NONE) {
            Box var2 = mc.player
                    .getBoundingBox()
                    .expand(this.waterExpandCheck.get(), this.waterExpandCheck.get(), this.waterExpandCheck.get());
    ...
    public AntiLiquid() {
        super("AntiLiquid");
        this.mode = this.builder(this.gR.add("mode"), AntiLiquid$Mode.class)
                .defaultValue(AntiLiquid$Mode.NONE)
                .build();
        this.ae = this.flagBuilder(this.gR.addEnable()).build();
        this.J = this.moduleEntry(
                        this.gR.addHotkey(), new MultiKeyBind(), this.gR.addEnable(), moduleMeta(() -> this.mode))
                .build();
        this.waterExpandCheck = this.doubleBuilder(this.gR.add("water-expand-check"))
                .defaultValue(0.2)
                .build();
        ...
        this.autoArmorFlyControl =
                this.flagBuilder(this.gR.add("auto-armor-fly-control")).build();
        this.autoArmorFlySwitchGt = this.intBuilder(this.gR.add("auto-armor-fly-switch-gt"))
                .defaultValue(5)
                .build();
        this.leaveWaterExpandCheck = this.doubleBuilder(this.gR.add("leave-water-expand-check"))
                .defaultValue(2.0)
                .build();
        this.gZ = 0;
        this.ha = 0;
    ...
    private void handleOutOfWater() {
        if (this.currentArmorGlidingSaveState && Tasks.b() > this.ha + this.autoArmorFlySwitchGt.get()) {
            Box var1 = mc.player
                    .getBoundingBox()
                    .expand(
                            this.leaveWaterExpandCheck.get(),
                            this.leaveWaterExpandCheck.get(),
                            this.leaveWaterExpandCheck.get());
