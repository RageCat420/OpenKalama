...
public class Flight extends BaseModule implements HackUtilHelperJ {
    ...
    public boolean shouldResetMotion() {
        ...
        if (this.hM == 0) {
            ...
            return var3 < this.hL
                    ? true
                    : MovTasks.h.checkEnvironmentCollision(
                            mc.player, mc.player.getPos().add(0.0, -0.096, 0.0), false);
        } else {
            this.hL = var1;
            this.hM++;
            return this.hM > 10;
    ...
    public Vec3d dispatchAntiKickMotion(Vec3d controlMotion) {
        boolean var2 = false;
        return !var2 && (!this.isActive() || !this.antikick.get())
                ? controlMotion
                : this.processAntiKickMotion(controlMotion, var2);
    }
    ...
    public Flight() {
        ...
        this.flightEnable = this.flagBuilder(this.hl.add("flight-enable"))
                .defaultValue(false)
                .build();
        this.flightMode = this.builder(this.hl.add("flight-mode"), Flight$Mode.class)
                .defaultValue(Flight$Mode.CREATIVE)
                .build();
        this.flightEnableHotkey = this.moduleEntry(
                        this.hl.add("flight-enable-hotkey"),
                        new MultiKeyBind(),
                        this.hl.add("flight-enable"),
                        moduleMeta(() -> this.flightMode))
                .build();
        this.flightModeSwitchHotkey = this.hotkey(this.hl.add("flight-mode-switch-hotkey"), new MultiKeyBind())
                .registerHotkey(HotKeyUtils.b(this::kT))
                .build();
        this.antikick = this.builder(this.hl.add("antikick"), Boolean.class)
                .defaultValue(true)
                .build();
        this.antikickPeriod =
                this.intBuilder(this.hl.add("antikick-period")).defaultValue(60).build();
        this.flySpeed = this.builder(this.hm.add("fly-speed"), Boolean.class)
                .defaultValue(false)
                .build();
        this.toggleFlightSpeed = this.toggleHotkey(
                        this.hm.add("toggle-flight-speed"), new MultiKeyBind(), this.hm.add("fly-speed"))
                .build();
        this.flySpeedCreative = this.builder(this.hm.add("fly-speed-creative"), Double.class)
                .defaultValue(0.8)
                .build();
        this.flySpeedSurvival = this.builder(this.hm.add("fly-speed-survival"), Double.class)
                .defaultValue(0.4)
                .build();
        this.walkSpeed = this.flagBuilder(this.hm.add("walk-speed")).build();
        this.toggleWalkSpeed = this.toggleHotkey(
                        this.hm.add("toggle-walk-speed"), new MultiKeyBind(), this.hm.add("walk-speed"))
                .build();
        this.walkSpeedOverride = this.builder(this.hm.add("walk-speed-override"), Double.class)
                .defaultValue(0.1)
                .build();
        this.ongroundWhenMine =
                this.flagBuilder(this.hl.add("onground-when-mine")).build();
        this.hB = false;
        this.hC = 0;
        this.hD = 0;
        this.hE = 0;
    ...
    public double getOverridingFlySpeed() {
        return mc.player != null && mc.interactionManager.getCurrentGameMode().isCreative()
                ? this.flySpeedCreative.get()
                : this.flySpeedSurvival.get();
    }
    ...
    public void applyPreTickModify(Event<LegalMovementManager> movementManagerEvent) {
        ...
        boolean var7 =
                switch ((Flight$Mode) this.flightMode.get()) {
                    case CREATIVE -> {
                        this.kU(var2);
                        yield true;
                    }
                    case MOTION -> {
                        if (this.isActive() && mc.player.getAbilities().flying) {
                            ...
                            Vec3d var11 = EntityUtils.movementInputToVelocity(
                                    var10, (float) (5.0 * this.getOverridingFlySpeed()), var2.getYaw());
                            var2.setVelocity(var11);
                            var11 = this.dispatchAntiKickMotion(var11);
                            FlightVelocity var6 =
                                    new FlightVelocity(var11, 5.0 * this.getOverridingFlySpeed(), HackUtilHelperA.rk);
    ...
    public void onStartMine(Event<PlayerActionC2SPacket> actionPacket) {
        if (this.ongroundWhenMine.get()
                && ((PlayerActionC2SPacket) actionPacket.e()).getAction() == Action.START_DESTROY_BLOCK) {
            this.hE = Tasks.b();
        }
    }
}
