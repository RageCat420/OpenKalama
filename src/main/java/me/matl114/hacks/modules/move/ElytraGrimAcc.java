...
public class ElytraGrimAcc extends BaseModule implements HackUtilHelperJ {
   ...
   private void Zp() {
      if (!this.fixKickFromLag.get() || !AntiChunkLag.INSTANCE.currentMayFaceLagChunk) {
         if (!this.fixKickFromLag.get() || Tasks.b() >= this.Kf + 20) {
            ...
            PlayerMoveC2SPacketAccess.setCause((PlayerMoveC2SPacket)(Object)this.Ad, PlayerMoveC2SPacketAccess.Cause.TRIGGER_SIMULATION);
         }
      }
   }
   ...
   public void bb(Event<LegalMovementManager> preTickEvent) {
      this.Kh = (this.enable.get() || this.Kh) && mc.player.isFallFlying() && !mc.player.isOnGround() && !MovTasks.ax().agj();
      if (this.Kh) {
         this.Kg = Tasks.b();
      }
   }
   ...
   public ElytraGrimAcc() {
      ...
      this.hotkey = this.toggleHotkey(this.Ka.add("hotkey"), new MultiKeyBind(), this.Ka.add("enable")).build();
      ...
      this.minAccelerateVelocity = this.builder(this.Ka.add("min-accelerate-velocity"), Double.class)
         ...
         .validator(Configs.doubleRange(0.0, 100.0))
         .build();
      this.fixKickFromLag = this.builder(this.Ka.add("fix-kick-from-lag"), Boolean.class).defaultValue(true).build();
      this.Ad = null;
      this.Ke = 0;
      this.Kf = 0;
      this.Kg = 0;
   ...
   public void Zm(Event<EntityVelocityUpdateS2CPacket> event) {
      ...
      if (mc.player != null
         ...
         if (var2.horizontalLengthSquared() < 0.01) {
            ...
            if (var3.horizontalLengthSquared() > 0.01 && var3.withAxis(Axis.Y, 0.0).dotProduct(var2.withAxis(Axis.Y, 0.0)) < 0.0) {
               event.cancel();
            }
         }
      }
