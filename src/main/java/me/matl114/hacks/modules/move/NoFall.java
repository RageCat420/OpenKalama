...
public class NoFall extends BaseModule implements HackUtilHelperJ {
    ...
    public void applyPreTickModify(Event<LegalMovementManager> movementManagerEvent) {
        ...
        if (var3 == null) {
            this.entityStage = 0;
        } else if (var2.getAbilities().invulnerable
                || this.disableWhenAllowFlying.get() && MovTasks.at().hB
                || this.checkInvulnerableEquipment()) {
            this.entityStage = 3;
            ...
            this.Ux = var2.getY();
            this.UF =
                    var2.getAttributeValue(EntityAttributes.GENERIC_SAFE_FALL_DISTANCE) + this.safeDistanceModify.get();
            if (var2.isOnGround()
    ...
    public NoFall() {
        super("NoFall");
        this.bypassMode = this.builder(this.Uq.add("bypass-mode"), NoFall$Mode.class)
                .defaultValue(NoFall$Mode.LAZY_MODE)
                .build();
        ...
        this.J = this.moduleEntry(
                        ...
                        new MultiKeyBind(),
                        this.Uq.add("toggle"),
                        moduleMeta(() -> this.bypassMode))
                .build();
        this.safeDistanceModify = this.intBuilder(this.Uq.add("safe-distance-modify"))
                .defaultValue(0)
                .build();
        this.equipmentIdBypassNofall = this.builder(this.Uq.add("equipment-id-bypass-nofall"), Regex.class)
                .defaultValue(new Regex("^(SLIME.*_BOOTS)$"))
                .build();
        this.disableWhenAllowFlying = this.builder(this.Uq.add("disable-when-allow-flying"), Boolean.class)
                .defaultValue(true)
                .build();
        this.Uw = -2.1474836E9F;
