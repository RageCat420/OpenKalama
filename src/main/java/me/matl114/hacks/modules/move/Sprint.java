...
public class Sprint extends BaseModule implements HackUtilHelperJ {
    ...
    public boolean postModify(Event<LegalMovementManager> movementManagerEvent, boolean enabledThisTick) {
        this.Om = false;
        this.On = false;
        if (!((LegalMovementManager) movementManagerEvent.b).c.b
                && ((LegalMovementManager) movementManagerEvent.b).c.a.isOnGround()) {
            this.On = true;
        }
    ...
    public Sprint() {
        super("Sprint");
        this.legalAutoSprint =
                this.flagBuilder(this.Of.add("legal-auto-sprint")).build();
        this.legalAutoSprintHotkey = this.toggleHotkey(
                        this.Of.add("legal-auto-sprint-hotkey"), new MultiKeyBind(), this.Of.add("legal-auto-sprint"))
                .build();
        ...
        this.fakeSprintMode = this.builder(this.Of.add("fake-sprint-mode"), Configs$BypassMode.class)
                .defaultValue(Configs$BypassMode.NO_BYPASS)
                .build();
        this.allDirectionSprint =
                this.flagBuilder(this.Of.add("all-direction-sprint")).build();
        this.bypassMode = this.builder(this.Of.add("bypass-mode"), Configs$BypassMode.class)
                .defaultValue(Configs$BypassMode.NO_BYPASS)
                .build();
        this.Om = false;
        this.On = false;
    ...
    public void applyPreTickModify(Event<LegalMovementManager> movementManagerEvent) {
        if (this.allDirectionSprint.get() && this.bypassMode.getValue().hasAc()) {
            ...
            if (this.mayWorkSprint()
                    && var2.rK()
                    && !var2.rE()
                    && var2.rF()
                    && !((LegalMovementManager) movementManagerEvent.b).d()) {
    ...
    public void onTick(Event<ClientPlayerEntity> event) {
        if (this.legalAutoSprint.get()
                && !mc.options.sprintKey.isPressed()
                && PlayerInputUtils.of(mc.options).ru()) {
