...
abstract class MoveSubHelperAj implements MoveSubHelperAh, HackUtilHelperJ {
    ...
    protected void ahI() {
        ...
        if (!ElytraExtra.INSTANCE.agj()
                && Tasks.b() % 5 == 0
                && MinecraftClient.getInstance().player.getVelocity().y < 0.0) {
    ...
    protected boolean ahK() {
        if (this.QL.pitch40AutoPullUp.get()
                && MinecraftClient.getInstance().player.getY() < this.QL.minHeight.get() - 16) {
            this.ahI();
        }

        if (this.QL.pitch40EndSafety.get()
                && MinecraftClient.getInstance().player.getY() < this.QL.minHeight.get() - 32) {
