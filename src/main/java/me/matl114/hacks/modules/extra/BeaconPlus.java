...
public class BeaconPlus extends BaseModule {
   ...
   public BeaconPlus() {
      super("BeaconPlus");
      this.enableBeaconEnhance = this.flagBuilder(this.ad.add("enable-beacon-enhance")).build();
   ...
   public void onScreenInitialize(Event<Screen> e) {
      if (this.enableBeaconEnhance.get() && e.e() instanceof BeaconScreen var3) {
         ...
         BeaconEffectSelectButton var6 = new BeaconEffectSelectButton(var4 + 167 - 23, var5 + 47 + 26, 22, 22, Text.literal("第一等级: "));
         var9.addDrawableChildTo(var6);
         BeaconEffectSelectButton var7 = new BeaconEffectSelectButton(var4 + 167 + 1, var5 + 47 + 26, 22, 22, Text.literal("第二等级: "));
         var9.addDrawableChildTo(var7);
         AtomicReference var8 = new AtomicReference<>(
            ButtonWidget.builder(
                  Text.literal("Send packet"),
                  b -> {
                     MinecraftClient.getInstance()
                        .getNetworkHandler()
                        .sendPacket(new UpdateBeaconC2SPacket(Optional.ofNullable(var6.fL()), Optional.ofNullable(var7.fL())));
