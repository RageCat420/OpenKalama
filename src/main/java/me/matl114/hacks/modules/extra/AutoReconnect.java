...
public class AutoReconnect extends BaseModule {
    ...
    public AutoReconnect() {
        ...
        this.delay = this.intBuilder(this.JI.add("delay"))
                .defaultValue(5)
                .validator(Configs.e)
                .build();
        this.counter = 0;
    ...
    public void onScreenInitialize(Event<DisconnectedScreen> event) {
        if (this.enableButtons.get()) {
            ...
            var3.Q(ExecutableWidget.instance(0, 25, 200, 20)
                    .eV(new ButtonElement(
                            el -> this.enable.get()
                                    ? Text.literal("Toggle Auto reconnect off (%d sec)"
                                            .formatted(Math.max(this.counter, 0) / 20))
                                    : Text.literal("Toggle Auto reconnect on"),
                            ButtonAction.a(this.enable::toggle))));
            var3.Q(ExecutableWidget.instance(0, 0, 200, 20)
                    .eV(new ButtonElement(TextProvider.c(Text.literal("Reconnect")), ButtonAction.a(() -> {
