...
public class AutoLogout extends BaseModule {
   ...
   public void onTick(Event<ClientPlayerEntity> event) {
      if (this.ae.get()) {
         ...
         if (this.totemLeft.get()) {
            double var3 = InventoryUtils.computePlayerInventory(v -> v.getItem() == Items.TOTEM_OF_UNDYING ? (double)v.getCount() : null, false);
   ...
   public AutoLogout() {
      super("AutoLogout");
      this.hotkey = this.toggleHotkey(this.Bq.add("hotkey"), new MultiKeyBind(), this.Bq.add("enable")).build();
      this.health = this.flagBuilder(this.Bq.add("health").add("enable")).build();
      this.health2 = this.builder(this.Bq.add("health").add("threshold"), IntRef.TYPE).defaultValue(4).build();
      this.totemLeft = this.flagBuilder(this.Bq.add("totem-left").add("enable")).build();
      this.totemLeft2 = this.builder(this.Bq.add("totem-left").add("threshold"), IntRef.TYPE).defaultValue(1).build();
      this.totemTrigger = this.flagBuilder(this.Bq.add("totem-trigger").add("enable")).build();
      this.totemTrigger2 = this.builder(this.Bq.add("totem-trigger").add("threshold"), IntRef.TYPE).defaultValue(99).build();
      this.minHeight = this.flagBuilder(this.Bq.add("min-height").add("enable")).build();
      this.minHeight2 = this.builder(this.Bq.add("min-height").add("threshold"), IntRef.TYPE).defaultValue(256).build();
      this.strangerPlayer = this.flagBuilder(this.Bq.add("stranger-player").add("enable")).build();
   ...
   public void onPacketTotem(Event<EntityStatusS2CPacket> event) {
      ...
      if (this.ae.get() && this.totemTrigger.get() && var2.getStatus() == 35 && mc.world != null && mc.player != null) {
         Entity var3 = var2.getEntity(mc.world);
         if (var3 != null && var3.getId() == mc.player.getId()) {
            int var4 = this.totemTrigger2.get();
            double var5 = InventoryUtils.computePlayerInventory(v -> v.getItem() == Items.TOTEM_OF_UNDYING ? (double)v.getCount() : null, false) - 1.0;
            if (var5 <= var4) {
               MainTasks.q();
               return;
            }
