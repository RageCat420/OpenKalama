...
public class EventNotify extends BaseModule {
    ...
    public final KeyBindRef J = this.moduleEntry(this.iE.addHotkey(), new MultiKeyBind(), this.iE.addEnable())
            .build();
    ...
    public void QJ(Event<Void> eventReconfiguration) {
        if (this.ae.get()
                && this.enableQueue.get()
                && !eventReconfiguration.<Boolean>getArgs(0)
                && this.lastOrder != 0) {
            this.lastOrder = 0;
            if (this.checkMin()) {
                return;
    ...
    public void addCustomWidgets(Consumer<DrawableWidget> acceptor, int dx, int dy, int dblank) {
        ...
        acceptor.accept(this.createExecuteButton(
                "widget.event-notify.test-usage", ButtonAction.a(this::registerAll), 0, dblank, dx, dy));
    }
    ...
    public EventNotify() {
        super("EventNotify");
        this.mode = this.builder(this.iE.add("mode"), NotifyType.class)
                .defaultValue(NotifyType.TRAY)
                .build();
        this.minOnly = this.flagBuilder(this.iE.add("min-only")).build();
        this.minWidth =
                this.doubleBuilder(this.iE.add("min-width")).defaultValue(0.0).build();
        ...
        this.queue3cTitleRegex = this.builder(this.iE.add("queue-3c-title-regex"), Regex.class)
                ...
                .build();
        this.order = this.builder(this.iE.add("order"), IntPrimitiveList.class)
                .defaultValue(new IntPrimitiveList(List.of(5, 10)))
                .build();
        this.enableDisconnectServer =
                this.flagBuilder(this.iE.add("enable-disconnect-server")).build();
        this.enablePopTotem = this.flagBuilder(this.iE.add("enable-pop-totem")).build();
        this.enableBaritoneEnd =
                this.flagBuilder(this.iE.add("enable-baritone-end")).build();
        this.lastOrder = 0;
