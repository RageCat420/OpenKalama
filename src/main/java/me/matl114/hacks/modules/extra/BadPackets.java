...
public class BadPackets extends BaseModule {
   ...
   public final FlagRef FI = this.builder(this.FH.add("fix-dup-sprint"), Boolean.class).defaultValue(true).build();
   public final FlagRef fixIllegalServerViewDistance;
   boolean FU;
   public final FlagRef fixBadBlockTags;
   float FY;
   public final FlagRef FJ = this.builder(this.FH.add("fix-dup-sneak"), Boolean.class).defaultValue(true).build();
   ...
   public BadPackets() {
      super("BadPackets");
      this.fixDupInput = this.builder(this.FH.add("fix-dup-input"), Boolean.class).defaultValue(true).build();
      this.exemptDupRot = this.builder(this.FH.add("exempt-dup-rot"), Boolean.class).defaultValue(true).build();
      this.filterDupRot = this.builder(this.FH.add("filter-dup-rot"), Boolean.class).defaultValue(false).build();
      this.fixFlyPackets = this.builder(this.FH.add("fix-fly-packets"), Boolean.class).defaultValue(true).build();
      this.fixDupRot = this.builder(this.FH.add("fix-dup-rot"), Boolean.class).defaultValue(true).build();
      this.fixFullDupRot = this.builder(this.FH.add("fix-full-dup-rot"), Boolean.class).defaultValue(false).build();
      this.fixIllegalServerViewDistance = this.builder(this.FH.add("fix-illegal-server-view-distance"), Boolean.class).defaultValue(true).build();
      this.fixBadBlockTags = this.builder(this.FH.add("fix-bad-block-tags"), Boolean.class).defaultValue(true).build();
      this.fixLitematicaTransmit = this.builder(this.FH.add("fix-litematica-transmit"), Boolean.class).defaultValue(true).build();
      this.fixInvalidPlayerEntryUpdate = this.flagBuilder(this.FH.add("fix-invalid-player-entry-update")).build();
      this.FU = false;
      this.FV = false;
   ...
   public void onSendSprint(Event<ClientCommandC2SPacket> event) {
      if (((ClientCommandC2SPacket)event.e()).getMode() == Mode.START_SPRINTING || ((ClientCommandC2SPacket)event.e()).getMode() == Mode.STOP_SPRINTING) {
      ...
      if (this.filterDupRot.get()
         ...
         if (var2.changesLook() && this.fixDupRot.get()) {
            if (this.fixFullDupRot.get() && var2 instanceof Full var9) {
               packetEvent.context(
                  PlayerMoveC2SPacketAccess.setCauseFrom(
                     VPacket.g(
               ...
               packetEvent.context(PlayerMoveC2SPacketAccess.setCauseFrom(VPacket.f(var11.isOnGround(), VPacket.getCollisionFlag(var11)), var11));
            }
         }
      } else {
         this.FX = var3;
   ...
   public void onAbilityUpdate(Event<UpdatePlayerAbilitiesC2SPacket> event) {
      if (!event.d() && this.fixFlyPackets.get() && !this.FV && ((UpdatePlayerAbilitiesC2SPacket)event.e()).isFlying()) {
         event.cancel();
      }
   }
