...
public class ClientExtra extends BaseModule {
   ...
   public final FlagRef lN = this.builder(this.ad.add("no-client-crash"), Boolean.class).defaultValue(false).build();
      ...
      if (this.noDisconnectOnPacketDecode.get()) {
         ...
         if (var3 instanceof ClientPlayPacketListener var5) {
            if (mc.player != null) {
               ...
               Debug.chat(var4.getClass().getSimpleName(), ":", Text.literal(var4.getMessage() == null ? "Exception: null" : var4.getMessage()));
            }
   ...
   public void onCrash(Event<MinecraftClient> event) {
      if (event.g() && ((MinecraftClient)event.e()).isRunning() && this.lN.get()) {
         ...
         List var9 = List.of(
            KalamaHelperHelperA.of(Text.literal("我已知晓, 继续游戏").formatted(Formatting.GREEN), Runnables.doNothing()),
   ...
   public void onBlockEntityException(Event<KalamaHelperHelperE> event) {
      if (this.noBlockEntityCrash.get()) {
         ...
         if (!var3.isRemoved()) {
            ...
            Debug.chat("Error while ticking blockEntity at world:", ChatUtils.t(Vec3d.of(var3.getPos())), "World:", var4.getRegistryKey().getValue());
            Debug.chat(var5.getClass().getSimpleName(), ":", Text.literal(var5.getMessage() == null ? "Exception: null" : var5.getMessage()));
   ...
   public ClientExtra() {
      super("ClientExtra");
      this.clientCrashKeepInServer = this.flagBuilder(this.ad.add("client-crash-keep-in-server")).build();
      this.noEntityCrash = this.flagBuilder(this.ad.add("no-entity-crash")).build();
      this.noBlockEntityCrash = this.flagBuilder(this.ad.add("no-block-entity-crash")).build();
      this.noDisconnectOnNetworkError = this.builder(this.ad.add("no-disconnect-on-network-error"), Boolean.class).defaultValue(false).build();
      this.noDisconnectOnPacketDecode = this.builder(this.ad.add("no-disconnect-on-packet-decode"), Boolean.class).defaultValue(false).build();
      this.noDisconnectOnPacketUnexpected = this.builder(this.ad.add("no-disconnect-on-packet-unexpected"), Boolean.class).defaultValue(false).build();
      this.keepGuiOpenOnPortal = this.flagBuilder(this.ad.add("keep-gui-open-on-portal")).build();
      this.clientBrandName = this.builder(this.ad.add("client-brand-name"), StringRef.TYPE).defaultValue("").build();
      this.lW = this.hotkey(Configs.j, this.ad.add("cursor-switch-hotkey").toPath())
         ...
         .build();
      this.fixPaletteException = this.flagBuilder(this.ad.add("fix-palette-exception")).build();
      this.logSelfServerLeaving = this.flagBuilder(this.ad.add("log-self-server-leaving")).build();
               ...
               if (var6 != mc.player) {
                  Debug.a(
                     "    - Name: "
                        ...
                        + ", Pos: "
                        + var6.getPos()
                        + ", dist: %.2f".formatted(var6.getPos().subtract(mc.player.getPos()).length())
                  );
               }
            }
         }
   ...
   public void onNetworkException(Event<KalamaHelperHelperE> event) {
      if (this.noDisconnectOnNetworkError.get()) {
         ...
         Throwable var5 = var2.exception();
         if (mc.player != null) {
            Debug.b(Text.literal("Error while handling a network packet: ").formatted(Formatting.RED).append(Text.literal(var3.getClass().getSimpleName())));
            Debug.chat(var5.getClass().getSimpleName(), ":", Text.literal(var5.getMessage() == null ? "Exception: null" : var5.getMessage()));
         }
   ...
   protected void checkClientData(Screen screen) {
      ...
      if (var4 && mc.player != null && mc.world != null && mc.inGameHud != null && mc.getNetworkHandler() != null && mc.interactionManager != null) {
