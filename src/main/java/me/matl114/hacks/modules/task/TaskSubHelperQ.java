...
public record TaskSubHelperQ(int selected, List<TaskSubHelperK> entries) {
   public static Codec<TaskSubHelperQ> CODEC = RecordCodecBuilder.create(
      oinstance -> oinstance.group(
            Codec.INT.fieldOf("index").forGetter(TaskSubHelperQ::Rj), Codec.list(TaskSubHelperK.dt).fieldOf("entries").forGetter(TaskSubHelperQ::Rk)
         )
