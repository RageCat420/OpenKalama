...
public record TaskSubHelperK(String name, ConnectionProxy$Type type, String address, int port, String userName, String password) {
   ...
   public static Codec<TaskSubHelperK> dt = RecordCodecBuilder.create(
      oinstance -> oinstance.group(
            Codec.STRING.fieldOf("name").forGetter(TaskSubHelperK::name),
            CodecUtils.enumCodec(ConnectionProxy$Type.class).fieldOf("type").forGetter(TaskSubHelperK::type),
