...
public class Proxy extends BaseModule {
    ...
    public DrawableWidget createEditRenderHandler(MutableRecord argsMap, MutableObject<MutableRecord> index) {
                ...
                .<ExecutableWidget>eV(new MultiLineTextElement(
                        el -> {
                            ...
                            return Text.literal("%s\n(%s:%s:%d)"
                                    .formatted(var2.name(), var2.type().name(), var2.address(), var2.port()));
                        },
        ...
        ExecutableWidget.instance(155, 0, 60, 20)
                .<ExecutableWidget>eV(new ButtonElement(
                        TextProvider.c(Text.translatable("widget.connection-proxy.open-editor")), ButtonAction.a(() -> {
    ...
    public void openProxyListEditScreen() {
        List<MutableRecord> var1 = new ArrayList<>(this.qC.Rk().stream()
                .map(s -> MutableRecord.of(TaskSubHelperK.ds, s))
                .toList());
        int var2 = this.qC.Rj();
        ...
        ListEntryWidgetController var4 = ListEntryWidgetController.mutable(
                var1,
                () -> MutableRecord.of(TaskSubHelperK.ds, TaskSubHelperK.EMPTY),
                v -> this.createEditRenderHandler(v, var3),
                30,
                220);
        KalamaHelperHelperD var5 = new KalamaHelperHelperD(var4, 0, 0, 320, 260);
        ConfirmingWidgetScreen var6 = new ConfirmingWidgetScreen(
                Text.translatable("widget.connection-proxy.proxy-list-editor.title"), var5, () -> true, () -> {
                    List<TaskSubHelperK> var3x =
                            var1.stream().map(s -> s.j(TaskSubHelperK.class)).toList();
                    ...
                    switch (var3.type()) {
                        case SOCKS:
                            if (var7.isEmpty()) {
                                ...
                                var2.addFirst(
                                        "socks5ClientProxy", new Socks5ProxyHandler(var8, var6 ? null : var5, var7));
                            }
                            break;
                        case HTTP:
                            var2.addFirst(
                                    "httpClientProxy",
                                    new HttpProxyHandler(var8, var6 ? null : var5, var7.isEmpty() ? null : var7));
                        case HTTPS:
                    }
                }
            }
