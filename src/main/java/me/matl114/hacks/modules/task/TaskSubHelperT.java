...
class TaskSubHelperT implements KalamaHelperHelperH {
   ...
   public List<String> b(CommandExecution sender, ArgumentInputStream streamArgs, ArgumentReader argsReader) {
      ...
      return this.a.LI().filter(s -> s.toLowerCase(Locale.ROOT).startsWith(var5.toLowerCase(Locale.ROOT))).toList();
   }
}
