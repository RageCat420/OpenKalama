...
public class ConfigManager extends BaseModule {
   ...
   public static final Codec<MapRef> yq = Codec.PASSTHROUGH.comapFlatMap(dynamic -> {
      Object var1 = dynamic.convert(ConfigOp.INSTANCE).getValue();
      return var1 instanceof MapRef var2 ? DataResult.success(var2) : DataResult.error(() -> "Config payload is not a MapRef: " + var1);
   ...
   public void LB(ArgumentInputStream args, ArgumentReader argsReader) {
      ...
      if (FileManager.getInstance().s(var4)) {
         ...
         Debug.b(Text.literal("点击本文本打开文件夹以查看或重命名").formatted(Formatting.YELLOW).styled(style -> style.withClickEvent(ChatUtils.getOpenFile(FileManager.f))));
      } else {
         ...
         if (!var5.isEmpty()) {
            ...
            for (BaseModule var8 : var5) {
               for (WrapperConfigRef var10 : var8.getEditableConfig()) {
                  ...
                  Ref var13 = var10.h();
                  var6.computeIfAbsent(var11.getRegistryKey().getValue(), k -> new MapRef()).setValue(var13, var12);
               }
            }
            ...
            for (Identifier var18 : new HashSet<>(var6.keySet())) {
               if (this.LL(var18, var15)) {
                  Debug.b(Text.literal("保存时跳过配置: " + var18 + " 以避免隐私信息泄露(可在设置中调整关键词)").formatted(Formatting.YELLOW));
                  var6.remove(var18);
               }
            }
   ...
   private List<BaseModule> LG(ArgumentReader argsReader) {
      Map var2 = HackModules.getModuleGroups()
         .stream()
   ...
   private boolean LL(Identifier identifier, List<String> privacyKeywords) {
      ...
      return privacyKeywords.stream().filter(keyword -> keyword != null && !keyword.isBlank()).anyMatch(var3::contains);
   }
            ...
            if (var6 != null) {
               for (BaseModule var8 : var5) {
                  for (WrapperConfigRef var10 : var8.getEditableConfig()) {
                     ...
                     String[] var12 = var10.getPath();
                     Ref var13 = var10.h();
                     MapRef var14 = var6.snapSnot().get(var11.getRegistryKey().getValue());
                     if (var14 != null) {
   ...
   private Stream<String> LJ(String configName) {
      if ("all".equalsIgnoreCase(configName)) {
         return Config.REGISTRY.stream().flatMap(config -> config.getVisiblePaths().stream()).filter(Objects::nonNull).distinct().sorted();
      } else {
         Config var2 = (Config)Config.REGISTRY.get(Identifier.tryParse(configName));
         return var2 == null ? Stream.empty() : var2.getVisiblePaths().stream().filter(Objects::nonNull).distinct().sorted();
      }
   }
   ...
   public Stream<String> LI() {
      return HackModules.getModuleGroups().stream().flatMap(s -> s.getModules().stream().map(BaseModule::getName)).distinct().sorted();
   }
   ...
   public void Ls(MainCommand mainCommand) {
      ...
      KalamaHelperHelperB var3 = KalamaHelperHelperA.a()
         .B("config_name")
         .c(
            KalamaHelperHelperF.g(
               () -> Config.REGISTRY.stream().map(Config::getRegistryKey).filter(Objects::nonNull).map(registryKey -> registryKey.getValue().toString())
            )
         )
         .v();
      KalamaHelperHelperB var4 = KalamaHelperHelperA.a()
         ...
         .v();
      KalamaHelperHelperB var5 = KalamaHelperHelperA.a().B("path").c(KalamaHelperHelperF.h((sender, configName) -> {
         Config var2x = (Config)Config.REGISTRY.get(Identifier.tryParse(configName));
         return var2x == null ? Stream.empty() : Stream.concat(var2x.getVisiblePaths().stream(), var2x.getPaths().stream()).distinct().sorted();
      })).v();
      KalamaHelperHelperB var6 = KalamaHelperHelperA.a().B("path").c(KalamaHelperHelperF.g(() -> Stream.of("<填写路径>"))).v();
      KalamaHelperHelperB var7 = KalamaHelperHelperA.a().B("path_prefix").b("").c(KalamaHelperHelperF.h((sender, configName) -> this.LJ(configName))).v();
      KalamaHelperHelperB var8 = KalamaHelperHelperA.a()
         .B("path")
         .c(KalamaHelperHelperF.g(() -> Stream.of("<填写路径>")))
         .c(KalamaHelperHelperF.g(me.matl114.utils.a.KalamaHelperHelperA.z(FileManager.f, sx -> sx.endsWith(".nbt") || sx.endsWith(".dat"))))
         .v();
         ...
         try (FileStorage var4 = FileManager.getInstance().t(fileName).t()) {
            ...
            Debug.b(
               Text.literal("成功保存配置快照: " + fileName + " ,点击本文本打开文件夹")
                  ...
                  .styled(style -> style.withClickEvent(ChatUtils.getOpenFile(var4.q().getParentFile())))
            );
         }
      }
   }
            ...
            for (Config var9 : Config.REGISTRY) {
               if (var9.getRegistryKey() != null) {
                  ...
                  if (this.LL(var10, var6)) {
                     Debug.b(Text.literal("保存时跳过配置: " + var10 + " 以避免隐私信息泄露(可在设置中调整关键词)").formatted(Formatting.YELLOW));
                  } else {
      ...
      if ("--confirm".equals(var3)) {
         ...
         Debug.chat(
            ...
            Text.literal("[确认]")
               ...
               .styled(style -> style.withClickEvent(ChatUtils.getSuggestCommand(MainCommand.l + "config resetall " + var2 + " --confirm")))
         );
      }
   }
   ...
   private void LM() {
      Debug.b(
         Text.literal("请将保存的 config 文件拖到配置快照目录中，点击本文本打开文件夹")
