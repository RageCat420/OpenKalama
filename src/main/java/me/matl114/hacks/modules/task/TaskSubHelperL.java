...
public record TaskSubHelperL(Map<String, String> ipToFolder, Map<String, String> ipProxy) {
   public static final Codec<TaskSubHelperL> CODEC = RecordCodecBuilder.create(
      oinstance -> oinstance.group(
            ...
            Codec.unboundedMap(Codec.STRING, Codec.STRING).optionalFieldOf("ip-proxies", Map.of()).forGetter(TaskSubHelperL::ipToFolder)
         )
         .apply(oinstance, TaskSubHelperL::new)
   );

   public TaskSubHelperL VE(Map<String, String> ipToFolder) {
      return this.ipProxy == ipToFolder ? this : new TaskSubHelperL(ipToFolder, this.ipToFolder);
   }
   ...
   public String VD(String ip) {
      return this.ipToFolder.getOrDefault(ip, ip);
   }
   ...
   public TaskSubHelperL VF(Map<String, String> ipProxy) {
      return this.ipToFolder == ipProxy ? this : new TaskSubHelperL(this.ipProxy, ipProxy);
   }

   public TaskSubHelperL(Map<String, String> ipToFolder, Map<String, String> ipProxy) {
      this.ipProxy = ipToFolder;
      this.ipToFolder = ipProxy;
   }

   public String VC(String ip) {
      String var2 = this.VD(ip);

      for (Entry var4 : this.ipProxy.entrySet()) {
