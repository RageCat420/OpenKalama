...
public class AutoEat extends BaseModule {
    ...
    private boolean isHealingPotion(ItemStack stack) {
        ...
        if (var2 == null && var3 == null) {
            return false;
                ...
                && Streams.of(var3.getEffects())
                        .<RegistryEntry>map(StatusEffectInstance::getEffectType)
                        .anyMatch(this.gv::contains)) {
            return true;
        } else {
            if (var2 != null) {
    ...
    public AutoEat() {
        ...
        this.J = this.toggleHotkey(this.ga.addHotkey(), new MultiKeyBind(), this.ga.addEnable())
                .build();
        this.log = this.builder(this.ga.add("log"), Boolean.class)
                .defaultValue(true)
                .build();
        this.inv = this.builder(this.ga.add("inv"), Boolean.class)
                .defaultValue(true)
                .build();
        this.leftClickToolForceEat =
                this.flagBuilder(this.ga.add("left-click-tool-force-eat")).build();
        this.enableHealth = this.builder(this.ga.add("enable-health"), Boolean.class)
                .defaultValue(true)
                .build();
        this.enableHunger = this.builder(this.ga.add("enable-hunger"), Boolean.class)
                .defaultValue(true)
                .build();
        this.healthLevel = this.doubleBuilder(this.ga.add("health-level"))
                .defaultValue(10.0)
                .validator(Configs.doubleRange(0.0, 20.0))
                .build();
        this.hungerLevel = this.intBuilder(this.ga.add("hunger-level"))
                .defaultValue(16)
                .validator(Configs.intRange(0, 20))
                .build();
        this.noEnemy = this.builder(this.ga.add("no-enemy"), Boolean.class)
                .defaultValue(true)
                .build();
        this.noEnemyDistanceAir = this.doubleBuilder(this.ga.add("no-enemy-distance-air"))
                .defaultValue(8.0)
        ...
        this.noEnemyDistanceGround = this.doubleBuilder(this.ga.add("no-enemy-distance-ground"))
                ...
                .build();
        this.cooldown =
                this.intBuilder(this.ga.add("cooldown")).defaultValue(20).build();
        this.gl = this.builder(this.ga.add("white-list-item"), EntrySet.<Item>parameter())
                ...
                .build();
        this.fireworkFix = this.builder(this.ga.add("firework-fix"), Boolean.class)
                .defaultValue(true)
                .build();
        this.autoFireworks = this.builder(this.ga.add("auto-fireworks"), Boolean.class)
                .defaultValue(false)
                .build();
        this.pauseInLiquid = this.builder(this.ga.add("pause-in-liquid"), Boolean.class)
                .defaultValue(false)
                .build();
        this.gq = null;
        this.gr = -1;
                    ...
                    if (this.gt != 0) {
                        ...
                        if (this.gs <= Tasks.b()
                                && (!this.pauseInLiquid.get()
                                        || !PlayerStateManager.INSTANCE.jy && !PlayerStateManager.INSTANCE.jG)) {
                            if (this.noEnemy.get()) {
                                double var6 = mc.player.isFallFlying()
                                        ? this.noEnemyDistanceAir.get()
                                        : this.noEnemyDistanceGround.get();
                                if (var6 > 1.0E-6
                                        && TargetSelector.INSTANCE.akK(var6, true, pl -> pl instanceof PlayerEntity)
                                                != null) {
                                    break label81;
                                }
                            }
                            ...
                            if (this.enableHealth.get() && var2.getHealth() <= this.healthLevel.get()) {
                                var3 = true;
                            } else if (this.enableHunger.get()
                                    && var2.getHungerManager().getFoodLevel() <= this.hungerLevel.get()) {
                                var3 = true;
                            }
                        }
                    }
    ...
    public boolean mayUseItem() {
        if (mc.player.isUsingItem()) {
            ...
            if (VItem.w().b(mc.player.getStackInHand(Hand.MAIN_HAND))
                    || VItem.w().b(mc.player.getStackInHand(Hand.OFF_HAND))) {
    ...
    private void onStatusConsumed(Event<EntityStatusS2CPacket> event) {
        if (this.gp
                ...
                && ((EntityStatusS2CPacket) event.b).getStatus() == 9) {
            this.stopEating();
            this.gs = Tasks.b() + this.cooldown.get();
        }
    }
    ...
    private KalamaHelperHelperK<ItemStack> findHandStack(boolean health) {
        ...
        if (var4 != null) {
            return var5 != null && var5 > var4
                    ? new KalamaHelperHelperK<>(40, var3)
                    : new KalamaHelperHelperK<>(InventoryUtils.getSelectedSlot(), var2);
        } else {
    ...
    private void tryStartEating(@Nonnull KalamaHelperHelperK<ItemStack> re, boolean offHand) {
        ...
        Runnable var5 =
                offHand ? InvExtra.INSTANCE.uh(re.index()) : InvExtra.INSTANCE.swapInventoryIndexToHand(re.index());
        if (var5 != null) {
