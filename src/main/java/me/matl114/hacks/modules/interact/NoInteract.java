...
public class NoInteract extends BaseModule {
    ...
    public final KeyBindRef J = this.toggleHotkey(this.pU.addHotkey(), new MultiKeyBind(), this.pU.addEnable())
            .build();
    public final NBTRef<EntrySet<Item>> pW;
    public final FlagRef swingHand;
    public final FlagRef autoCorrectAirPlace;
    public final NBTRef<EntrySet<Block>> pV = this.builder(
                    this.pU.add("no-interact-block"), EntrySet.<Block>parameter())
    ...
    public void gA(Event<KalamaHelperHelperI<ModulePreset>> event) {
        this.autoCorrectMode.set(
                Configs$LegalInteractMode.getFromPreset((ModulePreset) ((KalamaHelperHelperI) event.b).b()));
    ...
    public BlockHitResult handleMayAutoCorrect(BlockHitResult hitResult) {
        ...
        FlagEntry var3 = InteractionTasks.l(
                var2,
                this.autoCorrectAirPlace.get(),
                !this.autoCorrectMode.get().isLegal());
    ...
    public void onPreInteractBlock(Event<KalamaHelperHelperH> event) {
        if (this.ae.get() && (!this.noInteractVanillaOnly.get() || InteractManager.INSTANCE.Lq)) {
            ...
            if (this.autoDisableSameTickOffhand.get()
                    && var2 == Hand.OFF_HAND
                    && InteractManager.INSTANCE.Lq
                    && Tasks.b() == this.qe) {
                event.cancel();
    ...
    public NoInteract() {
        ...
        this.pW = this.builder(this.pU.add("no-interact-ignore-item"), EntrySet.<Item>parameter())
                ...
                .build();
        this.noInteractVanillaOnly = this.builder(this.pU.add("no-interact-vanilla-only"), Boolean.class)
                .defaultValue(true)
                .build();
        this.autoDisableSameTickOffhand =
                this.flagBuilder(this.pU.add("auto-disable-same-tick-offhand")).build();
        this.sneakIfMayInstant =
                this.flagBuilder(this.pU.add("sneak-if-may-instant")).build();
        this.autoCorrectPlacement =
                this.flagBuilder(this.pU.add("auto-correct-placement")).build();
        this.autoCorrectMode = this.builder(this.pU.add("auto-correct-mode"), Configs$LegalInteractMode.class)
                ...
                .build();
        this.autoCorrectAirPlace =
                this.flagBuilder(this.pU.add("auto-correct-air-place")).build();
        this.autoCorrectState =
                this.flagBuilder(this.pU.add("auto-correct-state")).build();
        this.swingHand = this.builder(this.pU.add("swing-hand"), Boolean.class)
                .defaultValue(true)
                .build();
        this.renderFailInteract =
                this.flagBuilder(this.pU.add("render-fail-interact")).build();
        this.renderFailInteractColor = this.builder(this.pU.add("render-fail-interact-color"), WrapColor.class)
        ...
        if (this.renderFailInteract.get()) {
            ...
            if (!var3.isEmpty()) {
                for (Box var5 : var3.getBoundingBoxes()) {
                    this.qf.submit(
                            var5.offset(interactAt),
                            this.renderFailInteractColor.get().withAlpha(255));
                }
            }
        }
    }
