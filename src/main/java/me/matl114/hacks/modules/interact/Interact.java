...
public class Interact extends BaseModule {
   ...
   private boolean processDelayMovementInteract(Entity target) {
      ...
      boolean var4 = RaycastUtils.x(mc.player, PlayerStateManager.INSTANCE.jl, PlayerStateManager.INSTANCE.jm, target, var2);
      if (var4) {
   ...
   public boolean placeBlock(BlockPos pos) {
      FlagEntry var2 = InteractionTasks.l(pos, !this.blockMode.get().isLegal(), !this.blockMode.get().isLegal());
   ...
   private boolean processExactInteract(
      ...
      if (vanillaSuccess && !var7.considerAntiShield(target)) {
         ...
         double var8 = CombatExtra.INSTANCE.getAttackAtTargetRange(player) + this.tpInteractRange.get().getValue();
   ...
   public boolean canInteract(BlockPos bp, double range) {
      ...
      if (!var4.isAir() && !var4.isLiquid() && this.DH.get().test(var4.getBlock())) {
         if (!InteractExtra.INSTANCE.fE(mc.player.getPos(), bp, range)) {
            ...
            return this.onlyInteractInteractableBlock.get() && !InteractUtils.t(mc.world, mc.player, bp, var4, mc.player.getStackInHand(Hand.MAIN_HAND))
               ? false
               : !this.interactBlockOnlyWhenHandNotBlock.get() || !(mc.player.getStackInHand(Hand.MAIN_HAND).getItem() instanceof BlockItem);
         }
      } else {
         return false;
      }
   ...
   public boolean interactBlock(BlockHitResult hitResult) {
      ...
      boolean var5 = RaycastUtils.canRaycastHit(mc.player, PlayerStateManager.INSTANCE.jl, PlayerStateManager.INSTANCE.jm, hitResult.getBlockPos(), var2);
      if (var5) {
   ...
   private boolean processLegacySnapInteract(Entity target) {
      ...
      if (var4) {
         ...
         if (this.tpInteractRange.get().positive() && !var7) {
            Vec3d var8 = MovTasks.tpAttackSearch(var5, target.getBoundingBox(), var2, 9.9, 1).stream().findFirst().orElse(null);
         ...
         if (var7) {
            ...
            var12.add(this.DO.nextDouble(-0.05, 0.05), this.DO.nextDouble(-0.05, 0.05), this.DO.nextDouble(-0.05, 0.05));
   ...
   public Interact() {
      ...
      this.enableEntity = this.builder(this.aD.add("enable-entity"), Boolean.class).defaultValue(true).build();
      this.enableBlock = this.builder(this.aD.add("enable-block"), Boolean.class).defaultValue(false).build();
      this.entityMode = this.builder(this.aD.add("entity-mode"), Configs$LegalTargetingMode.class).defaultValue(Configs$LegalTargetingMode.NONE).build();
      this.blockMode = this.builder(this.aD.add("block-mode"), Configs$LegalInteractMode.class).defaultValue(Configs$LegalInteractMode.NONE).build();
      this.entityWhitelist = this.builder(this.aD.add("entity-whitelist"), EntityTypeRegex.class)
         ...
         .build();
      this.ignoreBlockPlace = this.flagBuilder(this.aD.add("ignore-block-place")).build();
      ...
      this.DE = this.builder(this.aD.add("use-item-black-list"), EntrySet.<Item>parameter())
         ...
         .build();
      this.entityPriority = this.builder(this.aD.add("entity-priority"), Boolean.class).defaultValue(true).build();
      this.onlyInteractInteractableEntity = this.builder(this.aD.add("only-interact-interactable-entity"), Boolean.class).defaultValue(true).build();
      this.DH = this.builder(this.aD.add("block-whitelist"), EntrySet.<Block>parameter())
         ...
         .build();
      this.interactBlockOnlyWhenHandNotBlock = this.builder(this.aD.add("interact-block-only-when-hand-not-block"), Boolean.class).defaultValue(true).build();
      this.onlyInteractInteractableBlock = this.builder(this.aD.add("only-interact-interactable-block"), Boolean.class).defaultValue(true).build();
      ...
      this.renderTargetColor = this.builder(this.aD.add("render-target-color"), WrapColor.class).defaultValue(new WrapColor(Formatting.RED)).build();
      this.DN = null;
      this.DO = new Random();
   ...
   public void onPreTick(Event<ClientPlayerEntity> event) {
      ...
      if (this.ae.get()) {
         ...
         if (var2.getType() == Type.BLOCK) {
            if (!this.ignoreBlockPlace.get() && mc.player.getStackInHand(Hand.MAIN_HAND).getItem() instanceof BlockItem var7) {
               this.DN = var2;
            } else {
            ...
            if (InteractUtils.isInteractAcceptable(mc.world, mc.player, var6, mc.player.getStackInHand(Hand.MAIN_HAND))) {
               this.DN = var2;
            }
         }
   ...
   public void onRender3D(Event<MatrixStack> event) {
      if (this.renderTarget.get() && this.DN != null) {
         ...
         try {
            float var15 = (float)var7.getCenter().subtract(mc.player.getEyePos()).length();
            float var14 = Math.min(0.6F, 0.2F + var15 * 0.02F);
            RenderUtils.r((MatrixStack)event.b, var7.getMinPos(), var7.getMaxPos(), ColorUtils.h(this.renderTargetColor.get().color(), var14));
         } finally {
   ...
   public void gA(Event<KalamaHelperHelperI<ModulePreset>> event) {
      this.entityMode.set(Configs$LegalTargetingMode.getFromPreset((ModulePreset)((KalamaHelperHelperI)event.b).b()));
   ...
   public boolean Si(Entity target) {
      return this.entityMode.get().isLegal() ? this.processLegalInteract(target) : this.processIllegalInteract(target);
   }
   ...
   public boolean Se(Entity entity, double range) {
      if (!TargetSelector.INSTANCE.isTargetInRange(entity, range, 0)) {
         ...
         return !entity.isAlive() || entity.isSpectator() || !this.entityWhitelist.get().test(entity.getType())
            ? false
            : !this.onlyInteractInteractableEntity.get()
               || InteractUtils.isInteractAcceptable(mc.world, mc.player, entity, mc.player.getStackInHand(Hand.MAIN_HAND));
      }
   }
   ...
   private boolean processIllegalInteract(Entity target) {
      ...
      if (var2 == null) {
         ...
         boolean var5 = RaycastUtils.w(mc.player, PlayerStateManager.INSTANCE.jl, PlayerStateManager.INSTANCE.jm, target);
         ...
         if (!var12) {
            ...
            List var30 = Streams.concat(new Stream[]{var6.stream(), var7.stream()}).toList();
            ...
            if (var31 > 1) {
               ...
               if (Math.abs(var23 - var25) > var2.getAttributeValue(EntityAttributes.GENERIC_SAFE_FALL_DISTANCE) - 1.0) {
         ...
         if (this.ae.get()) {
            ...
            if (var2 == Hand.MAIN_HAND) {
               ...
               if (var3 != null && mc.world != null && this.DN != null && this.DN != mc.crosshairTarget) {
                  if (this.DN instanceof EntityHitResult var5 && var5.getType() == Type.ENTITY && this.Si(var5.getEntity())) {
                     hitResult.cancel();
                  } else if (this.DN instanceof BlockHitResult var6 && var6.getType() == Type.BLOCK && this.interactBlock(var6)) {
                     hitResult.cancel();
                  }
               }
            }
   ...
   public boolean placeBlockStrict(BlockPos pos, BlockState state) {
      FlagEntry var3 = InteractionTasks.l(pos, !this.blockMode.get().isLegal(), !this.blockMode.get().isLegal());
