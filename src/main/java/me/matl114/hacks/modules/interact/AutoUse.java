...
public class AutoUse extends BaseModule {
   ...
   public final KeyBindRef J = this.toggleHotkey(this.iE.addHotkey(), new MultiKeyBind(), this.iE.addEnable()).build();
   ...
   public boolean isUsableNotFood(ItemStack stack) {
      return this.cN.get().test(stack.getItem()) && (this.adQ(stack) || this.useFood.get() && VItem.w().h(stack));
   }

   public AutoUse() {
      super("AutoUse");
      this.log = this.flagBuilder(this.iE.add("log")).build();
      this.onlyWhenEnemyNear = this.flagBuilder(this.iE.add("only-when-enemy-near")).build();
      this.enemyNearDistance = this.doubleBuilder(this.iE.add("enemy-near-distance")).defaultValue(12.0).build();
   ...
   private boolean pass() {
      return !this.onlyWhenEnemyNear.get() || TargetSelector.INSTANCE.akK(this.enemyNearDistance.get(), true, pl -> pl instanceof PlayerEntity) != null;
   }
         ...
         if (!mc.player.isUsingItem()) {
            ...
            if (var4 != null) {
               ...
               if (mc.player.isUsingItem() && mc.player.getActiveHand() == var4) {
                  ...
                  if (this.log.get()) {
                     Debug.chat(ChatUtils.textFromLegacyString("&c[Use] &fStart to use"), VItem.w().l(mc.player.getActiveItem()));
                  }
               } else {
                  KeyBindAccess.of(mc.options.useKey).resetKeyState();
               }
   ...
   public boolean adQ(ItemStack stack) {
      if (VItem.w().b(stack)) {
         ...
         return var2 instanceof ShieldItem || var2 instanceof BowItem || var2 instanceof TridentItem || var2 instanceof CrossbowItem;
      }
   }
