...
public class GuiInteract extends BaseModule {
    ...
    public GuiInteract() {
        super("GuiInteract");
        this.wK = new HashSet<>();
        this.wL = Suppliers.memoize(
                () -> new KeyBinding[] {mc.options.useKey, mc.options.attackKey, mc.options.sprintKey});
        this.ae = this.builder(this.ad.addEnable(), Boolean.class)
                .defaultValue(false)
                .build();
        this.J = this.toggleHotkey(this.ad.addHotkey(), new MultiKeyBind(), this.ad.addEnable())
                .build();
        this.useTickWhenScreenOpen = this.builder(this.ad.add("use-tick-when-screen-open"), Boolean.class)
                .defaultValue(true)
                .build();
