...
public class AutoSurround extends BaseModule implements HackUtilHelperJ {
    ...
    public boolean checkSurround() {
        if (this.onlyGround.get() && !mc.player.isOnGround() && !CollisionUtil.isEntitySupported(mc.player, 1.5)) {
            ...
            if (this.onlyPlayerNear.get().isPresent()) {
                List var1 =
                        TargetSelector.INSTANCE.akC(this.onlyPlayerNear.get().getValue());
                if (var1.isEmpty()) {
                    return false;
                }
            }
            ...
            int var2 = Disabler.INSTANCE.isMultiRotPlaceCheckDisabled(
            ...
            if (this.antiPacketMine.get()) {
                var8 = MiningProgressManager.INSTANCE.getBreakingMap().values().stream()
                ...
                if (var12.isAir() || var12.isReplaceable()) {
                    ...
                    if (var14) {
                        if (this.autoAttackCrystal.get()) {
                            mc.world
                                    .getOtherEntities(null, MathUtils.r(var11), e -> e instanceof EndCrystalEntity)
                                    .forEach(endCrystalEntity -> {
                                        if (endCrystalEntity instanceof EndCrystalEntity var2x
                                                && !Attack.INSTANCE.Yh(endCrystalEntity)) {
                                            var5.add(var2x);
                                        }
                                    });
                        }
                        ...
                        if (this.canCubePlace(mc.player, var11, var5)) {
                            if (var3 == 0) {
                                ...
                                var6 |= var15.index() == 40;
                                var4 = var6
                                        ? InvExtra.INSTANCE.uh(var15.index())
                                        : InvExtra.INSTANCE.swapInventoryIndexToHand(var15.index());
                            }

                            InteractionTasks.g(
                                    this.mode.get(),
                                    (BlockHitResult) var13.val(),
                                    var6 ? Hand.OFF_HAND : Hand.MAIN_HAND,
                                    this.swingHand.get());
                            if (++var3 >= var2) {
                                break;
                            }
            ...
            if (var3 < var2) {
                for (BlockPos var18 : var9) {
                    ...
                    if (var3 == 0) {
                        ...
                        var6 |= var20.index() == 40;
                        var4 = var6
                                ? InvExtra.INSTANCE.uh(var20.index())
                                : InvExtra.INSTANCE.swapInventoryIndexToHand(var20.index());
                    }

                    InteractionTasks.g(
                            this.mode.get(), var19, var6 ? Hand.OFF_HAND : Hand.MAIN_HAND, this.swingHand.get());
                    if (++var3 >= var2) {
                        break;
                    }
                }
    ...
    public AutoSurround() {
        ...
        this.J = this.toggleHotkey(this.cz.addHotkey(), new MultiKeyBind(), this.cz.addEnable())
                .build();
        this.offhandEnable = this.flagBuilder(this.cz.add("offhand-enable")).build();
        this.delay = this.builder(this.cz.add("delay"), IntRef.TYPE)
                .defaultValue(1)
                .validator(Configs.e)
                .build();
        this.multiply = this.builder(this.cz.add("multiply"), IntRef.TYPE)
                .defaultValue(1)
                .validator(Configs.e)
                .build();
        this.mode = this.builder(this.cz.add("mode"), Configs$LegalInteractMode.class)
        ...
        this.autoAttackCrystal =
                this.flagBuilder(this.cz.add("auto-attack-crystal")).build();
        ...
        this.whiteList = this.builder(this.cz.add("white-list"), EntrySet.<Item>parameter())
                .defaultValue(new EntrySet<Item>(Registries.ITEM, List.of(Items.OBSIDIAN)))
                .build();
        this.onlyBlastResistance = this.builder(this.cz.add("only-blast-resistance"), Boolean.class)
                .defaultValue(true)
                .build();
        this.swingHand = this.builder(this.cz.add("swing-hand"), Boolean.class)
                .defaultValue(true)
                .build();
        this.cQ = false;
                ...
                item -> {
                    if (item.getItem() instanceof BlockItem var3) {
                        if (this.useWhiteList.get() && !this.whiteList.get().test(var3)) {
                            ...
                            return this.onlyBlastResistance.get()
                                            && var3.getBlock().getBlastResistance() < 600.0F
                                    ? null
    ...
    public Set<BlockPos> getTargetingPos() {
        ...
        var2 = var2.withMaxY(Math.max(
                var2.minY
                        + InteractExtra.INSTANCE.getPotentialEyeHeights().max().orElse(0.0),
                var2.maxY));
    ...
    private boolean canCubePlace(ClientPlayerEntity player, BlockPos pos, Set<EndCrystalEntity> pendingRemove) {
        ...
        VoxelShape var5 = var4.getCollisionShape(mc.world, pos, ShapeContext.of(mc.player))
                .offset(pos.getX(), pos.getY(), pos.getZ());
        return !CollisionUtil.hasAnyIntersects(
                mc.world, entity -> entity instanceof EndCrystalEntity var2 && pendingRemove.contains(var2), var5);
    }
}
