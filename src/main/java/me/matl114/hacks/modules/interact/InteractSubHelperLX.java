...
class InteractSubHelperLX implements HackUtilHelperJ {
    ...
    public void applyPreTickModify(Event<LegalMovementManager> movementManagerEvent) {
        ...
        Vec3d var3 =
                TargetSelector.INSTANCE.akn(MinecraftClient.getInstance().player.getPos(), this.et.getBoundingBox());
        if (var2.isFallFlying()) {
            var3 = var3.add(MinecraftClient.getInstance().player.getVelocity());
        }

        Vec3d var4 = var2.getPos();
        if (this.mG.tpInteractRange.get().positive()
                && this.et.getBoundingBox().squaredMagnitude(var3) > MathUtils.a(this.val$attackRange)) {
            Vec3d var5 = MovTasks.tpAttackSearch(var4, this.et.getBoundingBox(), this.val$attackRange, 9.9, 1).stream()
                    .findFirst()
                    .orElse(null);
        ...
        if (this.mD) {
            ...
            var9.add(
                    this.mG.DO.nextDouble(-0.05, 0.05),
                    this.mG.DO.nextDouble(-0.05, 0.05),
                    this.mG.DO.nextDouble(-0.05, 0.05));
