...
public class AutoPlate extends BaseModule {
   ...
   public AutoPlate() {
      ...
      this.expandRange = this.doubleBuilder(this.Mj.add("expand-range")).defaultValue(2.0).validator(Configs.doubleRange(0.0, 100.0)).build();
      this.interactRange = this.doubleBuilder(this.Mj.add("interact-range"))
         ...
         .build();
      this.fillDepth = this.intBuilder(this.Mj.add("fill-depth")).defaultValue(0).validator(Configs.d).build();
      this.copyState = this.builder(this.Mj.add("copy-state"), Boolean.class).defaultValue(true).build();
         ...
         this.Mp
            .addAll(
               this.Dp
                  .stream()
                  .flatMap(
                     s -> IntStream.range(0, this.fillDepth.get() + 1)
                        .mapToObj(j -> new BlockPos(s.getX() + mc.player.getBlockX(), var4 - j, s.getZ() + mc.player.getBlockZ()))
                  )
                  .toList()
            );
         this.Mp.forEach(s -> this.Du.submit(new Box(s), this.renderColor.get().withAlpha(255)));
         if (this.copyState.get()) {
            List<BlockPos> var5 = var3.stream().filter(s -> s.getY() == var4).toList();
            this.Mq = var5.stream().filter(s -> {
               BlockState var1x = mc.world.getBlockState(s);
               return !var1x.isAir() && !var1x.isLiquid();
            }).min(Comparator.comparingDouble(s -> s.getSquaredDistance(mc.player.getPos()))).map(mc.world::getBlockState);
         }
      }
   }
   ...
   public void tickPlace() {
      ...
      for (BlockPos var5 : this.Mp) {
         ...
         if (var6.isAir() || var6.isLiquid() || var6.isReplaceable()) {
            ...
            FlagEntry var14 = InteractionTasks.q(var5, var8, this.airPlace.get(), !this.mode.get().isLegal());
            if (InteractUtils.C(mc.player, var14)
               && InteractExtra.INSTANCE.fE(mc.player.getPos(), ((BlockHitResult)var14.val()).getBlockPos(), this.interactRange.get())
               && InteractUtils.getBlockPlacement(var8.getBlock(), mc.player, mc.world, (BlockHitResult)var14.val()) != null) {
               ...
               InteractionTasks.g(this.mode.get(), (BlockHitResult)var14.val(), Hand.MAIN_HAND, this.swingHand.get());
