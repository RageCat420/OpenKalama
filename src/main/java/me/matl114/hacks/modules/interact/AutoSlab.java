...
public class AutoSlab extends BaseModule {
    ...
    public void tickPlace() {
        ...
        for (BlockPos var5 : this.Ud) {
            ...
            if (var6 != null && ((ItemStack) var6.val()).getItem() instanceof BlockItem var8) {
                ...
                FlagEntry var9 = InteractionTasks.q(
                        var5, var13, this.airPlace.get(), !this.mode.get().isLegal());
                if (InteractUtils.C(mc.player, var9)
                        && InteractExtra.INSTANCE.fE(
                                mc.player.getPos(),
                                ((BlockHitResult) var9.val()).getBlockPos(),
                    ...
                    InteractionTasks.g(
                            this.mode.get(), (BlockHitResult) var9.val(), Hand.MAIN_HAND, this.swingHand.get());
                    if (++var1 >= var2) {
                        break;
                    }
                }
    ...
    public AutoSlab() {
        ...
        this.useBlockEntities =
                this.flagBuilder(this.Mj.add("use-block-entities")).build();
    ...
    public boolean isAvailable(Item item) {
        if (this.slabOnly.get()) {
            ...
            if (this.Uc.contains(var3)) {
                return !this.useBlockEntities.get() && var3 instanceof BlockWithEntity var4
                        ? false
                        : !this.TZ.get().test(var3);
            } else {
                return false;
            }
        } else {
    ...
    public void refreshBlocks() {
        ...
        for (Vec3i var3 : this.Dp) {
            ...
            if ((var5.isAir() || var5.isLiquid() || var5.isReplaceable())
                    && WorldUtils.canEntitySpawnAt(mc.world, var4, EntityType.CREEPER)) {
