      ...
      if (stack != null && !stack.isEmpty() && this.item != null) {
         if (!this.item.equals(stack.getItem())) {
            ...
            if (!InteractManager.INSTANCE.ignorePotionLevel.get()) {
               ...
               Set var3 = Streams.of(var2.getEffects()).map(StatusEffectInstance::getEffectType).collect(Collectors.toSet());
               Set var4 = Streams.of(((Potion)(Object)this.potionType.value()).getEffects()).map(StatusEffectInstance::getEffectType).collect(Collectors.toSet());
