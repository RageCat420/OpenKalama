...
public class Airplace extends BaseModule {
    ...
    int tE;
    int tC;
    public final KeyBindRef J = this.toggleHotkey(this.ty.add("hotkey"), new MultiKeyBind(), this.ty.add("enable"))
            .build();
    BlockPos ei;
    StateMachine tJ;
    ...
    public int onPlace(StateMachine machine) {
        ...
        while (var2.getY() != var3) {
            ...
            mc.interactionManager.sendSequencedPacket(
                    mc.world, seq -> new PlayerInteractBlockC2SPacket(this.tD.hand(), var8, seq));
    ...
    public Airplace() {
        super("Airplace");
        this.range = this.builder(this.ty.add("range"), DoubleRef.TYPE)
                .defaultValue(5.0)
                .validator(Configs.doubleRange(0.0, 10000.0))
                .build();
        this.render = this.flagBuilder(this.ty.add("render")).build();
        this.mode = this.builder(this.ty.add("mode"), Airplace$Mode.class)
                .defaultValue(Airplace$Mode.VANILLA)
                .updateListener(s -> this.De())
                .build();
        this.maxBatchPlace = this.intBuilder(this.ty.add("max-batch-place"))
                ...
                .build();
        this.invSleepTick = this.intBuilder(this.ty.add("inv-sleep-tick"))
                .defaultValue(2)
                .validator(Configs.e)
                .show(this::isState)
                .build();
    ...
    public void onGrimFastWall(BlockHitResult hitResult, Hand hand) {
        ...
        if (Disabler.INSTANCE.ajv()) {
            ...
            if (var8 != null) {
                this.tD = new InteractSubHelperJX(
                        var8.mutableCopy(),
                        var5,
                        var3.getCount(),
                        var3.copy(),
    ...
    public int onWait300MS(StateMachine machine) {
        ...
        if (this.tE > 8) {
            ...
            mc.interactionManager.sendSequencedPacket(
                    mc.world, seq -> new PlayerInteractBlockC2SPacket(this.tD.hand(), var4, seq));
            this.tD = null;
    ...
    public void onInputFastWall() {
        if (this.ae.get() && this.tD != null && this.tJ != null) {
            ...
            if (ItemStack.areItemsEqual(var4, var2)) {
                if (var1.toCenterPos().subtract(mc.player.getEyePos()).horizontalLengthSquared()
                        ...
                        switch ((Airplace$Mode) this.mode.get()) {
                            case VANILLA:
                                BlockHitResult var7 = new BlockHitResult(
                                        var6.getPos(), var6.getSide(), var6.getBlockPos(), var6.isInsideBlock());
                                event.context(var7);
                                return;
    ...
    public StateMachine Dn() {
        return new StateMachine(
                1, this::onUpdate, state -> 0, this::onPlace, this::onWaitSlotUpdate, this::onWait300MS);
    }
    ...
    public void onInputGrimWall() {
        if (this.ae.get()
                ...
                && var2 != Items.AIR
                && this.ei.toCenterPos().subtract(mc.player.getEyePos()).horizontalLengthSquared()
                        <= MathUtils.a(mc.player.getBlockInteractionRange() + 1.0)) {
            for (int var6 = 1; var6 < 256; var6++) {
                ...
                if (!var4.isAir() && !var4.isLiquid()) {
                    ...
                    InteractionTasks.b(
                            Hand.MAIN_HAND,
                            new BlockHitResult(var3.toBottomCenterPos().add(0.0, 1.0, 0.0), Direction.UP, var3, false),
                            false);
        ...
        if (this.tE == var2) {
            ...
            try {
                int var4 = mc.player
                        ...
                        .getSlotIndex(mc.player.getInventory(), this.tD.itemCount())
                        .orElse(-1);
                mc.interactionManager.clickSlot(
                        mc.player.currentScreenHandler.syncId, var4, 40, SlotActionType.SWAP, mc.player);
                mc.interactionManager.clickSlot(
                        mc.player.currentScreenHandler.syncId, var4, 40, SlotActionType.SWAP, mc.player);
            } finally {
