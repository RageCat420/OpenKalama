...
public class PrinterRewrite extends BaseModule {
    ...
    private boolean isDesyncStateInteractTransition(BlockState currentState, BlockState targetState) {
        if (currentState != null && targetState != null) {
            ...
            if (var3 instanceof NoteBlock) {
                ...
                return var3 instanceof CakeBlock && var4 instanceof CandleCakeBlock
                        ? true
                        : var3 instanceof CandleCakeBlock && var4 instanceof CakeBlock;
            }
        } else {
            return false;
        }
    ...
    public boolean doMultiReplace(BlockPos pos, BlockState currentState, BlockState targetState) {
        Pair var4 = InteractUtils.getNextInteractionStep(currentState, targetState).stream()
                .findAny()
                .orElse(null);
        if (var4 != null) {
        ...
        if (var19 == -1) {
            ...
            if (var7 != null
                    && InteractExtra.INSTANCE.fE(
                            mc.player.getPos(), ((BlockHitResult) var7.val()).getBlockPos(), this.range.get())
                    && InteractUtils.getBlockPlacement(var5, mc.player, mc.world, (BlockHitResult) var7.val())
                            != null) {
                if (var7.flag()) {
                    this.cQ = true;
    ...
    public void onPreInputEvent(Event<Void> event) {
        if (++this.countDown > this.delay.get()) {
            ...
            if (this.enable.get() && LitematicaHooks.getInstance().isEnabled()) {
                ...
                int var5 = Disabler.INSTANCE.isMultiRotPlaceCheckDisabled(
                                    ...
                                    && (var11.isAir()
                                                    && (var10.isLiquid()
                                                            || var10.getFluidState()
                                                                            .getFluid()
                                                                    == Fluids.WATER)
                                            || var11.getBlock() == var10.getBlock()
                                                    && var11.getFluidState() != var10.getFluidState())
                                    && !this.Dw.containsKey(var9)) {
                                FluidState var12 = var11.getFluidState();
                                FluidState var13 = var10.getFluidState();
                                if ((var12.getFluid() == Fluids.EMPTY
                                                || var12.getFluid() == Fluids.FLOWING_WATER
                                                || var12.getFluid() == Fluids.FLOWING_LAVA)
    ...
    private BlockHitResult createStateInteractHitResult(BlockPos pos) {
        Vec2f var2 =
                EntityUtils.q(pos.toCenterPos().subtract(mc.player.getEyePos()).normalize());
    ...
    public void addCustomWidgets(Consumer<DrawableWidget> acceptor, int dx, int dy, int dblank) {
        ...
        acceptor.accept(new KalamaHelperHelperJ<>(
                () -> this.supportWaterPlace.get() && this.useIceToFormWater.get() ? var5 : null, 0, 0));
    }
    ...
    public PrinterRewrite() {
        ...
        this.hotkey = this.toggleHotkey(this.Dl.add("hotkey"), new MultiKeyBind(), this.Dl.add("enable"))
                .build();
        this.mode = this.builder(this.Dl.add("mode"), Configs$LegalInteractMode.class)
                .defaultValue(Configs$LegalInteractMode.DELAY_MOVEMENT)
                .build();
        this.airPlace = this.flagBuilder(this.Dl.add("air-place")).build();
        this.delay = this.builder(this.Dl.add("delay"), IntRef.TYPE)
                .defaultValue(5)
                .validator(Configs.e)
                .build();
        this.multiply = this.builder(this.Dl.add("multiply"), IntRef.TYPE)
                .defaultValue(1)
                .validator(Configs.e)
                .build();
        this.autoSneak = this.builder(this.Dl.add("auto-sneak"), Boolean.class)
                .defaultValue(true)
                .build();
        this.supportWaterPlace = this.builder(this.Dl.add("support-water-place"), Boolean.class)
                .defaultValue(true)
                .build();
        this.useIceToFormWater = this.flagBuilder(this.Dl.add("use-ice-to-form-water"))
                .show(this.supportWaterPlace::get)
                .build();
        this.supportReplaceBlock = this.builder(this.Dl.add("support-replace-block"), Boolean.class)
                .defaultValue(true)
                .build();
        this.swingHand = this.builder(this.Dl.add("swing-hand"), Boolean.class)
                .defaultValue(true)
                .build();
        ...
        this.range = this.builder(this.Dl.add("range"), DoubleRef.TYPE)
                ...
                .build();
        this.render = this.builder(this.Dl.add("render"), Boolean.class)
                .defaultValue(true)
                .build();
        this.renderSuccessColor = this.builder(this.Dl.add("render-success-color"), WrapColor.class)
                .defaultValue(new WrapColor(Color.GREEN))
                .build();
        this.renderFailColor = this.builder(this.Dl.add("render-fail-color"), WrapColor.class)
    ...
    private int supplyInteractionItem(BlockState currentState, BlockState targetState) {
        if (currentState != null && targetState != null) {
            ...
            if (var3 instanceof RespawnAnchorBlock
                    ...
                    && (Integer) currentState.get(RespawnAnchorBlock.CHARGES)
                            < (Integer) targetState.get(RespawnAnchorBlock.CHARGES)) {
                ...
                KalamaHelperHelperK var11 = InventoryUtils.p(
    ...
    private boolean isStackedPlacementTransition(BlockState currentState, BlockState targetState) {
        if (currentState == null || targetState == null) {
            ...
            if (var3 instanceof SlabBlock) {
                ...
                return (Integer) currentState.get(SeaPickleBlock.PICKLES)
                        < (Integer) targetState.get(SeaPickleBlock.PICKLES);
            } else {
                return var3 instanceof FlowerbedBlock
                        ? (Integer) currentState.get(FlowerbedBlock.FLOWER_AMOUNT)
                                < (Integer) targetState.get(FlowerbedBlock.FLOWER_AMOUNT)
                        : false;
            }
        }
    }
