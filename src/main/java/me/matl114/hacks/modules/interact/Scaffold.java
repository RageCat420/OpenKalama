...
public class Scaffold extends BaseModule {
    ...
    public BlockHitResult guessTheBestPlacePositionForTargetingBlock(Vec3d predictedEyePos, BlockPos pos) {
        ...
        if (var10.isReplaceable() && InteractUtils.canCubePlace(mc.player, pos)) {
            FlagEntry var8 = InteractionTasks.n(
                    predictedEyePos,
                    pos,
                    this.airPlace.get(),
                    !this.legalTargeting.get().isLegal());
        ...
        for (Vec3i var13 : this.xb) {
            if (var13.getY() >= -this.expandInteractYDepth.get()) {
                ...
                if (var10.isReplaceable() && InteractUtils.canCubePlace(mc.player, var7)) {
                    FlagEntry var9 = InteractionTasks.l(
                            var7,
                            !this.legalTargeting.get().isLegal(),
                            !this.legalTargeting.get().isLegal());
                    if (var9 != null) {
    ...
    public Scaffold() {
        ...
        this.el = this.toggleHotkey(this.xc.addHotkey(), new MultiKeyBind(), this.xc.addEnable())
                .build();
        ...
        this.expandInteractYDepth = this.builder(this.xc.add("expand-interact-y-depth"), IntRef.TYPE)
                .defaultValue(0)
        ...
        this.expandInteractRange = this.builder(this.xc.add("expand-interact-range"), IntRef.TYPE)
                ...
                .validator(Configs.intRange(0, 3))
                .build();
        this.swingHand = this.builder(this.xc.add("swing-hand"), Boolean.class)
                .defaultValue(true)
                .build();
        this.delayTick = 0;
    ...
    public void onRightClick(Event<Void> rightClickEvent) {
        if (mc.player != null && this.ae.get()) {
            ...
            if (var5.isReplaceable()) {
                BlockHitResult var6 = this.guessTheBestPlacePositionForTargetingBlock(
                        var3.add(0.0, mc.player.dimensions.eyeHeight(), 0.0), var4);
                if (var6 != null) {
    ...
    private void placeBlockLegally(int hand, BlockHitResult result) {
        ...
        if (var4 != null) {
            try {
                if (!(mc.crosshairTarget instanceof BlockHitResult var6
    ...
    public void updateSearchRange(int range) {
        ...
        this.xb.sort(Comparator.comparingInt(
                v -> Math.max(Math.max(Math.abs(v.getX()), Math.abs(v.getY())), Math.abs(v.getZ()))));
    }

    public void IU(Event<KalamaHelperHelperI<ModulePreset>> event) {
        this.legalTargeting.set(
