...
public class InteractSubHelperR implements InteractSubHelperT {
   ...
   public void execute(InteractManager manager, PlayerEntity player) {
      ...
      if (var3 != null) {
         ...
         Runnable var5 = var4 ? InvExtra.INSTANCE.uh(var3.index()) : InvExtra.INSTANCE.swapInventoryIndexToHand(var3.index());
         if (var5 != null) {
            Hand var6 = var4 ? Hand.OFF_HAND : Hand.MAIN_HAND;
            ActionResult var7 = MinecraftClient.getInstance().interactionManager.interactItem(player, var6);
