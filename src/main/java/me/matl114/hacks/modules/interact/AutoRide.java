...
public class AutoRide extends BaseModule {
    ...
    public AutoRide() {
        ...
        this.J = this.toggleHotkey(this.mR.addHotkey(), new MultiKeyBind(), this.mR.addEnable())
                .build();
        this.interactCooldown = this.intBuilder(this.mR.add("interact-cooldown"))
                .defaultValue(4)
                .build();
        this.cd1 = 0;
    ...
    public void handleInput(Event<Void> event) {
        if (this.ae.get() && !mc.player.hasVehicle() && ++this.cd1 > this.interactCooldown.get()) {
            ...
            if (var6 != null) {
                ...
                EntityHitResult var12 =
                        new EntityHitResult(this.mS, var10.getCenter().add(0.0, var10.getLengthY() / 2.0, 0.0));
