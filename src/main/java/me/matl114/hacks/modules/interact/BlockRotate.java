...
public class BlockRotate extends BaseModule {
   ...
   public void onPreSendInteractBlockRotate(Event<PlayerInteractBlockC2SPacket> e) {
      if (!e.d()) {
         if (this.enable.get() && this.enableBlockRotateModify() && e.b instanceof PlayerInteractBlockC2SPacketAccess var3) {
            ...
            if (var3.hasUseContext()) {
               if (var3.getUseContext().blockPlace()) {
                  ...
                  BlockPos var13 = ((PlayerInteractBlockC2SPacket)e.b).getBlockHitResult().getBlockPos();
            ...
            if (var4 != null && ViaFabricPlusHooks.getInstance().getCurrentVersion().b(21, 0)) {
               if (this.yawDeceiveBypassMode.get() == Configs$BypassMode.BYPASS_GRIM) {
                  Listener.sendPacketNoEvents(
                     new PlayerInteractBlockC2SPacket(
                        Hand.OFF_HAND, ((PlayerInteractBlockC2SPacket)e.b).getBlockHitResult(), ((PlayerInteractBlockC2SPacket)e.b).getSequence() - 1
                     )
                  );
               }
               ...
               mc.getNetworkHandler()
                  .sendPacket(
                     new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, ((PlayerInteractBlockC2SPacket)e.b).getSequence(), var4.c(var10.y), var4.b(var10.x))
                  );
               PlayerInteractBlockC2SPacketAccess.of((PlayerInteractBlockC2SPacket)e.b).setSequence(NetworkUtils.generateNextSequence());
            }

            if (var4 != null && ViaFabricPlusHooks.getInstance().getCurrentVersion().c(20, 8)) {
               LegacySnapRotManager.INSTANCE.snapAt(var4.b(var10.x), var4.c(var10.y), true);
            }

            if (var5 != null && (this.rotateBypassMode.get().hasAc() || var4 != null && ViaFabricPlusHooks.getInstance().getCurrentVersion().c(20, 8))) {
               if (ViaFabricPlusHooks.isSupportDupRot()) {
                  PlayerMoveC2SPacket var12 = LegacySnapRotManager.INSTANCE.ahu(var5.subtract(mc.player.getEyePos()));
   ...
   public BlockRotate() {
      super("BlockRotate");
      this.enable = this.builder(this.Ax.add("enable"), Boolean.class).defaultValue(true).build();
      this.yawDeceiveBypassMode = this.builder(this.Ax.add("yaw-deceive-bypass-mode"), Configs$BypassMode.class)
         ...
         .build();
      this.rotateBypassMode = this.builder(this.Ax.add("rotate-bypass-mode"), Configs$BypassMode.class).defaultValue(Configs$BypassMode.NO_BYPASS).build();
      this.enable2 = this.builder(this.Ay.add("enable"), Boolean.class).defaultValue(true).build();
      this.clientStateTempFix = this.builder(this.Ay.add("client-state-temp-fix"), Boolean.class).defaultValue(true).build();
   ...
   public void registerAll() {
      super.registerAll();
      this.registerListener(Listener.ap().getChannel(PlayerInteractBlockC2SPacket.class), this::onPreSendInteractBlockRotate);
   ...
   public void handlePlaceCorrectTemperarySchematic(
      Item item, PlayerInteractBlockC2SPacket packet, PlayerInteractBlockC2SPacketAccess.UseContext useContext, Event<InteractSubHelperY> yawDeceive
   ) {
      if (this.enable2.get()) {
   ...
   public BlockHitResult handlePlaceCorrect(
      Item item, BlockPos modifyingBlockPos, BlockState targetState, PlayerInteractBlockC2SPacket packet, boolean forceCorrect
   ) {
      BlockHitResult var6 = packet.getBlockHitResult();
      BlockState var7 = mc.world.getBlockState(modifyingBlockPos);
      if (!targetState.isAir() && targetState.getBlock().asItem() == item && targetState.getBlock() == var7.getBlock()) {
         if (forceCorrect) {
   ...
   public static void handleYawDeceive(BlockState targetState, InteractSubHelperY deceive) {
      ...
      if (var2 instanceof WallMountedBlock var3
         && (targetState.get(WallMountedBlock.FACE) == BlockFace.FLOOR || targetState.get(WallMountedBlock.FACE) == BlockFace.CEILING)) {
         ...
         Vec2f var35 = EntityUtils.directionToPitchYaw(((Direction)targetState.get(PistonBlock.FACING)).getOpposite());
         deceive.a = var35.x;
         deceive.b = var35.y;
      } else if (var2 instanceof DispenserBlock var29) {
         Vec2f var34 = EntityUtils.directionToPitchYaw(((Direction)targetState.get(DispenserBlock.FACING)).getOpposite());
         deceive.a = var34.x;
         deceive.b = var34.y;
      } else if (var2 instanceof BarrelBlock var28) {
         Vec2f var33 = EntityUtils.directionToPitchYaw(((Direction)targetState.get(BarrelBlock.FACING)).getOpposite());
         deceive.a = var33.x;
         deceive.b = var33.y;
         ...
         Vec2f var7 = switch (var5) {
            case DOWN -> EntityUtils.q(Vec3d.of(var6.getVector()).add(0.0, -4.0, 0.0).normalize());
            case UP -> EntityUtils.q(Vec3d.of(var6.getOpposite().getVector()).add(0.0, 4.0, 0.0).normalize());
            default -> EntityUtils.q(Vec3d.of(var5.getOpposite().getVector()).normalize());
         };
         deceive.a = var7.x;
         deceive.b = var7.y;
   ...
   public void handlePlaceCorrectLitematica(
      ...
      if (this.enable3.get() && LitematicaHooks.getInstance().isEnabled()) {
         ...
         BlockHitResult var10 = this.handlePlaceCorrect(item, var8, var9, packet, useContext.oldState().isAir());
         if (var10 != null) {
            var6 = var10;
            BlockState var11 = mc.world.getBlockState(var8);
            if (var9 != var11) {
               BlockHitResult var12 = LitematicaHooks.getInstance().getEasyPlaceClickedPosition(var10, var9, var11);
               if (var12 != null) {
   ...
   public void handleInteractCorrectLitematica(BlockPos pos, BlockState newState, Event<InteractSubHelperY> yawDeceive) {
      if (this.enable3.get() && LitematicaHooks.getInstance().isEnabled()) {
