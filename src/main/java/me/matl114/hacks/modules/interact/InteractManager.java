...
public class InteractManager extends BaseModule {
   ...
   private static final String KW = "[hand|item_id] [once|inf|interval] [delay] <look|pos|entity> [pitch yaw|pos参数|@entity] 提交使用物品请求";
   ...
   private void aaz(MainCommand mainCommand, String name) {
      ...
      var3.subBuilder(SubCommand.bo()).u("list").x("message.command." + name + ".list.help").z(cmd -> cmd.executor(this::aaN)).r();
      var3.subBuilder(SubCommand.bo())
         .u("cancel")
         .x("message.command." + name + ".cancel.help")
         .A(KalamaHelperHelperA.a().B("id").d(() -> Stream.concat(Stream.of("all"), this.Lo.keySet().stream())).v())
         .z(cmd -> cmd.executor(this::aaO))
         .r();
      var3.subBuilder(SubCommand.bo()).u("clear").x("message.command." + name + ".clear.help").z(cmd -> cmd.executor(this::aaP)).r();
      var3.subBuilder(SubCommand.bo())
         .u("help")
         .x("message.command." + name + ".help.help")
         .A(new KalamaHelperHelperN<>("dispatch", KalamaHelperHelperA.a().B("dispatch").k(KV).v(), "all"))
   ...
   private void aay(MainCommand mainCommand) {
      ...
      var2.subBuilder(SubCommand.bp())
         ...
         .z(
            tree -> tree.<KalamaHelperHelperD, TaskSubCommand>subBuilder(SubCommand.bo())
               .u("entity")
   ...
   private static Stream<String> itemStackTabs(String token) {
      ...
      if (var2 >= 0) {
         ...
         return var4 != Items.AIR && hasPotionComponent(var4) ? Stream.concat(var3, Stream.of(var1 + "[")).distinct() : var3;
      }
   }
   ...
   private static InteractSubHelperFX parseItemStackSelector(String raw) {
      if (raw != null && !raw.isBlank()) {
         ...
         Item var4 = (Item)Registries.ITEM.getOrEmpty(Identifier.tryParse(var1)).orElse(null);
         if (var4 == null) {
            return null;
         } else {
            if (var2 != null) {
   ...
   private void sendHoldUseHelp(CommandExecution context) {
      context.sm("&c[Interact] &fholduseitem 用法: !!holduseitem [hand|item_id] [once|inf|interval] [delay] [release_ticks]");
      ...
      context.sn(
         ChatUtils.builder()
            .withColorString("1. 使用下界合金矛3次,一次30gt,40gt使用一次: &e" + MainCommand.getMainCommandPrefix() + "holduseitem netherite_spear 3 40 30")
            .withGlobal(Style.EMPTY.withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "holduseitem netherite_spear 3 40 30")))
            .end()
            .build()
      );
      context.sn(
         ChatUtils.builder()
            .withColorString("2. 使用下界合金矛3次,一次3gt,40gt使用一次: &e" + MainCommand.getMainCommandPrefix() + "holduseitem netherite_spear 3 40 3")
            .withGlobal(Style.EMPTY.withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "holduseitem netherite_spear 3 40 32")))
            .end()
            .build()
      );
      context.sn(
         ChatUtils.builder()
            .withColorString("3. 喝延时神龟药水3次,40gt喝一次: &e" + MainCommand.getMainCommandPrefix() + "holduseitem potion[long_turtle_master] 3 40 32")
            .withGlobal(
               Style.EMPTY.withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "holduseitem potion[long_turtle_master] 3 40 32"))
            )
            .end()
            .build()
      );
   ...
   private KalamaHelperHelperN<String> aaE(String name) {
      return new KalamaHelperHelperN<>(name, KalamaHelperHelperA.a().B(name).f().v(), "20", (execution, inputArgument) -> abm(inputArgument.a()) != null);
   }
   ...
   private String aaZ(CommandExecution context, String raw) {
      if (this.Lo.containsKey(raw)) {
         ...
         List var3 = this.Lo.keySet().stream().filter(id -> id.startsWith(raw)).toList();
         if (var3.isEmpty()) {
   ...
   public void aax(Event<PlayerMoveC2SPacket> eventPacket) {
      if ((this.disableLowVersionSpeedReset.get() && this.Lq || this.disableLowVersionSpeedResetWhenCommand.get() && this.Lr)
   ...
   private KalamaHelperHelperN<String> aaC(String name) {
      return new KalamaHelperHelperN<>(name, KalamaHelperHelperA.a().B(name).d(KT::stream).v(), "once", (execution, argument) -> abj(argument.a()));
   }
   ...
   private static InteractSubHelperX abk(String raw, int delay) {
      if (raw != null && !raw.isBlank()) {
         ...
         return (InteractSubHelperX)(switch (var2) {
   ...
   private void sendUseItemHelp(CommandExecution context) {
      ...
      context.sn(
         ChatUtils.builder()
            .withColorString(
               "1. 攻击距离玩家超过0.01的最近实体一次: &e" + MainCommand.getMainCommandPrefix() + "attack entity mainhand once 1 @e[distance=0.1..,limit=1,sort=nearest]"
            )
            .withGlobal(
               Style.EMPTY
                  .withClickEvent(
                     ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "attack entity mainhand once 1 @e[distance=0.1..,limit=1,sort=nearest]")
                  )
            )
            .end()
            .build()
      ...
      context.sn(
         ChatUtils.builder()
            .withColorString(
               "2. 攻击距离玩家超过0.01的最近实体16次,间隔10gt: &e" + MainCommand.getMainCommandPrefix() + "attack entity mainhand 16 10 @e[distance=1..,limit=1,sort=nearest]"
            )
            .withGlobal(
               Style.EMPTY
                  .withClickEvent(
                     ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "attack entity mainhand 16 10 @e[distance=1..,limit=1,sort=nearest]")
                  )
            )
            .end()
            .build()
      ...
      context.sn(
         ChatUtils.builder()
            .withColorString("3. 攻击脚下方块30次,1gt攻击一次: &e" + MainCommand.getMainCommandPrefix() + "attack block 30 1 ~ ~-1 ~")
            .withGlobal(Style.EMPTY.withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "attack block 30 1 ~ ~-1 ~")))
            .end()
            .build()
      );
      context.sn(
         ChatUtils.builder()
            .withColorString("2. 攻击脚下方块2次,100gt攻击一次(?): &e" + MainCommand.getMainCommandPrefix() + "attack block 2 200 ~ ~-1 ~")
            .withGlobal(Style.EMPTY.withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "attack block 2 200 ~ ~-1 ~")))
            .end()
            .build()
      );
   ...
   private static Stream<String> potionIdTabs() {
      return Registries.POTION.stream().<Identifier>map(Registries.POTION::getId).filter(Objects::nonNull).map(Identifier::getPath);
   }
   ...
   private KalamaHelperHelperN<String> aaD(String name) {
      return new KalamaHelperHelperN<>(name, KalamaHelperHelperA.a().B(name).f().v(), "1", (execution, inputArgument) -> abm(inputArgument.a()) != null);
   }
   ...
   private boolean onUseItem(CommandExecution context, ArgumentInputStream streamArgs, ArgumentReader reader) {
      if (!this.canSubmit(context)) {
         ...
         return !this.aaI(reader, context, "[hand|item_id] [once|inf|interval] [delay] <look|pos|entity> [pitch yaw|pos参数|@entity] 提交使用物品请求")
            ? true
            : this.submitRequest(
               context,
               new RecipeBookToggle(
                  this.aaY("holduseitem"), (InteractSubHelperX)var4.getSecond(), new InteractSubHelperR(this, (InteractSubHelperC)var4.getFirst(), var5)
               )
            );
      }
   }
   ...
   public InteractManager() {
      ...
      this.keepTaskWhenExit = this.flagBuilder(this.Lf.add("keep-task-when-exit")).build();
      this.ignorePotionLevel = this.flagBuilder(this.Lf.add("ignore-potion-level")).build();
      this.disableLowVersionSpeedReset = this.flagBuilder(this.Lf.add("disable-low-version-speed-reset")).build();
      this.disableLowVersionSpeedResetWhenCommand = this.flagBuilder(this.Lf.add("disable-low-version-speed-reset-when-command")).build();
   ...
   private boolean aaJ(CommandExecution context, ArgumentInputStream streamArgs, ArgumentReader reader) {
      if (!this.canSubmit(context)) {
         ...
         if (!KU.contains(var5)) {
            ...
            this.aaX(context, "[hand|item_id] [once|inf|interval] [delay] <look|pos|entity> [pitch yaw|pos参数|@entity] 提交使用物品请求");
            return true;
         } else {
            ...
            if (var7 == null) {
               ...
               return !this.aaI(reader, context, "[hand|item_id] [once|inf|interval] [delay] <look|pos|entity> [pitch yaw|pos参数|@entity] 提交使用物品请求")
                  ...
                  : this.submitRequest(
                     context,
                     new RecipeBookToggle(
                        this.aaY("useitem"), (InteractSubHelperX)var4.getSecond(), new InteractSubHelperAX((InteractSubHelperC)var4.getFirst(), var7)
                     )
                  );
            }
         }
         ...
         if (var6 == null) {
            ...
            return !this.aaI(reader, context, "[hand|item_id] [once|inf|interval] [delay] <look|pos|entity> [pitch yaw|pos参数|@entity] 提交使用物品请求")
               ...
               : this.submitRequest(
                  context,
                  new RecipeBookToggle(
                     this.aaY("attack"), (InteractSubHelperX)var4.getSecond(), new InteractSubHelperOX((InteractSubHelperC)var4.getFirst(), var6)
                  )
               );
         }
      }
         ...
         if (var6 == null) {
            ...
            return !this.aaI(reader, context, "[hand|item_id] [once|inf|interval] [delay] <look|pos|entity> [pitch yaw|pos参数|@entity] 提交使用物品请求")
               ...
               : this.submitRequest(
                  context,
                  new RecipeBookToggle(
                     this.aaY("mine"), (InteractSubHelperX)var4.getSecond(), new InteractSubHelperJ((InteractSubHelperC)var4.getFirst(), var6)
                  )
               );
         }
      }
   ...
   private void aaR(CommandExecution context) {
      context.sm("&c[Interact] &fuseitem 用法: !!useitem [hand|item_id] [once|inf|interval] [delay] <look|pos|entity> [pitch yaw|pos参数|@entity]");
      context.sm("&c[Interact] &fhand 可填 mainhand / offhand，也可直接填物品 id");
      context.sm("&c[Interact] &finterval 可填 once(执行一次)、inf(一直执行)，或具体循环次数");
      context.sm("&c[Interact] &fdelay 填非负整数,代表执行的间隔.若为0,则一次性执行至多9次");
      ...
      context.sn(
         ChatUtils.builder()
            .withColorString("1. 向上90度使用二级神龟喷溅药水: &e" + MainCommand.getMainCommandPrefix() + "useitem splash_potion[strong_turtle_master] look -90 ~")
            .withGlobal(
               Style.EMPTY
                  .withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "useitem splash_potion[strong_turtle_master] look -90 ~"))
            )
            .end()
            .build()
      );
      context.sn(
         ChatUtils.builder()
            .withColorString("2. 向上90度使用延时神龟喷溅药水: &e" + MainCommand.getMainCommandPrefix() + "useitem splash_potion[long_turtle_master] look -90 ~")
            .withGlobal(
               Style.EMPTY
                  .withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "useitem splash_potion[long_turtle_master] look -90 ~"))
            )
            .end()
            .build()
      );
      context.sn(
         ChatUtils.builder()
            .withColorString("3. 向下90度使用9个经验瓶,一次使用行完: &e" + MainCommand.getMainCommandPrefix() + "useitem experience_bottle 9 0 look 90 ~")
            .withGlobal(Style.EMPTY.withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "useitem experience_bottle 9 0 look 90 ~")))
            .end()
            .build()
      );
      context.sn(
         ChatUtils.builder()
            .withColorString("4. 向下90度使用9个经验瓶,一gt使用一次: &e" + MainCommand.getMainCommandPrefix() + "useitem experience_bottle 9 1 look 90 ~")
            .withGlobal(Style.EMPTY.withClickEvent(ChatUtils.getClickCopyText(MainCommand.getMainCommandPrefix() + "useitem experience_bottle 9 1 look 90 ~")))
            .end()
            .build()
      );
      context.sm("&c[Interact] &f指令建议配合BindCmd模块一起使用,通过快捷键自动发送");
