...
public class InteractExtra extends BaseModule {
   ...
   public void refreshInteractionRange(double val) {
      if (mc.player != null && this.lastRange != val) {
         ...
         var3.sort(Comparator.comparingDouble((Vec3d v) -> v.getX() * v.getX() + v.getY() * v.getY() + v.getZ() * v.getZ()));
         this.bS = var3;
   ...
   public InteractExtra() {
      ...
      this.useGrimExpandEyeHeight = this.flagBuilder(this.bR.add("use-grim-expand-eye-height")).build();
      this.reachDistance = this.builder(this.bR.add("reach-distance"), Double.class).defaultValue(0.0).build();
      this.noCoolDown = this.flagBuilder(this.bR.add("no-cool-down")).build();
      this.coolDownRewrite = this.intBuilder(this.bR.add("cool-down-rewrite")).defaultValue(4).build();
      this.allowRideInteract = this.builder(this.bR.add("allow-ride-interact"), FlagRef.TYPE).defaultValue(true).build();
      this.holdUse = this.builder(this.bR.add("hold-use"), FlagRef.TYPE).defaultValue(false).build();
      this.holdUseStartTick = this.intBuilder(this.bR.add("hold-use-start-tick")).defaultValue(4).build();
   ...
   public DoubleStream getPotentialEyeHeights() {
      if (this.useGrimExpandEyeHeight.get()) {
         ...
         return !mc.player.isFallFlying() && !mc.player.isUsingRiptide() && !mc.player.isSwimming()
            ? DoubleStream.concat(Arrays.stream(this.cd).map(s -> s * var1), DoubleStream.of(mc.player.dimensions.eyeHeight()))
            : DoubleStream.concat(Arrays.stream(this.cc).map(s -> s * var1), DoubleStream.of(mc.player.dimensions.eyeHeight()));
      } else {
