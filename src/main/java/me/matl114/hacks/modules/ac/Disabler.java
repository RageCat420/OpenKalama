...
public class Disabler extends BaseModule {
    ...
    public final FlagRef ae =
            this.builder(this.RC.addEnable(), Boolean.class).defaultValue(true).build();
    ...
    public Disabler() {
        super("Disabler");
        this.currentAc = this.builder(this.RC.add("current-ac"), DisablerManager$SupportAC.class)
                .defaultValue(DisablerManager$SupportAC.NONE)
                .build();
        this.J = this.moduleEntry(
                        this.RC.addHotkey(), new MultiKeyBind(), this.RC.addEnable(), moduleMeta(() -> this.currentAc))
                .build();
        this.grimSelfCheck = this.builder(this.RC.add("grim-self-check"), Boolean.class)
                .defaultValue(false)
                .build();
        this.grimMultiPlace = this.builder(this.RC.add("grim-multi-place"), Boolean.class)
                .defaultValue(true)
                .build();
        this.autoFlushMultiPlaceQueue = this.builder(this.RC.add("auto-flush-multi-place-queue"), Boolean.class)
                .defaultValue(true)
                .build();
        this.autoFlushPlaceBreakQueue = this.builder(this.RC.add("auto-flush-place-break-queue"), Boolean.class)
                .defaultValue(true)
                .build();
        this.RN = false;
        INSTANCE = this;
            ...
            if (this.RM
                ...
                PacketManager.b(
                        ...
                        () -> Listener.sendPacketNoEvents(new PlayerInteractBlockC2SPacket(
                                var6.getHand(), var6.getBlockHitResult(), NetworkUtils.generateNextSequence())));
            }
