...
public record MovTasks$MovInfo(Vec3d vec3d, Boolean oGroundOverride, boolean updatePlayer, Vec2f rotationOverride) {
    public MovTasks$MovInfo withUpdatePlayer(boolean updatePlayer) {
        return this.updatePlayer == updatePlayer
                ? this
                : new MovTasks$MovInfo(this.vec3d, this.oGroundOverride, updatePlayer, this.rotationOverride);
    }
    ...
    public MovTasks$MovInfo adE(Boolean oGroundOverride) {
        return this.oGroundOverride == oGroundOverride
                ? this
                : new MovTasks$MovInfo(this.vec3d, oGroundOverride, this.updatePlayer, this.rotationOverride);
    }
    ...
    public MovTasks$MovInfo adG(Vec2f rotationOverride) {
        return this.rotationOverride == rotationOverride
                ? this
                : new MovTasks$MovInfo(this.vec3d, this.oGroundOverride, this.updatePlayer, rotationOverride);
    }
    ...
    public boolean isEmptyTo(Vec3d currentPos) {
        return this.vec3d.squaredDistanceTo(currentPos) < 1.0E-4
                && this.rotationOverride == null
                && this.oGroundOverride == null;
    }

    public MovTasks$MovInfo adD(Vec3d vec3d) {
        return this.vec3d == vec3d
                ? this
                : new MovTasks$MovInfo(vec3d, this.oGroundOverride, this.updatePlayer, this.rotationOverride);
    }
}
