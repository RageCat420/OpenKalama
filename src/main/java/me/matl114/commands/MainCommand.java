...
public class MainCommand extends AbstractMainCommand {
   ...
   public MainCommand() {
      this.ay(new KalamaHelperHelperK("help", SubCommand.bo().a("help").g(s -> s.executor(new KalamaHelperHelperC(this))).k()));
      m = this;
      if (n != null) {
   ...
   public static CompletableFuture<Suggestions> tabCompleteClientCommand(String command, int cursorAt) {
      if (command.startsWith(l)) {
         ...
         return command.startsWith("/" + l) ? dispatchTabComplete(command.substring(l.length() + 1), cursorAt - l.length() - 1, true) : null;
      }
   }
