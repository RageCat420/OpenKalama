...
public class RegistryUtils {
   ...
   public static <W> RegistryEntry<W> getRegistryEntry(DynamicRegistryManager lookup, RegistryKey<? extends Registry<? extends W>> key, W value) {
   ...
   public static <W> RegistryEntry<W> g(WrapperLookup lookup, RegistryKey<W> key) {
      return lookup.getOptionalWrapper(key.getRegistryRef()).flatMap(registry -> registry.getOptional(key)).orElse(null);
   }
      ...
      try {
         for (Field var4 : RegistryKeys.class.getDeclaredFields()) {
            if (Modifier.isStatic(var4.getModifiers())
               ...
               if (var7.length == 1 && var7[0] instanceof ParameterizedType var9 && var9.getRawType() == Registry.class) {
