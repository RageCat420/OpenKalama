...
public class RotationArgumentType extends AbstractArgumentType<ExecuteRotation> implements ArgumentType<ExecuteRotation> {
   public Stream<String> tabCompleteArguments(CommandExecution sender, List<InputArgument<?>> args) {
      ...
      if (!(var3 instanceof KalamaHelperHelperG var4)) {
         ...
         if (var6 != 0 && !var5[0].isEmpty()) {
            ...
            return Stream.of(ExecuteRotation.fixed(var7.x, var7.y), ExecuteRotation.relative(3, 0.0F, 0.0F)).map(ExecuteRotation::aog);
         }
      }
   }
   ...
   @Nullable
   @Override
   public InputArgument<ExecuteRotation> consume(CommandExecution execution, List<InputArgument<?>> args, ArgumentReader reader) {
      if (!reader.hasNext()) {
