...
public interface InputArgument<W> {
   ...
   default double clampDouble(double low, double highEx) throws ArgumentException {
      ...
      if (var5 >= low && var5 < highEx) {
         ...
         throw new ValueOutOfRangeError(
            this.e(), this.i().getArgsName(), String.valueOf(low), String.valueOf(highEx), String.valueOf(var5), KalamaHelperHelperH.Qw
         );
      }
   }
   ...
   default <T extends Enum<T>> T A(Class<T> type) throws ArgumentException {
      ...
      throw new ValueOutOfRangeError(
         this.e(),
         this.i().getArgsName(),
         Arrays.stream((Enum[])type.getEnumConstants()).map(Enum::name).map(s -> s.toLowerCase(Locale.ROOT)).collect(Collectors.toList()),
         var2,
