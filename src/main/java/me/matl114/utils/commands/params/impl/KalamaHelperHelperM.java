...
public record KalamaHelperHelperM(String raw, Function<CommandExecution, List<Entity>> resolver) implements EntitySelector {
   ...
   public List<Entity> wo(CommandExecution execution) {
      return this.resolver.apply(execution).stream().filter(entity -> entity != null && !entity.isRemoved()).toList();
   }
