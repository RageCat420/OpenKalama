...
public class KalamaHelperHelperD<W extends AbstractArgumentType<T>, T> {
   ...
   public <R extends Enum<R>> KalamaHelperHelperD<W, T> q(R value) {
      ...
      this.d(() -> Arrays.stream((Enum[])var2.getEnumConstants()).map(s -> s.name().toLowerCase(Locale.ROOT))).b(value.name().toLowerCase(Locale.ROOT));
      return this;
   }
   ...
   public <R extends Enum<R>> KalamaHelperHelperD<W, T> p(Class<R> type) {
      this.d(() -> Arrays.stream((Enum[])type.getEnumConstants()).map(s -> s.name().toLowerCase(Locale.ROOT)));
      return this;
   }
