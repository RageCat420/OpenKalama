...
public class EntityArgumentType extends AbstractArgumentType<EntitySelector> implements ArgumentType<EntitySelector> {
   ...
   private static EntitySelector D(String raw) {
      if (raw.length() < 2) {
         ...
         KalamaHelperHelperB var1 = switch (raw.charAt(1)) {
      ...
      if (raw.isEmpty()) {
         ...
         try {
            if (var2 < 0) {
               ...
               return allowNegative || (var5 == null || var5 >= 0) && (var6 == null || var6 >= 0) ? new KalamaHelperHelperL(var5, var6) : null;
            }
   ...
   private static Stream<String> aa(String prefix, String option, String value) {
      if (!g.contains(option + "=")) {
         ...
         return ah(option, value) ? Stream.concat(var3, Stream.of(prefix + option + "=" + value + ",", prefix + option + "=" + value + "]")) : var3;
      }
   }

   private static Stream<String> ad() {
      return allEntities().stream().map(EntityArgumentType::entityName).filter(name -> name != null && !name.isBlank() && !name.contains(" ")).distinct();
   }
      ...
      if (raw.isEmpty()) {
         ...
         try {
            if (var2 < 0) {
               ...
               return allowNegative || (var7 == null || !(var7 < 0.0)) && (var8 == null || !(var8 < 0.0)) ? new KalamaHelperHelperC(var7, var8) : null;
            }
   ...
   @Nullable
   @Override
   public InputArgument<EntitySelector> consume(CommandExecution execution, List<InputArgument<?>> args, ArgumentReader reader) {
      if (!reader.hasNext()) {
   ...
   private static Stream<String> teamTabs() {
      return e.world == null ? Stream.empty() : allEntities().stream().map(EntityArgumentType::teamName).filter(s -> !s.isEmpty()).distinct();
   }

   private static Set<String> Z(String options) {
      List<KalamaHelperHelperT> var1 = Q(options.endsWith(",") ? options.substring(0, options.length() - 1) : options);
      return var1 != null && !var1.isEmpty() ? var1.stream().map(KalamaHelperHelperT::key).collect(Collectors.toSet()) : Set.of();
   }
   ...
   public static List<Entity> allEntities() {
      return e.world == null ? List.of() : StreamSupport.<Entity>stream(e.world.getEntities().spliterator(), false).toList();
   }

   private static Stream<String> entityTypeTabs(String value) {
      if (value.startsWith("!")) {
         return Stream.concat(Stream.of("!", "!#"), entityTypeTabs(value.substring(1)).map(tab -> "!" + tab));
      } else if (value.startsWith("#")) {
         return Stream.of("#").filter(tab -> tab.startsWith(value));
      } else {
         Stream var1 = Registries.ENTITY_TYPE.stream().<Identifier>map(Registries.ENTITY_TYPE::getId).filter(Objects::nonNull).map(Identifier::toString);
   ...
   private static Stream<String> af() {
      return e.world == null ? Stream.empty() : allEntities().stream().flatMap(entity -> entity.getCommandTags().stream()).distinct();
   }
   ...
   private static Stream<String> Y(String previousOptions, String partial) {
      ...
      return g.stream()
         ...
         .filter(option -> h.contains(option.substring(0, option.length() - 1)) || !var2.contains(option.substring(0, option.length() - 1)));
   }

   @Nullable
   private static KalamaHelperHelperT R(String token) {
      int var1 = S(token);
      return var1 < 1 ? null : new KalamaHelperHelperT(token.substring(0, var1).trim(), token.substring(var1 + 1).trim());
   }
   ...
   private static String entityName(Entity entity) {
      return entity instanceof PlayerEntity var1 ? var1.getNameForScoreboard() : entity.getName().getString();
   }
   ...
   private static List<Entity> resolveNamed(String raw) {
      return e.world == null ? List.of() : allEntities().stream().filter(entity -> matchesName(entity, raw)).toList();
   }

   public static boolean matchesName(Entity entity, String name) {
      return Objects.equals(entityName(entity), name) || Objects.equals(entity.getName().getString(), name);
   }
   ...
   public static String teamName(Entity entity) {
      return entity instanceof PlayerEntity var1 && var1.getScoreboardTeam() != null ? var1.getScoreboardTeam().getName() : "";
   }
}
