...
public class DispatchArgumentType<T> implements ArgumentType<T> {
   ...
   public DispatchArgumentType<T> registerDispatcher(BiPredicate<CommandExecution, List<InputArgument<?>>> predicate, ArgumentType<? extends T> type) {
      this.b.add(Pair.of(predicate, type));
      return this;
   }

   public DispatchArgumentType<T> registerArgumentDispatcher(int argumentIndex, String string, ArgumentType<? extends T> type) {
      this.registerDispatcher((execution, arguments) -> {
         try {
