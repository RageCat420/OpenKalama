...
public class WorldUtils {
   ...
   public static KalamaHelperHelperAk getWaypoint(String lookup) {
      ...
      return d().filter(s -> lookup.equalsIgnoreCase(s.getDisplayName()) || var2 != null && var2.equalsIgnoreCase(s.getDisplayName())).findFirst().orElse(null);
   }

   public static Stream<String> getPlayerListNames() {
      return mc.getNetworkHandler().getPlayerList().stream().<GameProfile>map(PlayerListEntry::getProfile).map(VRecord::getName);
   }
   ...
   public static float getPlayerBlockBreakingSpeedWithCanMineMultiply(PlayerEntity player, BlockState state, ItemStack stack) {
      ...
      if (player.isSubmergedIn(FluidTags.WATER)) {
         var3 *= (float)player.getAttributeInstance(EntityAttributes.PLAYER_SUBMERGED_MINING_SPEED).getValue();
      }
   ...
   public static float calcBlockBreakingDelta(BlockState state, BlockView world, BlockPos pos, float playerBreakSpeed) {
