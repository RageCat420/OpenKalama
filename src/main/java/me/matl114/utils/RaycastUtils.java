...
public class RaycastUtils {
    ...
    public static Optional<BlockPos> rayTraceSpecificBlock(Predicate<Block> blockPredicate) {
        if (a.world != null && a.player != null) {
            ...
            for (BlockPos var5 : r(
                    a.player.getEyePos(),
                    a.player.getEyePos().add(a.player.getRotationVector().multiply(6.0)))) {
    ...
    private static Vec3d findTargetPointOnFace(BlockState currState, BlockPos currPos, Direction direction) {
        List<Box> var3 = currState
                .getOutlineShape(a.world, currPos, ShapeContext.of(a.player))
                .getBoundingBoxes();
        return var3.stream()
                .map(it -> {
                ...
                .max(Comparator.<Pair<AlignedFace, Vec3d>>comparingDouble(it -> it.getSecond()
    ...
    public static EntityHitResult createRealHitResult(Entity entity, Vec3d playerEyePos) {
        ...
        return var4 != null && var4.isPresent()
                ? new EntityHitResult(entity, (Vec3d) var4.get())
    ...
    public static EntityHitResult raycastHitEntityExceptPlayerResult(Entity e, Vec3d from, Vec3d to) {
        return ProjectileUtil.raycast(
                e, from, to, new Box(from, to), es -> !es.isSpectator() && es.canHit() && es != a.player, 16384.0);
    }
    ...
    public static HitResult createEntityOnlyCrossHairResult(
        ...
        Vec3d var11 = var9.add(
                var10.x * entityInteractionRange, var10.y * entityInteractionRange, var10.z * entityInteractionRange);
        Box var12 = camera.getBoundingBox()
                .stretch(var10.multiply(entityInteractionRange))
                .expand(1.0, 1.0, 1.0);
        EntityHitResult var13 = ProjectileUtil.raycast(
                ...
                var11,
                var12,
                entity -> !entity.isSpectator() && entity.canHit() && (filter == null || filter.test(entity)),
                var7);
        return var13 != null && var13.getPos().squaredDistanceTo(var9) < var7
                ? ensureTargetInRange(var13, var9, entityInteractionRange)
                : null;
    }
    ...
    public static boolean raycastHitAnyEntity(Entity e, Vec3d from, Vec3d to) {
        EntityHitResult var3 =
                ProjectileUtil.raycast(e, from, to, new Box(from, to), es -> !es.isSpectator() && es.canHit(), 16384.0);
        return var3 != null && var3.getType() != Type.MISS;
    }
    ...
    public static HitResult createCrossHairHitResult(
        ...
        EntityHitResult var18 = ProjectileUtil.raycast(
                camera, var10, var15, var17, entity -> !entity.isSpectator() && entity.canHit(), var8);
