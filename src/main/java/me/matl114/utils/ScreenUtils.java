...
public class ScreenUtils {
   ...
   public static void simulateMouseButton(@Nonnull Screen screen, int button, int action, int mods) {
      ...
      if (a.getOverlay() == null) {
         double var7 = a.mouse.getX() * a.getWindow().getScaledWidth() / a.getWindow().getWidth();
         double var9 = a.mouse.getY() * a.getWindow().getScaledHeight() / a.getWindow().getHeight();
         if (var4) {
            screen.applyMousePressScrollNarratorDelay();
            Screen.wrapScreenError(() -> var6[0] = screen.mouseClicked(var7, var9, button), "mouseClicked event handler", screen.getClass().getCanonicalName());
         } else {
            Screen.wrapScreenError(
               () -> var6[0] = screen.mouseReleased(var7, var9, button), "mouseReleased event handler", screen.getClass().getCanonicalName()
            );
         }
      }
   }
   ...
   public static void simulateKeyAction(Screen screen, int key, int scancode, int action, int modifiers) {
      ...
      if (action == 1
         && (!(screen instanceof KeybindsScreen) || ((KeybindsScreen)screen).lastKeyCodeUpdateTime <= Util.getMeasuringTimeMs() - 20L)
         ...
         if (action == 0) {
            ...
            boolean var11 = InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), 292);
            if (var6) {
               if (var11) {
                  KeyBinding.setKeyPressed(var10, false);
               } else {
   ...
   public static void simulateMouseScroll(@Nonnull Screen screen, double horizontal, double vertical) {
      ...
      if (a.getOverlay() == null) {
         if (screen != null) {
            double var12 = a.mouse.getX() * a.getWindow().getScaledWidth() / a.getWindow().getWidth();
            double var14 = a.mouse.getY() * a.getWindow().getScaledHeight() / a.getWindow().getHeight();
            screen.mouseScrolled(var12, var14, var8, var10);
            screen.applyMousePressScrollNarratorDelay();
         } else if (a.player != null) {
            if (a.mouse.eventDeltaHorizontalWheel != 0.0 && Math.signum(var8) != Math.signum(a.mouse.eventDeltaHorizontalWheel)) {
               a.mouse.eventDeltaHorizontalWheel = 0.0;
            }

            if (a.mouse.eventDeltaVerticalWheel != 0.0 && Math.signum(var10) != Math.signum(a.mouse.eventDeltaVerticalWheel)) {
            ...
            if (a.player.isSpectator()) {
               if (a.inGameHud.getSpectatorHud().isOpen()) {
                  ...
                  float var19 = MathHelper.clamp(a.player.getAbilities().getFlySpeed() + var17 * 0.005F, 0.0F, 0.2F);
   ...
   public static boolean hasEnterDown() {
      return InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), 257)
         || InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), 355);
   }
