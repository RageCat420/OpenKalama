...
public class EntityUtils {
    ...
    public static float m(float newPitch) {
        return newPitch < 90.00001 && newPitch > -90.00001
                ? newPitch
                : (newPitch % 180.0F + 720.0F + 90.0F) % 180.0F - 90.0F;
    }
    ...
    public static Text getEntityDisplayable(Entity target) {
        return (Text)
                (target instanceof PlayerEntity var1
                        ? Text.literal(var1.getNameForScoreboard())
    ...
    public static Stream<String> getWorldPlayerNames(boolean containSelf) {
        return a.world.getPlayers().stream()
                .filter(i -> containSelf || i != a.player)
    ...
    public static double getEffectiveGravity(ClientPlayerEntity player) {
        ...
        return var1 && player.hasStatusEffect(StatusEffects.SLOW_FALLING)
    ...
    public static Identifier getSpawnedEntityId(NbtCompound nbt, String spawnDataKey) {
        if (nbt.contains(spawnDataKey)) {
            if (nbt.get(spawnDataKey) instanceof NbtCompound var3
                    && var3.get("entity") instanceof NbtCompound var6
                    && var6.get("id") instanceof NbtString var7) {
        ...
        try {
            ...
            if (Pattern.matches(value, "monster")) {
                for (EntityType var15 : Registries.ENTITY_TYPE) {
                    if (var15.getSpawnGroup() == SpawnGroup.MONSTER
                            && var15 != EntityType.ZOMBIFIED_PIGLIN
                            && var15 != EntityType.ENDERMAN) {
            ...
            while (var13.hasNext()) {
                ...
                if (!Pattern.matches(value, Registries.ENTITY_TYPE.getId(var18).getPath())
                        && Pattern.matches(
                                value, "!" + Registries.ENTITY_TYPE.getId(var18).getPath())) {
                    var13.remove();
                }
            }
    ...
    private static Vec3d simulateResetVerticalVelocityInFluid(Vec3d velocity, double y) {
        return a.player.horizontalCollision
                        && a.player.doesNotCollide(velocity.x, velocity.y + 0.6F - a.player.getY() + y, velocity.z)
    ...
    public static Vec3d calculateGlidingVelocity(
        ...
        if (var7 > 0.0) {
            ...
            var17 = var17.add(
                    (rotationVector.x * var21 - var17.x) * 0.1, 0.0, (rotationVector.z * var21 - var17.z) * 0.1);
        }
