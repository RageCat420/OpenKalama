...
public class RenderUtils {
    ...
    public static Quaternionf getBillboardRotation(BillboardMode renderState, float pitch, float yaw) {
        ...
        return switch (renderState) {
            ...
            case HORIZONTAL -> var3.rotationYXZ(
                    (float) (-Math.PI / 180.0) * yaw, (float) (Math.PI / 180.0) * A(var4.getPitch()), 0.0F);
            case VERTICAL -> var3.rotationYXZ(
                    (float) (-Math.PI / 180.0) * z(var4.getYaw()), (float) (Math.PI / 180.0) * pitch, 0.0F);
            case CENTER -> var3.rotationYXZ(
                    (float) (-Math.PI / 180.0) * z(var4.getYaw()),
                    (float) (Math.PI / 180.0) * A(var4.getPitch()),
                    0.0F);
    ...
    public static void j(MatrixStack matrixStack, List<Vec3d> path, Color color) {
        if (path.size() >= 2) {
            ...
            VRender.getInstance()
                    .p(matrixStack, path.stream().map(v -> v.subtract(var3)).toList(), color);
        }
    }
    ...
    public static void n(MatrixStack matrixStack, List<Vec3d> pairs, Color color) {
        if (pairs.size() >= 2) {
            Vec3d var3 = getCameraPos();
            VRender.getInstance()
                    .q(matrixStack, pairs.stream().map(v -> v.subtract(var3)).toList(), color);
        }
    }
