...
public record BlockLocation(RegistryKey<World> world, int x, int y, int z) {
    public static MapCodec<BlockLocation> JL = RecordCodecBuilder.mapCodec(o -> o.group(
    ...
    public static MapCodec<BlockLocation> wJ = RecordCodecBuilder.mapCodec(instance -> instance.group(
                    RegistryKey.createCodec(RegistryKeys.WORLD).fieldOf("world").forGetter(BlockLocation::vS),
    ...
    public boolean isLocationLoaded(World world) {
        return Objects.equals(world.getRegistryKey(), this.vS())
                ? world.getChunkManager().isChunkLoaded(this.x >> 4, this.z >> 4)
                : false;
    }

    public boolean YQ(BlockLocation location, double distance) {
        return Objects.equals(location.world, this.world)
                ? MathUtils.b(this.x - location.x) + MathUtils.b(this.z - location.z) <= MathUtils.a(distance)
                : false;
    }

    public boolean YP(BlockLocation location, double distance) {
        return Objects.equals(location.world, this.world)
                ? MathUtils.b(this.x - location.x) + MathUtils.b(this.z - location.z) + MathUtils.b(this.y - location.y)
                        <= MathUtils.a(distance)
                : false;
    }
