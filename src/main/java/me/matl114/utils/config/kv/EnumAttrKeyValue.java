...
public class EnumAttrKeyValue<T> extends BaseAttrKeyValue<T> {
    ...
    Class<T> identifier;
    public static final AttrKeyValue.CustomWidgetFactory<?> ENUM_WIDGET_FACTORY =
            (s, x, y, dx, dy) -> s instanceof EnumAttrKeyValue attrKeyValue
    ...
    public EnumAttrKeyValue(String key, T value, Class<T> clazz, Map<String, T> finiteValueMap) {
        super(
                key,
                value,
                (AttrKeyValue.CustomWidgetFactory<T>) ENUM_WIDGET_FACTORY,
                createFiniteMapLookup(finiteValueMap));
    ...
    public static <T> AttrKeyValue.CustomWidgetFactory<T> createFiniteLookupWidgetFactory(Map<String, T> map) {
        ...
        if (StringArgumentResult.class.isAssignableFrom(enumClass)) {
            flattenMap = map.entrySet().stream()
                    .map(entry -> new Pair<>(
                            entry.getKey(), (Supplier<Text>) ((StringArgumentResult) entry.getValue())::resultAsString))
                    .toList();
        } else {
    ...
    public DrawableWidget generateSwitchingButton(
        ...
        if (StringArgumentResult.class.isAssignableFrom(this.identifier)) {
            ...
            flattenMap = this.getValueMap().keySet().stream()
                    .map(v -> new Pair<>(v, (Supplier<Text>) () -> Text.translatableWithFallback(v, v)))
                    .toList();
        }
    ...
    public static DrawableWidget generateSwitchingButton(
        ...
        if (choices > 0) {
            ...
            subScreen.Q(ExecutableWidget.instance(1, 1, mainDx - 2, dy - 2)
                    .eV(new ButtonElement(
                                    ign -> {
                                        kvUpdater.run();
                                        return (Text) ((Supplier) flattenMap
                                                        .get(integer.get())
                                                        .getSecond())
                                                .get();
                                    },
            ...
            if (needSwitch) {
                ...
                subScreen.Q(ExecutableWidget.instance(dx - dy + 2, 2, dy - 4, dy - 4)
                        .eV(IconElement.co(
                                KalamaHelperHelperB.j,
                                KalamaHelperHelperB.k,
                                ButtonAction.a(() -> show.setValue(!show.booleanValue())),
                ...
                Supplier<KalamaHelperHelperCX> subScreenSupplier = Suppliers.memoize(() -> {
                    ...
                    for (int i = 0; i < choices; i++) {
                        ...
                        int finalI = i;
                        selectors.Q(ExecutableWidget.instance(0, height, dx - dy - 4, dy)
                ...
                ContentDelegateWidget<KalamaHelperHelperCX> dynamicDelegate =
                        new KalamaHelperHelperJ<>(() -> show.booleanValue() ? subScreenSupplier.get() : null, 2, dy);
            ...
            return ExecutableWidget.instance(x + 1, y + 1, dx - 2, dy - 2)
                    .eV(new ButtonElement(TextProvider.c(Text.empty()), ButtonAction.c())
                            .aO(TooltipHandler.ap(List.of(Text.translatable(ex.getKeyName())))));
        }
    }
}
