...
public interface PairLikeFactory<A, B, P> {
    PairLikeFactory<?, ?, Pair<?, ?>> PAIR_FACTORY =
            new PairLikeFactory.BasePairLikeFactory<>(Pair::of, Pair::getFirst, Pair::getSecond);
    ...
    default <D> PairLikeFactory<A, B, D> concat(WrapperFactory<P, D> mapper) {
        return new PairLikeFactory.BasePairLikeFactory<>(
                (a, b) -> mapper.create(this.create(a, b)),
                s -> this.getFirst(mapper.get(s)),
                s -> this.getSecond(mapper.get(s)));
    }
    ...
    public static class BasePairLikeFactory<A, B, P> implements PairLikeFactory<A, B, P> {
        ...
        public PairLikeFactory<B, A, P> swap() {
            return new PairLikeFactory.BasePairLikeFactory<>(
                    (b, a) -> this.creator.apply(a, b), this.secondGetter, this.firstGetter);
        }

        public BasePairLikeFactory(
                BiFunction<A, B, P> creator, Function<P, A> firstGetter, Function<P, B> secondGetter) {
