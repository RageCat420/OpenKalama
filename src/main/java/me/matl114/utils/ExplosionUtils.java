...
public final class ExplosionUtils {
    ...
    private static boolean rayCastAccept(
            KalamaHelperHelperHX stateAccess, Vec3d source, Vec3d pos, KalamaHelperHelperQ hitRule) {
    ...
    public static float calculateExplosionRawDamage(
            float power, Vec3d explosionPos, Box targetBox, KalamaHelperHelperHX access, KalamaHelperHelperQ hitRule) {
        Vec3d var5 = new Vec3d(
                (targetBox.minX + targetBox.maxX) / 2.0, targetBox.minY, (targetBox.minZ + targetBox.maxZ) / 2.0);
    ...
    public static double getImpact(double normalizedDistance, double exposure) {
        return !(normalizedDistance >= 1.0) && !(exposure <= 0.0)
                ? (1.0 - normalizedDistance) * MathHelper.clamp(exposure, 0.0, 1.0)
                : 0.0;
    }

    @KalamaHelperHelperA
    private static Set<BlockPos> collectPotentiallyDestroyedBlocks(
            float power, Vec3d explosionPos, KalamaHelperHelperHX access) {
        if (power <= 0.0F) {
            return Set.of();
        } else {
    ...
    public static float b(
            Box predictedPos, Vec3d explosionPos, float power, BlockView world, KalamaHelperHelperQ hitRule) {
        return calculateExplosionRawDamage(power, explosionPos, predictedPos, p(world), hitRule);
    }

    private static float getExposureSimplified(
            KalamaHelperHelperHX access, Vec3d source, Box box, KalamaHelperHelperQ hitRule) {
    ...
    public static float respawnAnchorDamage(
            Box predictedPos, Vec3d explosionPos, BlockView world, KalamaHelperHelperQ hitRule) {
        return calculateExplosionRawDamage(
                5.0F,
                explosionPos,
                predictedPos,
                q(world, Map.of(BlockPos.ofFloored(explosionPos), Blocks.AIR.getDefaultState())),
                hitRule);
    }
    ...
    public static float explosionDamage(
            LivingEntity target, Vec3d explosionPos, float power, BlockView world, KalamaHelperHelperQ hitRule) {
        return target == null
                ? 0.0F
                : calculateExplosionRawDamage(power, explosionPos, target.getBoundingBox(), p(world), hitRule);
    }
