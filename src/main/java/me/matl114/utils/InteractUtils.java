...
public class InteractUtils {
   ...
   public static boolean isInteractAcceptable(World world, PlayerEntity player, Entity entity, ItemStack interactStack) {
      if (world == null || player == null || entity == null) {
         ...
         if (isGenericAcceptedInteractItem(entity, var4)) {
            ...
            return isVehicleEntityInteractAcceptable(player, entity, var4) ? true : isSpecialEntityInteractAcceptable(player, entity, var4);
         }
      } else {
         return false;
      }
   ...
   public static boolean E(BlockState fromState, BlockState toState) {
      if (fromState == null || toState == null) {
         ...
         return fromState.equals(toState) ? true : !getNextInteractionStep(fromState, toState).isEmpty();
      }
   }

   @Nullable
   public static BlockState getBlockPlacement(Block block, PlayerEntity player, World world, BlockHitResult blockHitResult) {
   ...
   public static BlockPos getCurrentPlacePos(PlayerEntity player, BlockHitResult blockHitResult) {
      ItemPlacementContext var2 = new ItemPlacementContext(player, Hand.MAIN_HAND, new ItemStack(Blocks.STONE), blockHitResult);
   ...
   public static BlockState b(BlockItem blockItem, PlayerEntity player, World world, BlockHitResult blockHitResult) {
      ItemPlacementContext var4 = new ItemPlacementContext(player, Hand.MAIN_HAND, new ItemStack(blockItem), blockHitResult);
   ...
   private static boolean isSpecialEntityInteractAcceptable(PlayerEntity player, Entity entity, ItemStack stack) {
      if (entity instanceof ArmorStandEntity var18) {
      ...
      } else if (entity instanceof PigEntity var3 && var3.isSaddled() && !var3.hasPassengers() && !player.shouldCancelInteraction()) {
         return true;
      } else if (entity instanceof StriderEntity var6 && var6.isSaddled() && !var6.hasPassengers() && !player.shouldCancelInteraction()) {
         return true;
         ...
         if (var11.isTamed()) {
            if (var11.isBreedingItem(stack) && var11.getHealth() < var11.getMaxHealth()) {
               ...
               if (stack.getItem() instanceof DyeItem var20 && var20.getColor() != var11.getCollarColor()) {
                  ...
                  return var11.isInSittingPose() && var11.isWearingBodyArmor() && var11.getBodyArmor().isDamaged() ? true : true;
               }
            } else {
               return false;
            }
         ...
         return var8.isTamed() ? var8.isOwner(player) : stack.isIn(ItemTags.PARROT_FOOD) || stack.isIn(ItemTags.PARROT_POISONOUS_FOOD);
      } else {
   ...
   private static EquipmentSlot getArmorStandHitSlot(ArmorStandEntity armorStand, Vec3d hitPos) {
      ...
      if (var4 >= 0.1 && var4 < 0.1 + (var3 ? 0.8 : 0.45) && !armorStand.getEquippedStack(EquipmentSlot.FEET).isEmpty()) {
         var2 = EquipmentSlot.FEET;
      } else if (var4 >= 0.9 + (var3 ? 0.3 : 0.0) && var4 < 0.9 + (var3 ? 1.0 : 0.7) && !armorStand.getEquippedStack(EquipmentSlot.CHEST).isEmpty()) {
         var2 = EquipmentSlot.CHEST;
      } else if (var4 >= 0.4 && var4 < 0.4 + (var3 ? 1.0 : 0.8) && !armorStand.getEquippedStack(EquipmentSlot.LEGS).isEmpty()) {
         var2 = EquipmentSlot.LEGS;
      } else if (var4 >= 1.6 && !armorStand.getEquippedStack(EquipmentSlot.HEAD).isEmpty()) {
         var2 = EquipmentSlot.HEAD;
      } else if (armorStand.getEquippedStack(EquipmentSlot.MAINHAND).isEmpty() && !armorStand.getEquippedStack(EquipmentSlot.OFFHAND).isEmpty()) {
   ...
   public static Set<Pair<BlockState, Predicate<ItemStack>>> getNextInteractionStep(BlockState fromState, BlockState toState) {
      ...
      if (fromState != null && toState != null) {
         ...
         if (var3 == var4) {
            if (var3 instanceof SlabBlock && fromState.get(SlabBlock.TYPE) != SlabType.DOUBLE && toState.get(SlabBlock.TYPE) == SlabType.DOUBLE) {
               var2.add(Pair.of((BlockState)fromState.with(SlabBlock.TYPE, SlabType.DOUBLE), G(var3.asItem())));
            }

            int var5;
            if (var3 instanceof SnowBlock && (var5 = (Integer)fromState.get(SnowBlock.LAYERS)) < 8 && (Integer)toState.get(SnowBlock.LAYERS) > var5) {
               var2.add(Pair.of((BlockState)fromState.with(SnowBlock.LAYERS, var5 + 1), G(var3.asItem())));
            }

            if (var3 instanceof CandleBlock && (var5 = (Integer)fromState.get(CandleBlock.CANDLES)) < 4 && (Integer)toState.get(CandleBlock.CANDLES) > var5) {
            ...
            if (var3 instanceof FlowerbedBlock
               ...
               var2.add(Pair.of((BlockState)fromState.with(FlowerbedBlock.FLOWER_AMOUNT, var5 + 1), G(var3.asItem())));
            }

            if (var3 instanceof RepeaterBlock && !((Integer)toState.get(RepeaterBlock.DELAY)).equals(fromState.get(RepeaterBlock.DELAY))) {
               var2.add(Pair.of((BlockState)fromState.cycle(RepeaterBlock.DELAY), b));
            }

            if (var3 instanceof ComparatorBlock && toState.get(ComparatorBlock.MODE) != fromState.get(ComparatorBlock.MODE)) {
            ...
            if (var3 instanceof TrapdoorBlock && toState.get(TrapdoorBlock.OPEN) != fromState.get(TrapdoorBlock.OPEN)) {
               var2.add(Pair.of((BlockState)fromState.cycle(TrapdoorBlock.OPEN), b));
            }

            if (var3 instanceof FenceGateBlock && toState.get(FenceGateBlock.OPEN) != fromState.get(FenceGateBlock.OPEN)) {
               var2.add(Pair.of((BlockState)fromState.cycle(FenceGateBlock.OPEN), b));
            }

            if (var3 instanceof LeverBlock && toState.get(LeverBlock.POWERED) != fromState.get(LeverBlock.POWERED)) {
               var2.add(Pair.of((BlockState)fromState.cycle(LeverBlock.POWERED), b));
            }

            if (var3 instanceof ButtonBlock && !(Boolean)fromState.get(ButtonBlock.POWERED) && toState.equals(fromState.with(ButtonBlock.POWERED, true))) {
            ...
            if (var3 instanceof CandleBlock && (Boolean)fromState.get(CandleBlock.LIT) && toState.equals(fromState.with(CandleBlock.LIT, false))) {
            ...
            if (var3 instanceof RespawnAnchorBlock
               ...
               && (Integer)toState.get(RespawnAnchorBlock.CHARGES) > (Integer)fromState.get(RespawnAnchorBlock.CHARGES)) {
               var2.add(
                  Pair.of((BlockState)fromState.with(RespawnAnchorBlock.CHARGES, (Integer)fromState.get(RespawnAnchorBlock.CHARGES) + 1), G(Items.GLOWSTONE))
               );
            }

            if (var3 instanceof CakeBlock
               && (Integer)fromState.get(CakeBlock.BITES) < 6
               && toState.equals(fromState.with(CakeBlock.BITES, (Integer)fromState.get(CakeBlock.BITES) + 1))) {
               var2.add(Pair.of(toState, b));
            }
         ...
         if (var3 instanceof CakeBlock && var4 instanceof CandleCakeBlock && (Integer)fromState.get(CakeBlock.BITES) == 0) {
         ...
         if (var3 instanceof CandleCakeBlock && var4 instanceof CakeBlock && (Integer)toState.get(CakeBlock.BITES) == 1) {
            var2.add(Pair.of(toState, b));
         }
         ...
         if (var3 instanceof FlowerPotBlock var12
            ...
            var2.add(Pair.of(toState, (Predicate<ItemStack>)stack -> stack != null && stack.getItem() instanceof BlockItem var3x && var3x.getBlock() == var14));
         }
   ...
   private static boolean isGenericAcceptedInteractItem(Entity entity, ItemStack stack) {
      if (stack.isOf(Items.NAME_TAG) && entity instanceof LivingEntity) {
         ...
         return !var9.isBaby() && stack.isOf(Items.BOWL) ? true : var9.getVariant() == Type.BROWN && SuspiciousStewIngredient.of(stack.getItem()) != null;
         ...
         return !(entity instanceof ParrotEntity) ? false : stack.isIn(ItemTags.PARROT_FOOD) || stack.isIn(ItemTags.PARROT_POISONOUS_FOOD);
      }
   }
   ...
   public static boolean canShulkerOpen(World world, BlockPos pos, BlockState state) {
      if (world.getBlockEntity(pos) instanceof ShulkerBoxBlockEntity var4 && var4.getAnimationStage() != AnimationStage.CLOSED) {
         return true;
      } else {
         Box var5 = ShulkerEntity.calculateBoundingBox(1.0F, (Direction)state.get(ShulkerBoxBlock.FACING), 0.0F, 0.5F).offset(pos).contract(1.0E-6);
   ...
   public static BlockState c(PlayerEntity player, Hand hand, ItemStack stack, BlockHitResult hitResult) {
      return stack.getItem() instanceof BlockItem var5 ? var5.getPlacementState(new ItemPlacementContext(player, hand, stack, hitResult)) : null;
   }
   ...
   public static ActionResult simulateInteract(EntityHitResult entityHitResult) {
      ActionResult var1 = a.interactionManager.interactEntityAtLocation(a.player, entityHitResult.getEntity(), entityHitResult, Hand.MAIN_HAND);
      if (!var1.isAccepted()) {
   ...
   private static boolean canFenceConsume(World world, BlockPos pos, @Nullable PlayerEntity player) {
      if (player == null) {
         ...
         boolean var3 = player.getMainHandStack().getItem() instanceof LeadItem || player.getOffHandStack().getItem() instanceof LeadItem;
         if (!var3) {
            return false;
         } else {
   ...
   public static boolean canRespawnAnchorExplode(World world) {
      ...
      return Objects.equals(var1, "minecraft:overworld") || Objects.equals(var1, "minecraft:the_end") ? true : !world.getDimension().respawnAnchorWorks();
   }

   public static boolean isInteractAtAcceptable(World world, PlayerEntity player, Entity entity, Vec3d hitPos, ItemStack interactStack) {
      if (world == null || player == null || entity == null || hitPos == null) {
         ...
         if (entity instanceof ArmorStandEntity var6) {
            if (var6.isMarker()) {
               ...
               return !canArmorStandUseSlot(var6, var10) ? false : var10.getType() != net.minecraft.entity.EquipmentSlot.Type.HAND || var6.shouldShowArms();
            } else {
               EquipmentSlot var7 = getArmorStandHitSlot(var6, hitPos);
               if (canArmorStandUseSlot(var6, var7) && !var6.getEquippedStack(var7).isEmpty()) {
                  return true;
               } else {
                  EquipmentSlot var8 = EquipmentSlot.MAINHAND;
                  if (canArmorStandUseSlot(var6, var8) && !var6.getEquippedStack(var8).isEmpty()) {
                     return true;
                  } else {
                     EquipmentSlot var9 = EquipmentSlot.OFFHAND;
                     return canArmorStandUseSlot(var6, var9) && !var6.getEquippedStack(var9).isEmpty();
                  }
               }
            }
         } else {
   ...
   public static boolean z(World world, PlayerEntity player, ItemStack interactStack) {
      if (world == null || player == null || interactStack == null || interactStack.isEmpty()) {
         ...
         if (var3 instanceof BowItem) {
            return player.isInCreativeMode() || !player.getProjectileType(interactStack).isEmpty();
         } else if (var3 instanceof CrossbowItem) {
            ChargedProjectilesComponent var8 = (ChargedProjectilesComponent)interactStack.get(DataComponentTypes.CHARGED_PROJECTILES);
            return var8 != null && !var8.isEmpty() || !player.getProjectileType(interactStack).isEmpty();
         ...
         } else if (interactStack.getItem() instanceof Equipment var4 && (var4 instanceof ArmorItem || var4 instanceof ElytraItem)) {
         ...
         } else if (interactStack.getItem() instanceof ShieldItem || VItem.w().b(interactStack)) {
            return true;
   ...
   public static boolean t(World world, PlayerEntity player, BlockPos pos, BlockState state, ItemStack interactStack) {
      ...
      if (var5 instanceof RespawnAnchorBlock) {
         ...
         return world.getBlockEntity(pos) instanceof CampfireBlockEntity var21 && var21.getRecipeFor(interactStack).isPresent();
         ...
         if (var6 instanceof MinecartItem && state.isIn(BlockTags.RAILS)) {
            ...
            return true;
         } else if (!(var6 instanceof EndCrystalItem) || !state.isOf(Blocks.OBSIDIAN) && !state.isOf(Blocks.BEDROCK)) {
            if (var6 instanceof SpawnEggItem) {
               return state.hasBlockEntity() && world.getBlockEntity(pos) instanceof Spawner || state.getCollisionShape(world, pos).isEmpty();
            } else {
               ...
               if (var6 instanceof ShovelItem && d.contains(var5) && world.getBlockState(pos.up()).isAir()) {
                  return true;
               } else if (var6 instanceof HoneycombItem && HoneycombItem.getWaxedState(state).isPresent()) {
                  return true;
               } else {
