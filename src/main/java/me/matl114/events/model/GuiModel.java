...
public interface GuiModel {
   ...
   default void render(
      ...
      if (var8 != null) {
         if (var8.stackTransformer() != null) {
            ...
            itemRenderer.renderItem(var8.stack(), renderMode, leftHanded, matrices, vertexConsumers, light, overlay, var8.state());
            matrices.pop();
         } else {
            itemRenderer.renderItem(var8.stack(), renderMode, leftHanded, matrices, vertexConsumers, light, overlay, var8.state());
         }
   ...
   static GuiModel b(List<GuiModel> guiModelList) {
      if (guiModelList != null && !guiModelList.isEmpty()) {
         guiModelList = guiModelList.stream().filter(s -> !(s instanceof KalamaHelperHelperC)).toList();
         if (guiModelList.isEmpty()) {
            return EMPTY;
         } else {
            return (GuiModel)(guiModelList.size() == 1 ? (GuiModel)guiModelList.get(0) : new KalamaHelperHelperF(guiModelList));
         }
      } else {
         return EMPTY;
      }
