...
public class MoonriseVoxelShapeMixin implements MoonriseVoxelShapeAccess {
   ...
   private static final CachedShapeData moonrise$getOrCreateCachedShapeData(VoxelSet shape) {
      ...
      boolean hasSingleAABB = sizeX == 1 && sizeY == 1 && sizeZ == 1 && !isEmpty && discreteVoxelShape.contains(0, 0, 0);
      ...
      return new CachedShapeData(sizeX, sizeY, sizeZ, voxelSet, minFullX, minFullY, minFullZ, maxFullX, maxFullY, maxFullZ, isEmpty, hasSingleAABB);
   }
   ...
   private List<Box> toAabbsUncached() {
      ...
      if (this.singleAABBRepresentation != null) {
         ...
         this.field_1401
            .forEachBox(
               (minX, minY, minZ, maxX, maxY, maxZ) -> ret.add(
                  new Box(coordsX[minX] + offX, coordsY[minY] + offY, coordsZ[minZ] + offZ, coordsX[maxX] + offX, coordsY[maxY] + offY, coordsZ[maxZ] + offZ)
               ),
               true
            );
      }
   ...
   public VoxelShape method_1096(double x, double y, double z) {
      ...
      if (this.isEmpty) {
         ...
         ArrayVoxelShape ret = new ArrayVoxelShape(
            this.field_1401, offsetList(this.method_1109(Axis.X), x), offsetList(this.method_1109(Axis.Y), y), offsetList(this.method_1109(Axis.Z), z)
         );
         ...
         if (directionOpposite.lengthSquared() < 1.0E-7) {
            ...
            if (singleAABB != null) {
               return singleAABB.contains(fromBehindOffsetX, fromBehindOffsetY, fromBehindOffsetZ)
                  ? new BlockHitResult(
                     fromBehind, Direction.getFacing(directionOpposite.x, directionOpposite.y, directionOpposite.z).getOpposite(), offset, true
                  )
                  : raycast(singleAABB, from, to, offset);
            } else {
               return CollisionUtil.r((VoxelShape)(Object)this, fromBehindOffsetX, fromBehindOffsetY, fromBehindOffsetZ)
                  ? new BlockHitResult(
                     fromBehind, Direction.getFacing(directionOpposite.x, directionOpposite.y, directionOpposite.z).getOpposite(), offset, true
                  )
