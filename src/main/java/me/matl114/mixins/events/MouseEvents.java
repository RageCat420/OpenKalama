...
public abstract class MouseEvents {
    ...
    private void onMouseScroll(long handle, double xOffset, double yOffset, CallbackInfo ci) {
        if (MinecraftClient.getInstance().getOverlay() == null
                && SimpleInputManager.h().onMouseScroll(xOffset, yOffset)) {
            ci.cancel();
        }
    }
    ...
    private void onMouseClick(
            long handle, int button, int action, int mods, CallbackInfo ci, @Local(ordinal = 3) int i) {
    ...
    private void onScreenNull(CallbackInfo ci) {
        if (MinecraftClient.getInstance().currentScreen == null
            ...
            double f = this.method_1603()
                    * this.field_1779.getWindow().getScaledWidth()
                    / this.field_1779.getWindow().getWidth();
            double g = this.method_1604()
                    * this.field_1779.getWindow().getScaledHeight()
                    / this.field_1779.getWindow().getHeight();
            Event<MouseMoveAction> event = new Event<>(new MouseMoveAction((Mouse) (Object) this, f, g), true, false);
            Listener.bs().catchEvent(event);
            if (this.field_1780 != -1) {
                double h = this.field_1789
                        * this.field_1779.getWindow().getScaledWidth()
                        / this.field_1779.getWindow().getWidth();
                double i = this.field_1787
                        * this.field_1779.getWindow().getScaledHeight()
                        / this.field_1779.getWindow().getHeight();
