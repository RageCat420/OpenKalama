...
public class LancingUtils {
   ...
   public static float p(float t) {
      ...
      if (t == 0.0F) {
         ...
         return t < 0.5F ? (float)(-(Math.pow(2.0, 20.0 * t - 10.0) * var2) / 2.0) : (float)(Math.pow(2.0, -20.0 * t + 10.0) * var2 / 2.0 + 1.0);
      }
   }

   public static void positionArmForSpear(ModelPart arm, ModelPart head, boolean right, ItemStack itemStack, LivingEntity state) {
      int var5 = right ? 1 : -1;
      ...
      if (!(state.getItemUseTime() <= 0.0F) && (!state.isUsingItem() || state.getActiveHand() == (right ? Hand.MAIN_HAND : Hand.OFF_HAND))) {
         ...
         if (var6 != null) {
            ...
            arm.yaw = arm.yaw + -var5 * var7.raiseProgressStart() * (float) (Math.PI / 180.0) * var7.swayProgress() * 1.0F;
            arm.roll = arm.roll + -var5 * var7.raiseProgressMiddle() * (float) (Math.PI / 180.0) * var7.swayProgress() * 0.5F;
            arm.pitch = arm.pitch
               + (float) (Math.PI / 180.0)
                  * (-40.0F * var7.raiseProgressEnd() + 30.0F * var7.swayProgress() + -20.0F * var7.raiseProgressStart() + 20.0F * var7.raiseProgressMiddle() + 10.0F * var7.raiseProgressEnd() + 0.6F * var7.raiseProgressMiddle() * var7.swayProgress());
         }
      }
   }
   ...
   public static void applyHeldItemFeatureArm(MatrixStack matrixStack, float f, Arm arm, ItemStack itemStack) {
      ...
      if (var4 != null && f != 0.0F) {
         ...
         float var10 = 0.125F;
         float var11 = a(0.0F);
         matrixStack.translate(0.0, -var11 * 0.4, -var4.forwardMovement() * (var9 - var7.raiseProgressEnd()) + var11);
         matrixStack.multiply(RotationAxis.NEGATIVE_X.rotationDegrees(70.0F * (var7.raiseProgress() - var7.raiseProgressEnd()) - 40.0F * (var5 - var6)), 0.0F, -0.03125F, 0.125F);
         matrixStack.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(var8 * 90 * (var7.raiseProgress() - var7.raiseProgress() + 3.0F * var6 + var5)), 0.0F, 0.0F, 0.125F);
      }
   }
   ...
   public static void applySpearKineticTransform(Item item, float f, MatrixStack matrixStack, float g, Arm arm, ItemStack itemStack) {
      if (SWORD_KINETIC_MAP.containsKey(item)) {
         ...
         matrixStack.translate(
            var7 * (var6.raiseProgress() * 0.15F + var6.raiseProgressStart() * -0.05F + var6.raiseProgress() * -0.1F + var6.raiseProgressMiddle() * 0.005F),
            var6.raiseProgress() * -0.075F + var6.swayProgress() * 0.075F + var6.raiseProgressStart() * 0.01F,
            var6.raiseProgressEnd() * 0.05 + var6.raiseProgressStart() * -0.05 + var6.raiseProgressMiddle() * 0.005F
         );
         matrixStack.multiply(
            RotationAxis.POSITIVE_X.rotationDegrees(-65.0F * h(var6.raiseProgress()) - 35.0F * var6.raiseProgressMiddle() + 100.0F * var6.raiseProgressEnd() + -0.5F * var6.raiseProgressStart()), 0.0F, 0.1F, 0.0F
         );
         matrixStack.multiply(
            RotationAxis.NEGATIVE_Y.rotationDegrees(var7 * (-90.0F * lerpSpear(var6.raiseProgress(), 0.5F, 0.55F) + 90.0F * var6.raiseProgress() + 2.0F * var6.raiseProgressMiddle())),
            var7 * 0.15F,
            0.0F,
            0.0F
         );
