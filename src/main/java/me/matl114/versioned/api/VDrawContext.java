...
public interface VDrawContext {
   ...
   default void drawSprite(Sprite sprite, int x, int y, int z, int width, int height) {
      if (width != 0 && height != 0) {
         this.w(sprite.getAtlasId(), x, x + width, y, y + height, z, sprite.getMinU(), sprite.getMaxU(), sprite.getMinV(), sprite.getMaxV());
      }
   }
   ...
   default void T(Identifier texture, int x, int y, float u, float v, int width, int height, int textureWidth, int textureHeight) {
   ...
   void w(Identifier var1, int var2, int var3, int var4, int var5, int var6, float var7, float var8, float var9, float var10);
