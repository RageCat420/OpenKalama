...
public abstract class ViaFabricPlusHooks implements IHooks {
    ...
    public abstract static class AbstractViaFabricImpl extends ViaFabricPlusHooks {
        ...
        public SupportVersion getCurrentVersion() {
            ...
            if (!Objects.equals(currentProtocol, this.lastProtocol) || this.lastVersion == null) {
                try {
                    this.lastVersion = SupportVersion.parse((String) currentProtocol.getIncludedVersions().stream()
                            .findFirst()
                            .orElseThrow());
    ...
    public interface ViaPacketWrapper {
        default ViaFabricPlusHooks.ViaPacketWrapper writePacketType(String protocolVersion, PacketType<?> packetType) {
            return this.writePacketType(
                    protocolVersion, packetType.id().getPath().toUpperCase(Locale.ROOT));
        }
    ...
    public static class ViaPacketWrapperImpl implements ViaFabricPlusHooks.ViaPacketWrapper {
        ...
        static Map<String, Map<String, com.viaversion.viaversion.api.protocol.packet.PacketType>> types =
                new HashMap<>();
        static final Map<String, Type<?>> typeMap = new HashMap<>();
        static Map<String, Class<?>> protocolCache;

        private static Map<String, com.viaversion.viaversion.api.protocol.packet.PacketType> computeAndGuessTypes(
                ...
                for (String scannPath : availableScannPath) {
                    ...
                    try {
                        ...
                        if (Enum.class.isAssignableFrom(clazz)
                                && com.viaversion.viaversion.api.protocol.packet.PacketType.class.isAssignableFrom(
                                        clazz)) {
                            for (Object re : clazz.getEnumConstants()) {
                                types.put(((Enum) re).name(), (com.viaversion.viaversion.api.protocol.packet.PacketType)
                                        re);
                            }
                        }
        ...
        public static com.viaversion.viaversion.api.protocol.packet.PacketType getPacketType(
                String protocolVersion, String packetType) {
            String version = protocolVersion.startsWith("v") ? protocolVersion.substring(1) : protocolVersion;
            return types.computeIfAbsent(version, ViaFabricPlusHooks.ViaPacketWrapperImpl::computeAndGuessTypes)
                    .get(packetType);
        }
        ...
        public ViaFabricPlusHooks.ViaPacketWrapper writePacketType(String protocolVersion, String packetType) {
            com.viaversion.viaversion.api.protocol.packet.PacketType type =
                    Objects.requireNonNull(getPacketType(protocolVersion, packetType));
            if (this.delegate == null) {
                this.delegate =
                        PacketWrapper.create(type, ViaFabricPlus.getImpl().getPlayNetworkUserConnection());
            } else {
