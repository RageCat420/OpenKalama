...
public class MapDrawFactoryImpl implements IMapDrawFactory {
        ...
        return new MapDrawFeatureImpl(DrawFeatureFactory.ellipses(
                ...
                (windowRegionX, windowRegionZ, windowRegionSize, dimension) -> {
                    ...
                    return wrapperList.stream()
                            .map(wrapper -> wrapper.inject((EllipseWrapper<Ellipse> s) ->
                                    new Ellipse(s.centerX, s.centerZ, s.radiusX, s.radiusZ)))
    ...
    public IMapDrawFeature lines(
        ...
        return new MapDrawFeatureImpl(DrawFeatureFactory.lines(
                id,
                (windowRegionX, windowRegionZ, windowRegionSize, dimension) ->
                        lineSupplier.supplyElement(windowRegionX, windowRegionZ, windowRegionSize, dimension).stream()
                                .map(wrapper ->
                                        wrapper.inject((LineWrapper<Line> s) -> new Line(s.x1, s.z1, s.x2, s.z2)))
                                .map(ElementWrapper::getElement)
                                .toList(),
                colorSupplier,
    ...
    public IMapDrawFeature text(String id, ElementSupplier<Long2ObjectMap<TextWrapper<?>>> textSupplier) {
        return new MapDrawFeatureImpl(
                DrawFeatureFactory.text(id, (windowRegionX, windowRegionZ, windowRegionSize, dimension) -> {
                    Long2ObjectMap<TextWrapper<?>> long2ObjectMap =
                    ...
                    ObjectIterator<it.unimi.dsi.fastutil.longs.Long2ObjectMap.Entry<TextWrapper<?>>> var7 =
                            long2ObjectMap.long2ObjectEntrySet().iterator();

                    while (var7.hasNext()) {
                        it.unimi.dsi.fastutil.longs.Long2ObjectMap.Entry<TextWrapper<?>> key = var7.next();
                        map.put(
                                key.getLongKey(),
                                ((TextWrapper<?>) key.getValue())
                                        .<Text, TextWrapper<Text>>inject(
                                                (TextWrapper<Text> s) -> new Text(s.value, s.x, s.z, s.color, s.scale))
                                        .getElement());
                    }
