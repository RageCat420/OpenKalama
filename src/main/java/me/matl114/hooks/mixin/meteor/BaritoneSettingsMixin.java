...
public class BaritoneSettingsMixin {
   ...
   @ModifyArg(
      ...
      at = @At(
         value = "INVOKE",
         target = "Lmeteordevelopment/meteorclient/settings/SettingGroup;add(Lmeteordevelopment/meteorclient/settings/Setting;)Lmeteordevelopment/meteorclient/settings/Setting;",
         ordinal = 4
      ),
      require = 0,
      remap = false
   ...
   private Setting<?> hookCreateWrappers(Setting<?> setting, @Local baritone.api.Settings.Setting baritoneSide) {
      return (Setting<?>)(setting instanceof IntSetting intSetting && baritoneSide.value instanceof Long
         ? ((Builder)((Builder)((Builder)((Builder)((Builder)new Builder().name(baritoneSide.getName())).description(getDescription(baritoneSide.getName())))
