...
public class NBTType<T> implements WrapperFactory<NbtElement, T> {
    ...
    public T parse(NbtElement element) {
        return (T)
                ((Pair) (Object) this.typeCodec.decode(NbtOps.INSTANCE, element).getOrThrow()).getFirst();
    }

    public T createEmpty() {
        return (T) ((Pair) (Object) this.typeCodec
                        .decode(NbtOps.INSTANCE, (NbtElement) (Object) this.typeCodec
                                .encodeStart(NbtOps.INSTANCE, this.empty)
                                .getOrThrow())
                        .getOrThrow())
                .getFirst();
    }
    ...
    public NbtElement toNbt(T val) {
        return (NbtElement)
                (Object) this.typeCodec.encodeStart(NbtOps.INSTANCE, val).getOrThrow();
    }

    public BaseAttrKeyValue<T> createAttrKeyValue(String key, T value) {
        return new BaseAttrKeyValue<>(
                key,
                value,
                this.customWidgetFactory,
