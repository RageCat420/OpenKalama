...
public class EnumRef<T extends ConfigEnum> extends LazilyRegisterTypeRef<T, String> {
   ...
   protected void tryResolve() {
      if (!this.resolved) {
         ...
         if (re == null) {
            ...
            Preconditions.checkNotNull(val, "Unregistered enum value %s in enum type %s with %s".formatted(this.enumValue, this.enumType, re.toString()));
            this.resolved = true;
            this.set((T)val);
         }
      }
   ...
   protected T validateAndCast(Object val) {
      ...
      T configEnum = (T)val;
      Preconditions.checkArgument(Objects.equals(this.enumType, configEnum.getConfigEnumType()), "Enum type mismatch !");
      return configEnum;
   }
   ...
   public BaseAttrKeyValue<T> _createKeyValue0(String key) {
      ...
      if (this.resolved) {
         return AttrKeyValue.enumMap(key, this.getValue(), (Map<String, T>)this.getValue().getMap());
      } else {
